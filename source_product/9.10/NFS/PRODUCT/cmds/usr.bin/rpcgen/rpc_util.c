/*
 @(#)rpc_util:	$Revision: 1.6.109.1 $	$Date: 91/11/19 14:10:04 $  
*/
...
{
	printwhere();
	nl_fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,61, "%1$s, line %2$d: ")), infilename, linenum);
	nl_fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,62, "%s\n")), msg);
	crash();
}
	...
	char *file;
{
	if (nfiles < NFILES) {
		...
		f_print(stderr, (catgets(nlmsg_fd,NL_SETN,63, "too many files!\n")));
		crash();
	}
}
...
expected1(exp1)
	tok_kind exp1;
{
	s_print(expectbuf, (catgets(nlmsg_fd,NL_SETN,64, "expected '%s'")),
		toktostr(exp1));
	error(expectbuf);
}
...
expected2(exp1, exp2)
	tok_kind exp1, exp2;
{
	s_print(expectbuf, (catgets(nlmsg_fd,NL_SETN,65, "expected '%s' or '%s'")),
		toktostr(exp1),
		toktostr(exp2));
	error(expectbuf);
}
...
{
	s_print(expectbuf, (catgets(nlmsg_fd,NL_SETN,66, "expected '%s', '%s' or '%s'")),
		toktostr(exp1),
		toktostr(exp2),
		toktostr(exp3));
	error(expectbuf);
