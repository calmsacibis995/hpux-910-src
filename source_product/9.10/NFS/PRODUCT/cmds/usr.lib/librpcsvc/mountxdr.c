/*	@(#)mountxdr.c	$Revision: 1.22.109.1 $	$Date: 91/11/19 14:24:58 $
...
/* Moved the three following routines to libc.a in a file  */
/* called xdr_fh.c					   */
/*
	...
	register struct mountlist **mlp;
{
	...
	while (TRUE) {
		...
		/* HPNFS This routine can only send a message of size   */
		/* HPNFS UDPMSGSIZE.  So, we calculate how much space   */
		/* HPNFS the next entry will take (name and path), plus */
		/* HPNFS 4 bytes each for rounding to the next byte,    */
		/* HPNFS 4 bytes each for the lenght to be transmitted  */
		/* HPNFS and an extra 4 bytes for padding.		*/

		if ( more_elements && (xdrs->x_op == XDR_ENCODE))
		{
		   if ((strlen((*mlp)->ml_name) + strlen((*mlp)->ml_path) + 20 + XDR_GETPOS(xdrs)) > UDPMSGSIZE )
			more_elements = FALSE;
		}
