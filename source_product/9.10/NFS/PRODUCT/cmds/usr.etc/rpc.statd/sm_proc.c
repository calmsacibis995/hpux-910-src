/* @(#)rpc.statd:	$Revision: 1.9.109.3 $	$Date: 93/12/20 11:02:53 $
*/
/* (#)sm_proc.c	1.1 87/08/05 3.2/4.3NFSSRC */
/* (#)sm_proc.c	1.2 86/12/30 NFSSRC */
#ifndef lint
static char sccsid[] = "(#)sm_proc.c 1.1 86/09/24 Copyr 1984 Sun Micro";
#endif

#ifdef PATCH_STRING
static char *patch_3453="@(#) PATCH_9.0: sm_proc.o $Revision: 1.9.109.3 $ 93/12/20 PHNE_3453"
;
#endif
...
sm_name *namep;
{
	...
	if(debug)
	    logmsg((catgets(nlmsg_fd,NL_SETN,21, "proc sm_stat: mon_name = %s\n")), namep);
...
my_id *myidp;
{
	...
	if(debug)
	     logmsg((catgets(nlmsg_fd,NL_SETN,22, "proc sm_unmon_all: [%1$s, %2$d, %3$d, %4$d]\n")), myidp->my_name, myidp->my_prog, myidp->my_vers, myidp->my_proc);
...
{
	if(debug)
	     logmsg((catgets(nlmsg_fd,NL_SETN,23, "proc sm_simu_crash\n")));
/*
	if(monitor_q != NULL)
*/
	sm_crash();
...
mon *monp;
{
	...
	my_id *my_idp, *nl_idp;
                        ...
                        /* tjs 11/93 use xmalloc */
	if((new = (mon_entry *) xmalloc(sizeof(mon_entry))) == 0) {
		log_perror((catgets(nlmsg_fd,NL_SETN,24, "insert_mon: malloc")));
		abort();
	}
	memset(new, 0, sizeof(mon_entry));
	new->id = *monp;
	monp->mon_id.mon_name = NULL;
	monp->mon_id.my_id.my_name = NULL;

	if(debug)
	     logmsg((catgets(nlmsg_fd,NL_SETN,25, "add_mon(%1$x) %2$s (id=%3$d)\n")), new, new->id.mon_id.mon_name, * ((int * )new->id.priv));
	...
	if(monitor_q == NULL) {
		...
		while (next != NULL)  { /* look for other mon_name */
			if(strcmp(next ->id.mon_id.mon_name,
				  new->id.mon_id.mon_name) == 0) { /* found */
				nl_idp = &next->id.mon_id.my_id;
				if(strcmp(new->id.mon_id.my_id.my_name,
				    nl_idp->my_name) == 0 &&
				my_idp->my_prog == nl_idp->my_prog &&
				my_idp->my_vers == nl_idp->my_vers &&
				my_idp->my_proc == nl_idp->my_proc) {
					/* already exists an identical one */
                                        /* tjs 11/93 use xfree instead free */ 
					xfree(new->id.mon_id.mon_name);
					xfree(new->id.mon_id.my_id.my_name);
					xfree(new);
					return;
				}
				else {
					new->nxt = next->nxt;
                                        /* tjs 8/93 fix backward pointer */
                                        new->prev = next;
					if(next->nxt != NULL)
						next->nxt->prev = new;
					next->nxt = new;
	...
	while((nl = next) != NULL) {
		...
		if(mon_name == NULL || (mon_name != NULL &&
	 	strcmp(nl ->id.mon_id.mon_name, mon_name) == 0)) {
			...
			my_idp->my_proc == nl_idp->my_proc) {
				/* found */
				if(debug) logmsg((catgets(nlmsg_fd,NL_SETN,26, "delete_mon(%1$x): %2$s\n")), nl, mon_name);
				if(nl->prev != NULL)
					nl->prev->nxt = nl->nxt;
				else {
					monitor_q = nl->nxt;
				...
				if(nl->nxt != NULL)
					nl->nxt->prev = nl->prev;
                                /* tjs 11/93 use xfree instead free */ 
				xfree(nl->id.mon_id.mon_name);
				xfree(nl_idp->my_name);
				xfree(nl);
			}
		} /* end of if mon */
	}
...
/*
				if(debug) printf("problem with notifying %s failure, give up\n", mon_name);
*/
			}
		}
		next = next->nxt;
...
int state;
{
	...
	if(debug)
	     logmsg((catgets(nlmsg_fd,NL_SETN,27, "statd_call_lockd: %1$s state = %2$d\n")), stat.mon_name, stat.state);
	if((err = call_tcp(my_idp->my_name, my_idp->my_prog, my_idp->my_vers,
		...
		logclnt_perrno(err);
		logmsg((catgets(nlmsg_fd,NL_SETN,28, "statd cannot contact local lockd on %s status change, give up!\n")), mon_name);
		return(-1);
		}
	else
		return(0);
...
pr_mon()
{
	...
	if(!debug)
		return;
	if(monitor_q == NULL) {
	     logmsg((catgets(nlmsg_fd,NL_SETN,29, "*****monitor_q = NULL\n")));
		return;
	}

	nl= monitor_q;
	logmsg((catgets(nlmsg_fd,NL_SETN,30, "*****monitor_q: ")));
	while(nl != NULL) {
		logstr((catgets(nlmsg_fd,NL_SETN,31, "(%x), ")), nl);
                logstr("\nmon_name = %s, my_name = %s, prog = %d, vers = %d, proc = %d\n", nl->id.mon_id.mon_name, nl->id.mon_id.my_id.my_name, nl->id.mon_id.my_id.my_prog, nl->id.mon_id.my_id.my_vers, nl->id.mon_id.my_id.my_proc);  
		nl = nl->nxt;
	}
	logstr((catgets(nlmsg_fd,NL_SETN,32, "\n")));
}
