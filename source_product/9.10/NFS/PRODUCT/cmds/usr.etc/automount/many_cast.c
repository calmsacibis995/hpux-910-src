/* @(#)automount:	$Revision: 1.5.109.1 $	$Date: 91/11/19 14:14:02 $
*/

#ifndef lint
static char sccsid[] = 	"(#)many_cast.c	1.3 90/07/24 4.1NFSSRC Copyr 1990 Sun Micro";
#endif
	...
	int		timeout;	/* timeout (sec) */
{
	...
	if ((sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
		syslog(LOG_ERR, (catgets(catd,NL_SETN,701, "Cannot create socket for broadcast rpc: %m")));
		stat = RPC_CANTSEND;
		goto done_broad;
	}
	mask = (1 << sock);
		...
		for (i = 0; addrs[i].s_addr != 0; i++) {
			...
			*((u_long *)outbuf) = htonl(xid + i);
			if (sendto(sock, outbuf, outlen, 0,
				...
				syslog(LOG_ERR,
				    (catgets(catd,NL_SETN,702, "Cannot send broadcast packet: %m")));
				stat = RPC_CANTSEND;
				goto done_broad;
			}
		}
	recv_again:
		...
		case -1:  /* some kind of error */
			if (errno == EINTR)
				goto recv_again;
			syslog(LOG_ERR, (catgets(catd,NL_SETN,703, "Many_cast select problem: %m")));
			stat = RPC_CANTRECV;
			goto done_broad;
	...
	try_again:
		...
		if (inlen < 0) {
			if (errno == EINTR)
				goto try_again;
			syslog(LOG_ERR,
			    (catgets(catd,NL_SETN,704, "Cannot receive reply to many_cast: %m")));
			stat = RPC_CANTRECV;
			goto done_broad;
		}
