/* @(#)automount:	$Revision: 1.4.109.6 $	$Date: 93/12/13 16:33:35 $
*/
#ifndef lint
...
#ifdef PATCH_STRING
static char *patch_2501="@(#) PATCH_9.0: auto_look.o $Revision: 1.4.109.6 $ 93/07/06 PHNE_2501 PHNE_2967"
;
#endif
	...
	if (me == NULL) {
		if (*name == '=' && cred->aup_uid == 0) {
			if (isdigit(*((u_char *)name+1))) {
				trace = atoi(name+1);
				(void) fprintf(stderr, (catgets(catd,NL_SETN,201, "automount trace = %d\n")),
					trace);
			} else {
				diag(name+1);
			}
	...
	char *s;
{
	...
	case 'v':	/* toggle verbose */
		verbose = !verbose;
		strcpy(msg1, catgets(catd,NL_SETN,203, "on"));
		strcpy(msg2, catgets(catd,NL_SETN,204, "off"));
		syslog(LOG_ERR, (catgets(catd,NL_SETN,202, "verbose %s")), verbose ? msg1 : msg2);
		break;
		...
		for (i = 0; i < FH_HASH_SIZE; i++) {
			...
			for (; vnode; vnode = NEXT(struct vnode, vnode)) {
				if (vnode->vn_type == VN_LINK) {
					...
					(void) fprintf(stderr, (catgets(catd,NL_SETN,205, "link: %s/ %s ")),
					...
					(void) fprintf(stderr, (catgets(catd,NL_SETN,206, "dir : %s %s -%s\n")),
	...
	if ((fp = fopen(mapname, "r")) != NULL) {
		...
		for (;;) {
			...
			if (verbose && syntaxok && isspace(*(u_char *)lp)) {
				syntaxok = 0;
				syslog(LOG_ERR,
				(catgets(catd,NL_SETN,207, "leading space in map entry \"%s\" in %s")),
				lp, mapname);
			}
			 ...
			 */
			 ...
			 /*
                          * HPNFS - Fix DTS# INDaa16344 - 11/15/93 hv
                          * Don't do sanity check if the first character
                          * of the entry is '+', included map.
                          */
                       if (verbose && syntaxok && *word != '+') { /* sanity check entry */
				if (*key == '/') {
					if (*word != '/') {
						syntaxok = 0;
						syslog(LOG_ERR,
						(catgets(catd,NL_SETN,208, "bad key \"%s\" in direct map %s\n")),
						word, mapname);
					}
				} else {
					if (strchr(word, '/')) {
						syntaxok = 0;
						syslog(LOG_ERR,
						(catgets(catd,NL_SETN,209, "bad key \"%s\" in indirect map %s\n")),
						word, mapname);
					}
				}
			}
	...
	char *lp, *lq, *mapname, *mapopts, *key;
{
	...
	if (w[0] == '-') {	/* default mount options for entry */
		if (syntaxok && (p = opt_check(w+1))) {
			syntaxok = 0;
			syslog(LOG_ERR,
			(catgets(catd,NL_SETN,210, "WARNING: \"%s\" ignored for %s in %s")),
			p, key, mapname); 
		}
	...
	while (*w == '/' || implied) {
		...
		if (w[0] == '-') {	/* mount options */
			if (syntaxok && (p = opt_check(w+1))) {
				syntaxok = 0;
				syslog(LOG_ERR,
				(catgets(catd,NL_SETN,211, "WARNING: \"%s\" ignored for %s in %s")),
				p, key, mapname); 
			}
		...
		if (me->map_mntopts == NULL)
			goto alloc_failed;
		if (w[0] == '\0') {
			syslog(LOG_ERR, (catgets(catd,NL_SETN,212, "map %s, key %s: bad")), mapname, key);
			goto bad_entry;
		}
...
alloc_failed:
	syslog(LOG_ERR, (catgets(catd,NL_SETN,213, "Memory allocation failed: %m")));
bad_entry:
	free_mapent(ms);
	return ((struct mapent *) NULL);
}
...
bad_entry:
	syslog(LOG_ERR, (catgets(catd,NL_SETN,214, "bad entry in map %s \"%s\"")), mapname, w);
	return 1;
}
...
alloc_failed:
	syslog(LOG_ERR, (catgets(catd,NL_SETN,215, "Memory allocation failed: %m")));
	free_mapent(me);
	return ((struct mapent *)NULL);
}
	...
	char *mapopts, *host;
{
	...
	if (cl == NULL) {
		...
		if (cl == NULL) {
			syslog(LOG_ERR, (catgets(catd,NL_SETN,218, "client create: %s")), clnt_spcreateerror(host));
			return ((struct mapent *) NULL);
		}
	}

	timeout.tv_sec = mount_timeout; /* total time for results to return */
	timeout.tv_usec = 0;
        if (clnt_stat = clnt_call(cl, MOUNTPROC_EXPORT,
		xdr_void, 0, xdr_exports, &ex, timeout)) {
		syslog(LOG_ERR, (catgets(catd,NL_SETN,216, "%s: exports: %s")), host, clnt_sperrno(clnt_stat));
		clnt_destroy(cl);
		return ((struct mapent *) NULL);
	}
	clnt_destroy(cl);
...
alloc_failed:
	syslog(LOG_ERR, (catgets(catd,NL_SETN,217, "Memory allocation failed: %m")));
	free_mapent(ms);
	freeex(exlist);
	return ((struct mapent *)NULL);
}
