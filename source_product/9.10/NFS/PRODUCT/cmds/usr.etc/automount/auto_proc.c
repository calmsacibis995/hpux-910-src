/* @(#)automount:	$Revision: 1.4.109.2 $	$Date: 92/05/28 15:06:19 $
*/
#ifndef lint
static char sccsid[] = 	"(#)auto_proc.c	1.4 90/07/24 4.1NFSSRC Copyr 1990 Sun Micro";
#endif
	...
	readdirargs *args;
{
	...
	for (link = TAIL(struct link, dir->dir_head); link;
		...
		*entp = (struct entry *) malloc(sizeof(entry));
		if (*entp == NULL) {
			syslog(LOG_ERR, (catgets(catd,NL_SETN,601, "Memory allocation failed: %m")));
			break;
		}
