/* @(#)automount:	$Revision: 1.5.109.3 $	$Date: 92/04/08 15:07:20 $
*/
	...
	int		timeout;	/* timeout (sec) */
{
	...
	if ((sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
		syslog(LOG_ERR, (catgets(catd,NL_SETN,801, "Cannot create socket for nfs: %m")));
		stat = RPC_CANTSEND;
		goto done_broad;
	}
	FD_ZERO(&mask);
		...
		for (i = 0; addrs[i].s_addr != 0; i++) {
			...
			*((u_long *)outbuf) = htonl(xid + i);
			if (sendto(sock, outbuf, outlen, 0,
				...
				syslog(LOG_ERR,
				    (catgets(catd,NL_SETN,802, "nfscast: Cannot send packet: %m")));
				continue;
			}
			sent++;
		}
	...
	recv_again:
		...
		case -1:  /* some kind of error */
			if (errno == EINTR)
				goto recv_again;
			syslog(LOG_ERR, (catgets(catd,NL_SETN,803, "nfscast: select: %m")));
			stat = RPC_CANTRECV;
			goto done_broad;
	...
	try_again:
		...
		if (inlen < 0) {
			if (errno == EINTR)
				goto try_again;
			syslog(LOG_ERR,
			    (catgets(catd,NL_SETN,804, "nfscast: cannot receive reply: %m")));
			stat = RPC_CANTRECV;
			goto done_broad;
		}
