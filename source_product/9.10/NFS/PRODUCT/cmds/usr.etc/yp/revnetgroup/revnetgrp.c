#ifndef lint
static  char rcsid[] = "@(#)revnetgroup:	$Revision: 1.20.109.1 $	$Date: 91/11/19 14:22:26 $  ";
#endif
	...
	char *argv[];
{
...
#ifdef NLS
	nl_init(getenv("LANG"));
	nlmsg_fd = catopen("revnetgroup",0);
#endif NLS
	...
	if (argc == 2 && argv[1][0] == '-' 
		...
		(void) fprintf(stderr,
			(catgets(nlmsg_fd,NL_SETN,1, "usage: %s -h (by host), %s -u (by user)\n")),argv[0],argv[0]);
		exit(1);
	}
...
/* 
 *	Get the next netgroup from the netgroup file (/etc/netgroup?)
 */
static char * 
nextgroup() 
{ 
...
/*
 * Load up a hash table with the info in the netgroup file (/etc/netgroup?)
 */
static void
loadtable(nf)
    FILE *nf;
