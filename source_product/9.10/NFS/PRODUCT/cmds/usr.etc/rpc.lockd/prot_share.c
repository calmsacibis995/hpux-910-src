...
static char *PHNE_2916_patch =
	"@(#) PATCH_9.0: prot_share.o $Revision: 1.3.109.2 $ 93/07/26 PHNE_2916";
#endif
...
init_nlm_share()
{
	...
	if (debug)
		logmsg((catgets(nlmsg_fd,NL_SETN,300,"init_nlm_share\n")));
	for (i = 0; i < SH_HASH; i++)
		fh_to_sx[i] = NULL;
}

 ...
 */
	...
	if (debug) {
		logmsg((catgets(nlmsg_fd,NL_SETN,301,"proc_nlm_share: %1$s from %2$s\n")),
			SHARE_REQ?"share":"unshare",
			req->share.caller_name);
		logmsg((catgets(nlmsg_fd,NL_SETN,302,"oh=")));
		pr_fh(&req->share.oh);
		logmsg((catgets(nlmsg_fd,NL_SETN,303,"\nfh="))); 
		pr_fh(&req->share.fh);
		logmsg((catgets(nlmsg_fd,NL_SETN,304,"\n"))); 
		if (SHARE_REQ)
		    logmsg((catgets(nlmsg_fd,NL_SETN,305,"share: mode=%1$d, access=%$\n")),
 ...
 */
	if (sxh == NULL) {
		if (SHARE_REQ) {
			if (debug)
			    logmsg((catgets(nlmsg_fd,NL_SETN,306,"...null sxh - new file\n")));
			sxh = (struct sx *)xmalloc(sizeof(struct sx));
			sxh->bck = NULL;
			sxh->fwd = fh_to_sx[hv];
                              /* fix double link list backward pointer  x */
                              /* which caused file lock problem tjs 7/93  */
                        if (fh_to_sx[hv] != NULL) fh_to_sx[hv]->bck = sxh;
			fh_to_sx[hv] = sxh;
 ...
 */
	if (debug)
		logmsg((catgets(nlmsg_fd,NL_SETN,307,"...sxh @0x%x\n")), sxh);
	sxxh = sxh->sxxp;
	while (sxxh != NULL) {
 ...
 */
	if (sxxh != NULL) {
		if (debug)
			logmsg((catgets(nlmsg_fd,NL_SETN,308,"...matching sxx @0x%x\n")), sxxh);
		if (UNSHARE_REQ) {
/*
 ...
 */
	if (SHARE_REQ) {
		if (!compatible(sxh->sxxp, &req->share)) {
			if (debug)
			    logmsg((catgets(nlmsg_fd,NL_SETN,309,"...not compatible\n")));
			result = nlm_denied;
			goto common_exit;
		}
		sxxh = new_sxx(&req->share);
		if (debug)
		    logmsg((catgets(nlmsg_fd,NL_SETN,310,"...compatible. new sxx @0x%x\n")), sxxh);
		sxh->sxxp->bck = sxxh;
		sxxh->fwd = sxh->sxxp;
		sxh->sxxp = sxxh;
	}
	...
	char *client;
{
	...
	if (debug)
		logmsg((catgets(nlmsg_fd,NL_SETN,311,"destroy_client_shares for %s\n")), client);
 ...
 */
					if (debug)
					    logmsg((catgets(nlmsg_fd,NL_SETN,312,"... zap sxx @0x%x\n")), sxxh);
					if (sxxh->bck == NULL)
						sxh->sxxp = sxxh->fwd;
					else
						sxxh->bck->fwd = sxxh->fwd;
 ...
 */
					if (sxh->sxxp == NULL) {
					if (debug)
						logmsg((catgets(nlmsg_fd,NL_SETN,313,"...  &  sx @0x%x\n")), sxh);
						if (sxh->bck == NULL)
							fh_to_sx[hv] = sxh->fwd;
						else
							sxh->bck->fwd = sxh->fwd;
...
{
	struct sxx *new;
	new = (struct sxx *)xmalloc(sizeof(struct sxx));
	...
	while (ss != NULL) {
		if (((int)ss->sh.mode & (int)s->access) ||
			...
			if (report_sharing_conflicts)
				logmsg((catgets(nlmsg_fd,NL_SETN,314,"File sharing request from %1$s (mode %2$d, access %3$d) conflicts\nwith sharing for %4$s (mode %5$d, access %6$d)\n")),
