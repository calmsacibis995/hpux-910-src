/* @(#)rpc.lockd:	$Revision: 1.19.109.2 $	$Date: 94/10/21 10:36:34 $
*/
/* (#)prot_lock.c	1.1 87/08/05 3.2/4.3NFSSRC */
/* (#)prot_lock.c	1.3 87/06/18 NFSSRC */
#ifndef lint
...
reclock *a;
{
	...
	if(fp == NULL)  { 	/*create new fe entry*/
		if((fp = (struct fs_rlck *) a->pre_fe) == NULL) {
			logmsg(catgets(nlmsg_fd,NL_SETN,41, "%1$s: (%2$x)pre_fe == NULL : ABORT in add_reclock\n"), progname, a);
				abort();
		}
		a->pre_fe = NULL;
		insert_fe(fp);
		...
		if( insrtp != NULL && same_proc(insrtp, a) && same_op(insrtp, a) && inside(a, insrtp)) { /* lock exists */
			if(insrtp == a) { /* extra protection */
				logmsg(catgets(nlmsg_fd,NL_SETN,42, "%1$s: add_reclock: insrtp = a = %2$x should not happen!!!\n"), progname, a);
			}
			else {
				a->rel = 1;
			}
		...
		if(same_proc(nl, a) && nl->lck.ub > a->lck.lb) { /*overlap*/
			...
			if(inside(nl, a)) { /*delete complete*/
				...
				if((new = a->pre_le) == NULL) { 
				 /* no more lock entry */
					logmsg(catgets(nlmsg_fd,NL_SETN,43, "%1$s: (%2$x) pre_le is NULL : ABORT in delete_reclock\n"), progname, a);
					abort();
				}
				else {
					a->pre_le = NULL;
			 ...
			 since add_mon returns -1 only with "new" mp addition */
					if(add_mon(new, 1) == -1)
						logmsg(catgets(nlmsg_fd,NL_SETN,44, "%s: add_mon = -1 in delete, should not happen\n"), progname);
				...
				find_insert(fp, nl);
			    	...
			    	logmsg(catgets(nlmsg_fd,NL_SETN,45, "%s: impossible!\n"), progname);
		}
	}
...
reclock *a;
{
	...
	if((nl = wait_q) == NULL) {
		...
		while(nl != NULL) {
			if(same_lock(nl, a)) {
				if(debug)
					logmsg(catgets(nlmsg_fd,NL_SETN,46, "%s: same blocking lock already exists\n"), progname);
				a->rel = 1;
				return;
			}
			next = nl;
		...
		while(nl != NULL) {
			if(overlap(nl, a) &&
			 blocked(&fp, &insrtp, nl) == 0) {
	                        /* not blocked - by other remote locks */
	                        ...
	                        flock.l_whence = 0;
	                        flock.l_start = nl->lck.l_offset;
	                        flock.l_len = nl->lck.l_len;
				ENABLEPRIV(SEC_REMOTE);
	                        if (nfs_fcntl(nl->lck.fh.n_bytes, f_cmd, &flock) != -1) {
				   DISABLEPRIV(SEC_REMOTE);
				   if((remote_clnt(nl)) && (add_call(nl) == -1)) { 
				      logmsg(catgets(nlmsg_fd,NL_SETN,47, "%1$s: wakeup(%2$x) cannot take place due to add_call malloc error\n"), progname, nl);
				   }
				   else {
				      ...
				      if (! remote_clnt(nl) &&
				         ((msgp = retransmitted(nl, KLM_LOCK)) != NULL)) {
					  remote_result *resp;

					  /* Send a reply if we know where to
					   * send it to and we can malloc the
					   * space.
					   */
				   ...
				   DISABLEPRIV(SEC_REMOTE);
				   if (errno != EACCES)
	      			      logmsg((catgets(nlmsg_fd,NL_SETN,54, "%1$s: fcntl (wakeup): errno = %2$d!\n")), progname, errno);
				}
			}
			nl = nl->wait_nxt;
		}
...
reclock *a;
{
	...
	if((fp = get_fe()) == NULL) {	/*no more fe entry*/
		logmsg(catgets(nlmsg_fd,NL_SETN,48, "%s: get_fe: out of fs_rlck entry\n"), progname);
		return(NULL); 
	}
...
struct netobj *a, *b;
{
	...
	if(a->n_len != b->n_len) 
		return(FALSE);
	if(memcmp(&a->n_bytes[0], &b->n_bytes[0], a->n_len) != 0) 
		return(FALSE);
	else
		return(TRUE);
}
...
netobj *a, *b;
{
	if(b == NULL) {
		...
		if(debug)
			logmsg(catgets(nlmsg_fd,NL_SETN,49, "%1$s: obj_copy(a = %2$x, b = NULL), a\n"), progname);
		return(0);
	}
...
reclock *nl, *a;
{
	if(nl->lck.ub <= a->lck.lb || nl->lck.lb >= a->lck.ub)
		return(FALSE);
	if(strcmp(nl->lck.svr, a->lck.svr) != 0)
		return(FALSE);
	if(obj_cmp(&nl->lck.fh, &a->lck.fh))
		return(TRUE);
	else
		return(FALSE);
}
...
int i;
{
	if(strcmp(a->lck.svr, a->lck.clnt) == 0)
		...
		return(0);
	if(remote_data(a)) {
		if(mond(hostname, PRIV_RECOVERY, a, i, 0) == -1)
			return(-1);
		if(mond(a->lck.svr, PRIV_RECOVERY, a, i, 1) == -1)
			return(-1);
		}
	else
		if(mond(a->lck.clnt, PRIV_CRASH, a, i, 1) == -1)
			return(-1);
		return(0);
...
int i;
{
	...
	case 1:
		...
		break;
	default:
		logmsg(catgets(nlmsg_fd,NL_SETN,50, "%1$s: unknown contact monitor (%2$d) : ABORT in contact_monitor\n"), progname, i);
		abort();
	}
	...
	if((priv_size = sizeof(struct priv_struct)) > 16)   /* move to init */
		logmsg(catgets(nlmsg_fd,NL_SETN,51, "%1$s: contact_mon: problem with private data size (%2$d) to status monitor\n"),
		    progname, priv_size);
	...
	if(resp->res_stat == stat_succ) {
		if(resp->sm_stat == stat_succ) {
			...
			return(0);
		     ...
		     logmsg(catgets(nlmsg_fd,NL_SETN,52, "%1$s: site %2$s not subscribe to status monitor service \n"),
			    progname, new->svr);
			return(-1);
		}
	}
	else {
		logmsg(catgets(nlmsg_fd,NL_SETN,53, "%s: lockd cannot contact local statd\n"), progname);
		return(-1);
	}
}
