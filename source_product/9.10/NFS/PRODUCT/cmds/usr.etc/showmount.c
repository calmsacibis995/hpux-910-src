#ifndef lint
static char sccsid[] = 	"@(#)showmount.c	1.4 90/07/23 4.1NFSSRC Copyr 1990 Sun Micro";
#endif
	...
	char **argv;
{
	...
	if (host == NULL) {
		if (gethostname(hostbuf, sizeof(hostbuf)) < 0) {
			perror((catgets(catd,NL_SETN,1, "showmount: gethostname")));
			exit(1);
		}
		host = hostbuf;
	}
	...
	if (err = clnt_call(cl, MOUNTPROC_DUMP,
		...
		fprintf(stderr, (catgets(catd,NL_SETN,2, "showmount: %s\n")), clnt_sperrno(err));
		exit(1);
	}
	tb = table;
	...
	if (ml != NULL && tb == &table[NTABLEENTRIES])
		printf((catgets(catd,NL_SETN,3, "showmount:  table overflow:  only %d entries shown\n")),
			NTABLEENTRIES);
	endtb = tb;
	if (dflg)
...
usage()
{
	fprintf(stderr, (catgets(catd,NL_SETN,4, "showmount [-a] [-d] [-e] [host]\n")));
}
	...
	char *host;
{
	...
	if (err = clnt_call(cl, MOUNTPROC_EXPORT,
		...
		fprintf(stderr, (catgets(catd,NL_SETN,5, "showmount: %s\n")), clnt_sperrno(err));
		exit(1);
	}

	if (ex == NULL) {
		fprintf(stdout, (catgets(catd,NL_SETN,6, "no exported file systems for %s\n")), host);
	} else {
		fprintf(stdout, (catgets(catd,NL_SETN,7, "export list for %s:\n")), host);
	}
	max = 0;
	...
	while (ex) {
		...
		gr = ex->ex_groups;
		if (gr == NULL) {
			fprintf(stdout, (catgets(catd,NL_SETN,8, "(everyone)")));
		}
		while (gr) {
