...
static char scan_buf[100];

gets(cp)
{
	strcpy(cp,kdbgetline());
	return cp;
}
...
char *fmt;
{
	strcpy(scan_buf,kdbgetline());
	return sscanf(scan_buf,fmt,args);
}
...
register int **argp;
{
	...
	case '\n':
	case '\t': 
		while((ch1 = getc(sstring))==' ' || ch1=='\t' || ch1=='\n')
			;
		if(ch1 != EOF)
			ungetc(ch1, sstring);
		break;
...
int *eofptr;
{
	...
	*eofptr = 0;
	...
	if(type == 's')
		ignstp = SPC;
	while(_sctab[ch = getc(sstring)] & ignstp)
		if(ch == EOF)
			break;
	ignstp = SPC;
	if(type == 'c')
