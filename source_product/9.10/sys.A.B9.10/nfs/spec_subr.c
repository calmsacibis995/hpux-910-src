/* $Source: /source/hpux_source/kernel/sys.SWT68K_800/nfs/RCS/spec_subr.c,v $
 * $Revision: 1.7.83.4 $	$Author: kcs $
 * $State: Exp $   	$Locker:  $
 * $Date: 93/09/17 19:09:19 $
 */
	...
	dev_t dev;
{
	...
	if ((sp = sfind(dev, vp)) == NULL) {
		if (vp->v_type == VFIFO) {
			...
			STOV(sp)->v_fstype = VNFS_SPEC;

			/* init the times in the snode to those in the vnode */
			if (!VOP_GETATTR(vp, &va, u.u_cred,VSYNC)) {
