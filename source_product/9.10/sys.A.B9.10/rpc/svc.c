/* $Source: /source/hpux_source/kernel/sys.SWT68K_800/rpc/RCS/svc.c,v $
 * $Revision: 1.12.83.4 $	$Author: marshall $
 * $State: Exp $   	$Locker:  $
 * $Date: 93/12/06 22:49:03 $
 */
...
static char rcsid[] = "@(#) $Header: svc.c,v 1.12.83.4 93/12/06 22:49:03 marshall Exp $ (Hewlett-Packard)";
#endif
...
extern struct rsstatstr rsstat;
	...
	while (TRUE) {
		s = splnet();
	 	if  (xprt->xp_sock->so_rcv.sb_cc == 0) {
			   ...
			   if ( retval ) {
				/* We must give up our semaphore here.
				** Where we jump to, when we longjmp(),
				** must assume no semaphores owned, and
				** will re-acquire, if necessary, before
				** proceeding.
				*/
#ifndef _WSIO
				VSEMA(&up_io_sema);
...
svc_run()
{
	...
	while (TRUE) {
		/*	HPNFS	jad	87.09.10
		**	THIS IS AN ERROR!  svc_run() will blindly
