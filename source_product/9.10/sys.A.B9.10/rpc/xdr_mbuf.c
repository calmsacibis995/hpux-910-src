/* $Source: /source/hpux_source/kernel/sys.SWT68K_800/rpc/RCS/xdr_mbuf.c,v $
 * $Revision: 1.9.83.4 $	$Author: marshall $
 * $State: Exp $   	$Locker:  $
 * $Date: 93/12/15 12:02:56 $
 */
...
******************************************************************************
****								
**** 	xdr_mbuf - XDR implementation on kernel mbufs.
****								
******************************************************************************
* Description
*	The XDR mbuf routines provide the functions necessary for getting
*	the XDR data to and from kernel mbufs.
*
* Externally Callable Routines
*	xdrmbuf_init - initialize an XDR stream using mbufs
*	xdrmbuf_destroy - null routine
*	xdrmbuf_getlong - get a long from the mbuf XDR stream
*	xdrmbuf_putlong - put a long into the mbuf XDR stream
* 	nfs_bcopy - front-end byte copy routine for NFS
*	xdrmbuf_getbytes - get bytes from the mbuf XDR stream
*	xdrmbuf_putbytes - put bytes into the mbuf XDR stream
*       xdrmbuf_getvec - get data from the mbuf XDR stream
*                        and set up the iovec array
*	xdrmbuf_putbuf - attach data to mbuf chain using special mbuf
*	xdrmbuf_getpos - determine the current position in the XDR stream
*	xdrmbuf_setpos - set the current position in the XDR stream
*	xdrmbuf_inline - ask for a block of contiguous data in the input stream
*
* Test Module
*	$SCAFFOLD/nfs/*
*
...
*	
*     02/03/92		kls	Add xdrmbuf_getvec.
*
******************************************************************************
* END_IMS_MOD */

#if	defined(MODULEID) && !defined(lint)
static char rcsid[] = "@(#) $Header: xdr_mbuf.c,v 1.9.83.4 93/12/15 12:02:56 marshall Exp $ (Hewlett-Packard)";
#endif
	...
	long		*lp;
{
	...
	if ((xdrs->x_handy -= sizeof(long)) < 0) {
		 ...
		 */
			...
			bcopy(xdrs->x_private, holdit, (int)amt);
			more = 'y';
		}
		if (xdrs->x_base) {
			register struct mbuf *m =
			...
			xdrs->x_private = mtod(m, caddr_t);
			if (more == 'y') {
				bcopy(xdrs->x_private, &(holdit[amt]), (int)
				(sizeof(long)-amt)); 
...
-----------------------------------------------------------------------------*/
	privlbcopy(KERNELSPACE, from, KERNELSPACE, to, len);
}
	...
	register u_int	 len;
{
	...
	while ((xdrs->x_handy -= len) < 0) {
		if ((xdrs->x_handy += len) > 0) {
			NFSBCOPY(xdrs->x_private, addr, (int)xdrs->x_handy);
			addr += xdrs->x_handy;
			len -= xdrs->x_handy;
		}
		if (xdrs->x_base) {
	...
	NFSBCOPY(xdrs->x_private, addr, (int)len);
	xdrs->x_private += len;
	return (TRUE);
}
	...
	register u_int		 len;
{
	...
	while ((xdrs->x_handy -= len) < 0) {
		if ((xdrs->x_handy += len) > 0) {
			bcopy(addr, xdrs->x_private, (int)xdrs->x_handy);
			addr += xdrs->x_handy;
			len -= xdrs->x_handy;
		}
		if (xdrs->x_base) {
...

/* BEGIN_IMS xdrmbuf_getvec *
 ********************************************************************
 ****
 ****		xdrmbuf_getvec(xdrs, iov, iovcntp, maxiovcnt, lenp, maxlen)
 ****
 ********************************************************************
 * Input Parameters
 *	xdrs		handle passed to the routines
 *	iov		pointer to iov array in the nfs_writeargs structure
 *	iovcntp		pointer to the counter for iov array numbers 
 *	maxiovcnt	maximum value of iovcnt
 *	lenp		pointer to length of the xdr bytes 
 *	maxlen		maximum value of len
 *
 * Output Parameters
 *      iov             pointer to iov array in the nfs_writeargs
 *	iovcntp		pointer to the counter for the iov array numbers 
 *	lenp		pointer to the length of the xdr bytes 
 *
 * Return Value
 *	TRUE/FALSE	depending upon the sucess of the operation
 *
 ...
 *
 * Description
 *      call xdrmbuf_getlong to decode the length field (wa_count),
 *      check for maximum length and zero.
 * 
 *      calculated the padding
 *      make local copies to the xdr references to the mbuf.
 *      go through xdrs xdr_base chain and set iov array iov_base and
 *      iov_len.
 *
 *      If padding is necessary, point next iov to the same bytes
 *      referenced by the previous iov.
 *	
 *      This change has dependence on changes to svc_getargs and
 *      svc_freeargs. The ud_imbuf is freed in svc_freeargs.
 *
 *
 * Modification History
 *
 ...
 *
 * Called By (optional)
 *	xdr_writeargs (server side)
 *
 ********************************************************************
 * END_IMS xdrmbuf_getvec */
bool_t
xdrmbuf_getvec(xdrs, iov, iovcntp, maxiovcnt, lenp, maxlen)
	register XDR	*xdrs;
	struct iovec	*iov;
	...
	u_int		 maxlen;
{
	...
	int		 okay;
	 ...
	 /* first decode  the length since xdr bytes are counted
	 */

	if (! xdrmbuf_getlong(xdrs, lenp)) {
		return (FALSE);
	}
	len = *lenp;
	if (len > maxlen) {
	...
	/*
	 * Make local copies of xdrs references to the mbuf.  Not only
	 * is this much more efficient in a volatile world, but it is
	 * also necessary to not modify these xdrs fields if a copy is
	 * required.
	 */
	basem = (struct mbuf *)xdrs->x_base;
	private = xdrs->x_private;
	handy = xdrs->x_handy;

	/*
	 * Create the iovec structure 
	 */
	iovcnt = 0;
	while ((handy -= len) < 0) {
		if ((handy += len) > 0) {
		...
		else if ((basem = basem->m_next) == NULL) {
			okay = FALSE;
			goto done;
		}
		else {
			private = mtod(basem, caddr_t);
			handy = basem->m_len;
		} 
	}
	if (iovcnt >= maxiovcnt) {
	...
	if (padding) {
		...
		iovcnt++;
		/*
		 * Padding necessary, point the next iov to the same bytes
		 * referenced by the previous iov.
		 *
		 * WARNING: this algorithm doesn't work if padding > len !
		 */
		iov->iov_base = private;
		iov->iov_len  = padding;
		iov++;
...
done:	xdrs->x_base = (caddr_t)basem;
	xdrs->x_private = private;
	xdrs->x_handy = handy;

copy:	/*
	 * Returning FALSE and IOVCNT > MAXIOVCNT indicates to
	 * caller that a copy is required (too many fragments).
	 */
	*iovcntp = iovcnt;
	return (okay);
}
	...
	int		 arg;
{
	...
	(void) xdrmbuf_putlong(xdrs, &llen);
	/*
	 * Calculate the necessary padding needed to get to a
	 * multiple of four bytes. e.g. if len = 5, padding=3.
	 */
