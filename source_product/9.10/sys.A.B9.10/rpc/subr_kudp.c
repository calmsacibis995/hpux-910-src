/* $Source: /source/hpux_source/kernel/sys.SWT68K_800/rpc/RCS/subr_kudp.c,v $
 * $Revision: 1.8.83.3 $	$Author: kcs $
 * $State: Exp $   	$Locker:  $
 * $Date: 93/09/17 19:27:50 $
...
static char rcsid[] = "@(#) $Header: subr_kudp.c,v 1.8.83.3 93/09/17 19:27:50 kcs Exp $ (Hewlett-Packard)";
#endif
	...
	*from = *mtod(m, struct sockaddr_in *);
	...
	while(m) {
		...
		if (first) 
		   ...
		   m1->m_next = m;
		/* m_pullup could return a null pointer if the m_get fails 
		 * there. We should then free up the beginning part of the
		 * chain (pointed to by m0)
		 *	-- arm 06/15/88 DTS fix
		 */
		if (m == NULL) {
		   if (!first) 
			/* there is something to free up */
			m_freem(m0);
 ...
 *	05/05/92        kls	Change ku_fastsend error return -2 to EAGAIN
 *
 * External Calls
	...
	struct sockaddr_in *addr;
{
...
#endif
	Sendtries++;
	s = splnet(); /* protect ourselves -- Sandip */
	/* This was done as a result of a bug which shows up when
	   netisr is running on the ICS. If an inbound packet comes in,
	   a different thread of execution starts off and mbufs get
	   messed up -- Sandip 6/2/89 */
	...
	/*
	 *  if ku_fastsend returns EAGAIN, then we can try to send m the
