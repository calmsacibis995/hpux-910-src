/* $Source: /source/hpux_source/kernel/sys.SWT68K_800/rpc/RCS/svc_kudp.c,v $
 * $Revision: 1.11.83.4 $	$Author: marshall $
 * $State: Exp $   	$Locker:  $
 * $Date: 93/12/15 12:18:38 $
 */
...
static char rcsid[]="@(#) $Header: svc_kudp.c,v 1.11.83.4 93/12/15 12:18:38 marshall Exp $";
#endif
...
/*------------------------------------*/
/* Larger buffer for NFS server       */
/*------------------------------------*/

#define SVCKUDP_SBRESERVE   256*1024
...
/* Move this structure to svc.h, and add "int rsruns;". Hence svc_run 
 * procedure can accumulate rsruns counter.
...
struct rsstatstr rsstat;


/*----------------------------------------------------------
---
---
nfs_macct is removed for 4.3 support
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
        ...
        if ( sock->so_rcv.sb_hiwat < SVCKUDP_SBRESERVE  )
	   (void) sbreserve_big( &sock->so_rcv, SVCKUDP_SBRESERVE) ;
 ...
 * Modification History
 *	11/06/87	ew	added comments
 *      11/27/91        rb      ifdef MP to work with MP lan driver
 *
 * External Calls
 ...
 *      11/27/91        rb      ifdef MP sleep_lock to work with 
 *                                    MP lan driver
 *
 * External Calls
	...
	struct rpc_msg *msg; 
{
	...
	rpc_debug(4, "svckudp_send %x\n", xprt);
        ...
        while (ud->ud_flags & UD_BUSY) {
                ud->ud_flags |= UD_WANTED;
                sleep_then_unlock((caddr_t)ud, PZERO-2,old);
                if(!(ud->ud_flags & UD_BUSY))
                { goto loop_done; }
                old  = sleep_lock();
        }
        sleep_unlock(old);
loop_done:
        ud->ud_flags |= UD_BUSY;  /* can this be a problem */
#else /* not MP */
	s = splimp();
	while (ud->ud_flags & UD_BUSY) {
		ud->ud_flags |= UD_WANTED;
 ...
 * Modification History
 *	11/07/87	ew	added comments
 *      01/15/91        kls     ud_inmbuf freed by svc_freesrgs.
 *                              to support xdrmbuf_getvec  functions
 *
 * External Calls
	...
	caddr_t		 args_ptr;
{
	...
	/* NIKE IOVEC changes - ud_inmbuf freed by svc_freeargs  */
        return(ret);
 ...
 *	11/07/87	ew	added comments
 *      01/15/91        kls     ud_inmbuf freed by svc_freesrgs.
 *                              to support xdrmbuf_getvec function for
 *                              write
 *	
 * External Calls
