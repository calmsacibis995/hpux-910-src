 # $Source: /source/hpux_source/kernel/sys.SWT68K_300/s200io/RCS/pdis.s,v $
 # $Revision: 1.3.84.3 $	$Author: kcs $
 # $State: Exp $   	$Locker:  $
 # $Date: 93/09/17 21:16:43 $
 # HPUX_ID: @(#)pdis.s	55.1		88/12/23
...
escape:
	...
	jmp	recover(%a5)		#| jump to the recover routine
...

 #*****************************************************************************
 #									     *
 #     int pdi_inqueue( desc )				     		     *
 #									     *
 #     struct pdi_global	*desc;						     *
 #									     *
 #----------------------------------------------------------------------------*
 #									     *
...

 #*****************************************************************************
 #									     *
 #     int pdi_control( desc, register, value )				     *
 #									     *
 #     struct pdi_global	*desc;						     *
 #     int register, value;						     *
 #									     *
 #----------------------------------------------------------------------------*
 #									     *
 #     This procedure does a direct control to the pdi device.		     *
...
_pdi_control:
 ...
 #
	...
	mov.l	&10000,%d0		#| set loop counter
...

 ...
 #----------------------------------------------------------------------------*
 #									     *
 #     This function examines the status	of a pdi device.		     *
...
sts11:	bsr	find_RXBUF		#| check	for at least 3 control blocks
	bsr	find_CTRL_AREA		#|
	bsr	TXCTRLBUFFroom		#|
...

 #*****************************************************************************
 #									     *
 #     int pdi_qcontrol(	desc, register,	value )				     *
 #									     *
 #     struct pdi_global	*desc;						     *
 #     int register, value;						     *
 #									     *
 #----------------------------------------------------------------------------*
 #									     *
 #     This procedure queues a control block to the pdi device, to	     *
 #     be executed later.						     *
 #									     *
 #     This routine returns zero	if the control was successful		     *
 #     or a negative number indicating an error:				     *
...

 #*****************************************************************************
 #									     *
 #     int pdi_enter( desc, buf,	nbyte, stat )				     *
 #									     *
 #     struct pdi_global	*desc;						     *
 #     char *buf;							     *
 #     unsigned nbyte;							     *
 ...
 #----------------------------------------------------------------------------*
 #									     *
 #     This routine will	transfer characters from a pdi card.		     *
 #     It will transfer:							     *
...

 ...
 #----------------------------------------------------------------------------*
 #									     *
 #     This routine will	transfer up to nbyte characters	to a pdi card.	     *
 #     The variables are:						     *
 #	  desc	--  pdi	internal variables.				     *
 #	  buf	--  buffer to transfer data.				     *
 #	  nbyte	--  maximum size of buffer.				     *
...

 ...
 #----------------------------------------------------------------------------*
 #									     *
 #     This procedure initializes the pdi cards	global variable	space	     *
 #									     *
 #									     *
 #									     *
 #									     *
...
release_access: 
	mov.b	%d0,SEMAPHORE(%a3)	#Store dont-care into semaphore.
	mov.w	%d7,%sr			#restore old sr
...

 ...
 #									     *
 #     This routine uses	the cards SEMAPHORE to	gain access.		     *
 #									     *
...

 ...
 #	      pointer in the cards Receive data buffer.		     *
...
gc3:	bclr	&0,%d1			#| Fix up the 68000 pointer to be the
	rol.w	&7,%d1			#| cards type of pointer
	bsr	gain_access
	movp.w	%d1,EMPTY(%a6)		#| Remember d1 =	cards EMPTY pointer.
	bsr	release_access
...
RX_stuff_avail: 
 ...
 #						d5 = RXDATABUFF_SIZE
	...
	bsr	release_access		#| were	just checking equality)
	clr.l	%d2
	clr.l	%d0
...
setbit1:	addq.b	&2,%d0			#| Set "ctrl not	empty" bit
 ...
 #					| this control block is	at the head of
	...
	cmpi.b	TERMFIELD(%a0),&255	#| else if its a TERM=255 control block
...
pc3:	bclr	&0,%d1			#| Fix up the 68000 pointer to be the
	rol.w	&7,%d1			#| cards type of pointer
	bsr	gain_access
	movp.w	%d1,DATA_AREA+FILL(%a2)	#| Remember d1 =	cards FILL pointer.
	bsr	release_access
pcdone:	
...

 ...
 #									     *
 #     Upon exit:							     *
 #	      FILL in the cards transmit control buffer is updated.	     *
 #	      a1, d4 and d5 are	left with the values from find_CTRL_AREA.    *
 #	      This bashes d0, d1, d2, d3, d4, d5, a0 and a1.		     *
 #									     *
 #     This routine uses	the cards SEMAPHORE to	gain access.		     *
