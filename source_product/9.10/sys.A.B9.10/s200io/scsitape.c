/* $Source: /source/hpux_source/kernel/sys.SWT68K_300/s200io/RCS/scsitape.c,v $
 * $Revision: 1.10.84.9 $	$Author: paul $
 * $State: Exp $   	$Locker: paul $
 * $Date: 94/11/17 13:18:02 $
 */
 ...
 */
#ifdef PATCHED
static char scsitape_patch_id[] =
"@(#) PATCH 9.0: scsitape.c $Revision: 1.10.84.9 $ $Date: 94/11/17 13:18:02 $";
#endif /* PATCHED */
...
/* Device configuration / control data */
struct st_type {
	char *vendor;                   /* vendor id */
	char *product;                  /* product id */
	u_int media;                    /* general media type */
/*	u_int mt_type;                  /* type for MTIOCGET ioctl */
	u_int block_size;               /* default fixed block size */
	u_int close_marks;              /* filemarks for rew close */
	u_int partitions;               /* number of partitions */
	u_int characteristics;          /* functional characteristics */
};

#define SET_PF_BIT	0x1
#define WRITE_FM	0x2
#define HP		0x4
#define EXABYTE		0x8	/* Exabyte does things a little differently */

struct st_type st_dev_type[] = {
	{ "HP      ", "7980S",  MT_ISSTREAM, 256, 2, 1, WRITE_FM | SET_PF_BIT },

        { "HP      ", "HP35450", MT_ISDDS2,  256, 2, 2, WRITE_FM | SET_PF_BIT },

	{ "HP      ", "HP354",  MT_ISDDS2,   256, 2, 2, WRITE_FM | SET_PF_BIT },

	{ "HP      ", "C1533A", MT_ISDDS2,   256, 2, 2, WRITE_FM | SET_PF_BIT },

	{ "EXABYTE ", "EXB-8505", MT_ISSTREAM, 1024, 1, 1, EXABYTE|SET_PF_BIT },

	{ "EXABYTE ", "EXB-8200", MT_ISSTREAM, 1024, 1, 1, EXABYTE },

	{ "EXABYTE ", "EXB8500C", MT_ISSTREAM, 1024, 1, 1, EXABYTE|SET_PF_BIT },

	{ "EXABYTE ",  NULL,    MT_ISSTREAM,   1024, 1, 1, EXABYTE },

	{ "ARCHIVE ", "VIPER 60", MT_ISSTREAM,  512, 1, 1, 0 },

	{ "ARCHIVE ", "VIPER",   MT_ISSTREAM,   512, 1, 1, 0 },

	{ "WANGTEK ", "5525ES",  MT_ISSTREAM,   512, 1, 1, 0 }
};

/*
 * Generic device type data for devices not found above.
 * The correct tape device type is deterimined by using the SCSI
 * density code. The density types identify medium data format
 * which typically also indicates the general device type.
 */
struct st_type st_dev_generic = {
	 "Generic ", "Tape", MT_ISSTREAM, 256, 2, 1, 0 
};

struct st_type hp_generic = {
	 "Generic ", "Tape", MT_ISSTREAM, 256, 2, 1, SET_PF_BIT 
};
...
/* #define 		B_SCRACH4	NO LONGER USED */
...
struct scsitape_od {
	...
	struct st_type	*type;		/* device type data             */
...
int scsitape_ticks[] = {
	...
	360 * HZ,	/* MTREW    5 */
	180 * HZ,	/* MTOFFL   6 */
	120 * HZ,	/* MTNOP (Generic timeout) 7 */
 	180 * HZ,	/* MTEOD    8 */
	180 * HZ,	/* MTWSS    9 */
	180 * HZ,	/* MTFSS   10 */
	180 * HZ,	/* MTBSS   11 */
	180 * HZ,	/* Mode Sense   12 */
	180 * HZ 	/* Mode Select  13 */
};
...
register struct buf *bp;
{
	...
	dp->dev_queue.b_flags &= ~(IOB_EOT_SEEN | NOT_AT_BOT);
...
register struct buf *bp;
{
	...
	if (count &&
	    (parm == MTFSF || parm == MTFSR || parm == MTEOD || parm == MTFSS))
		bp->b_queue->b_flags |= NOT_AT_BOT;
...
register struct buf *bp;
{
	...
	pf = dp->type->characteristics & SET_PF_BIT ? PF : 0;
...
struct scsitape_od *dp;
{
	...
	if (inq->dev_type != 1)
	    ...
	    /* Look for device in device table */
	    for (i = 0; i < sizeof(st_dev_type) / sizeof(st_dev_type[0]); i++)
	    {
		tp = &st_dev_type[i];

		if (tp->vendor != NULL &&
		    strncmp(tp->vendor, inq->vendor_id,
		            strlen(tp->vendor)))
		{
			continue;
		}
		...
		if (tp->product != NULL &&
		    strncmp(tp->product, inq->product_id,
		            strlen(tp->product)))
		{
			continue;
		}
	...
	dp->type = &st_dev_generic;

done:
	SCSI_TRACE(DIAGs) ("Vendor: %s Product: %s\n", 
		dp->type->vendor, dp->type->product);
	return(0);
}
...
int flag;
{
	...
	if (err=scsitape_control(dp, scsi_inquiry, dp->name, 0, 0, MTNOP))
		goto open_failed;
	else /* Check that it is a sequential access device
	       * Determine tape device type.
	       */
	    if (err=determine_device_type(dp)) {
	...
	if (err=scsitape_control(dp, scsi_test_unit, NULL, 0, 0, MTNOP)) {
	 ...
	 dp->flags |= FLAG_QUIET;
	 if (((struct inquiry *)dp->name)->ansi == 1)
	 ...
	 if (select_req) {
	    ...
	    select_cnt=28;
	    if (err=scsitape_control(dp, scsi_mode_select_cmd,
...
post_compression:
	...
	if ((dev & MT_DENSITY_MASK) == 0x80)	{
		 ...
		 * mode (length == 0), set to appropriate size based on st_type
		 */
		    if (dp->dev_queue.b_flags & NOT_AT_BOT) {
			err = ENXIO;
			goto open_failed;
			}
		    length = dp->type->block_size;
		    mode_data.blk_len[0] = length>>16&0xff;
		    mode_data.blk_len[1] = length>>8&0xff;
		    mode_data.blk_len[2] = length&0xff;
...
int flag;
{
	...
	switch	(order)	{
		...
		case MTIOCTOP:	/* control operation */
			...
			if (op == MTNOP)
				count = 0;
			/* Non-HP devices have difficult time with 
			 * write_0_filemarks.  Use space command instead.
			 */
			if (op == MTNOP && 
				!(dp->type->characteristics & WRITE_FM))
		...
		case MTIOCGET:
			...
			err=scsitape_control(dp, scsi_mode_sense_cmd,
				(char *)&mode_sense_data, 0, 0, 12);
			mtget->mt_type = dp->type->media;
			if (dp->od_resid >= 0)
				mtget->mt_resid = 0;
			else
...
#define	LRS	0x2	/* Long records supported */
			...
			if (dp->type->characteristics & EXABYTE) {
			    unsigned char *ptr = (unsigned char *)&xsense;
			    if (ptr[19]&0x1) {
				mtget->mt_dsreg1 |= BOT,
