...
typedef struct	proc {
	...
	struct b_sema *p_bsema;	/* beta: head of per-process semaphore list */
	struct sema   **p_sema_reaquire;
				/* pointer to semaphore reaquire vector */
...
#else
#define PID_MAXSYS      6  /* Used in dux/getpid.c */
#endif
