...
typedef struct lock
{
	...
	struct lock	*sl_g_link;/* list of all spinlocks */
	struct lock	*sl_link;	/* link of owned locks */
	u_int	sl_lock_caller;	 /* rp of last caller for lock operation */
	u_int	sl_unlock_caller;/* rp of last caller for unlock operation */
#ifdef GPROF
/* Begin SPINLOCK_CONTENTION */
	u_int	sl_cost;	/* FORMERLY lock_cost */
	u_int	sl_spares[6];	/* FORMERLY lock_spares */
/* end SPINLOCK_CONTENTION */
	u_int	sl_holder
#else	/* ! GPROF */
/* Begin SPINLOCK_CONTENTION */
	u_int	sl_cost;	/* FORMERLY lock_cost */
	u_int	sl_spares[7];	/* FORMERLY lock_spares */
/* end SPINLOCK_CONTENTION */
#endif /* ! GPROF */
#else
#ifdef GPROF
...
#ifdef SPINLOCK_DEBUG
#define spinlock(lock)			spinlock_dbg(lock)
#define	spinunlock(lock)		spinunlock_dbg(lock)
#endif /* SPINLOCK_DEBUG */
...
#ifdef  __hp9000s800
#ifdef  SPINLOCK_DEBUG
#define PFAIL_DISABLE(X)        (X = pfail_disable())
#define PFAIL_ENABLE(X)         pfail_enable(X)
#else   /* ! SPINLOCK_DEBUG */
