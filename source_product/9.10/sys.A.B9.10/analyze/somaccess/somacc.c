/* UNISRC_ID: @(#)somaccess.c	4.00	85/10/01  */
...
struct space_dictionary_record *space;    /* temp. storage for space_record */
   ...
   *index = 0;              /* set index */
   ...
   bad = fseek(location->hi32.fptr, SPACE_DIC_LOC, FILE_BEGIN); /*set f. ptr */
   if (bad) {  /* if problem with fseek the som file then exit */
      stat->error = FD_SP_FSEEK_ERR0;   /* set status to fseek error */
      return(BAD);
   }

   bad = fread(&sp_loc, sizeof(sp_loc), ONE, location->hi32.fptr);/* read loc */
   if (bad != ONE)  {
   ...
   bad = fread(&sp_total, sizeof(sp_total), ONE, location->hi32.fptr);/* read to
tal */
   ...
   bad = fseek(location->hi32.fptr, SPACE_STR_LOC, FILE_BEGIN);
   if (bad) {  /* if problem with fseek the som file then exit */
      stat->error = FD_SP_FSEEK_ERR1;   /* set status to fseek error */
      return (BAD);               /* return unsuccessfully */
   }

   bad = fread(&str_loc, sizeof(str_loc), ONE, location->hi32.fptr);/* read loc
*/
   if (bad != ONE)   {
   ...
   bad = fread(&str_size, sizeof(str_size), ONE, location->hi32.fptr);
   ...
   for (i=0; i <= sp_total ; i++)  {
      ...
      bad = fseek(location->hi32.fptr, sp_loc, FILE_BEGIN);
      if (bad) {
      ...
      bad = fread(space, sizeof(*space), ONE, location->hi32.fptr);
      if(bad!=ONE) {
...
composed above, read length first */
         ...
         bad = fseek(location->hi32.fptr, offset, FILE_BEGIN);
         if(bad) {
            stat->error = FD_SP_FSEEK_ERR4;
            break;
         }
         bad = fread(&num_char, sizeof(num_char), ONE, location->hi32.fptr);
         if(bad!=ONE) {
         ...
         bad = fread(the_name, sizeof(*the_name), num_char, location->hi32.fptr);
         if(bad != num_char){
...
et = offset to this space_record (sp_loc).  */
            ...
            space_ptr->hi32.fptr = location->hi32.fptr;
            space_ptr->offset = sp_loc;
...
tal */
   ...
   bad = fseek(location->hi32.fptr, SUBSP_DIC_LOC, FILE_BEGIN); /*set f. ptr */
   if (bad) {  /* if problem with fseek the som file then exit */
      stat->error = FD_SUB_FSEEK_ERR1;   /* set status to fseek error */
      return(BAD);
   }
   bad = fread(&sub_loc, sizeof(sub_loc), ONE, location->hi32.fptr);/* read loc
*/
   ...
   bad = fread(&sub_total, sizeof(sub_total), ONE, location->hi32.fptr);/* read
total */
...
*/
   ...
   bad = fread(&str_size, sizeof(str_size), ONE, location->hi32.fptr);/* read si
ze */
   if (bad != ONE) {
...
ound or run out of subspace_record ( bigger than quantity) */
   ...
   for (i=0; i <= quantity ; i++)  {
      ...
      bad = fseek(location->hi32.fptr, start_sub, FILE_BEGIN);
      if (bad) {
      ...
      bad = fread(subsp, sizeof(*subsp), ONE, location->hi32.fptr);
      if(bad!=ONE)  {
...
et = offset to this subsp_record (sp_loc).  */
            ...
            subsp_ptr->hi32.fptr = location->hi32.fptr;
            subsp_ptr->offset = start_sub;
            break;   /* get out of loop1 */
         }
      }
