...
# $Source: /source/hpux_source/kernel/sys.SWT68K_300/float/RCS/bindec.s,v $
# $Revision: 1.3.84.3 $      $Author: kcs $
# $State: Exp $         $Locker:  $
# $Date: 93/09/17 20:46:48 $
...
#
         ...
         andi.w    &0xe000,%d0      # isolate stag bits
...
un_de_norm:                            
         move.w    (%a0),%d0        
         andi.w    &0x7fff,%d0      # strip sign of normalized exp
         move.l    4(%a0),%d1       
         move.l    8(%a0),%d2       
norm_loop:                            
         sub.w     &1,%d0           
...
a2_str:                             
         ...
         andi.l    &0x7fffffff,fp_scr2(%a6) # create abs(X)
...
a3_cont:                            
         ...
         move.w    &0x3fff,fp_scr2(%a6) # replace exponent with 0x3fff
...
a9_loop:                            
         ...
         move.b    &0x10,0x44(%a7)  # set fcefpte[15] bit
         move.w    &0x0023,0x40(%a7) # load cmdreg1b with mul command
         move.b    &0xfe,0x8(%a7)   # load all 1s to cu savepc
...
a11_st:                             
         ...
         andi.l    &0x00000030,user_fpcr(%a6) # set size to ext, 
#					;block exceptions
...
do_fint:                            
         ...
         bsr.l     sintdo           # sint routine returns int in fp0
...
a14_st:                             
         ...
         subi.l    &0x3ffd,%d0      # sub bias less 2 to make fract
         tst.l     %d0              # check if > 1
...
no_zr:                              
         ...
         addi.l    &0x00000080,%d3  # inc at bit 7
         addx.l    %d1,%d2          # continue inc
         andi.l    &0xffffff80,%d3  # strip off lsb not used by 882
zer_m:                              
...
convrt:                             
         ...
         subi.w    &0x3ffd,%d0      # subtract off bias
...
x_loop_fin:                            
         clr.l     %d1              # put zero in d1 for addx
         addi.l    &0x00000080,%d3  # inc at bit 6
         addx.l    %d1,%d2          # continue inc
         andi.l    &0xffffff80,%d3  # strip off lsb not used by 882
...
a16_st:                             
         ...
         andi.b    &0x0f,fp_scr1(%a6) # clear first nibble of FP_SCR1 
