/* @(#) $Header: sds_parse.c,v 1.1.83.3 93/09/17 20:34:14 kcs Exp $ */
...
/*
 * Certain devices (e.g. Stingray) have a 2k physical block size, so
 * we need to always perform I/O on a 2k boundary.  To be safe, we
 * define IO_BSIZE to be 4k, HP-UX would have a hard time handling
 * devices that had a physical block size larger than our page size
 * (for swapping, etc), so this seems safe.
 */
#define IO_BSIZE	4096
 ...
 * size of the SDS_INFO file.  We then add 2 * IO_BSIZE so that we
 * can round a request down to a IO_BSIZE boundary and up to a
 * IO_BSIZE boundary (necessary for block device access).
 */
#define MAX_LIFIO  \
    ((LIFSIZE > SDS_MAXINFO ? LIFSIZE : SDS_MAXINFO) + (2 * IO_BSIZE))
 ...
 *    file system buffer.
 *
 *    Since we must perform all I/O on IO_BSIZE boundaries, we
 *    truncate the offset to a IO_BSIZE boundary.  We then must
 *    round the size up so that the resulting request ends on a
 *    IO_BSIZE boundary.  Consequently, (user-land only) the static
 *    buffer must have 2*IO_BSIZE more bytes than the largest
...
lif_state_t *lif_state;
{
    ...
    /*
     * Round the offset down to the nearest IO_BSIZE boundary.
     */
    rem = offset % IO_BSIZE;
    if (rem)
    {
    ...
    /*
     * Round the size up to the nearest IO_BSIZE boundary.
     */
    srem = size % IO_BSIZE;
    if (srem)
	size += (IO_BSIZE - srem);
