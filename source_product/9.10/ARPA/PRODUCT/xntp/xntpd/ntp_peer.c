/* $Header: ntp_peer.c,v 1.2.109.3 94/11/09 10:43:08 mike Exp $
...
# define	INIT_PEER_ALLOC		15
				/* initialize space for 15
				   peers */
# define	INC_PEER_ALLOC		5   /* when we run out, add 5
				   more */

/*
 * Miscellaneous statistic counters which may be queried.
 */
U_LONG  peer_timereset;		/* time stat counters were
				   zeroed */
U_LONG  findpeer_calls;		/* number of calls to
				   findpeer */
U_LONG  assocpeer_calls;	/* number of calls to
				   findpeerbyassoc */
U_LONG  peer_allocations;	/* number of allocations
				   from the free list */
U_LONG  peer_demobilizations;	/* number of structs freed
				   to free list */
int     total_peer_structs;	/* number of peer structs
				   in circulation */
