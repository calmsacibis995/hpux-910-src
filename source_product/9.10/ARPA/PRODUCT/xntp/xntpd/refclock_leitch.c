/*
 * $Header: refclock_leitch.c,v 1.2.109.3 94/11/09 10:56:15 mike Exp $
...
# 	if defined(SYSV_TTYS)
     ...
     *
     * The LEITCHCLK support uses a 1-pps signal and level converter
...
# 	if defined(STREAM)
	...
	if (ioctl (fd232, TCGETS, (char    *)&ttyb) < 0)
	    {
	    syslog (LOG_ERR,
		    "leitch_start: ioctl(%s, TCGETS): %m", leitchdev);
	    goto screwed;
	    }
    ttyb.c_iflag = IGNBRK | IGNPAR | ICRNL;
	ttyb.c_oflag = 0;
	...
	if (ioctl (fd232, TCSETS, (char    *)&ttyb) < 0)
	    {
	    syslog (LOG_ERR,
		    "leitch_start: ioctl(%s, TCSETS): %m", leitchdev);
	    goto screwed;
	    }
# 		if defined(LEITCHCLK)
...
# 	endif	/* DEBUG */
	...
	return;			/* The date is return with
				   a trailing newline,
				   discard it. */

    switch (leitch -> state)
        {
	case STATE_IDLE:	/* unexpected, discard and
				   resync */
	    return;
	case STATE_DATE:
...
# 	 endif /* MSUTOTSF(leitch->fudge1, leitch->reftime3.l_uf); */
...
# endif	/* defined (REFCLOCK) && (defined(LEITCH) || defined(LEITCHCLK) || defined(LEITCHPPS)) */
