/* $Header: ntp_proto.c,v 1.2.109.3 94/11/09 10:43:45 mike Exp $
...
s_fp    sys_rootdelay;		/* distance to current sync
				   source */
u_fp    sys_rootdispersion;	/* dispersion of system
				   clock */
U_LONG  sys_refid;		/* reference source for
				   local clock */
l_fp    sys_offset;		/* combined offset from
				   clock_select */
u_fp    sys_maxd;		/* dispersion of selected
				   peer */
l_fp    sys_reftime;		/* time we were last
				   updated */
l_fp    sys_refskew;		/* accumulated skew since
				   last update */
struct peer    *sys_peer;	/* our current peer */
u_char  sys_poll;		/* log2 of desired system
				   poll interval */
LONG    sys_clock;		/* second part of current
				   time */
LONG    sys_lastselect;		/* sys_clock at last
				   synch-dist update */

/*
 * Non-specified system state variables.
 */
int     sys_bclient;		/* we set our time to
				   broadcasts */
U_LONG  sys_bdelay;		/* default delay to use for
				   broadcasting */
int     sys_authenticate;	/* authenticate time used
				   for syncing */

U_LONG  sys_authdelay;		/* ts fraction, time it
				   takes for encrypt() */

/*
 * Statistics counters
 */
U_LONG  sys_stattime;		/* time when we started
				   recording */
U_LONG  sys_badstratum;		/* packets with invalid
				   incoming stratum */
U_LONG  sys_oldversionpkt;	/* old version packets
				   received */
U_LONG  sys_newversionpkt;	/* new version packets
				   received */
U_LONG  sys_unknownversion;	/* don't know version
				   packets */
U_LONG  sys_badlength;		/* packets with bad length 
				*/
U_LONG  sys_processed;		/* packets processed */
U_LONG  sys_badauth;		/* packets dropped because
				   of authorization */
U_LONG  sys_wanderhold;		/* sys_peer held to prevent
				   wandering */
...
# endif	/* DEBUG */
    ...
    peer_timer = 1 << max (min (peer -> ppoll, peer -> hpoll), peer -> minpoll);
...
# endif	/* DEBUG */
	...
	peer = newpeer (&rbufp -> recv_srcadr, rbufp -> dstadr, mymode,
		(int)PKT_VERSION (pkt -> li_vn_mode), NTP_MINDPOLL,
	...
	case MODE_BCLIENT:
		...
		case MODE_ACTIVE:
		    ...
		    peer2 = newpeer (&rbufp -> recv_srcadr,
			    ...
			    (int)PKT_VERSION (pkt -> li_vn_mode),
				...
				*/
{
    ...
    peer -> org = p_xmt;	/* reuse byte-swapped
				   pkt->xmt */
...
# endif	/* DEBUG */
    ...
    t10_uf = p_xmt.l_uf;
    M_SUB (t10_ui, t10_uf, peer -> rec.l_ui, peer -> rec.l_uf);
				/* peer->rec==t0 */
	...
	t23_uf = p_rec.l_uf;
	M_SUB (t23_ui, t23_uf, p_org.l_ui, p_org.l_uf);
				/* pkt->org==t3 */
        }

    /* now have (t2 - t3) and (t0 - t1).  Calculate (ci),
       (di) and (ei) */
    ci.l_ui = t10_ui;
    ci.l_uf = t10_uf;
...
# endif	/* DEBUG */
    ...
    newpoll = max (min (peer -> ppoll, peer -> hpoll), peer -> minpoll);
...
# endif	/* DEBUG */
	...
	if (ord[i] == peer -> filter_nextpt)
	    break;
    for (; i < NTP_SHIFT - 1; i++)  /* i is current, move
				   everything up */
	ord[i] = ord[i + 1];
