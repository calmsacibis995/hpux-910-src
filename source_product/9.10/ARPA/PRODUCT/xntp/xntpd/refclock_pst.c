/*
 * $Header: refclock_pst.c,v 1.2.109.3 94/11/09 11:00:35 mike Exp $
 ...
 * Definitions
 */
# 	define	MAXUNITS	4	/* maximum number of PST
				   units permitted */
# 	define	PSTDEV	"/dev/pst%d"	/* device we open.  %d is
				   unit number */
# 	define	NPSTSAMPS	12	/* take 12 PST samples per
				   minute */
...
# 	define	PSTHSREFID	0x7f7f030a
				/* 127.127.3.10 refid for
				   hi strata */
...
struct pstunit
{
    ...
    u_char  timezone;		/* hour offset to time zone
				   */
    u_char  errors;		/* number of errors
				   detected */
    ...
    u_short     timesincesync;	/* time since radio got
				   sample */
 ...
 */
# 	define	STATE_IDLE	0	/* not doing anything in
				   particular */
 ...
 * Status flags
 */
# 	define	PST_LEAPYEAR	0x1	/* pst clock thinks it is a
				   leap year */
...
# 	define	PST_DOQV	0x20	/* get version, reinit
				   delays */
...
{
    U_LONG  remainder;		/* left over submillisecond
				   remainder */
    char    msbchar;		/* character for high order
				   bits */
    char    lsbchar;		/* character for low order
				   bits */
};
...
# 	if defined(SYSV_TTYS)
     ...
     *
     * The PSTCLK support uses a 1-pps signal and level converter
...
# 	if defined(STREAM)
	...
	if (ioctl (fd232, TCGETS, (char    *)&ttyb) < 0)
	    {
	    syslog (LOG_ERR,
		    "pst_start: ioctl(%s, TCGETS): %m", pstdev);
	    goto screwed;
	    }
    ttyb.c_iflag = IGNBRK | IGNPAR | ICRNL;
	ttyb.c_oflag = 0;
	...
	if (ioctl (fd232, TCSETS, (char    *)&ttyb) < 0)
	    {
	    syslog (LOG_ERR,
		    "pst_start: ioctl(%s, TCSETS): %m", pstdev);
	    goto screwed;
	    }
# 		if defined(PSTCLK)
...
struct recvbuf *rbufp;
{
    ...
    char   *company;
    char    buf[20];
    static char     wwvdelay[6] =
        {
	'S', 'C', '\0', 'S', 'E', '\0'
        };
    static char     wwvhdelay[6] =
        {
	'S', 'H', '\0', 'S', 'G', '\0'
        };
...
# endif	/* defined (REFCLOCK) && (defined(PST) || defined(PSTCLK) || defined(PSTPPS)) */
