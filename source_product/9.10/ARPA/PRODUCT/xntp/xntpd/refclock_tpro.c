/*
 * $Header: refclock_tpro.c,v 1.2.109.3 94/11/09 11:01:23 mike Exp $
...
# 	define	TPRODESCRIPTION	"KSI/Odetics TPRO-S IRIG-B Reader"
				/* who we are */
# 	define	TPROHSREFID	0x7f7f0c0a
				/* 127.127.12.10 refid hi
				   strata */
...
struct tprounit
{
    ...
    struct refclockio   io;	/* given to the I/O handler
				   */
    struct tproval  tprodata;	/* data returned from tpro
				   read */
    ...
    char    bcd[100];		/* bcd timecode for check 
				*/
    /* 
     * Status tallies
     */
...
struct peer    *peer;
{
    ...
    tptr = &tpro -> tprodata;
    if (read (tpro -> io.fd, tptr, sizeof (struct tproval)) < 0)
        {
...
gettstamp (&tpro -> lastrec);
    ...
    record_clock_stats (&(tpro -> peer -> srcadr), tpro -> bcd);
...
# endif	/* defined (REFCLOCK) && defined(TPRO) */
