/* $Header: ntp_request.c,v 1.2.109.3 94/11/09 10:45:21 mike Exp $
...
struct req_proc
{
    ...
    short   needs_auth;		/* true when authentication
				   needed */
    short   sizeofitem;		/* size of request data
				   item */
...
static struct req_proc  xntp_codes[] =
{
	...
	REQ_PEER_LIST_SUM, NOAUTH, 0, peer_list_sum
        ...
        {
	REQ_PEER_INFO, NOAUTH, sizeof (struct info_peer_list),
	    peer_info
        }  ,
        {
	REQ_PEER_STATS, NOAUTH, sizeof (struct info_peer_list),
	    peer_stats
        }  ,
        {
	...
	REQ_TIMER_STATS, NOAUTH, 0, timer_stats
        ...
        {
	REQ_CONFIG, AUTH, sizeof (struct conf_peer),
	    do_conf
        }  ,
        {
	REQ_UNCONFIG, AUTH, sizeof (struct conf_unpeer),
	    do_unconf
        }  ,
        {
	...
	REQ_GET_RESTRICT, NOAUTH, 0, list_restrict
        ...
        {
	REQ_RESADDFLAGS, AUTH, sizeof (struct conf_restrict),
	    do_resaddflags
        }  ,
        {
	REQ_RESSUBFLAGS, AUTH, sizeof (struct conf_restrict),
	    do_ressubflags
        }  ,
        {
	REQ_UNRESTRICT, AUTH, sizeof (struct conf_restrict),
	    do_unrestrict
        }  ,
        {
	REQ_MON_GETLIST, NOAUTH, 0, mon_getlist
        ...
        {
	REQ_RESET_STATS, AUTH, sizeof (struct reset_flags),
	    reset_stats
        }  ,
        {
	REQ_RESET_PEER, AUTH, sizeof (struct conf_unpeer),
	    reset_peer
        }  ,
        {
	...
	REQ_TRAPS, NOAUTH, 0, req_get_traps
        ...
        {
	REQ_ADD_TRAP, AUTH, sizeof (struct conf_trap),
	    req_set_trap
        },
        {
	REQ_CLR_TRAP, AUTH, sizeof (struct conf_trap),
	    req_clr_trap
        },
        {
...
U_LONG  numrequests;		/* number of requests we've
				   received */
U_LONG  numresppkts;		/* number of resp packets
				   sent with data */

U_LONG  errorcounter[INFO_ERR_AUTH + 1];
				/* lazy way to count
				   errors, indexed */
/* by the error code */
...
init_request ()
{
    ...
    numresppkts = 0;
    auth_timereset = 0;
    info_auth_keyid = 0;	/* by default, can't do
				   this */
