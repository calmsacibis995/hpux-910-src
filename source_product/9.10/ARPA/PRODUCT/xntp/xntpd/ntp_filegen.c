/*
 * ntp_filegen.c
 * $Header: ntp_filegen.c,v 1.2.109.3 94/11/09 10:33:26 mike Exp $
 *
 ...
 * other constants
 */
# define FGEN_AGE_SECS   (24*60*60)
				/* life time of FILEGEN_AGE
				   in seconds */
...
U_LONG  newid;
{
	...
	case FILEGEN_DAY:
	    /* You can argue here in favor of using MJD,
	       but * I would assume it to be easier for
	       humans to interpret dates * in a format they
	       are used to in everyday life. */
	    caljulian (newid, &cal);
...
# endif	/* DEBUG */
    ...
    if (fp == NULL)
        {
	/* open failed -- keep previous state * * If the
	   file was open before keep the previous
	   generation. * This will cause output to end up
	   in the 'wrong' file, * but I think this is still
	   better than loosing output * * ignore errors due
...
U_LONG  now;
{
	...
	case FILEGEN_WEEK:
	    /* Would be nice to have a calweekstart()
	       routine */
	    /* so just use a hack ... */
	    /* just round time to integral 7 days period
	       for actual year  */
