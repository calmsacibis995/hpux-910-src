/*
 * $Header: refclock_goes.c,v 1.2.109.3 94/11/09 10:53:03 mike Exp $
 ...
 */
# 	define	GOESMAXDISPERSE	(FP_SECOND>>1)	/* max error for
				   synchronized clock (0.5
				   s as an u_fp) */
# 	define	GOESSKEWFACTOR	17	/* skew factor (for about
				   32 ppm) */
# 	define	GOESPRECISION	(-10)	/* precision assumed (about
				   1 ms) */
# 	define	GOESREFID	"GOES"	/* reference id */
# 	define	GOESDESCRIPTION	"Kinemetrics GOES Receiver"
				/* who we are */
# 	define	GOESHSREFID	0x7f7f050a  /* 127.127.5.10 refid hi
				   strata */
...
# 	define	LENGOES2	21	/* format 2 timecode length
				   */
# 	define FMTGOESU	0	/* unknown format timecode
				   id */
...
# 	if defined(SYSV_TTYS)
     ...
     *
     * The GOESCLK support uses a 1-pps signal and level converter
...
# 	if defined(STREAM)
	...
	if (ioctl (fd232, TCGETS, (char    *)&ttyb) < 0)
	    {
	    syslog (LOG_ERR,
		    "goes_start: ioctl(%s, TCGETS): %m", goesdev);
	    goto screwed;
	    }
    ttyb.c_iflag = IGNBRK | IGNPAR | ICRNL;
	ttyb.c_oflag = 0;
	...
	if (ioctl (fd232, TCSETS, (char    *)&ttyb) < 0)
	    {
	    syslog (LOG_ERR,
		    "goes_start: ioctl(%s, TCSETS): %m", goesdev);
	    goto screwed;
	    }
# 		if defined(GOESCLK)
...
# endif	/* defined (REFCLOCK) && (defined(GOES) || defined(GOESCLK) || defined(GOESPPS)) */
