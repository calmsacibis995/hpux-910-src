/* $Header: ntp_config.c,v 1.2.109.6 94/11/09 10:31:34 mike Exp $
 ...
 */
# ifndef	CONFIG_FILE
# 	ifdef	__bsdi__		/* HACK to avoid BSDI
				   configure bug */
# 		define	CONFIG_FILE	"/usr/local/etc/xntp.conf"
# 	else	/* ! __bsdi__		/* HACK to avoid BSDI */
# 		ifdef CONFINS
# 			define CONFIG_FILE	CONFINS
# 		else	/* ! CONFINS */
# 			define CONFIG_FILE	"/usr/contrib/etc/ntp.conf"
# 		endif	/* CONFINS */
# 	endif				/* __bsdi__ */
# endif				/* CONFIG_FILE */
...
# define	MAXLINE		1024	/* maximum length of line 
				*/
# define	MAXFILENAME	128	/* maximum length of a file
				   name (alloca()?) */
 ...
 */
static FILE    *res_fp;
static char     res_file[20];	/* enough for
				   /tmp/xntpXXXXXX\0 */
...
# ifdef DEBUG
    ...
    optind = 0;			/* reset optind to restart
				   getopt */
 ...
 */
	...
	register LONG   i;
	register char **ap;
	/* 1 progname + 5 -d's + 1 -r + keyid + keyfile +
	   tempfile + 1 */
	char   *argv[15];
	char    numbuf[15];
	/* 
 ...
 */
	...
	extern char    *req_file;   /* name of the file with
				   configuration info */
	extern U_LONG   info_auth_keyid;
	...
	/* save keyid so we will accept config requests
	   with it */
	info_auth_keyid = req_keyid;

	req_file = res_file;	/* set up pointer to res
				   file */
	...
	if (i == 0)
	    {
	    /* this used to close everything * I don't
