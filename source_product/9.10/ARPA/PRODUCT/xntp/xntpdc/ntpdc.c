/*
 * $Header: ntpdc.c,v 1.2.109.2 94/11/03 15:48:24 mike Exp $
 */
 ...
 */
static int  interactive = 0;	/* set to 1 when we should
				   prompt */
static char    *prompt = "xntpdc> ";
				/* prompt to ask him about 
				*/
 ...
 */
static struct timeval   tvout =
{
    DEFTIMEOUT, 0
};				/* time out for reads */
static struct timeval   tvsout =
{
    DEFSTIMEOUT, 0
};				/* secondary time out */
static l_fp     delay_time;	/* delay time */
static char     currenthost[LENHOSTNAME];
				/* current host name */
static struct sockaddr_in   hostaddr =
{
    0
};				/* host address */
static int  showhostnames = 1;	/* show host names by
				   default */
...
# 	ifdef SO_RCVBUF
        {
	int     rbufsize = INITDATASIZE + 2048;
				/* 2K for slop */
...
char  **rdata;
{
    ...
    *rsize = 0;
    ...
    numrecv = 0;
    *rdata = datap = pktdata;
    lastseq = 999;		/* too big to be a sequence
				   number */
