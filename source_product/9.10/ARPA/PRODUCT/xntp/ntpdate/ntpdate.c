# ifndef lint
static char rcsid[] = "$Header: ntpdate.c,v 1.2.109.3 94/11/08 13:32:33 mike Exp $";
# endif	/* not lint */
...
# define PEER_MAXDISP    (64*FP_SECOND)
				/* maximum dispersion (fp
				   64) */
# define NTP_INFIN       15	/* max stratum, infinity a
				   la Bellman-Ford */
# define NTP_MAXWGT      (8*FP_SECOND)
				/* maximum select weight 8
				   seconds */
# define NTP_MAXLIST     5	/* maximum select list size
				   */
# define PEER_SHIFT      8	/* 8 suitable for crystal
				   time base */
 ...
 */
int     sys_samples = DEFSAMPLES;
				/* number of samples/server
				   */
U_LONG  sys_timeout = DEFTIMEOUT;   /* timeout time, in
				   TIMER_HZ units */
struct server **sys_servers;	/* the server list */
int     sys_numservers = 0;	/* number of servers to
				   poll */
int     sys_maxservers = 0;	/* max number of servers to
				   deal with */
int     sys_authenticate = 0;	/* true when authenticating
				   */
U_LONG  sys_authkey = 0;	/* set to authentication
				   key in use */
U_LONG  sys_authdelay = 0;	/* authentication delay */
int     sys_version = NTP_VERSION;
...
static RETSIGTYPE   input_handler P ((int));

/* XXX ELIMINATE: merge BIG slew into adj_systime in lib/systime.c */
# ifdef	ELIMINATE		/* XXX FIXME: ultrix cc has
				   redeclaration */
static int  adj_systime P ((l_fp *));
# endif	/* ELIMINATE		/* XXX FIXME: ultrix cc has */
...
char   *argv[];
{
     ...
     */
    while ((c = getopt (argc, argv, "a:bde:k:o:p:qst:v")) != EOF)
	switch (c)
	    {
	    case 'a':
		c = atoi (optarg);
...
struct recvbuf *rbufp;
{
    ...
    register struct server *server;
    register s_fp   di;
    register U_LONG     t10_ui,
                        t10_uf;
    register U_LONG     t23_ui,
                        t23_uf;
    l_fp    org;
    l_fp    rec;
    l_fp    ci;
...
# endif	/* notdef */
    ...
    t23_uf = rec.l_uf;
    M_SUB (t23_ui, t23_uf, org.l_ui, org.l_uf);
				/* pkt->org == t3 */
...
clock_adjust ()
{
    ...
    if (dostep)
        {
	if (simple_query || step_systime (&server -> offset))
	    {
        ...
        {
	if (simple_query || adj_systime (&server -> offset))
	    {
...
/* XXX ELIMINATE adj_systime: similar to ntpdate.c, ntp_unixclock.c */
/*
...
/* XXX ELIMINATE getnetnum: similar to ntp_config.c, gizconf.c, ntpdc.c, ntpq.c */
/*
