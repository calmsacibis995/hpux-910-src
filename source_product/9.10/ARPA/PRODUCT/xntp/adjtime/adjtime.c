...
# ifndef lint
static char     RCSid[] = "$Header: adjtime.c,v 1.2.109.5 94/10/28 15:44:25 mike Exp $";
# endif				/* not lint */
...
# include <signal.h>
# include "adjtime.h"
# undef _XPG2			/* Make SURE this sym is
				   undefined */
...
/* Make the following routines visible iff adjtime() isn't a syscall */
# ifndef SYS_ADJTIME
...
register struct timeval    *olddelta;
{
    ...
    /* If they are giving us seconds, ignore up to current
       threshold saved */
    if (delta -> tv_sec)
        {
	saveup = 0L;
