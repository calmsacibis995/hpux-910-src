...
# ifndef lint
static char     RCSid[] = "$Header: adjtimed.c,v 1.2.109.4 94/10/28 15:52:34 mike Exp $";
# endif				/* not lint */
...
# endif				/* TIOCNOTTY */
    ...
    if (verbose)
	printf ("adjtimed: started (rate %.2f%%)\n", RATE * 100.0);
 ...
 */
	...
	if (verbose > 2)
	    printf ("adjtimed: delta is too small: %dus\n", dt);
	if (sysdebug > 2)
	    syslog (LOG_INFO, "delta is too small: %dus", dt);
	return (1);
        }
    ...
    if (verbose > 2)
	printf ("adjtimed: leftover %dus\n", leftover);
    if (sysdebug > 2)
	syslog (LOG_INFO, "leftover %dus", leftover);
    rate = dt * RATE;
...
ResetClockRate ()
{
    ...
    if (verbose > 2)
	puts ("adjtimed: resetting the clock");
    if (sysdebug > 2)
	syslog (LOG_INFO, "resetting the clock");
