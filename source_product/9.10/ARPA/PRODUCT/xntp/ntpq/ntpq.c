/* $Header: ntpq.c,v 1.2.109.5 94/12/09 17:47:31 mike Exp $
 ...
 */
int     interactive = 0;	/* set to 1 when we should
				   prompt */
...
# define	RF	10		/* refid (sometimes string,
				   sometimes not) */
...
struct ctl_var  sys_var[] =
{
	...
	CS_LEAP, LP, "leap"
        ...
        {
	CS_STRATUM, UI, "stratum"
        }  ,			/* 2 */
        {
	CS_PRECISION, IN, "precision"
        }  ,			/* 3 */
        {
	CS_ROOTDELAY, FU, "rootdelay"
        }  ,			/* 4 */
        {
	CS_ROOTDISPERSION, FU, "rootdispersion"
        }  ,			/* 5 */
        {
	CS_REFID, RF, "refid"
        }  ,			/* 6 */
        {
	CS_REFTIME, TS, "reftime"
        }  ,			/* 7 */
        {
	CS_POLL, UI, "poll"
        }  ,			/* 8 */
	...
	CS_DRIFT, FS, "freq"
        ...
        {
	CS_COMPLIANCE, UI, "compliance"
        }  ,			/* 12 */
        {
	CS_CLOCK, TS, "clock"
        }  ,			/* 13 */
        {
	CS_LEAPIND, LP, "leapindicator"
        }  ,			/* 14 */
        {
	CS_LEAPWARNING, LP, "leapwarning"
        }  ,			/* 15 */
        {
	CS_PROCESSOR, ST, "processor"
        }  ,			/* 16 */
        {
	CS_SYSTEM, ST, "system"
        }  ,			/* 17 */
...
struct ctl_var  peer_var[] =
{
	...
	CP_CONFIG, UI, "config"
        ...
        {
	CP_AUTHENABLE, UI, "authenable"
        }  ,			/* 2 */
        {
	CP_AUTHENTIC, UI, "authentic"
        }  ,			/* 3 */
        {
	CP_SRCADR, HA, "srcadr"
        }  ,			/* 4 */
        {
	CP_SRCPORT, UI, "srcport"
        }  ,			/* 5 */
        {
	CP_DSTADR, NA, "dstadr"
        }  ,			/* 6 */
        {
	CP_DSTPORT, UI, "dstport"
        }  ,			/* 7 */
        {
	CP_LEAP, LP, "leap"
        }  ,			/* 8 */
	...
	CP_HPOLL, UI, "hpoll"
        ...
        {
	CP_PRECISION, IN, "precision"
        }  ,			/* 13 */
        {
	CP_ROOTDELAY, FS, "rootdelay"
        }  ,			/* 14 */
        {
	CP_ROOTDISPERSION, FU, "rootdispersion"
        }  ,			/* 15 */
        {
	CP_REFID, RF, "refid"
        }  ,			/* 16 */
        {
	CP_REFTIME, TS, "reftime"
        }  ,			/* 17 */
        {
	CP_ORG, TS, "org"
        }  ,			/* 18 */
	...
	CP_OFFSET, AR, "offset"
        ...
        {
	CP_DISPERSION, FU, "dispersion"
        }  ,			/* 26 */
        {
	CP_KEYID, UI, "keyid"
        }  ,			/* 27 */
        {
	CP_FILTDELAY, AR, "filtdelay"
        }  ,			/* 28 */
        {
	CP_FILTOFFSET, AR, "filtoffset"
        }  ,			/* 29 */
        {
	CP_PMODE, ST, "pmode"
        }  ,			/* 30 */
        {
	CP_RECEIVED, UI, "received"
        }  ,			/* 31 */
        {
	CP_SENT, UI, "sent"
        }  ,			/* 32 */
        {
	CP_FILTERROR, AR, "filterror"
        }  ,			/* 33 */
        {
	CP_FLASH, ST, "flash"
        }  ,			/* 34 */
     ...
     */
        {
	CP_SRCADR, HA, "peeraddr"
        }  ,			/* 4 */
        {
	CP_SRCPORT, UI, "peerport"
        }  ,			/* 5 */
        {
	CP_PPOLL, UI, "peerpoll"
        }  ,			/* 11 */
        {
	CP_HPOLL, UI, "hostpoll"
        }  ,			/* 12 */
        {
	0, EOV, ""
        }
...
struct ctl_var  clock_var[] =
{
	...
	CC_TYPE, UI, "type"
        ...
        {
	CC_TIMECODE, ST, "timecode"
        }  ,			/* 2 */
        {
	CC_POLL, UI, "poll"
        }  ,			/* 3 */
        {
	CC_NOREPLY, UI, "noreply"
        }  ,			/* 4 */
        {
	CC_BADFORMAT, UI, "badformat"
        }  ,			/* 5 */
        {
	CC_BADDATA, UI, "baddata"
        }  ,			/* 6 */
        {
	CC_FUDGETIME1, FL, "fudgetime1"
        }  ,			/* 7 */
        {
	CC_FUDGETIME2, FL, "fudgetime2"
        }  ,			/* 8 */
        {
	CC_FUDGEVAL1, IN, "fudgeval1"
        }  ,			/* 9 */
        {
	CC_FUDGEVAL2, IN, "fudgeval2"
        }  ,			/* 10 */
        {
	CC_FLAGS, UI, "flags"
        }  ,			/* 11 */
...
struct xcmd     builtins[] =
{
    ...
    /* { "poll",	ntp_poll,	{ OPT|UINT,
       OPT|STR, NO, NO }, { "n", "verbose", "", "" }, "poll
       an NTP server in client mode `n' times" }, */
        {
	"passwd", passwd,
	    {
	    NO, NO, NO, NO
...
# define	DEFSTIMEOUT	(2)	/* 2 second time out after
				   first */
# define	DEFDELAY	0x51EB852   /* 20 milliseconds, l_fp
				   fraction */
# define	DEFHOST		"localhost" /* default host name */
# define	LENHOSTNAME	256	/* host name is 256
				   characters LONG */
# define	MAXCMDS		100	/* maximum commands on cmd
				   line */
# define	MAXHOSTS	100	/* maximum hosts on cmd
				   line */
# define	MAXLINE		512	/* maximum line length */
# define	MAXTOKENS	(1+MAXARGS+2)
				/* maximum number of usable
				   tokens */
# define	MAXVARLEN	256	/* maximum length of a
				   variable name */
# define	MAXVALLEN	256	/* maximum length of a
				   variable value */
# define	MAXOUTLINE	72	/* maximum length of an
				   output line */
...
int     showhostnames = 1;	/* show host names by
				   default */
 ...
 * alignment.
 */
# define	MAXFRAGS	24	/* maximum number of
				   fragments */
...
int     numassoc = 0;		/* number of cached
				   associations */
...
{
    if (havehost)
	(void)close (sockfd);	/* cleanliness next to
				   godliness */
    exit (0);
}
 ...
 */
int     out_chars;		/* number of characters
				   output */
int     out_linecount;		/* number of characters
				   output on this line */
