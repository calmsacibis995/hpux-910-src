/*
 *  $Header: authspeed.c,v 1.2.109.4 95/01/12 16:57:21 mike Exp $
 * authspeed - figure out how LONG it takes to do an NTP encryption
 */

#if defined(HPUX) || defined(_AUX_SOURCE) || defined(SOLARIS)
#define FAKE_RUSAGE
#endif	/* HPUX) || defined(_AUX_SOURCE) || defined(SOLARIS) */
...
char   *argv[];
{
    ...
    progname = argv[0];
    loops = DEFLOOPS;
    while ((c = getopt (argc, argv, "mn:")) != EOF)
	switch (c)
	    {
	    case 'c':
		totalcost++;
	...
	printf ("a single authenticated packet.\n");
        ...
        {
	/* 	printf("authdelay in the configuration
	   file\n"); printf("real authentication delay:
	   %.6f\n", rtime/(double)loops);
	   printf("authentication delay in CPU time:
	   %.6f\n", vtime/(double)loops); printf("\nThe CPU
	   delay is probably the best bet for\n");
