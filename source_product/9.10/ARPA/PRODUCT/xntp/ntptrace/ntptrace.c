/* $Header: ntptrace.c,v 1.2.109.2 94/11/03 14:33:38 mike Exp $
...
int     nonames = 0;		/* if set, don't print
				   hostnames */
 ...
 */
int     sys_retries = 5;	/* # of retry attempts per
				   server */
...
int     sys_numservers = 0;	/* number of servers to
				   poll */
int     sys_maxservers = NTP_MAXSTRATUM + 1;
				/* max number of servers to
				   deal with */
...
char   *argv[];
{
     ...
     */
    while ((c = getopt (argc, argv, "do:nr:t:v")) != EOF)
	switch (c)
	    {
	    case 'd':
		++debug;
