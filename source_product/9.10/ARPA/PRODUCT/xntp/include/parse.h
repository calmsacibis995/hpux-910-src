/*
 * @(#)parse.h: $Revision: 1.2.109.1 $ $Date: 94/10/26 16:55:24 $
 * $Locker:  $
 */

/*
 * /src/NTP/REPOSITORY/v3/include/parse.h,v 3.2 1993/07/09 11:37:05 kardel Exp
 *
 * parse.h,v 3.2 1993/07/09 11:37:05 kardel Exp
 *
...
#define __PARSE_H__
  ...
  static char parsehrcsid[]="parse.h,v 3.2 1993/07/09 11:37:05 kardel Exp FAU";
#endif
...
struct parsetime
{
  ...
  struct timeval parse_time;	/* PARSE timestamp */
  TIMESTAMP	 parse_stime;	/* telegram sample timestamp */
  struct timeval parse_ptime;	/* PPS time stamp */
...
struct parse			/* parse module local data */
{
   ...
   */
  TIMESTAMP      parse_lastchar;	/* time stamp of last received character */

  /*
   * time code input buffer (from RS232 or PPS)
   */
  int            parse_index;	/* current buffer index */
...
extern int  parse_ioread(/* parse_t *parseio, char ch, TIMESTAMP *ctime */);
...
extern void syn_simple P((parse_t *, TIMESTAMP *, struct format *, unsigned LONG));
extern unsigned LONG pps_simple P((parse_t *, int status, struct timeval *));
#endif
