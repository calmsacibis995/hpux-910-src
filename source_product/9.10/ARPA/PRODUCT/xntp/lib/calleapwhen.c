/* $Header: calleapwhen.c,v 1.2.109.2 94/10/28 17:13:34 mike Exp $
...
LONG    calleaptab[10] =
{
           -(JAN + FEBLEAP) * SECSPERDAY,
    /* leap previous to cycle */
            (MAR + APR + MAY + JUN) * SECSPERDAY,
    /* end of June */
            (MAR + APR + MAY + JUN + JUL + AUG + SEP + OCT + NOV + DEC) * SECSPERDAY,
    /* end of Dec */
           ...
           *SECSPERDAY + 3 * SECSPERYEAR,
    /* next after current cycle */
};
