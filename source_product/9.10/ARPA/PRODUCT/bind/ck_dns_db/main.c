...
** Hewlett Packard
**
** usage: ck_dns_db [-v] [-F] [-o origin] -f dns_file [-o origin] \
**                                                      [-f dns_file ...]
**
...
static char rcsid[] = "@(#) $Header: main.c,v 1.8.109.1 91/11/21 11:39:16 kcs Exp $";
...
usage()
{
	fprintf(stderr, "ck_dns_db [-v] [-F] [-o origin] -f db_file [-o origin] [-f db_file]\n");
	exit(1);
}
	...
	while ((c = getopt(argc, argv, "Ff:o:v")) != EOF)
		switch(c){
			case 'F':
					...
					break;
			case 'f':
					errs +=db_load(optarg,origin,&zones[1]);
					break;
			case 'o':
					origin = optarg;
					break;
