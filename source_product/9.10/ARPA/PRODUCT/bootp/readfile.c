...
#ifndef lint
static char rcsid[] = "$Header: readfile.c,v 1.8.109.2 93/10/29 10:30:14 randyc Exp $";
#endif
...
char *src;
{
	...
	switch (retval) {
		...
		report(LOG_ERR, "bad vendor magic cookie for host \"%s\"\n",
				host->hostname->string);
		return -1;
	    case E_BAD_BROADADDR:
		report(LOG_ERR, "bad reply broadcast address for host \"%s\"\n",
				host->hostname->string);
		return -1;
	    case E_END_OF_ENTRY:
	    default:
...
struct host *hp;
{
	...
	case SYM_BOOTSIZE:
        ...
        case SYM_BROADCAST_REPLY:
	    switch (optype) {
		case OP_ADDITION:
		    if (!strncmp(*symbol, "auto", 4)) {
			hp->broadaddr.s_addr = INADDR_ANY;
			hp->flags.broadcast_reply = TRUE;
			hp->flags.broadcast_auto = TRUE;
		    } else if (prs_inetaddr(symbol, &value) < 0) {
			return E_BAD_BROADADDR;
		    } else {
			hp->broadaddr.s_addr = value;
			hp->flags.broadcast_reply = TRUE;
			hp->flags.broadcast_auto = FALSE;
		    }
		    break;
		case OP_DELETION:
		    hp->flags.broadcast_reply = FALSE;
		    break;
		case OP_BOOLEAN:
		    hp->broadaddr.s_addr = INADDR_ANY;
		    hp->flags.broadcast_reply = TRUE;
 ...
 * According RFC1123, the restriction on the first character is relaxed
...
{
    do {
	if (!isalnum(*hostname++)) {	/* First character must be a 
	                                   alphanumeric */
	    return FALSE;
	}
#ifdef hpux
	/* Added the character "_" as a leagal character in the hostname, */
	/* since HPUX supports "_" as a leagal character in its hostname. */
...
char **src;
{
	...
	if (!hp->flags.broadcast_reply) {
	    if (hp->flags.broadcast_reply = hp2->flags.broadcast_reply) {
		hp->flags.broadcast_auto = hp2->flags.broadcast_auto;
		hp->broadaddr = hp2->broadaddr;
	    }
	}
    }
}
