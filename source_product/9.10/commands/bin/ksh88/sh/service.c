...
int flag;
{
	...
	while(1) 
	{
		...
		switch(count++)
		{
			...
			case 4:
			{
				 ...
				 *  Try getcwd(3).  getcwd(3) calls 
				 *  /bin/pwd if it needs to, so we don't
				 *  call it here.  Error message is take
				 *  care of in builtin.c:sh_builtin().
				 *  if *dfault != '/';
				 */
				char buff[PATH_MAX+1];
				extern char *getcwd();
	...
	/* DSDe408239: 
	 * 	if cp is the same as the value of PWDNOD, don't 
	 *	bother to put the same value into PWDNOD.
	 */
	if (strcmp(cp,PWDNOD->value.namval))
...
@*/
{
	...
	if(flag)
	{
		...
		while((sh.lastpath=path_absolute(name,1))==0 && pruned)
...
char	*path_absolute(name, exec_flag)
register const char *name; 
int exec_flag;
/*@
	assume name!=NULL;
...
/* The following does not work because of bad parameters to access()  call
 * has been fixed by changing (S_IRUSR...) to just R_OK ---- POSIX3 development
 *			f = sh_access(stak_word(),(S_IRUSR|S_IRGRP|S_IROTH));
 */
			f = sh_access(stak_word(),R_OK);
	}
#ifdef VPIX
	...
	if(*stak_word()!='/')
		pwd_path_join(path_pwd(1),(char*)stak_end(sh.staktop));
...
@*/
{
	...
	/* UCSqm00665: ksh sometimes ignores absolute path-names
	p = path_relative(p);
	*/
#ifdef XENIX
	if(ex_xenix(p))
	...
	openok:
		...
		case EINVAL:
			/*  trying to run S800 executable on S300 or v-v */
			sh_fail(p, e_wrongbinary);
...
@*/
{
	...
	if(com)
	{
		...
		execflg &= ~EXECPR;
/*
 * The st structure is saved before calling the function and is 
 * set to previous values after the function call.
 *
 * The st.cmdadr keeps the name of the function if it is ksh
 * else it continues with the name of the script as required by
 * the POSIX.2 Draft 11.2
 * 
 * com array is the array of char pointers pointing at the function
 * name and the arguments to the function.
 */
