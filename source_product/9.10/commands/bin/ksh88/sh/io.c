...
char *buf;
{
	...
	if(!fp)
	{
		...
		fp->flag = 0;
	/*
	 *  Invalidate savefd if we are modifying
	 *  the file descriptor that we have info about
	 */
	if (savefd == fd)
		savefd = -1;
	fp->fdes = fd;
...
register int f2;
{
	...
	{
		/*
		 *  Need to invalidated the saved tty
		 *  info if one of the fd's is the one
		 *  we saved.
		 */
		if (savefd == f2 || savefd == f1)
			savefd = -1;
		if(fs==0)
...
#ifdef KEYSHELL
        if (1)
                n = KeyshellRead(fno, fp->base, MAXLINE-2);
        else
#else
#   ifdef ESH
...
void io_sync()
{
	...
	if(fp)
	{
		...
		if(count=fp->last-fp->ptr)
		    ret_val = lseek(filenum(fp),-((off_t)count),SEEK_CUR);
		
		/*
		 *  We don't want to erase any characters
		 *  we have read into the input buffer if
		 *  we failed on the lseek (a socket or pipe)
		 */
		if(ret_val != -1)
		{
...
struct ionod	*iop;
{
	...
	for(;iop;iop=iop->ionxt)
	{
		...
		if(*ion)
		{
			...
			{
				/* don't allow <> from restricted shells */
				if(is_option(RSHFLG))
