...
int mode;
{
	...
	struct argnod *ap;
        char *stakdata;
        int staksize;
        if (staksize = stak_offset()) {
          stakdata = stak_fix();
        }
        staksav = stak_word();
	comptr = (struct comnod*)stak_alloc(sizeof(struct comnod));
	...
	stak_reset(staksav);
        ...
        if (staksize) {
          stak_begin();
          memcpy(stak_word(), stakdata, staksize);
          stak_set(staksize);
        }
#endif /* STAK_FIX */
	opt_flags = savflags;
