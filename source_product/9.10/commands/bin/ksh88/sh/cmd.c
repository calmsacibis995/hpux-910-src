	...
	switch(sh.wdval)
	{
		...
		case ';':
			if(e=sh_parse(sym,flg|MTFLG))
				i=makelist(TLST, i, e);
			/*
			 *  The ksh88 check for spurious null commands
			 *  has been taken out for backward compatbility
			 *  with ksh-i.  With this check left in, the 
			 *  following constructs, legal in ksh-i, are 
			 *  syntax errors in ksh88:
			 *
			 *          ;
			 *	    cmd ; ; ; 
			 *          function x
...
#ifdef POSIX2
	/*
         *  Check for the POSIX negation operator "!"
	 */
...
#ifdef HP_BUILTIN
						if(!bltin && hp_builtin)
						   bltin=sh_lookup(argp->argval,tab_hp_builtins);
#endif /* HP_BUILTIN */						
						if(bltin&SYSDECLARE)
							key_on = KEYFLG;
					}
...
#ifdef POSIX3
/* 
 * The library routine wordexp() uses Posix Shell itself for
 * work expansion. Hence the following change in the error code.
 * The code may not work for the cases where sh.wdval does not
 * store the last token of the input
 * 
...
static void chkflags(fexp)
{
	...
	{
		register int flag = 0;
		char nquote = 0;	/* whether or not we are inside "..." */
		char *sp=argp->argval;
		while(c= *sp++)
		{
			if(c==ESCAPE)
