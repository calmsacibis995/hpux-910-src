	...
	if(argscan!=argi)
	{
		...
		*argscan = sep;
		...
		if(sep == '=')
		{
			...
			/*
			 *  restricted shells can't set SHELL
			 */
			if(n==SHELLNOD)
			{
				/*
				 ...
				 *  ksh, we don't do the ksh88 check
				 *  against "*r*sh" but just check for
				 *  an 'r' in the  basename.
				 * Well, this will be changed in 10.0
				 * time frame.
				 * UCSqm00385.
				 */
				sim = path_basename(argscan);
				if (strmatch(sim,"rksh"))
					rsflag = 0;
			}
			if(initenv)
			{
			...
			{
				/* 
				 *  turn on vi or emacs option if editor 
				 *  name matches *vi or *macs
				 */
			...
			{
			        /*
				 *  LC_* override the LANG setting.  We have
				 *  to do a setlocale on lang and then override
				 *  for the rest of the defined LC_* env vars.
				 *  Theses values are only for the shell's use.
				 *  all forked programs will get started and do
				 *  their own setlocale()'s.
				 *
				 *  Currently the shell only uses LC_CTYPE (for 
				 *  the isupper(),... routines) and LC_COLLATE
				 *  for equiv classes etc.).  If any others are 
				 *  needed, they will also have to be checked here.
				 *  and will also need their own if statement below.
				 */
				setlocale(LC_ALL,argscan);
				if(!isnull(LCTYPENOD))
...
#   endif /* MULTIBYTE */
			...
			else if(n==LCCOLLATENOD)
				setlocale(LC_COLLATE,argscan);
#endif /* _locale_  */
#ifdef MULTIBYTE
			...
			{
			 	/*
				 *  If we reset the index for getopts
				 *  we also have to reset some other internal
				 *  counters
				 */
			   	opt_char = 0;
			}
...
int	env_init()
{
	...
	initenv = 0;
	/*
	 *  Do the path_pwd(0) in all cases.  There are bugs
...
#ifdef SIGWINCH
	/*
	 *  Need to make sure that we have noticed any change
	 *  in the window size before we generate the new
	 *  environment.
	 */
	window_change();
#endif /* SIGWINCH */
#ifndef POSIX2
	/* dont do special things to _ variable for POSIX shell */
...
#ifdef HP_BUILTIN
		/*
		 *  Check to see if it is an HP special
		 *  builtin command.
		 */
		else if(hp_builtin  &&
			sh_lookup(a1,tab_hp_builtins))
		{
		        if(flag)
			   a1 = is_builtin;
		}
...
#ifdef POSIX2
		/*
		 *  We put the goto search before dofunction
		 *  so that we skip the function stuff if we 
		 *  come in through the if(1).  This is because
		 *  we have already done the funtion lookup by the
		 *  time we hit if(1).
		 */
		goto search;
		dofunction:
			...
			/*  check for unset function and skip */
			if(isnull(np) && !nam_istype(np, N_FUNCTION))
				goto search;
		}
		else
		{
...
#ifdef POSIX3
/*
 * Remove the nodes in tree <root> 
 */

void env_remove(root)
struct Amemory *root;
{
	register char **argv;
	register struct namnod *np;
	register int namec;
	struct namnod *onp = 0;
	namec = gscan_some((void(*)())0,root,0,0);
	argv = argnam  = (char**)stak_alloc((namec+1)*BYTESPERWORD);
	namec = gscan_some(pushnid,root,0,0);
#ifdef NLS
	gsort(argv,namec,strcoll);
#else
	gsort(argv,namec,strcmp);
