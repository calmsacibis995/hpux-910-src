...
register char *v[];
{
	...
	/* UCSqm00290:
	 * ksh dumps core if appln uses reserved file descs
	 */
#ifdef KSHELL
	close(INIO);
	close(TMPIO);
...
#ifdef NLS
	/*
	 *  This must go before the nam_init() because
	 *  if the user has specified a LANG, nam_init()
	 *  will set it, and we don't want to override it.
	 */
	if (!setlocale(LC_ALL, ""))
	{
		p_setout(ERRIO);
		p_str(_errlocale("ksh"),NL);
	}
#endif /* NLS */

#ifdef POSIX2
	/*
	 * POSIX-shell is not internationalized and misinterprete some Asian
	 * characters in filenames. We'll need to switch to Bourne-shell if
	 * posix-shell is being called by libc, e.g. popen().
	 */
	if ((c>1) && (strcmp(v[1],"-cQ")==0) && (__nl_char_size>1))
		(void) execl("/bin/sh", "sh", "-c", v[2], 0);
#endif
	...
	/* Set up jobs structure */
	job.freejobs = (unsigned char *)malloc(JBYTES);
	 ...
	 *
	 *  ksh88 originaly checked against the pattern
	 *  "*r*sh" but we check against "*rsh" for backward
	 *  compatibility.
	 */
	if(c>0 && (strmatch(sim,"*rsh") || strmatch(sim, "rksh")))
		rsflag=0;
...
#else
		/* check if -Q option is used */
		if(!is_option(EVFLAG) &&
		   *sim && (input = path_open(sim,NULLSTR)) >= 0)
#endif
		{
			/* Do the $ENV file */
			st.cmdadr = sh_heap(sim);
			env_monitor = NEUTRAL;	/* will be set if set +/-m */
			exfile();
			free(st.cmdadr);
			st.cmdadr = command;
			io_fclose(input);
...
static void	exfile()
{
	...
	register struct history *hisp = 0;
	...
	while(1)
	{
		...
		if((st.states&PROMPT) && notstacked() && !fiseof(fp))
		{
			...
			/*
			 *  This job_wait() is to force the update of
			 *  the jobs sturcture for jobs that have been
			 *  stopped.  Doing a built-in does not call 
			 *  job_wait() so we are not notified at the 
			 *  next prompt of a stopped job unless we do
			 *  a job_wait() here (before we do the job_list()).
			 */
			job_wait(0);			
		...
		if(sh_timeout>0)
			alarm(0);
		if((st.states&PROMPT) && hisp && st.peekn)
		{
			hist_eof();
			p_char(st.peekn);
			p_setout(ERRIO);
...
#ifdef POSIX2
				;		/* spawn processes: DSDe406849 */
#else
				execflags |= 1;	/* for ksh, dont spawn processes */
#endif /* POSIX2 */
			st.execbrk = 0;
			p_setout(ERRIO);
			sh_exec(t,execflags);
