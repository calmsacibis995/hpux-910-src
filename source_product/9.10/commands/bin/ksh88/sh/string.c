...
/* string_to_long() is an onionskin for strtol which does error checking.   */
/* The string str is converted to a long int, using the base given by base. */
/* flg is an error indicator, which will be 1 if underflow, overflow, or    */
/* some conversion error occurs, and zero otherwise.  flg needs to be       */
/* checked by the calling routine to determine error status.                */

long string_to_long(str, base, flg)
char *str;
int base, *flg;
{
    char *ptr;
    ...
    ptr = (char *)0;
    errno = 0;
    result = strtol(str, &ptr, base);
    err = errno;

    *flg = 0;
    if( (result == 0 && ptr == str) || (err) ) {
        *flg = 1;
        result = 0;
    }
