...
#define SYNBAD	2
#ifdef POSIX3
/* This exit code is introduced in the POSIX shell for the first time
 * There is a library routine wordexp() introduced in the HPUX 9.0
 * which does the word expansion
 *
 * int wordexp(const char *words, wordexp_t *pwordexp, int flags);
 * This new function performs word expansions and places the list of
 * expanded words into the structure pointed to by pwordexp.
 *
 * The expansion would be the same as would be performed by the POSIX shell,
 * if the words argument is the part of a
...
/* print interface routines */
    ...
    extern void p_list(int,const char*[],int);
...
extern MSG	e_wrongbinary;
extern MSG	e_bad_signal;
...
extern int	jobtty;			/* current jobtty: 0, 1, or 2 */
...
/*
 *  There were several places in the old ksh that required
 *  special flags to determine one state from another.  Instead
 *  of adding lots of little flags all over the place, I'm adding
 *  a global bitmask here (and in defs.c) to take care of these
 *  problems as they arise.  This variable, hp_flags1, will be
 *  initalized to zero.  All of the macros for the bit positions
 *  are defined below.
 *
 *  The meanings of the flags are:
 *
 *	DOING_FC	When executing the fc command on a series of
 *                      commands, we do not want the invoked editor
 *			to be stoppable, so we set this flag and then
 *			check it in xec.c.  Before this fix, if you
 *			stopped the editor, the commands were executed
 *			immediately.  There would be too much redesign to
 *			get everything else to work, so we don't allow it.
 *
 *	HPCOMSUB	Turned on just before we call sh_exec() for a 
 *			command substitution.  Turned off after the 
 *			call to sh_exec() returns.  Korn sometimes turns
 *			off the comsub flag at inconvenient times.
 */

extern unsigned long hp_flags1;

#define	DOING_FC	01
#define HPCOMSUB	02

/*
 *  These are values used by env_monitor. See sh/defs.c for
 *  more info.
 */
#define NEUTRAL	0	/* initial state */
#define ON	1	/* set -m has occured */
#define	OFF	2	/* set +m has occured */

extern int env_monitor;

#ifdef HP_DEBUG
/*
 *  This is the bit-vector that
 *  tells which debug options are
 *  currently on.  See also sh/args.c.
 */

extern optflag hp_debug;
extern SYSTAB debug_options;

#endif /* HP_DEBUG */

#ifdef HP_BUILTIN
extern int hp_builtin;
#endif /* HP_BUILTIN */
