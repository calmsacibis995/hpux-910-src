...
#ifdef POSIX2
		/*
		 *  POSIX.2 requires that if HOME is unset
		 *  that the tilde should be expanded to the
		 *  users home directory
		 */
#include "builtins.h"	   
	   	char *home = nam_strval(HOME);
		if (!home)
		{
		 	char *cuserid();
			/*  check for trailing '/' and preserve */
			if((cp=strrchr(sp,'/')) != NULL)
				*cp=0;
			sp = logdir(cuserid((char *)0));
			if(cp)
			   *cp='/';
			return(sp);
