...
#if defined(u3b) || defined(u3b5) || defined(hp9000s800)
	{"time"},
	{"msgque"},
	{"sema"},
	{"shmem"},
...
/* C2 additions for cleaning up enviroment variables */
extern	char	**environ;
extern	int	cleanenv();
 ...
 */
...
#if defined(SecureWare) && defined(B1)
			if(ISB1)
                            ipcs_tags(md, msginfo.msgmni, (long)nl[MSGTAG].n_value);
#endif
...
#ifdef B1
                            if (((ISB1)&&
				(!ipcs_allow_print(i, &qds.msg_perm))) ||
                                ((!ISB1) && (!ipcs_allow_print(&qds.msg_perm))))
#else
                            if (!ipcs_allow_print(&qds.msg_perm))
#endif
                            {
                                i++;
                                continue;
...
#endif
			hpp('q', "SRrw-rw-rw-", &qds.msg_perm, i++, msginfo.msgmni);
			if(oflg)
	...
	if(mflg) {
...
#if defined(SecureWare) && defined(B1)
			if(ISB1)
                            ipcs_tags(md, shminfo.shmmni, (long)nl[SHMTAG].n_value);
#endif 
			lseeke(md, (long)nl[SHM].n_value, 0);
			if(oflg || bflg || tflg || !qflg || pflg || !nl[MSG].n_value)
				printf("%s%s%s%s%s%s\n", chdr,
					cflg ? chdr2 : "",
					oflg ? " NATTCH" : "",
					bflg ? "  SEGSZ" : "",
		...
		while(i < shminfo.shmmni) {
			...
			if(ISSECURE){
#ifdef B1
                            if (((ISB1) && (!ipcs_allow_print(i, &mds.shm_perm)))||
                                ((!ISB1) && (!ipcs_allow_print(&mds.shm_perm))))
#else
                            if (!ipcs_allow_print(&mds.shm_perm))
#endif
                            {
                                i++;
                                continue;
...
#endif
			hpp('m', "DCrw-rw-rw-", &mds.shm_perm, i++, shminfo.shmmni);
			if(oflg) {
				printf("%7u", mds.shm_nattch);
			}
			if(bflg)
				printf(" %6d", mds.shm_segsz);
			if(pflg)
	...
	if(sflg) {
...
#if defined(SecureWare) && defined(B1)
			if(ISB1)
                            ipcs_tags(md, seminfo.semmni, (long)nl[SEMTAG].n_value);
#endif
		...
		while(i < seminfo.semmni) {
			...
			if(ISSECURE){
#ifdef B1
                            if (((ISB1) && (!ipcs_allow_print(i, &sds.sem_perm)))||
                               ((!ISB1) && (!ipcs_allow_print(&sds.sem_perm))))
#else
                            if (!ipcs_allow_print(&sds.sem_perm))
#endif
                            {
                                i++;
                                continue;
...
#endif
			hpp('s', "--ra-ra-ra-", &sds.sem_perm, i++, seminfo.semmni);
			if(bflg)
...
/*
**	hpp - common header print
*/

hpp(type, modesp, permp, slot, slots)
