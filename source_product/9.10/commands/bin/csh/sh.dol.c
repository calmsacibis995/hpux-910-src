	...
	register CHAR *cp;
{
	...
	if (gargc != 1) {
		setname(to_char(cp));
		bferr((catgets(nlmsg_fd,NL_SETN,1, "Ambiguous")));
	}
	cp = savestr(gargv[0]);
	...
	CHAR **v;
{
	...
	Dvp = v; Dcp = nullstr;		/* Setup input vector for Dreadc */
	...
	case '"':
		...
		for (;;) {
			...
			if (c == c1)
				break;
			if (c == '\n' || c == DEOF)
				error((catgets(nlmsg_fd,NL_SETN,2, "Unmatched %c")), c1);
			if ((c & (QUOTE|TRIM)) == ('\n' | QUOTE))
				--wp, ++i;
			if (--i <= 0)
				goto toochars;
...
pack:
	...
	for (;;) {
		...
		if (--i <= 0)
toochars:
			error((catgets(nlmsg_fd,NL_SETN,3, "Word too long")));
		*wp++ = c;
	}
ret:
	Gcat(nullstr, wbuf);
...
#ifndef NONLS
CHAR	*nulvec[] = { 0 };
CHAR 	nulargv1[] = {'a', 'r', 'g', 'v', '\0'};
struct	varent nulargv = { nulvec, nulargv1, 0 };
#else
char	*nulvec[] = { 0 };
...
{
	register CHAR *np;
	register char *b_np;		/* NLS: for $<. Buffer for read call */
	register struct varent *vp;
	CHAR name[82];
	
	/* changed from	CHAR name[20];   DSDe408290, DSDe414890 */
	
	int c, sc;
	int subscr = 0, lwb = 1, upb = 0;
	bool dimen = 0, isset = 0;
	char wbuf[WRDSIZ];		/* Only used for $<; remain char */
	...
	case '<'|QUOTE:
		if (dimen || isset)
			goto syntax;		/* No $?<, $#< */
		/* NLS: np changed to b_np because read returns bytes */
		/* and np is now CHAR */
#ifdef SIGWINCH
		sighold(SIGWINCH);	/* DSDe410384: block signal,
					 * else, WINCH satisfies read! */
#endif /* SIGWINCH */
		for (b_np = wbuf; read(OLDSTD, b_np, 1) == 1; b_np++) {
			if (b_np >= &wbuf[WRDSIZ-1])
				error((catgets(nlmsg_fd,NL_SETN,4, "$< line too long")));
			if (*b_np == 0 || *b_np == '\n')  
				break;
		}
#ifdef SIGWINCH
		sigrelse(SIGWINCH);		/* Release WINCH after read */
#endif /* SIGWINCH */
		*b_np = 0;
		/*
	...
	default:
		...
		if (digit(c)) {
			...
			if (subscr == 0) {
				if (isset) {
					dolp = file ? CH_one : CH_zero;
					goto eatbrac;
				}
				if (file == 0)
					error((catgets(nlmsg_fd,NL_SETN,5, "No file for $0")));
				c = DgetC(0);
				if (c == ':') {
					c = DgetC(0), dolmcnt = 1;
					if (c == 'g')
					...
					if (!any(c, "htrqxe"))
						error((catgets(nlmsg_fd,NL_SETN,8, "Bad : mod in $")));
					dolmod = c;
					if (c == 'q')
						dolmcnt = 10000;
				} else
...
syntax:
				error((catgets(nlmsg_fd,NL_SETN,6, "Variable syntax")));
		}
		*np++ = 0;
		unDredc(c);
		vp = adrof(name);
	...
	if (isset) {
		dolp = ((int)vp || (int)getenv(to_char(name))) ? CH_one : CH_zero;
		goto eatbrac;
	}
	if (vp == 0) {
		np = to_short(getenv(to_char(name)));
		if (np) {
			addla(np);
			goto eatbrac;
		}
	...
	if (dimen == 0 && subscr == 0 && c == '[') {
		...
		*np = 0, np = name;
		...
		if (digit(*np)) {
			...
			if ((i < 0 || i > upb) && !any(*np, "-*")) {
oob:
				setname(to_char(vp->name));
				error((catgets(nlmsg_fd,NL_SETN,7, "Subscript out of range")));
			}
			lwb = i;
			if (!*np)
				upb = lwb, np = CH_star;
	...
	CHAR *term;
{
	register int c;
	CHAR *Dv[2];
	CHAR obuf[WRDSIZ], lbuf[WRDSIZ], mbuf[WRDSIZ];
	int ocnt, lcnt, mcnt;
	register CHAR *lbp, *obp, *mbp;
	CHAR **vp;
	bool quoted;
                      
	if (creat(to_char(shtemp), 0600) < 0)
		Perror(to_char(shtemp));
	close(0);
	if (open(to_char(shtemp), 2) < 0) {
		int oerrno = errno;

		unlink(to_char(shtemp));
		errno = oerrno;
		Perror(to_char(shtemp));
	}
	unlink(to_char(shtemp));			/* 0 0 inode! */
	Dv[0] = term; Dv[1] = NOSTR; gflag = 0;
	scan(Dv, trim); rscan(Dv, Dtestq); quoted = gflag;
	ocnt = WRDSIZ; obp = obuf;
	for (;;) {
		/*
		 * Read up a line
		 */
		lbp = lbuf; lcnt = WRDSIZ - 4;
		for (;;) {
			c = readc(1);		/* 1 -> Want EOF returns */
			if (c < 0) {
				setname(to_char(term));
				bferr((catgets(nlmsg_fd,NL_SETN,9, "<< terminator not found")));
			}
			if (c == '\n')
				break;
			if (c &= TRIM) {
				*lbp++ = c;
#ifdef NLS16
				/* if c is kanji, we want to decrement by 2 bytes */
				if (((CHAR) c) & KMASK)
					lcnt--;
#endif
				if (--lcnt < 0) {
					setname("<<");
					error((catgets(nlmsg_fd,NL_SETN,10, "Line overflow")));
				} 
			}
		}
		*lbp = 0;
		 ...
		 */
		if (Eq(lbuf, term)) {
			write(0, to_char(obuf), WRDSIZ - ocnt);
			lseek(0, 0l, 0);
			return;
		}
			...
			*lbp++ = '\n'; *lbp = 0;
			for (lbp = lbuf; c = *lbp++;) {
				*obp++ = c;
				...
				if (--ocnt <= 0) {
#ifdef NLS16
				/* if a kanji is at the WRDSIZ boundary, ocnt will be -1 */
				    if (ocnt == -1)
					write(0, to_char(obuf), WRDSIZ+1);
				    else
#endif
					write(0, to_char(obuf), WRDSIZ);
				    obp = obuf; ocnt = WRDSIZ;
				}
			}
			continue;
		}
		 ...
		 */
		Dcp = lbuf; Dvp = Dv + 1; mbp = mbuf; mcnt = WRDSIZ - 4;
		for (;;) {
			c = DgetC(DODOL);
			if (c == DEOF)
				break;
			...
			*mbp++ = c;
			...
			if (--mcnt <= 0) {
				setname("<<");
				bferr((catgets(nlmsg_fd,NL_SETN,11, "Line overflow")));
			}
		}
		*mbp++ = 0;
		...
		for (; *vp; vp++) {
			for (mbp = *vp; *mbp; mbp++) {
				*obp++ = *mbp & TRIM;
#ifdef NLS16
				/* if *mbp is kanji, we want to decrement by 2 bytes */
				if ((*mbp & TRIM) > 0377)
					ocnt--;
#endif
				if (--ocnt <= 0) {
