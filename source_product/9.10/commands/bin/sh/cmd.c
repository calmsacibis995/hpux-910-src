...
register int	esym;
{
	...
	else
	{
		...
		for (;;)
		{
			...
			{
				argp= wdarg;
				wdarg = (struct argnod *)alloc(CHARSIZE*tlength(argp->argval) + BYTESPERWORD);
				tmovstr(argp->argval, wdarg->argval);
			}
...
prsym(sym)
{
	if (sym & SYMFLG)
	{
		...
		prs(nl_msg(637,endoffile));
	else
	{
		if (sym & SYMREP)
			prc(sym);
		if (sym == NL)
			prs((nl_msg(101, "newline or ;")));
		else
			prc(sym);
	}
}
...
{
	prp();
	prs(nl_msg(605,synmsg));
	if ((flags & ttyflg) == 0)
	{
		prs(nl_msg(639,atline));
		prn(standin->flin);
	}
	prs(colon);
	prc(LQ);
	if (wdval)
		...
		prst_cntl(wdarg->argval);
	prc(RQ);
	prs(nl_msg(638,unexpected));
	newline();
	exitsh(SYNBAD);
}
