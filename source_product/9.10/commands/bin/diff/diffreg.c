...
int	slen[2];
int	pref, suff = 0;	/* length of prefix and suffix */
...
diffreg()
{
	...
	if ((stb1.st_mode & S_IFMT) == S_IFDIR) {
		...
		if (!strcmp(file2, "-")) {
			fprintf(stderr, (catgets(nl_fn,NL_SETN,23, "diff: can't specify - -\n")));
			done(NOSIG);
		}
		file1 = copytemp();
		name1 = stdin_name;
	...
	for (;;) {
		...
		for (j = 0; j < i; j++) {
		    if(iflag) {
			if (tolower(buf1[j]) != tolower(buf2[j]))
				goto notsame;
		    }
		    else
			if(buf1[j] != buf2[j])
...
same:
	if (opt == D_CONTEXT && anychange == 0)
		printf((catgets(nl_fn,NL_SETN,15, "No differences encountered\n")));
	done(NOSIG);
}
...
copytemp()
{
	char buf[BUFSIZ];
	register int i, f;
	char *tmpdir, tmpstr[MAXPATHLEN + 1];
	...
	if ((tmpdir = getenv("TMPDIR")) != 0) {
	    sprintf(tmpstr,"%s/dXXXXX",tmpdir);
	    tempfile = strdup(mktemp(tmpstr));
	}
	else
	    tempfile = strdup(mktemp("/tmp/dXXXXX"));
#ifdef DBUG
	printf("In copytemp: tempfile = %s\n",tempfile);
#endif
	f = creat(tempfile,0600);
	if (f < 0) {
		fprintf(stderr, (catgets(nl_fn,NL_SETN,16, "diff: temporary input file ")));
		perror(tempfile);
		done(NOSIG);
	}
...
#ifdef DBUG
	printf("\nSplice arguments dir = %s, file = %s\n",dir, file);
#endif

	if (!strcmp(file, "-")) {
		fprintf(stderr, (catgets(nl_fn,NL_SETN,18, "diff: can't specify - with other arg directory\n")));
		done(NOSIG);
	}
	a1 = dir;
	a2 = file;
...
prune()
{
	...
	for(pref=0;pref<len[0]&&pref<len[1]&&
		...
		pref++ ) ;
	/*
	 *  skip next loop if called from merge.  suff will not be
	 *  assigned a value, but it is declared globally and thus
	 *  will default to zero.  This is fine.
	 */
	if(! Mflag){     
...
char *s;
{
	...
	if (opt == D_IFDEF) {
		if (inifdef)
			/* comment delimiters added so output is
			   ANSI C conformant */
...
char *str;
{
	...
	if(!bflag && !wflag) {
			for(shift=0;(t=getc(f))!='\n';shift+=7) {
				if(t==EOF){
					if(shift){
						fprintf(stderr,(catgets(nl_fn,NL_SETN,19,"Warning : missing newline at end of file %s\n")), str); 
						break;
					} else
						return(0);
				}
				if (iflag) t = (long) tolower(t);
				sum += (long)t << (shift
				    %= HALFLONG);
			}
	} else {
	...
	FILE *f;
{
	...
	while (--cnt >= 0)
#ifdef NLS
	    if(laststatus = (BYTE_STATUS(*cp++,laststatus)) > 2) 
#else
		if (*cp++ & 0200)
#endif
			return (0);
