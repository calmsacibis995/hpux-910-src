...
#ifdef	hpux
	/* no lseek needed, just opened file */
	(void) lockf(fileno(df), F_LOCK, 0L);
#else	hpux
...
putitime()
{
	...
	fd = fileno(df); /*get the file descptr number given the stream name */
#ifdef	hpux
	/* no lssek needed, file was just opened */
	...
	struct	idates	*what;
{
   ...
   strcpy(tempstr,"LANG=");
   strcat(tempstr,langstr);
   putenv(tempstr);
   strcpy(tempstr,"LC_TIME=");
   strcat(tempstr,lctimestr);
   putenv(tempstr);
	...
	struct	idates	*idatep;
{
	...
	recno = 0;
	if ((fgets(buf, BUFSIZ, df)) != buf) /*fgets,gets a string from stream*/
		return(-1);
	recno++;
	...
	char	*buf;
{
	char	un_buf[128];
 	...
 	/*read from string buf , format is DUMPINFM*/
