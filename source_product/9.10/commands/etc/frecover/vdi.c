...
*
* File:         vdi.c
* RCS:          $Header: vdi.c,v 70.5.1.2 94/10/12 15:19:00 hmgr Exp $
...
#ifndef lint
 static char RCSid[]="$Header: vdi.c,v 70.5.1.2 94/10/12 15:19:00 hmgr Exp $";
#endif
...
{
  int n;
  static struct stat local_sbuf;
  static int vdi_read_first = 1;

  vdi_errno = 0;

  /*
   * stat the file descriptor for raw reads later.
   * If the stat fails, return an error.  This code
   * was added to help the resolution of defect
   * DSDe415807.
   */
  if (vdi_read_first) {
    if (fstat(fd, &local_sbuf) != 0) {
      vdi_errno = errno;
      return(-1);
    }
    vdi_read_first = 0;
  ...
  case VDI_DISK:
    /*
     * if this is a disk and is a raw device,
     * force the read to a block boundary to make it succeed.
     * Otherwise, let the original behavior prevail.
     * This code is put in place to resolve DSDe415807.
     */
    if ( (type == VDI_DISK) && ((local_sbuf.st_mode & S_IFMT) == S_IFCHR) ) {
#     define	roundup(x, y)	((((x)+((y)-1))/(y))*(y))
      int newbsize, dum_count, beginning_slop;
      char *dum_buf;
      long fpos;

      /*
       * mark the current file position, compute,
       * the beginning slop for the reads, and
       * position to the beginning of the block.
       */
      fpos = (long)lseek(fd, (off_t)0, SEEK_CUR);
      if (fpos == -1) {
	vdi_errno = errno;
	return(-1);
      }
      if (roundup(fpos, 1024) == fpos)
        beginning_slop = 0;
      else
        beginning_slop = 1024 - (roundup(fpos, 1024) - fpos);
      if (lseek(fd, (off_t)fpos - (off_t)beginning_slop, SEEK_SET) == -1) {
	vdi_errno = errno;
	return(-1);
      }
      newbsize = roundup(nbyte + beginning_slop, 1024);
      dum_buf = (char *)malloc(newbsize);
      if (dum_buf == NULL) {
	vdi_errno = errno;
	return(-1);
      }
      /*
       * read the buffer's worth of data
       */
      if ((n = read(fd, dum_buf, newbsize)) < 0) {
        vdi_errno = errno;
	free(dum_buf);
	return(n);
      }
      memcpy(buf, &dum_buf[beginning_slop], nbyte);
      free(dum_buf);
      if ((long)lseek(fd, (off_t)fpos + (off_t)nbyte, SEEK_SET) == -1) {
	vdi_errno = errno;
	return(-1);
      }
      break;
