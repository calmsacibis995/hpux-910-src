	...
	register daddr_t bn;
{
	...
	for (; j <= NIADDR; j++) {
		if (blknos[j] != nb) {
			...
			io->i_cc = io->i_fs.fs_bsize;
#ifdef SDS
			if (sds_read(io) != io->i_fs.fs_bsize) {
#else /* ! SDS */
...
register int count;
{
	...
	register char *p;
	register int lbn, off, size, diff;
	...
	if (io->i_flgs & F_REMOTE) {	/* not a blocked device */
		...
		/* i_offset is previous offset into file */
		diff = io->i_ino.i_size - io->i_offset;
		if (diff <= 0) 
			return (-1);
		/* These several lines determines which logical block
		 * on disk i_offset points into.
		 */
		fs = &io->i_fs;
		...
		/* Size of filesystem block or fragment size */
		...
		if (old_bn != io->i_bn) {
			...
			io->i_cc = size;
			io->i_ma = io->i_buf;
			/* 'devread()' returns count of bytes not read.
			 * Should be reported?
			 */
#ifdef SDS
			if (sds_read(io) < 0) {
		...
		/* These three lines appear to have no purpose !! 
		 * i_cc always set immediately prior to call to 'devread' PQP
		 */
		io->i_cc = size;
...
register int count;
{
	register struct iob *io;
	register char *p;
	register int ret=0, size;

	/* i_offset is an offset into io->i_buf;  If i_offset is beyond 
	 * i_buf (i.e. i_offset == MAX_BSIZE) then we need new data.
	 * i_offset should be initialized to MAX_BSIZE for first entry.
	 */
	io = &iob[fdesc];
	do {
		if (MAX_BSIZE <= io->i_offset) {
			/* Fresh block required: call 'devread()' */
			io->i_cc = MAX_BSIZE;  /* Fixed size read */
			io->i_ma = io->i_buf;
#ifdef SDS
			if (sds_read(io) < 0) {
#else /* ! SDS */
			...
			io->i_offset = 0;
			io->i_bn    += MAX_BSIZE/DEV_BSIZE; 
		}
		p = &io->i_buf[io->i_offset];
		size = min(MAX_BSIZE - io->i_offset, count);
		count -= size;
		io->i_offset += size;
		ret += size;
		while (size--) *buf++ = *p++;
	...
	register char *buf;
{
	...
	if (file->i_flgs&F_ISATAPE) { /* If this a tape device ... */
		if ((i = count) <= 0)
			return (0);
		if (taperead(fdesc, buf, i) < 0)
			return(-1);
		else
			return (count);
		}
	...
	if (copyin(fdesc, buf, i) == -1)
		...
		return (count); /* Should 'count' be hard-coded? */
}

int	openfirst = 1;

/* Two modes of accessing devices exists:
 *	block mode used for just about everything (goes through a devices
 *	file system) and "raw mode" for tape drives.
 */
open(str, flag)
	register char *str;
	int flag;
...
gotfile:	/* Zero out iob (file) entry */
	{
		register char *cp;
	...
	dump_value("how",how);
#endif
	/* Open flag must request "raw" tape driver */
	if (flag == F_ISATAPE) {
		/* Do initialization for scsi tape special case */
		file->i_bn = 0;
		file->i_offset = MAX_BSIZE;
		/* 
		 * For CD-INSTALL we need to use the raw driver, but
		 * is not really a tape, so change how to be F_ISATAPE
		 * (why does it do a get_how() above anyway?, Can't it
		 * trust 'flag'?)
		 */
		how = F_ISATAPE;
		goto tapefile_opened;
		}
	if (how & F_REMOTE) {
		int junk;
		short m1 = -1;
...
int flag;
{
	...
	strcpy(prefix,path);
	if (((how = get_how()) == F_REMOTE) || (flag == F_ISATAPE))
	    return(open(path, flag)); /* open_CDF used only for local disk */
	cnode_name[0] = '/';
