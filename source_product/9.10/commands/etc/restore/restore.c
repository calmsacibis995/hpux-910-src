...
createfiles()
{
	...
	for (;;) {
		...
		last = upperbnd(last);
		/*
		/* Check if the next available file on from the dump
		 * is what we expected.  If not, then the specified
		 * file was not dumped to tape.  The file(s) were lost
		 * while doing the dump.  Causes could be the file was
		 * removed during the dump, or was open for write.
		 *
		 * Since this file is not on the tape, we will clear the 
		 * flags from the symbol table.
		 *
		 */

		 while ( curfile.ino > first ) {
		    ep = lookupino(first);
		    if ( ep == NIL )
		       panic("corrupted symbol table\n");
		    fprintf(stderr,"%s: not found on tape\n",myname(ep) );
