...
{
	register char *cp;
	register struct entry *epsave = ep;
	struct stat	statbuf;

	static char namebuf[MAXPATHLEN];

	for (cp = &namebuf[sizeof(namebuf)-2]; cp > &namebuf[ep->e_namlen]; ) {
...
#if defined(DUX) || defined(DISKLESS)
			if (epsave->e_CDF == C_CDF) {
				namebuf[sizeof(namebuf)-3] = '\0';
			} else if (epsave->e_CDF == C_UNKNOWN) {
				namebuf[sizeof(namebuf)-2] = '+';
				if (lstat(cp, &statbuf) < 0) {
					namebuf[sizeof(namebuf)-2] = '\0';
					if (lstat(cp, &statbuf) == 0)
						epsave->e_CDF = C_NOCDF;
						
				} else {
					namebuf[sizeof(namebuf)-2] = '\0';
					if (S_ISCDF(statbuf.st_mode))
						epsave->e_CDF = C_CDF;
					else
						epsave->e_CDF = C_NOCDF;
				}
...
static struct strhdr strtblhdr[allocsize(MAXPATHLEN / STRTBLINCR) / STRTBLINCR];
