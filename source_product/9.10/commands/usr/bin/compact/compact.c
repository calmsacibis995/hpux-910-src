...
char *argv [ ];
{
	...
	union cio c;
	longint ic, n;
	char *cp, *cpargv, fname [LNAME];
	struct stat cfstatus;

	flist=(struct index *)0;

	/*dir [513] . next = NULL;*/
	dir [DIRSIZE-1] . next = NULL;
	for (head = dir + (j = DIRSIZE-1); j--; ) {
		dirp = head--;
		head -> next = dirp;
	}
	bottom = dirp -> pt = dict;
	...
	for (i = 1; ; i++) {
		...
		if (i >= argc) {
			...
			for (cp = fname, cpargv = argv[i] ; cp < &fname[LNAME-3] && (*cp = *cpargv++) ; cp++);

			if (i < argc) {		/* add ".C" suffix */
				*cp++ = '.';
				*cp++ = 'C';
				*cp   = '\0';
			}
...
#ifdef ACLS
					/*
					 * If the uncompacted file has optional
					 * ACL entries, they should be copied
					 * to the compacted file.
					 * Note this does an fstat to get the 
					 * uid and gid of the compacted file
					 * in case the file already existed and
					 * has different ownerships (in this 
					 * case the cpacl or chmod may fail).
					 */
					if (status.st_acl) {
						fstat(fileno (cfp), &cfstatus); 
						fcpacl(fileno (uncfp), fileno (cfp), status.st_mode, status.st_uid, status.st_gid, cfstatus.st_uid, cfstatus.st_gid);
					}
					else
					{
					/*
					 * File has no optional ACL entries or
					 * is remote.  Chmod is then more 
					 * efficient to use than cpacl.
					 */
		...
		if (ferror (uncfp) || (cfp != NULL && ferror (cfp)))
