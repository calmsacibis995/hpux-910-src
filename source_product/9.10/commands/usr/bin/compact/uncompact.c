...
#ifdef ACLS
			/*
			 * If the compacted file has optional ACL entries, they 
			 * should be copied to the uncompacted file.
			 * Note this does an fstat to get the uid and gid of 
			 * the uncompacted file in case the file already 
			 * existed and has different ownerships (in this 
			 * case the cpacl or chmod may fail).
			 */
			if (status.st_acl) {
				fstat(fileno (uncfp), &uncfstatus); 
				fcpacl(fileno (cfp), fileno (uncfp), status.st_mode, status.st_uid, status.st_gid, uncfstatus.st_uid, uncfstatus.st_gid);
			}
			else
			{
			/*
