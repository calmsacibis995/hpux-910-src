...
int		err_f = 0;	/* set if checkf() returns NULL */

#ifdef NLS
nl_catd		catd;
#endif NLS

#define USAGE() { fprintf(stderr,catgets(catd, NL_SETN, 1, "Usage: pg [-number] [-p string] [-cefns] [+line] [+/pattern/] files\n")); exit(1); }
...
char *argv[];
{
	register char	*s;
	register char	*p;
	char		*env[5]; 	/* the environment options */
	int		prnames = 0, not_done = 1, i = 1; 
	char		*getenv();
	FILE		*checkf();

#ifdef NLS
	if (!setlocale(LC_ALL,"")) {
		fputs(_errlocale("pg"), stderr);
		putenv("LANG=");
		catd = (nl_catd)-1;
	} else
		catd = catopen("pg", 0);
	...
	/* check if PG has environment options set */
	if (s = getenv ("PG")) { 
		fnames = env;
		do {
			for (p = s; (*s != ' ' && *s != '\0' && *s != '\t'); s++) 
...
/* set program options according to arguments stored in an array */
argscan ()
{
	...
	while (--nfiles > 0) {
		if ((ch = (*++fnames)[0]) == '-') {
			...
			if (*++s == '/') {
				...
				initopt = 0;
#ifdef	NLS16
				for (++s, p=initbuf, b_status = BYTE_STATUS((unsigned char)*s, b_status);
				*s!='\0' && !(*s=='/' && b_status==ONEBYTE);)
#else
				for (++s, p=initbuf; *s!='\0' && *s!='/';)
					...
					if (p < initbuf + sizeof(initbuf)) {
						*p++ = *s++;
#ifdef	NLS16
						b_status = BYTE_STATUS((unsigned char)*s, b_status);
#endif
					}
					else {
						fprintf(stderr,catgets(catd, NL_SETN, 4, "pg: pattern too long\n"));
						exit(1);
					}
				*p = '\0';
			}
...
register char *s;
{
	...
	fprintf(stderr,catgets(catd, NL_SETN, 5, "pg: prompt too long\n"));
	exit(1);
#ifdef lint
	return(NULL);
#endif
	...
	for (;;) {
		...
		if (clropt) {
			...
			if (start < old_ss.first_line)
				sopr(catgets(catd, NL_SETN, 6, "...skipping backward\n"),0);
			else
			if (start > old_ss.last_line + 1)
				sopr(catgets(catd, NL_SETN, 7, "...skipping forward\n"),0);
		}
		...
		if (new_ss.is_eof) {
			if (!eof_pause || eofl_no == 1)
				return(1);
			hadchance++;
			error(catgets(catd, NL_SETN, 8, "(EOF)"));
		}
		old_ss = new_ss;
...
char *filename;
{
	...
	for (;;) {
		...
		case 'd':
			...
			case 1:
			case -1:
				new_ss.last_line = old_ss.last_line + nlines*sign*window/2;
				new_ss.first_line = new_ss.last_line - window + 1;
				break;
			case 0:
				new_ss.first_line = (nlines-1) * window/2 + 1;
				new_ss.last_line = new_ss.first_line + window - 1;
				break;
			}
			inwait = 0;
			...
			*/
			...
			if (setjmp(restore) != 0) {
				...
				if ((sf=fopen(cmdptr,"w")) == NULL) {
					error(catgets(catd, NL_SETN, 9, "cannot open save file"));
					break;
				}
				kill_line();
				sopr(catgets(catd, NL_SETN, 10, "saving file "),1);
				sopr(cmdptr,1);
				fflush(stdout);
				save_input(sf);
				error(catgets(catd, NL_SETN, 11, "saved"));
			}
			fclose(sf);
			break;
		case 'q':
...
#ifdef	NLS16
			for (i = cmdptr; i < cmdend; i++) {
				c = *i;
				b_status = BYTE_STATUS((unsigned char)c, b_status);
			}
#endif
			if ( (cmdend > cmdptr + 1)
				&& (*cmdptr ==  *(cmdend - 1))
		...
		case '!':       /* shell escape */
			...
			if ((id = fork ()) < 0) {
				error(catgets(catd, NL_SETN, 12, "cannot fork, try again later"));
				break;
			}
			if (id == 0) {
				/*
				...
				*/
				...
				fprintf(stderr,catgets(catd, NL_SETN, 13, "exec failed\n"));
				exit(1);
			}
...
ttyin ()
{
	...
	int save_bstatus[LINSIZ];
	int *status_ptr = save_bstatus;
...
#define		ANDISASCII
	...
	fixterm();
	(void) set_state(&state,' ',NULL);   /* initialize state processing */
	sptr = cmdbuf;
	while(state != 10) {
		ch = readch();
#ifdef	NLS16
		...
		if (ch == '\n' && !slash)
			break;
		if ((ch == erasechar() ANDISASCII) && !slash) {
			if (sptr > cmdbuf) {
				--promptlen;
				pr("\b \b");
				--sptr;
				...
				--status_ptr;
#endif
				if ((unsigned char)*sptr < ' ') {
					--promptlen;
					pr("\b \b");
				}
#ifdef	NLS16
				/* backspace over 2-byte character */
				else if (*status_ptr == SECOF2) {
					--promptlen;
					pr("\b \b");
					--sptr;
					--status_ptr;
		...
		if ((ch == killchar() ANDISASCII) && !slash) {
			if (hard_copy)
				putchar(ch);
			resetterm();
			return(1);
...
help()
{
	...
	pr("-------------------------------------------------------\n");
	pr(catgets(catd, NL_SETN, 14, "   h               help\n"));
	pr(catgets(catd, NL_SETN, 15, "   q or Q          quit\n"));
	pr(catgets(catd, NL_SETN, 16, "   <blank> or \\n   next page\n"));
	pr(catgets(catd, NL_SETN, 17, "   l               next line\n"));
	pr(catgets(catd, NL_SETN, 18, "   d or ^D         display half a page more\n"));
	pr(catgets(catd, NL_SETN, 19, "   . or ^L         redisplay current page\n"));
	pr(catgets(catd, NL_SETN, 20, "   f               skip the next page forward\n"));
	pr(catgets(catd, NL_SETN, 21, "   n               next file\n"));
	pr(catgets(catd, NL_SETN, 22, "   p               previous file\n"));
	pr(catgets(catd, NL_SETN, 23, "   $               last page\n"));
	pr(catgets(catd, NL_SETN, 24, "   w or z          set window size and display next page\n"));
	pr(catgets(catd, NL_SETN, 25, "   s savefile      save current file in savefile\n"));
	pr(catgets(catd, NL_SETN, 26, "   /pattern/       search forward for pattern\n"));
	pr(catgets(catd, NL_SETN, 27, "   ?pattern? or\n"));
	pr(catgets(catd, NL_SETN, 28, "   ^pattern^       search backward for pattern\n"));
	pr(catgets(catd, NL_SETN, 29, "   !command         execute command\n"));
	pr("\n");
	pr(catgets(catd, NL_SETN, 30, "Most commands can be preceded by a number, as in:\n"));
	pr(catgets(catd, NL_SETN, 31, "+1\\n (next page); -1\\n (previous page); 1\\n (page 1).\n"));
	pr("\n");
	pr(catgets(catd, NL_SETN, 32, "See the manual page for more detail.\n"));
	pr("-------------------------------------------------------\n");
}
...
register int nskip;
{
	if (fnum + nskip < 0) {
		nskip = -fnum;
		if (nskip == 0)
			error(catgets(catd, NL_SETN, 33, "No previous file"));
	}
	else
	if (fnum + nskip > nfiles - 1) {
		nskip = (nfiles - 1) - fnum;
		if (nskip == 0)
			error(catgets(catd, NL_SETN, 34, "No next file"));
	}
	return(nskip);
}
...
register char *fs;
{
	...
	if (stbuf.st_mode & S_IFDIR) {
		fprintf(stderr,catgets(catd, NL_SETN, 35, "pg: %s is a directory\n"),fs);
		return (NULL);
	}
	if (stbuf.st_mode & S_IFREG) {
		...
		if (f != stdin) {
			fprintf(stderr,catgets(catd, NL_SETN, 36, "pg: special files only handled as standard input\n"));
			return(NULL);
		}
		else {
			(void) mktemp(tmp_name);
			if ((fd=creat(tmp_name,0600)) < 0) {
			    fprintf(stderr,catgets(catd, NL_SETN, 37, "pg: Can't create temp file\n"));
			    return(NULL);
			}			
			close(fd);
			if ((tmp_fou = fopen(tmp_name, "w")) == NULL) {
				fprintf(stderr,catgets(catd, NL_SETN, 38, "pg: Can't get temp file for writing\n"));
				return(NULL);
			}
			if ((tmp_fin = fopen(tmp_name, "r")) == NULL) {
				fprintf(stderr,catgets(catd, NL_SETN, 39, "pg: Can't get temp file for reading\n"));
				return(NULL);
			}
			pipe_in = 1;
		}
...
int i;
{
	...
	static struct messages {
		...
		"Pattern not found",				1,	/* catgets 40 */
		"Range endpoint too large",			11,	/* catgets 41 */
		"Bad number",					16,	/* catgets 42 */
		"`\\digit' out of range",			25,	/* catgets 43 */
		"Illegal or missing delimeter",			36,	/* catgets 44 */
		"No remembered search string",  		41,	/* catgets 45 */
		"\\( \\) imbalance",				42,	/* catgets 46 */
		"Too many \\(",					43,	/* catgets 47 */
		"More than two numbers given in \\{ \\}",	44,	/* catgets 48 */
		"} expected after \\",				45,	/* catgets 49 */
		"First number exceeds second in \\{ \\}",	46,	/* catgets 50 */
		"[] imbalance",					49,	/* catgets 51 */
		"Regular expression overflow",			50,	/* catgets 52 */
		"Bad regular expression",		 	0	/* catgets 53 */
		};

	for (j = 0; re_errmsg[j].number != 0; j++ )
		if (re_errmsg[j].number == i )
			break;
	error(catgets(catd, NL_SETN, 40+j, re_errmsg[j].message));
...
#ifdef	NLS16
	for (i = buf; i <= endbuf; i++)
		b_status = BYTE_STATUS((unsigned char)*i, b_status);
	if((*endbuf++ != '\\' || (*(endbuf-1) == '\\' && b_status == SECOF2))
	    && *endbuf == '$') {
#else
	if(*endbuf++ != '\\' && *endbuf == '$') {
#endif
...
save_pipe()
{
	if (!doliseof)
		while (fgetputc(stdin) != EOF)
			if (brk_hit) {
				brk_hit = 0;
				error(catgets(catd, NL_SETN, 54, "Piped input only partially saved"));
				break;
			}
	fclose(tmp_fou);
}
...
compact()
{
	fprintf(stderr, catgets(catd, NL_SETN, 55, "pg: no more memory - line %d\n"),dol->l_no);
	end_it();
}
...
terminit()	/* set up terminal dependencies from termlib */
{
	...
	if ((freopen("/dev/tty","r+",stdout)) == NULL) {
		fprintf(stderr,catgets(catd, NL_SETN, 56, "pg: cannot reopen stdout\n"));
		exit(1);
	}
	...
	if (err_ret != 1) {
		fprintf(stderr,catgets(catd, NL_SETN, 57, "pg: cannot find terminal type\n"));
		exit(1);
	}
...
char *filename;
{
	...
	int pagenum;
	if (filename != NULL) {
		sopr(catgets(catd, NL_SETN, 58, "(Next file: "),1);
		sopr(filename,1);
		sopr(")",1);
	}
	else {
