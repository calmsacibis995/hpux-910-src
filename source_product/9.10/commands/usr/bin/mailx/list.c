...
number:
			if (star) {
				printf((catgets(nl_fn,NL_SETN,1, "No numbers mixed with *\n")));
				return(-1);
			}
			mc++;
			other++;
		...
		case TSTRING:
			if (beg != 0) {
				printf((catgets(nl_fn,NL_SETN,2, "Non-numeric second argument\n")));
				return(-1);
			}
			other++;
			if (lexstring[0] == ':') {
				...
				if (colresult == 0) {
					printf((catgets(nl_fn,NL_SETN,3, "Unknown colon modifier \"%s\"\n")),
					    lexstring);
					return(-1);
				}
				colmod |= colresult;
		...
		case TSTAR:
			if (other) {
				printf((catgets(nl_fn,NL_SETN,4, "Can't mix \"*\" with anything\n")));
				return(-1);
			}
			star++;
			break;
	...
	*np = NOSTR;
	...
	if (star) {
		...
		if (mc == 0) {
			printf((catgets(nl_fn,NL_SETN,5, "No applicable messages.\n")));
			return(-1);
		}
		return(0);
	}
	...
	if (np > namelist) {
		...
		if (mc == 0) {
			printf((catgets(nl_fn,NL_SETN,6, "No applicable messages from {%s")),
				namelist[0]);
	...
	if (colmod != 0) {
		...
		if (mp >= &message[msgCount]) {
			...
			printf((catgets(nl_fn,NL_SETN,8, "No messages satisfy")));
			for (colp = &coltab[0]; colp->co_char; colp++)
				if (colp->co_bit & colmod)
...
check(mesg, f)
{
	...
	if (mesg < 1 || mesg > msgCount) {
		printf((catgets(nl_fn,NL_SETN,9, "%d: Invalid message number\n")), mesg);
		return(-1);
	}
	mp = &message[mesg-1];
	if ((mp->m_flag & MDELETED) != f) {
		printf((catgets(nl_fn,NL_SETN,10, "%d: Inappropriate message\n")), mesg);
		return(-1);
	}
	return(0);
}
	...
	char **argv;
{
	...
	*ap = NOSTR;
	if (*cp != '\0') {
		printf((catgets(nl_fn,NL_SETN,11, "Warning:  only first %d arguments of %s command processed.\n")),MAXARGC-1,com->c_name);
	}
	return(ap-argv);
}
	...
	char **sp;
{
	...
	if (quotec && c == 0)
		fprintf(stderr, (catgets(nl_fn,NL_SETN,12, "Missing %c\n")), quotec);
	*sp = --cp;
	*cp2 = '\0';
	return(TSTRING);
}
...
{
	if (++regretp >= REGDEP)
		panic((catgets(nl_fn,NL_SETN,13, "Too many regrets")));
...
mark(mesg)
{
	...
	i = mesg;
	if (i < 1 || i > msgCount)
		panic((catgets(nl_fn,NL_SETN,14, "Bad message number to mark")));
...
unmark(mesg)
{
	...
	i = mesg;
	if (i < 1 || i > msgCount)
		panic((catgets(nl_fn,NL_SETN,15, "Bad message number to unmark")));
	message[i-1].m_flag &= ~MMARK;
}
...
metamess(meta, f)
{
	...
	case '+':
		...
		for (mp = dot + 1; mp < &message[msgCount]; mp++)
			if ((mp->m_flag & MDELETED) == f)
				return(mp - &message[0] + 1);
		printf((catgets(nl_fn,NL_SETN,17, "Referencing beyond last message\n")));
		return(-1);
	...
	case '-':
		...
		for (mp = dot - 1; mp >= &message[0]; mp--)
			if ((mp->m_flag & MDELETED) == f)
				return(mp - &message[0] + 1);
		printf((catgets(nl_fn,NL_SETN,18, "Referencing before first message\n")));
		return(-1);
	...
	case '$':
		...
		for (mp = &message[msgCount-1]; mp >= &message[0]; mp--)
			if ((mp->m_flag & MDELETED) == f)
				return(mp - &message[0] + 1);
		printf((catgets(nl_fn,NL_SETN,19, "No applicable messages\n")));
		return(-1);
	...
	case '.':
		...
		if ((dot->m_flag & MDELETED) != f) {
			printf((catgets(nl_fn,NL_SETN,20, "%d: Inappropriate message\n")), m);
			return(-1);
		}
		return(m);

	default:
		printf((catgets(nl_fn,NL_SETN,21, "Unknown metachar (%c)\n")), c);
		return(-1);
	}
}
