...
netmap(name, from)
	char name[], from[];
{
	char nbuf[GENBUFSIZ], ret[GENBUFSIZ];
	register char *cp;
	char *unuucp();
	...
	char str[];
{
	...
	char buf[GENBUFSIZ], path[GENBUFSIZ];
	register int c, host;
	...
	char from[];
{
	...
	if (cp == NOSTR)
		cp = rindex(name, '%');
	if (cp == NOSTR) {
		fprintf(stderr, (nl_msg(1, "Somethings amiss -- no @ or % in arpafix\n")));
		return(name);
	}
	cp++;
	arpamach = netlook(cp, '@');
...
#define	XHSIZE		50		/* Size of extra hash table */
...
minit()
{
	...
	midfree = 0;
	tp = &xtab[0];
	for (xp = &xtrahash[0]; xp < &xtrahash[XHSIZE-1]; xp++) {
		xp->xh_name = NOSTR;
		xp->xh_mid = 0;
		xp->xh_attnet = 0;
	...
	char name[];
{
	...
	if (xp == (struct xtrahash *) 0) {
		printf((nl_msg(2, "Ran out of machine id spots\n")));
		return(0);
	}
	if (xp->xh_name == NOSTR) {
		if (midfree >= XHSIZE) {
			printf((nl_msg(3, "Out of machine ids\n")));
			return(0);
		}
		xtab[midfree] = xp;
...
mlook(mid)
{
	...
	m = mid & 0177;
	if (m >= midfree) {
		printf((nl_msg(4, "Use made of undefined machine id\n")));
		return(NOSTR);
	}
...
mtype(mid)
{
	...
	m = mid & 0177;
	if (m >= midfree) {
		printf((nl_msg(5, "Use made of undefined machine id\n")));
		return(0);
	}
...
optim(net, name)
	char net[], name[];
{
	char netcomp[GENBUFSIZ], netstr[40], xfstr[40];
	register char *cp, *cp2;
	register int c;
	...
	for (;;) {
		...
		*cp2 = 0;
		...
		if ((c = netlook(netcomp, *cp)) == 0) {
			printf((nl_msg(6, "No host named \"%s\"\n")), netcomp);
err:
			strcpy(name, net);
			return;
		}
	...
	if (strlen(netcomp) == 0) {
		printf((nl_msg(7, "net name syntax\n")));
		goto err;
	}
	...
	while (*cp) {
		if ((cp2 = netname(*cp++)) == NOSTR) {
			printf((nl_msg(8, "Made up bad net name\n")));
			printf((nl_msg(9, "Machine code %c (0%o)\n")), cp[-1], cp[-1]);
			printf((nl_msg(10, "Sorry.\n")));
			goto err;
		}
		strcat(name, cp2);
		stradd(name, *cp++);
	...
	char net[];
{
	...
	if (strlen(cp) == 0)
		return;
	if ((strlen(cp) % 2) != 0) {
		printf((nl_msg(11, "Strange arg to optiboth\n")));
		return;
	}
	while (*cp) {
		cp2 = rpair(cp+2, *cp);
...
best(src, dest)
{
	...
	fflush(stdout);
	if (stype == 0 || dtype == 0) {
		printf((nl_msg(12, "ERROR:  unknown internal machine id\n")));
		return(0);
	}
	if ((stype & dtype) == 0)
		return(0);
