...
register char *s;
{
	...
	if ((vp2 = lookup(s)) == NOVAR) {
		if (!sourcing) {
			printf((catgets(nl_fn,NL_SETN,1, "\"%s\": undefined variable\n")), s);
			return(1);	
		}
		return(0);
	}
	...
	char str[];
{
	...
	if ( (top=calloc(strlen(str)+1, 1)) == NULL )
		panic((catgets(nl_fn,NL_SETN,2, "Out of memory")));
	cp = top;
	cp2 = str;
	while (*cp++ = *cp2++)
		;
...
struct grouphead *prev_gh;  /* Pointer to previous grouphead */
	...
	char name[];
{
	...
	if ((gh = findgroup(name)) == NOGRP) {
		printf((catgets(nl_fn,NL_SETN,3, "\"%s\": not a group\n")), name);
		return;
	}
	printf("%s\t", gh->g_name);
