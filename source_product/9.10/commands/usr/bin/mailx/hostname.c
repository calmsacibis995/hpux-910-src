...
inithost()
{
	...
	for (np = netmach; np->nt_machine != 0; np++)
		if (strcmp(np->nt_machine, EMPTY) == 0)
			break;
	if (np->nt_machine == 0) {
		printf((catgets(nl_fn,NL_SETN,1, "Cannot find empty slot for dynamic host entry\n")));
		exit(1);
	}
	np->nt_machine = host;
	np++;
...
struct utsname *uptr;
{
	...
	fd = fopen(HDRFILE, "r");
	if (fd == NULL) {
		fprintf(stderr, (catgets(nl_fn,NL_SETN,2, "Cannot open %s\n")), HDRFILE);
		exit(1);
	}
	
	for (;;) {	/* each line in the file */
		if (fgets(buf, sizeof buf, fd) == NULL) {
			fprintf(stderr, (catgets(nl_fn,NL_SETN,3, "no sysname in %s\n")), HDRFILE);
			exit(2);
		}
