	...
	struct name *names;
{
	...
	/* Sendmail does the mailability check because its smarter
	   than we can be.  So does the Bell mailer, but it puts 
	   dead.letter in the current directory rather than HOME.
	   We thus define SENDMAIL and don't do the check.
	*/
#ifdef DELIVERMAIL
	return(names);
#else
	...
	while (np != NIL) {
		...
		while (*cp == '\\')
			cp++;
		strcpy(username, MAILNAME);
		strcat(username, cp);
		if ((fi = fopen(username, "r")) != NULL) {
			if (fread(username, 1, FRWRDLEN, fi) == FRWRDLEN &&
			strncmp(username, FORWARD, FRWRDLEN)==0) {
				fclose(fi);
				np = np->n_flink;
				continue;
			}
...
/*
 * check for names which are files or pipes, and just drop them off
 * the list.  (Silently; it's for rmail.)
 */
	...
	struct name *names;
{
	...
	register struct name *np, *top, *x, *t;
	...
	struct name *np;
{
	...
	/* we first try setting argv[0] to "sendmail" for DELIVERMAIL
	 * or the user defined delivery agent; if that agent doesn't exist,
	 * we'll set argv[0] to "rmail" for the default mailer MAIL
	 * (see send.c)
	 */
	*ap++ = "sendmail";
...
/* 
 * Strip only localname from address 
 */ 
char *
do_allnet(address) 
	register char *address;
{
	...
	strcpy(adr, address);
	if ((n = charcount(adr,'@')) >= 2 || (m = charcount(adr,'%')) >= 2)
	return(address);
	if ((cp = rindex(adr,'!')) != NULL)
		cp++;
	else
		cp = adr;
	if ((cp1 = rindex(adr,'@')) != NULL)
		*cp1 = 0; 
	if ((cp2 = rindex(adr,'%')) != NULL)
 		*cp2 = 0;
	if ((ad = rindex(cp,'(')) != NULL)
		cp = ++ad;
	if ((ad = rindex(cp,')')) != NULL)
		*ad = 0;
	return(cp);
