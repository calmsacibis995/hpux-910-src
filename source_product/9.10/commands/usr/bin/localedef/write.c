...
int mod;
{
	...
	for (i = 0; i < sizeof(lcmesg_head); i++)
		(void) fprintf(fp, "%c", ((char *)&lcmesg_head)[i]);

	(void) fprintf(fp, "%s", info_tab[YESEXPR]);
	(void) fprintf(fp, "%c", (char)null);
	(void) fprintf(fp, "%s", info_tab[NOEXPR]);
	(void) fprintf(fp, "%c", (char)null);
	 (void) fprintf(fp, "%s", info_tab[YESSTR]);
	 (void) fprintf(fp, "%c", (char)null);
	  (void) fprintf(fp, "%s", info_tab[NOSTR]);
	...
	for (i = cmlog[LC_MESSAGES].cm[mod].pad; i; i--)
		(void) fprintf(fp, "%c", (char)null);
}
/* plc_coll
...
int mod;
{
	...
	/* Posix 11.2 for __ctype2  table */
	for (i = 0; i < CTYPESIZ; i++)
		(void) fprintf(fp, "%c", ctype5[i]);
	...
	(void) fprintf(fp, "%s", info_tab[CODE_SCHEME]);
	(void) fprintf(fp, "%c", (char)null);

	(void) fprintf(fp, "%s", info_tab[CSWIDTH]);
	(void) fprintf(fp, "%c", (char)null);
#endif /* EUC */
...
int mod;
{
	...
	/* Posix 11.2 */
	(void) fprintf(fp, "%s", info_tab[T_FMT_AMPM]);
        (void) fprintf(fp, "%c", (char)null);        
