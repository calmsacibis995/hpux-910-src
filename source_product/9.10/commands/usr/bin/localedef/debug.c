...
/* dlc_messages:
** write LC_MESSAGES category information into the 'debug' file.
*/
void
dlc_messages()
{
#ifdef MESGDBG
	(void) fprintf(dfp, "\nLC_MESSAGES HEADER\n");
	(void) fprintf(dfp, "yes_addr: %d\n", lcmesg_head.yes_addr);
	(void) fprintf(dfp, "no_addr: %d\n", lcmesg_head.no_addr);
	(void) fprintf(dfp, "yesstr_addr: %d\n", lcmesg_head.yesstr_addr);
	(void) fprintf(dfp, "nostr_addr: %d\n", lcmesg_head.nostr_addr);

	(void) fprintf(dfp, "\nLC_MESSAGES DATA\n");
	(void) fprintf(dfp, "yesexpr: %s\n", info_tab[YESEXPR]);
	(void) fprintf(dfp, "noexpr: %s\n", info_tab[NOEXPR]);
	(void) fprintf(dfp, "yesstr: %s\n", info_tab[YESSTR]);
	(void) fprintf(dfp, "nostr: %s\n", info_tab[NOSTR]);
#endif
}
...
dlc_ctype()
{
	...
	(void) fprintf(dfp, "ctype2\n");
	for (i = 1; i < CTYPESIZ; ) {
		for (j = 0; j < 16; j++)
			(void) fprintf(dfp, "%x ",ctype5[i++]);
		(void) fprintf(dfp, "\n");
	}
	(void) fprintf(dfp, "\nKANJI1\n");
	(void) fprintf(dfp, "%x\n",ctype3[0]);
...
dlc_mntry()
{
	...
	(void) fprintf(dfp, "\nMONETARY DATA\n");
	(void) fprintf(dfp, "int_frac_digits: ");
	if ( *mntry_tab[INT_FRAC] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[INT_FRAC]);
	(void) fprintf(dfp, "frac_digits: ");
	if ( *mntry_tab[FRAC_DIGITS] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[FRAC_DIGITS]);
	(void) fprintf(dfp, "p_cs_precedes: ");
	if ( *mntry_tab[P_CS] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[P_CS]);
	(void) fprintf(dfp, "p_sep_by_space: ");
	if ( *mntry_tab[P_SEP] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[P_SEP]);
	(void) fprintf(dfp, "n_cs_precedes: ");
	if ( *mntry_tab[N_CS] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[N_CS]);
	(void) fprintf(dfp, "n_sep_by_space: ");
	if ( *mntry_tab[N_SEP] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[N_SEP]);
	(void) fprintf(dfp, "p_sign_posn: ");
	if ( *mntry_tab[P_SIGN] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[P_SIGN]);
	(void) fprintf(dfp, "n_sign_posn: ");
	if ( *mntry_tab[N_SIGN] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[N_SIGN]);
	(void) fprintf(dfp, "currency_symbol_lc: ");
	if ( *mntry_tab[CURRENCY_LC] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
		(void) fprintf(dfp, "%d\n",*mntry_tab[CURRENCY_LC]);
	(void) fprintf(dfp, "currency_symbol_li: ");
	if ( *mntry_tab[CURRENCY_LI] == CHAR_MAX )
		(void) fprintf(dfp, "-1\n");
	else
	...
	(void) fprintf(dfp, "mon_grouping: ");
	for (i=0;i<strlen(mntry_tab[MON_GROUPING]);i++) 
	    if ( *(mntry_tab[MON_GROUPING]+i) != CHAR_MAX )
	      (void) fprintf(dfp, "%d;", *(mntry_tab[MON_GROUPING]+i));
	    else
	      (void) fprintf(dfp, "-1");
	(void) fprintf(dfp,"\n");
...
dlc_nmrc()
{
	...
	(void) fprintf(dfp, "\nNUMERIC DATA\n");
	(void) fprintf(dfp, "grouping: ");
	for (i=0;i<strlen(nmrc_tab[GROUPING]);i++) 
	    if ( *(nmrc_tab[GROUPING]+i) != CHAR_MAX )
	      (void) fprintf(dfp, "%d;", *(nmrc_tab[GROUPING]+i));
	    else
	      (void) fprintf(dfp, "-1");
	(void) fprintf(dfp,"\n");
...
dlc_time()
{
	...
	/* Posix 11.2 */
	(void) fprintf(dfp, "t_fmt_ampm: %d\n", lctime_head.t_fmt_ampm);
