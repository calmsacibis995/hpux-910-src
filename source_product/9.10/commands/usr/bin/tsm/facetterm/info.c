...
** All Rights Reserved.							    **
*****************************************************************************/
/* @(#) $Header: info.c,v 70.1 92/03/09 15:44:49 ssa Exp $ */
#include "wins.h"
#include "ptypath.h"
#ifndef FACET
#include "printer.h"
	...
	int	max;
{
	...
	blank_trim( in );
	wants_scrambled = remove_scramble_string( in, &method, &key );
	...
	else if ( strcmp( in, "no_prompt_hotkey" ) == 0 )
	{
		if ( Opt_no_prompt_hotkey < 0 )
			strcpy( out, "none" );
		else if ( Opt_no_prompt_hotkey < ' ' )
			sprintf( out, "^%c", Opt_no_prompt_hotkey + 0x40 );
		else if ( Opt_no_prompt_hotkey > 0x7F )
			sprintf( out, "\\%03o", Opt_no_prompt_hotkey );
		else
			sprintf( out, "%c", Opt_no_prompt_hotkey );
	}
	else if ( strcmp( in, "termio" ) == 0 )
	{
		window_termio_in_hex( out );
	}
	else if ( strcmp( in, "ltchars" ) == 0 )
	{
		window_ltchars_in_hex( out );
	}
							/* not FACET */
#endif
	else
	...
	if ( wants_scrambled )
	{
		...
		scrambled = scramb( out, method, key );
		strncpy( out, scrambled, max - 1 );
		out[ max - 1 ] = '\0';
	}
}
