...
while (c=get1char())
	...
	switch(c)
		...
		default:
			if (c != tab)
			error((nl_msg(4, "bad table specification character")));
		...
		case '.': /* end of table specification */
			if (lefline[icol][nclin]>0) {icol++; rightl++;};
			ncol = max(ncol, icol);
			if(sawchar)
				nclin++;
			if (nclin>=MAXHEAD)
				error((nl_msg(5, "too many lines in specification")));
			icol=0;
			if (ncol==0 || nclin==0)
				error((nl_msg(6, "no specification")));
			if (c== '.')
				{
				while ((c=get1char()) && c != '\n')
					if (c != ' ' && c != '\t')
						error((nl_msg(7, "dot not last character on format line")));
				...
				if (oncol == 0)
					...
					error((nl_msg(8, "tried to widen table in T&, not allowed")));
				return;
				}
			sawchar=0;
			continue;
		...
		case 'c': case 's': case 'n': case 'r': case 'l':  case 'a':
			style[icol][nclin]=c;
			if (c== 's' && icol<=0)
				error((nl_msg(9, "first column can not be S-type")));
			if (c=='s' && style[icol-1][nclin] == 'a')
				{
				fprintf(tabout, ".tm ");
				fprintf(tabout, (nl_msg(10, "warning: can't span a-type cols, changed to l\n")));
				style[icol-1][nclin] = 'l';
				}
			if (c=='s' && style[icol-1][nclin] == 'n')
				{
				fprintf(tabout, ".tm ");
				fprintf(tabout, (nl_msg(11, "warning: can't span n-type cols, changed to c\n")));
				style[icol-1][nclin] = 'c';
				}
			icol++;
			if (c=='^' && nclin<=0)
				error((nl_msg(12, "first row can not contain vertical span")));
			if (icol>qcol)
				error((nl_msg(13, "too many columns in table")));
			sawchar=1;
			continue;
		case 'b': case 'i': 
			c += 'A'-'a';
		...
		case 'f': case 'F':
			...
			if (stopc) if (get1char()!=stopc)
				error((nl_msg(14, "Nonterminated font name")));
			continue;
		case 'P': case 'p':
			if (icol<=0) continue;
			...
			while (c = get1char())
				...
				if (snp-temp>4)
					error((nl_msg(15, "point size too large")));
				}
			*snp = 0;
			if (atoi(temp)>36)
				error((nl_msg(16, "point size unreasonable")));
			un1getc (c);
			continue;
		case 'V': case 'v':
			if (icol<=0) continue;
			...
			while (c = get1char())
				...
				if (snp-temp>4)
					error((nl_msg(17, "vertical spacing value too large")));
				}
			*snp=0;
			un1getc(c);
			continue;
		...
		/* end commented out code ... */
			...
			*snp=0;
			if (snp-cll[icol-1]>CLLEN)
				error ((nl_msg(18, "column width too long")));
			if (!stopc)
				un1getc(c);
			continue;
		case 'e': case 'E':
...
error((nl_msg(19, "EOF reading table specification")));
}
findcol()
{
# define FLNLIM 1600
/* this counts the number of columns for all spec lines, */
/* then puts these lines back and returns the largest number of columns. */
char *s, line[FLNLIM+2], *p=line;
int c, n=0, inpar=0, maxn=0;
...
for(s=line; *s = c = get1char(); s++)
	...
	if (c=='\n' || c == EOF || c=='.' || c==',') {
		...
		if (s>=line+FLNLIM)
			error((nl_msg(20, "too long spec line")));
	}
}
garray(qcol)
{
...
x = calloc(a,b);
if (x==0) 
	error((nl_msg(21, "Couldn't get memory")));
return(x);
}
freearr()
{
