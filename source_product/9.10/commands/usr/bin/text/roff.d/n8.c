...
#ifdef NLS16
	if ( j&BMASK2ND ) { 
		return(1);	/* 16 bit code is always "alpha" */
	} else {
		j |= (j&FLAG8) >> 17;
		j &= 0377;
		return( isalpha(j) );
...
#ifdef NLS16
/* This function maps the specified char "c" into its lower case */
/* and returns the mapped char.					 */
nl_maplow(c)
int c;
{
	int i;
...
casehw(){
	register i, k;
#ifdef NLS16
	/* To store 8/16-bit code in "j" string */
	register int *j;
#else
	register char *j;
#endif
	...
	while(!skip()){
		...
		full:
			prstr((nl_msg(52, "Exception word list full.\n")));
			*nexth = 0;
			return;
		}
		while(1){
...
#ifdef NLS16
		while((*e && (w <= hyend)) &&
		      ((*e & (CMASK&~0200)) == maplow(*w & CMASK))){e++; w++;};
#else
		while((*e && (w <= hyend)) &&
...
#ifdef NLS16
	if ( i&BMASK2ND ) return(0);
	else if ( i&FLAG8 ) return(0);
	else if(i < 'a') i -= 'A'; 
	else i -= 'a';
#else
	if(i < 'a')i -= 'A'; else i -= 'a';
...
#ifdef NLS16
	i = t[(maplow(a)-'a')&0177][(j=(maplow(b)-'a')&0177)/2];
#else
