...
setov(){
	...
	/* if more than 9 overstriking chars.,
	   write error message and continue  -jh */
	if(k==NOV) prstr((nl_msg(54, "Too many overstriking characters.\n")));
	o[k] = w[k] = 0;
	if(o[0])
	    do {
		j = 0;
...
int x;
{
	...
	for(j=0;1;j++){
		if((tabtab[j] & TMASK)== 0){
			if(x==savfc)prstr((nl_msg(53, "Zero field width.\n")));
			j = 0;
			goto rtn;
		}
