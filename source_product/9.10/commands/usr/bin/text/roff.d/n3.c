...
#ifdef NLS16
/* 32 bit fields are used to express one char. */
typedef unsigned int tint;	/* type of characters stored in tmp file */
#else
...
casern(){
	...
	if (j)	{
		if (freenslot == -1)	{
		    if (i == -1)	{
			prstr((nl_msg(30, "too many names for rn")));
			ferrex();	}
		    freenslot = i;	}
...
int mn;
{
	...
	if(app && (oldmn >= 0) && (nametab[oldmn].ename & MMASK)){
		...
		if ((i == -1) || (i == oldmn) || !alloc())	{
			...
			prstr((nl_msg(31, "Too many string/macro names.\n")));
			edone(04);
			offset.o = 0;
			return(offset.b = 0);
		}
...
copyb()
{
	...
	j &= BMASK;
#ifdef NLS16
	/* Convert 8-bit code intto internal one to compare them */
	/* with chars gotten later.				 */
	j |= (j&0200)<<17;
	j &= ~0200;
	k |= (k&0200)<<17;
	k &= ~0200;
...
filep alloc()
{
	...
	if (i == NBLIST+1) i = 0;
			 ...
			 if ((memblist[i] =
				...
				prstr((nl_msg(32, "out of memory for store")));
				exit(1);	}
#endif
			}
	nextb.o = 0;
...
int i;
{
	...
	if (!(offset.o = ++offset.o & (BLK - 1)))	{
		...
		if(blist[offset.b] == (filep)-1){
			if(alloc() == 0){
				prstr((nl_msg(33, "Out of temp file space.\n")));
				done2(01);	}
...
wbfl(){
	...
	/* memcpy(memblist[woff.b]+woff.o,wbuf.wnt,wbfi*sizeof(tint)); */
#endif
	for (i=0; i<NB; i++)
	    if (woff.b == roff[i])	{
		for (j=0; j<wbfi; j++)
...
filep blk;		/* find a place in the cache and get a block */
{
	...
	if (read(ibf, rbuf.rp[i], BLK * sizeof(tint)) == 0)	{
		prstr((nl_msg(34, "Macro read failed\n")));
		ferrex();	}
#else
	bp = memblist[blk];
	for (j=0; j<BLK; j++)
		rbuf.rnt[i][j] = *bp++;	/* get buffer */
	/* memcpy(rbuf.rnt[i],memblist[blk],BLK*sizeof(tint)); */
#endif
	roff[i] = blk;
	return i;
}
errbsa()
{
	prstr((nl_msg(35, "Bad storage allocation\n")));
	done2(-5);
}
popi(){
	register struct s *p;
...
filep newip;
{
	...
	if (argstrp < NMACS)
		...
		prstr((nl_msg(36, "too many macro calls")));
		exit(1);	}
	...
	if ((nxf = (struct s *)malloc(sizeof(struct s))) == NULL) {
		prstr((nl_msg(37, "out of memory for macro call")));
		exit(1);	}
#endif
	maclev++;
	macbcnt = 0;
...
#define ALIGN(addr,grain)  ((addr+sizeof(grain)-1) & ~(sizeof(grain)-1))
	...
	if((i = sbrk(x)) == (char *)-1){
		prstrfl((nl_msg(38, "Core limit reached.\n")));
		edone(0100);	}
	    else enda = i;
...
collect()
{
	...
	if ((argpp = (int **)malloc(MARGL*sizeof(int))) == NULL) {
		prstr((nl_msg(39, "out of memory for macro call")));
		exit(1);	}
	hsstrp = (int *)argpp;
	...
	while((argpp != argppend.dbl) && (!skip())){
		*argpp++ = strp;
		...
		while(1){
			...
			*strp++ = i;
			if(strflg && (strp >= lim)){
				prstrfl((nl_msg(40, "Macro argument too long.\n")));
				copyf--;
				edone(004);
			}
#ifndef INCORE
...
casepm(){
	...
	for(i = 0; i<NM; i++){
		...
		if(!tot){
#ifdef NLS16
			/* Prevent char from being masked on 8 bit */
			*p++ = xx & 0377;
			if(!(*p++ = (xx >> BYTE) & 0377))*(p-1) = ' ';
#else
			*p++ = xx & 0177;
