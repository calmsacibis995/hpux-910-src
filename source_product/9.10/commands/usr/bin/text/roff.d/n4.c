...
extern int nlsbs1;
extern int nlsbs2;
extern int *nlsexpbuf1;
extern int *nlsexpbuf2;
#endif
setn()
{
	register i,j;
	...
	if((i & 0177) == '.')switch(i>>BYTE){
		...
		case 'z': i = dip->curd;
			...
			cbuf[2] = 0;
#ifdef NLS16
			/* Convert 8-bit code type into internal one	*/
			/* because 8-bit macro name is registered as	*/
			/* common 8-bit code.				*/
			cbuf[0] |= (cbuf[0]&0200) << 17;
			cbuf[1] |= (cbuf[0]&0200) << 17;
			cbuf[0] &= ~0200;
			cbuf[1] &= ~0200;
#endif
			setcp(cbuf);
...
int i;
{
	...
	if(!numerr)prstrfl((nl_msg(42, "Too many number registers.\n")));
...
#ifdef NLS16
		/* To support 2 more user-defined numeric expression */
		/* n/N in.af.					     */
		case 'n':
		case 'N': return(nlsexp(i,f) + j );
#endif
	}
}
#ifdef NLS16
/* Following function is prepared for making user-defined numeric */
/* expression according to the specified value in "i".		  */
nlsexp(i,f)
int i, (*f)();
{
	int j, k;
	...
	int delimnum;
	if ( nform == 'n' ) {
		nlsexpbuf=nlsexpbuf1;
		nlsbs=nlsbs1;
	} else {
		nlsexpbuf=nlsexpbuf2;
		nlsbs=nlsbs2;
	}
	
	if ( !nlsbs ) {
		/* When no message catalog is specified. */
		nform=1;
		return( decml(i,f) );
	}
	if ( j=i/nlsbs ) {
	...
	j=i%nlsbs;
	delimnum=0;
	for(p=nlsexpbuf+1; *p && delimnum!=j ; p++ ) {
		if ( *p==nlsexpbuf[0] ) delimnum++;		
	}
	for(; *p && *p!=nlsexpbuf[0]; p++ ) {
		j = (*f)( *p | nrbits );
		if ( j==0 ) break;
	}
}
...
a0:
	...
	switch (ch_CMASK)	{
		...
		case '/':
			...
			if(nonumb)break;
			if(i == 0){
				prstrfl((nl_msg(43, "Divide by zero.\n")));
				acc = 0;
			}else acc /= i;
			goto a0;
		case '%':
...
#ifdef NLS16
		case 'N':	/* Add n/N numeric expression in .af */
		case 'n':
#endif
		case 'A':
		case 'a':
