...
casefi(){
#ifdef NLS16
	int i, j;
	/* To support formatting restriction. */
	while (1) {
		if ( skip() ) break;
		getch();
		switch(ch_CMASK) {
		case 'p':
			/* Register prefix formatting restriction. */
			i=setchar(prbuf,PRRN);
			break;
		case 's':
			/* Register suffix formatting restriction. */
			i=setchar(subuf,SURN/2);
			break;
		case 't':
			/* Register terminal restriction. */
			i=setchar(tebuf,TERN);
			break;
		case 'e':
			/* Register extended terminals restriction. */
			i=setchar(exbuf,EXRN);
			break;
		default:
			eat(' ');
			ch=ch_CMASK;
	...
	for (i=0; i < SURN && subuf[i]; i++);	/* find end of string */
	for (j=0; i < SURN && j < TERN && tebuf[j]; i++, j++)
		subuf[i] = tebuf[j];
	subuf[i] = 0;
#endif
	tbreak();
...
#ifdef NLS16
/* Call getch() until first returned char is detected again.	*/
setchar(s, len)
int *s, len;
{
	int i, delim;
	delim=getch()&CMASK;
	if (delim==' '||delim=='\n') return(0);
	for(i=0; i<len && (getch()&CMASK)!=delim; i++ ) {
		s[i]=ch_CMASK;
	}
	if (i==len) eat(delim);
	s[i]=0;		/* Set null terminator */
	return(i);
}
#endif
casenf(){
...
casewh(){
	...
	if(k == NTRAP){
		prstrfl((nl_msg(44, "Cannot plant trap.\n")));
		return;
	}
	mlist[k] = j;
	nlist[k] = i;
...
casetm(x) int x;{
	...
	lgf++;
	copyf++;
	if(skip() && x)prstrfl((nl_msg(45, "User Abort.")));
#ifdef NLS16
	for(i=0; i<NTM-2;i++) {
		tmbuf[i]=getch();
		if( ch_CMASK == '\n' ) {
			i++;
			break;
		} else if ( ch_CMASK == ('}' & ~CMASK | RIGHT) ){
			/* don't output masked \}  -jh */
			i--;
			continue;
		} else {
			/* Convert char type from internal into common  */
			/* to output on terminal.			*/
			tmbuf[i] |= (ch_CMASK&FLAG8) >> 17;
			if ( ch_CMASK&BMASK2ND ) {
				tmbuf[i+1]=(ch_CMASK&BMASK2ND) >> 16;
				i++;
			}
		}
	}
...
e0:
		...
		if((nxev >= NEV) || (nxev < 0) || (evi >= EVLSZ)){
			prstrfl((nl_msg(46, "Cannot do ev_\n")));
...
caseie(){
	if(ifx >= NIF){
		prstr((nl_msg(47, "if-else overflow.\n")));
		ifx = 0;
		edone(040);
	}
	caseif(1);
...
rdtty(){
#ifdef NLS16
	/* Prevent returned 8-bit value of rdtty() is	*/
	/* considered as minus.				*/
...
casetr(){
	...
	while (getch() && ((i = ch_CMASK) != '\n')) {
		...
		if (ch_CMASK == '\n') ch_CMASK = ' ';
#ifdef NLS16
		/* To support 8-bit code on .tr	*/
		if ( i&BMASK2ND || ch_CMASK&BMASK2ND ) {
		} else if ( i&FLAG8 ) {
			trtab[(i&0177)+256] = ch_CMASK;
		} else {
			trtab[i] = ch_CMASK;
		}
#else
...
casemc(){
	...
	/* clear ic immediately if no words on current line */
	if (icf > 1 && nwd == 0 && wch == 0) ic = 0;
	icf = 0;
	if(skip())return;
	ic = getch();
	icf = 1;
