...
int c;
{
	...
	switch(j = c & CMASK){
		...
		default:
			...
			if( j&(CMASK&~BMASK&~MOT) ) {
				if ( !(j&BMASK2ND) ) {
					c = (trtab[(j&0177)+256]&CMASK) | (c&~CMASK);
				} else {
					c = (j&CMASK) | (c&~CMASK);
				}
			} else {
				c = (trtab[j] & CMASK) | (c & ~CMASK);
			}
#else
...
flusho(){
	...
	if(!ptid){
		while((ptid=open(ptname,1)) < 0){
			if(++waitf <=2)prstr((nl_msg(26, "Waiting for Typesetter.\n")));
			sleep(15);
		}
	}
#endif
...
#ifndef tso
		if(stop && (ptid != 1)) prstr((nl_msg(27, "Troff finished.\n")));
#endif
	}
#endif
	flusho();
...
casepi(){
	...
	if(toolate || skip() || !getname() || (pipe(id) == -1) ||
	   ((i=fork()) == -1)){
		prstr((nl_msg(28, "Pipe not created.\n")));
		return;
	}
	ptid = id[1];
	if(i>0){
