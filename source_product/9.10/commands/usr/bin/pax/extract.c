/* $Source: /misc/source_product/9.10/commands.rcs/usr/bin/pax/extract.c,v $
 *
 * $Revision: 66.3 $
 *
 ...
 *
 * $Log:	extract.c,v $
 * Revision 66.3  90/11/27  04:49:36  04:49:36  kawo
 * changes done for cnode handling
 * 
 * Revision 66.2  90/07/10  08:26:55  08:26:55  kawo (#Wolfgang Kapellen)
 * Changes for cnode specific device files:
 *  - New function cnode_swap().
 *  - Calls to cnode_swap() from: inswab(), inascii(), inbinary().
 * 
 * Revision 66.1  90/05/11  08:23:48  08:23:48  michas (#Michael Sieber)
 * inital checkin
 * 
 * Revision 2.0.0.4  89/12/16  10:35:13  mark
 ...
 * 
 * Revision 2.0.0.3  89/10/13  02:34:52  mark
 * Beta Test Freeze
 * 
 */

#ifndef lint
static char        *ident = "$Id: extract.c,v 2.0.0.4 89/12/16 10:35:13 mark Exp Locker: mark $";
...
static void	    cnode_swap P((Stat *));
static int 	    inbinary P((char *, char *, Stat *));
static int	    inascii P((char *, char *, Stat *));
static int	    inswab P((char *, char *, Stat *));
static int	    readtar P((char *, Stat *));
static int	    readcpio P((char *, Stat *));
...
get_header(name, asb)
    ...
    Stat               *asb;
{
    DBUG_ENTER("get_header");
    if (ar_format == TAR) {
	DBUG_RETURN(readtar(name, asb));
    } else {
	DBUG_RETURN(readcpio(name, asb));
    }
}
...
/* cnode_swap - swaps values in structure stat if cnode specific device files
 *		are supported
 *
 * DESCRIPTION
 *
 *	Checks if a cnode specific device file has to be restored.
 *
 * PARAMETERS
 *
 *	Stat	*asb	- stat block for the file (modified and passed back
	...
	if ( (S_ISCHR(asb->sb_mode))  ||
	    ...
	    if ( ((asb->sb_rdev >= 0)  &&
		 ...
		 (asb->sb_rdev == LOCALMAGIC) ) {
	        asb->sb_rcnode	= (cnode_t)asb->sb_rdev;
	        asb->sb_rdev	= (dev_t)  asb->sb_size;
	        asb->sb_size	= (off_t)  0;
	    }
#endif
}
