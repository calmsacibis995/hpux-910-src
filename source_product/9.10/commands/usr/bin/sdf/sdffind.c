...
char **expr;
{
	int dp;
	struct stat statbuf;
	char buf[256], new_name[17];
	struct direct direct;
	void sdfdir2str(), process1();

	dp = sdfopen(path, 0);				/* open file/dir */
	if (dp == -1) {
	...
	while (sdfread(dp, (char *) &direct, sizeof(direct))==sizeof(direct)) {
		if (direct.d_ino == 0)
			continue;
		sdfdir2str(direct.d_name);
		strncpy(new_name, direct.d_name, DIRSIZ + 2);
		new_name[DIRSIZ + 2] = '\0';	/* seal off long name */
