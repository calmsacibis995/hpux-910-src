static char *RCS_ID="@(#)$Revision: 70.1 $ $Date: 91/11/07 10:49:15 $";
/*	@(#) $Revision: 70.1 $	*/
#ifndef NLS
...
#define USAGE	strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,882, "[-xNUM] [-uNUM]")))
...
char **envp;
{
	...
	while ((i = getopt(argc, argv, "u:x:")) != EOF) {
		...
		case 'u':
			...
			break;
		default:
			(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,883, "\tusage: %s %s\n"))),
			    Progname, USAGE);
			cleanup(1);
		}
	}
	if (argc != optind) {
		(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,884, "\tusage: %s %s\n"))), Progname, USAGE);
		cleanup(1);
	}

	DEBUG(9, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,885, "Progname (%s): STARTED\n"))), Progname);
	fp = fopen(LMTUUSCHED, "r");
	if (fp == NULL) {
		DEBUG(1, (catgets(nlmsg_fd,NL_SETN,886, "No limitfile - %s\n")), LMTUUSCHED);
	} else {
		(void) fscanf(fp, "%d", &maxnumb);
		(void) fclose(fp);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,887, "Uusched limit %d -- ")), maxnumb);
		...
		if (i == maxnumb) {
		    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,888, "found %d -- cleanuping\n ")), maxnumb);
		    cleanup(0);
		}
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,889, "continuing\n")), maxnumb);
	}
	...
	for (num=0, m=M; m->mach[0] != '\0'; m++, num++) {
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,890, "machine: %s, ")), M[num].mach);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,891, "ccount: %d\n")), M[num].ccount);
	}

	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,892, "Execute num=%d \n")), num);
	while (num > 0) {
	    snumber = (time((long *) 0) % (long) (num));  /* random num */
	    (void) strcpy(Rmtname, M[snumber].mach);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,893, "num=%d, ")), num);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,894, "snumber=%d, ")), snumber);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,895, "Rmtname=%s\n")), Rmtname);
	    (void) sprintf(lckname, "%s.%s", LOCKPRE, Rmtname);
	    if (checkLock(lckname) != FAIL && callok(Rmtname) == 0) {
		/* no lock file and status time ok */
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,896, "call exuucico(%s)\n")), Rmtname);
		exuucico(Rmtname);
	    }
	    else {
		/* system locked - skip it */
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,897, "system %s locked or inappropriate status--skip it\n")),
		    Rmtname);
	    }
...
char	*name;
{
	...
	namelen = strlen(name);
	DEBUG(9, (catgets(nlmsg_fd,NL_SETN,898, "machine(%s) called\n")), name);
...
char *name;
{
	...
	DEBUG(3, (catgets(nlmsg_fd,NL_SETN,900, "ret=%d, ")), ret);
	DEBUG(3, (catgets(nlmsg_fd,NL_SETN,901, "errno=%d\n")), errno);
}
