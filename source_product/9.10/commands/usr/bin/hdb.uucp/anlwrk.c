...
char *file, **wvec;
{
	...
	if (fgets(str, MAXRQST, fp) == NULL) {
		...
		(void) unlink(afile);
		DEBUG(4,(catgets(nlmsg_fd,NL_SETN,3, "Finished Processing file: %s\n")),file);
		*file = '\0';
		CLEAN_RETURN(fp);
	}
...
char *file, *wrkvec;
{
	...
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,5, "gtwvec: dir %s\n")), RemSpool);
	while ((nargs = anlwrk(file, wrkvec, wcount)) == 0) {
		if (!iswrk(file))
			return(0);
	}
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,6, "        return - %d\n")), nargs);
	return(nargs);
}
...
char *file;
{
	...
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,7, "insert(%s)  ")), file);
	for (i = Nfiles; i>0; i--) {
	...
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,8, "insert %s ")), p);
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,9, "at %d\n")), i);
	Fptr[i] = p;
	Nfiles++;
}
