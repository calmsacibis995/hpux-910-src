/*	@(#) $Revision: 72.2 $	*/
static char *RCS_ID="@(#)$Revision: 72.2 $ $Date: 93/03/25 11:12:34 $";
#ifndef NLS
...
/*	/sccs/src/cmd/uucp/s.uugetty.c
	uugetty.c	1.3	7/29/85 16:34:00
*/
#include	"uucp.h"
...
#include	<utmp.h>
/* AT&T undoc feature. HP doesn't support it now */
/* -Port */
...
struct Gdef {
	...
	struct termiox	gx_iflags;	/* initial h/w flow control flags */
	struct termiox	gx_fflags;	/* final h/w flow control flags */
};
...
#define	ISANE	(BRKINT|IGNPAR|ICRNL|IXON)  /* deleted ISTRIP from original
                                               source -Port    */
#define	OSANE	(OPOST|ONLCR)
#define	CSANE	(CS8|CREAD)                 /* Was CS7 and PARENB and CREAD
                                                   -Port       */
...
/* modem control modes - MCSETAW calls */
struct Symbols mmodes[] = {
	"CRTS", CRTS,
	NULL,	0
};

/* hardware flow control - TCSETX calls */

struct Symbols xmodes[] = {
	"RTSXOFF",	RTSXOFF,
	"CTSXON", 	CTSXON,
	NULL, 0
};
...
#define	LDISC0	0
#endif
#define TERM_NONE 0       /* Port to make uugetty compile */
...
/* does the hardware/driver support hardware flow control	*/

int	hwfctl=0;

/* does the hardware/driver support modem control	*/
...
char **argv;
{
	...
	register struct Symbols *answer;
        /* AT&T bug fixes brought over from getty.c. See getty.c for
           the details. */
	...
	struct termiox termiox;		/* For h/w flow control.	*/
	/* Again, unsupported.
	...
	/*	Get the initial modem control settings.	*/
...
/* timeout argument. */
				...
				if(isdigit(*(ptr = *++argv)))
					sscanf(ptr,"%d",&timeout);
				else error((catgets(nlmsg_fd,NL_SETN,838, "getty: timeout argument invalid. \"%s\"\n")), *argv);
			}
			break;
...
/* Check a "gettydefs" file mode. */
		case 'c':
			...
			if(--argc == 0) {
				fprintf(stderr,
				    (catgets(nlmsg_fd,NL_SETN,839, "Check Mode Usage: getty -c gettydefs-like-file\n")));
				exit(1);
			}
			check = TRUE;
			checkgdfile = *++argv;
...
/* Attempt to open the check gettydefs file. */
			if((fp = fopen(checkgdfile,"r")) == NULL) {
				fprintf(stderr,(catgets(nlmsg_fd,NL_SETN,840, "Cannot open %s\n")),checkgdfile);
				exit(1);
			}
			fclose(fp);
...
/* "init" from working too hard. */
	if(argc < 1) {
		error((catgets(nlmsg_fd,NL_SETN,841, "getty: no terminal line specified.\n")));
		sleep(20);
		exit(1);
	} else line = *argv;
...
/* of the "getty_defs" file as the initial settings. */
	if(--argc > 0 ) {
		if((speedef = find_def(*++argv)) == NULL) {
			error((catgets(nlmsg_fd,NL_SETN,842, "getty: unable to find %s in \"%s\".\n")),
			    *argv,GETTY_DEFS);
...
/* If a terminal type was supplied, try to find it in list. */
	/* Port.Unsupported. Just ignore input.
         if(--argc > 0) {
...
/* If a line discipline was supplied, try to find it in list. */
	/* Port. Unsupported. Just ignored user input.
	if(--argc > 0) {
...
#ifdef RAYDEBUG
	{
	   int junkfd1, junkfd2, junkfd3, junkfd4;

	   /* 
	    * open a bunch of files to force the fd value for the error
	    * to be larger than anything uugetty should use.
	    */ 
	   junkfd1=open("/etc/passwd",O_RDONLY);
	   junkfd2=open("/etc/group",O_RDONLY);
	   junkfd3=open("/etc/inittab",O_RDONLY);
	   junkfd4=open("/etc/checklist",O_RDONLY);
	   if ((debugfd=fopen("/tmp/uugetty.log","a+")) == NULL)
	   {
	      fprintf(stderr,"unable to open debug file \'/tmp/uugetty.log\' ");
	   }
	   fprintf(debugfd,"start up of uugetty\n");
	   fflush(debugfd);
	   close(junkfd1);
	   close(junkfd2);
	   close(junkfd3);
...
/* line. */
	chdir("/dev");
/* Blow away lockfiles that was created by the pervious uugetty process */
	    (void) sprintf(lckname, "%s.%s", LOCKPRE, line);
            (void) checkLock(lckname);
...
/* the protections to only allow uucp to read the line. */
	stat(line,&statb);
        pwd=getpwnam("uucp");
	chown(line,pwd->pw_uid,statb.st_gid); /* for uugetty use uucp uid */
	chmod(line,0622);
	...
	if ( (fopen(line, "r+")) == NULL) { /* this opens stdin */
		error((catgets(nlmsg_fd,NL_SETN,843, "getty: cannot open \"%s\". errno: %d\n")),line,errno);
		sleep(20);
		exit(1);
	}
	/*
	 *   don't close stderr until here.  If the fopen fails, "error"
	 *   will try to write to stderr.
	 */
	(void) fclose(stderr);
	(void) close(2);
        /*
	 *   Added 9-30-91 by Ray Spear
	 *   There was a problem were cu would connect to the remote host
	 *   and end of with a continuous stream of CR LF commin out on the
	 *   screen.  This appears to be an interaction problem between
	 *   uugetty, cu, and the 4 port mux driver.  My guess is the
	 *   line reconfigure code in uugetty somehow confused the daylights
	 *   out of the mux driver.  The following code was added to prevent
	 *   uugetty from doing a line configure if the line was own by 
	 *   someone else.  The sleep(xx) needs to be long enough so uugetty
	 *   isn't eating the system alive busy waiting and short enough so
	 *   uugetty gets in quickly when the line becomes free.  The longer
	 *   uugetty takes to "get in" the longer a user calling into this
	 *   system will have to wait for the connection.  The sleep value
	 *   use to be 60.  This "felt" too long.  I changed it to 30.
	 */
	if ( ! checklinelock(line)) { /*  There is a lock file already */
	    /* some process is using the line for output */
	    (void) fclose(stdin);
	    (void) sprintf(lckname, "%s.%s", LOCKPRE, line);
#ifdef RAYDEBUG
	    fprintf(debugfd,"waiting for lock file \'%s\' to go away\n",
		  lckname);
	    fflush(debugfd);
#endif
...
#ifdef RAYDEBUG
	    fprintf(debugfd,"got lock file \'%s\'\n", lckname);
	    fflush(debugfd);
#endif
	    exit(0);
        }
	...
	if (wait_read) {	  /* wait to read the first character */
#ifdef RAYDEBUG
		fprintf(debugfd,"wait for one character\n");
		fflush(debugfd);
#endif
		 ...
		 *
		 * Checking for EOT also needs to be done in routine
		 * getname()
		 */
...
#ifdef RAYDEBUG
		fprintf(debugfd,"character received %#X\n", *buffer);
		fflush(debugfd);
#endif
	}
	    
#ifdef RAYDEBUG
        fprintf(debugfd,"checking on lock file\n");
	    fflush(debugfd);
#endif
...
#ifdef RAYDEBUG
	fprintf(debugfd,"opening line \'%s\'\n",line);
	fflush(debugfd);
#endif
...
/*               Unsupported VVVVVVVV Port 
		if(termtype == TERM_NONE) { */
#ifdef RAYDEBUG
	    fprintf(debugfd,"termtype == TERM_NONE. doing putc(CR);putc(NL)\n");
	    fflush(debugfd);
#endif
...
/* Generate a message with the system identification in it. */
		if (uname(&utsname) != FAILURE) {
			sprintf(buffer,(catgets(nlmsg_fd,NL_SETN,844, "%.9s\r\n")), utsname.nodename);
...
#ifdef RAYDEBUG
	    fprintf(debugfd,"trying to open the ISSUE_FILE\n");
	    fflush(debugfd);
#endif
...
#ifdef RAYDEBUG
	    fprintf(debugfd,"printing the log message \'%s\'\n",
			speedef->g_message);
	    fflush(debugfd);
#endif
...
#ifdef RAYDEBUG
	    fprintf(debugfd,"process the response\n");
	    fflush(debugfd);
#endif
		switch(getname(user,&termio)) {
		case GOODNAME:
#ifdef RAYDEBUG
	    		fprintf(debugfd,"name is GOODNAME\n");
	    		fflush(debugfd);
#endif
			if (timeout) alarm(0);
...
/* on when the terminal type was set. */
			...
			termio.c_line = lined;
#ifdef RAYDEBUG
	    		fprintf(debugfd,"do fioctl(TCSETAW,&termio)\n");
	    		fflush(debugfd);
#endif
			fioctl(stdin,TCSETAW,&termio);

			/* Set the final h/w flow control parameters	*/
			/* Fix for DSDe407887	*/

			termiox.x_hflag	= speedef->gx_fflags.x_hflag;
			termiox.x_cflag	= speedef->gx_fflags.x_cflag;
			termiox.x_sflag	= speedef->gx_fflags.x_sflag;
/*
**	At the moment, if the driver or the hardware does not support
**	hardware flow control, TCSETX will return an error. But this may not
**	be the case in the future.
*/

			if (hwfctl && (ioctl(0, TCSETX, &termiox) < 0)) {
				exit(NO_HWFCTL);
			}
...
/* spaces. */
          ...
          /* Bug fixes brought over from getty.c */
                        largs[MAXARGS] = (char *) NULL;

/* Exec "login". */

#ifdef RAYDEBUG
			fprintf(debugfd,"execv /bin/login\n");
			fflush(debugfd);
...
/* won't overwrite it in the process of looking for new entry. */
			...
			if ((speedef = find_def(newspeed)) == NULL) {
				error((catgets(nlmsg_fd,NL_SETN,845, "getty: pointer to next speed in entry %s is bad.\n")),
				oldspeed);
...
/* If the old entry has disappeared, then quit and let next "getty" try. */
					error((catgets(nlmsg_fd,NL_SETN,846, "getty: unable to find %s again.\n")),
						oldspeed);
					exit(1);
				}
			}
...
/* utmp file, then attempt to append to the end of the wtmp file. */
	if (u != NULL && (fp = fopen(WTMP_FILE,"r+")) != NULL) {
         /* Bug fix brought over from getty.c 
            Prevent multiprocess open the wtmp at the same time
            and seek to the same place and start write to the
            same location. Use append mode. We can't do a a+
            because we don't want to create the file if it does not
            exist */
                finfo = fcntl(fileno(fp),F_GETFL);
                if (fcntl(fileno(fp), F_SETFL, finfo | O_APPEND) == EOF ){
...
#ifdef RAYDEBUG
    fprintf(debugfd,"in search routine looking for \'%s\'\n", target);
    fflush(debugfd);
#endif
...
int termtype,lined;
{
	...
	/*struct termcb termcb;  Unsupported stuff again. Port */
...
/*
**	Some hardware (CIO) do not support hardware flow control, but the
**	drivers present in the machine has the relevant routines. So
**	even if the hardware does not support hardware flow control, 
**	TCGETX does not return an error.
*/

	/* Check if h/w and driver supports h/w flow control	

	if(ioctl(0, TCGETX, &termiox) != -1)
			hwfctl = 1;
	*/
...
/* Pause briefly while terminal settles. */
	...
	/* Set the initial h/w flow control parameters	*/
	/* Fix for DSDe407887.	*/

	termiox.x_hflag	= speedef->gx_iflags.x_hflag;
	termiox.x_cflag	= speedef->gx_iflags.x_cflag;
	termiox.x_sflag	= speedef->gx_iflags.x_sflag;
/*
**	At the moment, if the driver or the hardware does not support
**	hardware flow control, TCSETX will return an error. But this may not
**	be the case in the future.
*/
	...
	/* Set the modem control parameters	*/
...
/* If a EOT character was typed, the user on the other end disconnected
 * so terminate this invocation of uugetty.
 * NOTE: Cu on a direct line when ~. is encountered will
 * send EOTs to the other side.  EOT=\004
*/
		if ((c = (rawc & 0177)) == '\004')
...
/* line. */
			...
			ptr = user;
			fputs((catgets(nlmsg_fd,NL_SETN,847, "\r\n")),stdout);
...
/* If unable to open, complain and then use the built in default. */
	if (fp == NULL) {
		error((catgets(nlmsg_fd,NL_SETN,848, "getty: can't open \"%s\".\n")),gdfile);
		return(&DEFAULT);
	}
...
/* start correctly on next line. */
		if (rawc != EOF && c != '\n') {
			...
			if (check) fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,849, "Entry too long.\n")));
		}
...
/* last character stored. */
		...
		if (check) fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,850, "\n**** Next Entry ****\n%s\n")),line);
...
/* Initialize "def" and "gptr". */
		...
		/* Initialize all control flags to zero.	*/

		gptr->gx_iflags.x_hflag	= 0;
		gptr->gx_iflags.x_cflag	= 0;
		gptr->gx_iflags.x_sflag	= 0;

		gptr->gx_fflags.x_hflag	= 0;
		gptr->gx_fflags.x_cflag	= 0;
		gptr->gx_fflags.x_sflag	= 0;
...
/* Extract the "g_iflags" */
			case IFLAGS:
				if ((ptr = fields(ptr,&gptr->g_iflags, &gptr->gx_iflags)) == NULL) {
					field = state;
					state = FAILURE;
				} else {
...
/* Extract the "g_fflags". */
			case FFLAGS:
				if ((ptr = fields(ptr,&gptr->g_fflags, &gptr->gx_fflags)) == NULL) {
					field = state;
					state = FAILURE;
				} else {
...
/* In check mode print out the results of the parsing. */
				...
				fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,851, "id: %s\n")),gptr->g_id);
				fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,852, "initial flags:\niflag- %o oflag- %o cflag- %o lflag- %o xflag- %o\n")),
					gptr->g_iflags.c_iflag,
					gptr->g_iflags.c_oflag,
					gptr->g_iflags.c_cflag,
					gptr->g_iflags.c_lflag,
					gptr->gx_iflags.x_hflag);
				fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,853, "final flags:\niflag- %o oflag- %o cflag- %o lflag- %o xflag- %o \n")),
					gptr->g_fflags.c_iflag,
					gptr->g_fflags.c_oflag,
					gptr->g_fflags.c_cflag,
					gptr->g_fflags.c_lflag,
					gptr->gx_fflags.x_hflag);
				fprintf(stdout,"crtsflag- %d\n", (int) mb);
				fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,854, "message: %s\n")),gptr->g_message);
				fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,855, "next id: %s\n")),gptr->g_nextid);
			}
...
/* the bad line. */
			...
			*++ptr = '\0';
			fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,856, "Parsing failure in the \"%s\" field\n\
...
/*	field.								*/
/*	New argument termiox addeded to take care of h/w flow control	*/
/*	support.							*/

char *fields(ptr,termio, termiox)
register char *ptr;
struct termio *termio;
struct termiox *termiox;
{
...
/* that are needed for SANE tty behavior. */
			...
			/* Does it represent h/w flow control information */
			/* Fix for DSDe407887.	*/

			else if ((symbol = search(word,xmodes)) != NULL) {
				hwfctl	= 1;
				termiox->x_hflag |= symbol->s_value;
			}
			/* Does it represent h/w flow control information */
			/* Fix for DSDe407887.	*/

			else if ((symbol = search(word,mmodes)) != NULL)
				mb |= symbol->s_value;
			else if (check) fprintf(stdout,(catgets(nlmsg_fd,NL_SETN,857, "Undefined: %s\n")),word);
		}
