 ...
 */
#define CDEBUG(l, f, s) if (Debug >= l) fprintf(stderr, f, s); fflush(stderr);
#ifndef SMALL
#define DEBUG(l, f, s) if (Debug >= l) fprintf(stderr, f, s); fflush(stderr);
#else
#define DEBUG(l, f, s)
#endif SMALL
...
#define MAXNAMESIZE	256	/* long file names */
/*#define MAXNAMESIZE	64	/* /usr/spool/uucp/<14 chars>/<14 chars>+slop */
#define MAXMSGTIME 33
#define MAXEXPECTTIME 180 /*45*/
#define MAXCHARTIME 60 /*15*/
...
struct nstat {
	...
	clock_t	t_scall;	/* start call to system			*/
	clock_t	t_ecall;	/* end call to system			*/
	time_t	t_tacu;		/* acu time				*/
	time_t	t_tlog;		/* login time				*/
	clock_t	t_sftp;		/* start file transfer protocol		*/
	time_t	t_sxf;		/* start xfer 				*/
	time_t	t_exf;		/* end xfer 				*/
	clock_t	t_eftp;		/* end file transfer protocol		*/
...
#endif
/* The following three lines are out here because we do
   things such as strcpy(msg,catgets(....)). Instead of ifdef every
   line of code that do those things, we will just declared it */
extern char msg1[];  /* message catalog buffers */
extern char msg2[];
extern char msg3[];
...
#define VERSION(x)	;
/* We don't use sccs, and the sscsid will be meaningless anyway. Besides,
   it will confuse the whatstrings */ 
#define sccsid	" "    
/* was :assert(s1, s2, i1, sccsid, __FILE__, __LINE__);\ */
...
/* We are doing this because we are partially 4.2 */
#define BSD4_2
#ifndef BSD4_2
#define DIR FILE
...
extern time_t	time();
extern int	strlen(), strcmp(), strncmp(), sprintf();
...
extern clock_t	millitick();				/* logent.c */
