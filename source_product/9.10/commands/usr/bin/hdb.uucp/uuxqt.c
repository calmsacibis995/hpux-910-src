/*	@(#) $Revision: 70.2 $	*/
static char *RCS_ID="@(#)$Revision: 70.2 $ $Date: 91/11/07 10:49:34 $";
#ifndef NLS
...
/*	/sccs/src/cmd/uucp/s.uuxqt.c
	uuxqt.c	1.11.1.1	7/29/85 16:34:23
*/
#include "uucp.h"
...
#define BAD_FILE	2
#define USAGE	strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,1064, "[-xDEGUG] [-sSYSTEM]")))
...
char	*argv[], *envp[];
{
	...
	while ((ret = getopt(argc, argv, "s:x:")) != EOF) {
		...
		default:
			(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1066, "\tusage: %s %s\n"))),
			    Progname, USAGE);
			exit(1);
		}
	}
	if (argc != optind) {
		(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1067, "\tusage: %s %s\n"))), Progname, USAGE);
		exit(1);
	}

	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1068, "\n\n** START **\n")),  "");
	fp = fopen(LMTUUXQT, "r");
	if (fp == NULL) {
		DEBUG(1, (catgets(nlmsg_fd,NL_SETN,1070, "No limitfile - %s\n")), LMTUUXQT);
	} else {
		(void) fscanf(fp, "%d", &maxnumb);
		(void) fclose(fp);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1071, "Uuxqt limit %d -- ")), maxnumb);
		ret = cuantos(X_LOCKPRE, X_LOCKDIR);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1072, "found %d -- ")), ret);
		if (maxnumb >= 0 && ret >= maxnumb) {
			DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1073, "exiting\n")), maxnumb);
			exit(0);
		}
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1074, "continuing\n")), maxnumb);
	}
	...
	guinfo(Euid, User);
	setuucp(User);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1075, "User - %s\n")), User);
	guinfo(Uid, Loginuser);

	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1076, "process %s\n")), "");
...
char	*file;
{
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1080, "stashing %s\n")), file);
...
xsort()
{
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1081, "xsort:  first was %s\n")), xcache[0]);
	qsort(xcache, xcachesize, MAXBASENAME + 1, xcompare);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1082, "xsort:  first is %s\n")), xcache[0]);
}
...
char	*file;
{
	if (xcachesize > 0) {
		...
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1083, "xget: returning %s\n")), file);
		return(1);
	} else {
...
register char	*file, *dir;
{
	...
	if (xcachesize == 0) {
		...
		while (gnamef(pdir, file) == TRUE) {
			DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1084, "gt_Xfile got %s\n")), file);
			/* look for x prefix */
			if (file[0] != XQTPRE)
				continue;
...
rm_Xfiles()
{
	...
	if ((fp = fopen(_Xfile, "r")) == NULL) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1085, "rm_Xfiles: can't read %s\n")), _Xfile);
		return;
	}
...
mv_Xfiles()
{
	...
	if ((fp = fopen(_Xfile, "r")) == NULL) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1086, "mv_Xfiles: can't read %s\n")), _Xfile);
		return;
	}
...
unmv_Xfiles()
{
	...
	if ((fp = fopen(xfull, "r")) == NULL) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1088, "unmv_Xfiles: can't read %s\n")), xfull);
		return;
	}
...
char *ptr;
{
	...
	while ((ptr = getprm(ptr, prm)) != NULL) {
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1090, "prm='%s'\n")), prm);
	    switch(*prm) {
...
{
	    expfile(file);
	    DEBUG(7, (catgets(nlmsg_fd,NL_SETN,1091, "fullname: %s\n")), file);
...
char	*user, *rmt, *file, *cmd, *buf, *errfile;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,1093, "retosndr %s, ")), msg);
...
char	*rmt, *tofile, *errfile, *cmd, *buf;
{
	...
	if (*errfile != '\0' && NOTEMPTY(errfile)
	   && (fi = fopen(errfile, "r")) != NULL) {
		fputs((catgets(nlmsg_fd,NL_SETN,1095, "\n\t===== stderr was =====\n")), fp);
		if (xfappend(fi, fp) != SUCCESS)
			fputs((catgets(nlmsg_fd,NL_SETN,1096, "\n\t===== well, i tried =====\n")), fp);
		(void) fclose(fi);
		fputc('\n', fp);
	}
...
char *dirname;
{
	...
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1097, "_Xfile - %s\n")), _Xfile);
	...
	*incmd = *retaddr = NULLCHAR;
	...
	while (fgets(buf, BUFSIZ, xfp) != NULL) {
		...
		case X_STDIN:	/* standard input */
				...
				DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1098, "badfile - in: %s\n")), fin);
				badfiles = 1;
			    }
			    break;
		...
		case X_STDOUT:	/* standard output */
				...
				*p = NULLCHAR;	/* these can be dangerous! */
				...
				badfiles = 1;
				DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1099, "badfile - out: %s\n")), fout);
			    }
			    break;
	...
	fclose(xfp);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1100, "fin - %s, ")), fin);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1101, "fout - %s, ")), fout);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1102, "sysout - %s, ")), sysout);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1103, "user - %s\n")), user);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1104, "incmd - %s\n")), incmd);
	...
	if (badfiles || (cmdok == BAD_COMMAND) || cmdok == BAD_FILE) {
	    ...
	    logent(incmd, lbuf);
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1110, "bad command %s\n")), incmd);
	...
	logent(_Cmd, lbuf);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1112, "cmd %s\n")), _Cmd);
	...
	mask = umask(0);
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,1113, "full cmd: %s\n")), _Cmd);
