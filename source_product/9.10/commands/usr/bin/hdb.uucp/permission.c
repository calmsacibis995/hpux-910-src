...
/*	/sccs/src/cmd/uucp/s.permission.c
	permission.c	1.3	7/29/85 16:33:17
*/
#ifndef UUCHECK
#include "uucp.h"
#endif
...
{
	int ret;
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,557, "logFind called (name: %s, ")), name);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,558, "rmtname: %s)\n")), rmtname);

	ret = validateFind (rmtname);
	if (ret == SUCCESS) { /* found VALIDATE entry */
	    ret = userFind (name, rmtname, U_VALIDATE);
	    if (ret) {
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,559, "machine/login match failed")), "");
		return(FAIL);
	    }
	}
	else
	...
	strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,560, "_Request (%s), ")));
        strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,561, "TRUE")));
        strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,562, "FALSE")));
	DEBUG(7, msg1, requestOK() ? msg2 : msg3);

	strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,563, "_Switch (%s), ")));
	strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,564, "TRUE")));
        strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,565, "FALSE")));
	DEBUG(7, msg1, switchRole() ? msg2 : msg3);

	strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,566, "_CallBack (%s), ")));
        strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,567, "TRUE")));
        strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,568, "FALSE")));
	DEBUG(7, msg1, callBack() ? msg2 : msg3);


	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,569, "_MyName (%s), ")), _MyName);
	return(ret);
}
...
{
	register i, ret;
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,570, "mchFind called (%s)\n")), name);
	...
	/*DEBUG(7, (catgets(nlmsg_fd,NL_SETN,574, "_Request (%s), ")),
	    requestOK() ? (catgets(nlmsg_fd,NL_SETN,575, "TRUE")) : (catgets(nlmsg_fd,NL_SETN,576, "FALSE")));
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,577, "_Switch (%s), ")),
	    switchRole() ? (catgets(nlmsg_fd,NL_SETN,578, "TRUE")) : (catgets(nlmsg_fd,NL_SETN,579, "FALSE")));
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,580, "_CallBack (%s), ")),
	    callBack() ? (catgets(nlmsg_fd,NL_SETN,581, "TRUE")) : (catgets(nlmsg_fd,NL_SETN,582, "FALSE")));
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,583, "_MyName (%s), ")), _MyName);*/
	...
	for (i=0; _Commands[i] != NULL; i++)
	    DEBUG(7, (catgets(nlmsg_fd,NL_SETN,584, "_Commands %s\n")),  _Commands[i]);
	return(ret);
}
...
#else UUCHECK
		if (_Pubdir == NULL) {
		    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,585, "malloc() error\n")));
		    exit(1);
		}
#endif UUCHECK
...
char *list[];
{
	...
	while (*p && num < maxlist) {
		...
		if (*p == ':')
			*p++ = NULLCHAR;
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,586, "list (%s) ")), list[num]);
		num++;
	}
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,587, "num = %d\n")), num);
	list[num] = NULL;
	return(num);
}
...
int type;
{
	...
	if ( (Fp = fopen(PFILE, "r")) == NULL) {
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,588, "can't open %s\n")), PFILE);
		return(FAIL);
	}
	...
	for (;;) {
		...
		(void) fclose(Fp);
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,589, "name (%s) not found; return FAIL\n")), name);
		return(FAIL);
	    }
...
char *name;
{
	...
	if ( (Fp = fopen(PFILE, "r")) == NULL) {
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,590, "can't open %s\n")), PFILE);
		return(FAIL);
	}

	for (;;) {
	    if (parse_tokens (_Flds) != 0) {
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,591, "validateFind (%s) FAIL\n")), name);
		(void) fclose(Fp);
		return(FAIL);
	    }
...
#else UUCHECK
		if (i >= NUMFLDS) {
			DEBUG(3, (catgets(nlmsg_fd,NL_SETN,593, "bad option (%s) in PERMISSIONS\n")),pair.name);
			(void) printf((catgets(nlmsg_fd,NL_SETN,594, "\n*****************************\n")));
			(void) printf((catgets(nlmsg_fd,NL_SETN,595, "**BAD OPTION in PERMISSIONS file: %s\n")),
				pair.name);
			(void) printf((catgets(nlmsg_fd,NL_SETN,596, "*****************************\n")));
			Uerrors++;
			return(0);
		}
#endif UUCHECK
...
char	*cmd, *fullcmd;
{
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,597, "cmdOK(%s, )\n")), cmd);
...
char *fullname;
{
	...
	for (i=0; _Commands[i] != NULL; i++) {
		...
		if (name[0] != '/')
			...
			bname = _Commands[i];
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,598, "bname=%s\n")), bname);
