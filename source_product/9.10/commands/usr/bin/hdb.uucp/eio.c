...
char type;
{
	...
	*s = '\0';
	...
	s1 = strlen(bufr) + 1;
	if (setjmp(Failbuf)) {
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,462, "ewrmsg write failed\n")), "");
		return(FAIL);
	}
	alarm(60);
...
register char *str;
{
	...
	if(setjmp(Failbuf)) {
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,463, "erdmsg read failed\n")), "");
		return(FAIL);
	}
...
register FILE *fp1;
{
	...
	if (setjmp(Failbuf)) {
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,464, "ewrdata failed\n")), "");
		return(FAIL);
	}
	bytes = 0L;
	...
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,465, "ewrmsg write %d\n")), statbuf.st_size);
	while ((len = fread(bufr, sizeof (char), BUFSIZ, fp1)) > 0) {
		alarm(60);
		ret = write(fn, bufr, (unsigned) len);
		alarm(0);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,466, "ewrmsg ret %d\n")), ret);
		if (ret != len)
			return(FAIL);
		if ((msglen -= len) <= 0)
			break;
...
register FILE *fp2;
{
	...
	bytes = msglen;
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,468, "erdblk msglen %d\n")), msglen);
	for (;;) {
		len = erdblk(bufr, MIN(msglen, BUFSIZ), fn);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,469, "erdblk ret %d\n")), len);
		if (len < 0) {
			DEBUG(7, (catgets(nlmsg_fd,NL_SETN,470, "erdblk failed\n")), "");
			return(FAIL);
		}
		if ((msglen -= len) < 0) {
			DEBUG(7, (catgets(nlmsg_fd,NL_SETN,471, "erdblk read too much\n")), "");
			return(FAIL);
		}
	...
	ticks = millitick();
	sprintf(text, (catgets(nlmsg_fd,NL_SETN,472, "<- %ld / %ld.%.3d secs")), bytes, ticks / 1000, ticks % 1000);
	DEBUG(4, "%s\n", text);
	syslog(text);
	return(0);
}
...
register char *blk;
{
	...
	if(setjmp(Failbuf)) {
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,473, "erdblk timeout\n")), "");
		return(FAIL);
	}

	for (i = 0; i < len; i += ret) {
		alarm(60);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,474, "ask %d ")), len - i);
		if ((ret = read(fn, blk, (unsigned) len - i)) < 0) {
			alarm(0);
			DEBUG(7, (catgets(nlmsg_fd,NL_SETN,475, "read failed\n")), "");
			return(FAIL);
		}
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,476, "got %d\n")), ret);
		blk += ret;
		if (ret == 0)
			break;
	}
