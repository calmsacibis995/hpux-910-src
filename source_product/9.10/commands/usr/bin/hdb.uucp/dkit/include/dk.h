...
#include <sys/ioctl.h>
#endif KERNEL
#define	DKIODIAL	(IOC_VOID|('k'<<8)|0)	/* dial out */
#define	DKIOCNEW	(IOC_VOID|('k'<<8)|1)	/* offer a service */
#define	DKIOCREQ	(IOC_VOID|('k'<<8)|2)	/* request service (SU only) */
#define	DKIORESET	(IOC_VOID|('k'<<8)|3)	/* reset interface */
#define DKKMCSET	(IOC_VOID|('k'<<8)|4)	/* associate logical interface
					   with physical KMC # */

#define DKIOCSPL	(IOC_VOID|('s'<<8)|1)	/* splice two circuits together (SU only) */
#define	DIOCSWAIT	(IOC_VOID|('s'<<8)|2)	/* wait for splice to take place */


/*     driver control        */

#define DIOCEXCL	(IOC_VOID|('d'<<8)|1)	/* exclusive use */
#define DIOCNXCL	(IOC_VOID|('d'<<8)|2)	/* reset exclusive use */
#define	DIOCRMODE	(IOC_VOID|('d'<<8)|3)	/* set receiver termination modes */
#define	DIOCQQABO	(IOC_VOID|('d'<<8)|4)	/* inquire status of last read */
#define	DIOCSIG		(IOC_VOID|('d'<<8)|5)	/* start short read, signal when done */

#define	DIOCXCTL	(IOC_VOID|('d'<<8)|8)	/* send ctl envelope on next write */
#define DIOCFLUSH	(IOC_VOID|('d'<<8)|9)	/* flush output */
#define DIOCSETK	(IOC_VOID|('d'<<8)|10)	/* debug info from kmc xmit&recv */
#define	DIOCQSTAT	(IOC_VOID|('d'<<8)|11)	/* return 3B hw/fw log data */
#define	DIOCBSIZE	(IOC_VOID|('d'<<8)|12)	/* set URP block size */
#define	DIOCTIME	(IOC_VOID|('d'<<8)|13)	/* set stagnation timeout value */
#define	DIOCTRAP	(IOC_VOID|('d'<<8)|14)	/* activate trsave trace for channel */

/*	interface memory read/write codes	 */

#define	DIOCHWREAD	(IOC_VOID|('d'<<8)|15)	/* read interface RAM */
#define	DIOCHWRITE	(IOC_VOID|('d'<<8)|16)	/* write interface RAM */

/*	diagnostic control codes	*/

#define	DIOCDKDGN	(IOC_VOID|('d'<<8)|17)	/* execute on-line diagnostics */
#define	DIOCDGNSET	(IOC_VOID|('d'<<8)|18)	/* initialize diagnostic mode */
#define	DIOCDGNCLR	(IOC_VOID|('d'<<8)|19)	/* clear diagnostic mode */

/*	3b2/PE codes			*/
#define TCDKIDLD	(IOC_VOID|('d'<<8)|20)	/* download the PE */
#define TCDKIFCF	(IOC_VOID|('d'<<8)|21)	/* Force call to function */
#define TCDKIRST	(IOC_VOID|('d'<<8)|22)	/* Reset the PE board */
#define	TCDKISYSG	(IOC_VOID|('d'<<8)|23)	/*sysgen the PE board */

/* Get info from driver */
#define DIOCINFO	(IOC_VOID|('d'<<8)|24)	/* get chans per interface*/
#define	TCDKIBUG	(IOC_VOID|('d'<<8)|25)	/*turn debug on on the PE board */
#define	DIOOPEN 	(IOC_VOID|('d'<<8)|26)	/* inquire channel open status*/
/*	special codes used by dkxstdio	*/

#define	DXIOEXIT	_IOW('D','T',int)		/* process exit code */
#define DIOCSBSIZE	(IOC_VOID|('D'<<8)|27)	/* buffer size for select read */
#define DIOCDEBUG	(IOC_VOID|('D'<<8)|28)	/* for debugging */
#define DKDEBUG_GET	01
#define		DKDEBUG_DKDEV	01
#define		DKDEBUG_DKCHAN	02
#define		DKDEBUG_DKTTY	03	
#define		DKDEBUG_CBLOCK	04
#else
