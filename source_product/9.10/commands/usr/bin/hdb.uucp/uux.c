static char *RCS_ID="@(#)$Revision: 70.4 $ $Date: 91/11/13 08:35:23 $";
#ifndef NLS
...
#define USAGE	strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,1033, "[-aNAME] [-b] [-c] [-C] [-j] [-gGRADE] [-n] [-p] [-r] [-sFILE] [-xNUM] [-z] command-string")))
...
char **envp;
{
 ...
 /* we want this to run as uucp, even if the kernel doesn't */
	...
	/*
	 *  The following code was moved to later in main.
	 *  It is executed after we determine the name of the
	 *  target system.  This allows us to use aliases
	 *  based on the name of the target system.
	 *
	 * uucpname(Myname);
	 *
	 *  do we have an aliases?
	 *    mchFind will find it if we do.
	 *    If we don't myName will again call uucpname.
	 *
	 * mchFind(Myname);
	 * myName(Myname);
	 */
	...
	*_Sfile = '\0';
	...
	while ((i = getopt(argc, argv, "a:bcCjg:nprs:x:z")) != EOF) {
		...
		default:
			(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1034, "\tusage: %s %s\n"))), Progname, USAGE);
			exit(1);
		}
	}

	DEBUG(4, "\n\n** %s **\n", (catgets(nlmsg_fd,NL_SETN,1036, "START")));
	...
	for (; optind < argc; optind++) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1037, "arg - %s:")), argv[optind]);
		(void) strcat(inargs, " ");
	 ...
	 * to spool directory
	 */
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1038, "arg - %s\n")), inargs);
	gwd(Wrkdir);
	if(fopt){
		if(*fopt != '/')
	...
	if (chdir(WORKSPACE) != 0) {
	    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1039, "No spool directory - %s - get help\n")), WORKSPACE);
	    cleanup(-2);
	}
	/*
	...
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1040, "xsys %s\n")), xsys);
	...
	if (versys(xsys) != 0) {
		 ...
		 * bad system name
		 */
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1041, "bad system name: %s\n")), xsys);
		if (fprx != NULL)
			(void) fclose(fprx);
		if (fpc != NULL)
			(void) fclose(fpc);
		cleanup(101);	/* long live code 101! */
	}

	/*
	 *  Does this system have an aliases?
	 *    mchFind will find it if we do.
	 *    If we don't myName will call uucpname.
	 */
	mchFind(xsys);
	myName(Myname);
	...
	DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1042, "User %s\n")), User);
	if (retaddr == NULL)
		retaddr = User;
	...
	*cmd = '\0';
	...
	DEBUG(9, (catgets(nlmsg_fd,NL_SETN,1043, "cfile = %s\n")), cfile);
	...
	gename(DATAPRE, xsys, _Grade, rxfile);
	DEBUG(9, (catgets(nlmsg_fd,NL_SETN,1044, "rxfile = %s\n")), rxfile);
	...
	if (pipein) {
		...
		gename(DATAPRE, Myname, _Grade, dfile);
	...
	ap = inargs;
	while ((ap = getprm(ap, prm)) != NULL) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1050, "prm - %s\n")), prm);
		...
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1051, "syspart -> %s, ")), syspart);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1052, "rest -> %s, ")), rest);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1053, "ret -> %d\n")), ret);
		...
		if (syspart[0] == '\0') {
			...
			DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1054, "syspart -> %s\n")), syspart);
		} else if (versys(syspart) != 0) {
			/*
			 * bad system name
			 */
			fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1055, "bad system name: %s\n")), syspart);
			if (fprx != NULL)
				(void) fclose(fprx);
			if (fpc != NULL)
				(void) fclose(fpc);
		...
		if (EQUALSN(syspart, Myname, SYSNSIZE)) {
			...
			if (ckexpf(rest))
				cleanup(2);
			DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1056, "rest %s\n")), rest);

			/* see if I can read this file as read uid, gid */
			if (uidstat(rest, &stbuf) != 0) {
			    (void) fprintf(stderr,
			      (catgets(nlmsg_fd,NL_SETN,1057, "can't get file status %s\n")), rest);
			    cleanup(2);
			}
			if ( !(stbuf.st_mode & ANYREAD)
				...
				fprintf(stderr,(catgets(nlmsg_fd,NL_SETN,1058, "permission denied %s\n")), rest);
				cleanup(1);
			}

			/* D. file for sending local file */
			gename(DATAPRE, xsys, _Grade, dfile);

			if (cflag || !(stbuf.st_mode & ANYREAD)) {
				/* make local copy */
				if (uidxcp(rest, dfile) != 0) {
				    fprintf(stderr,(catgets(nlmsg_fd,NL_SETN,1059, "can't copy %s\n")), rest);
				    cleanup(1);
				}
		...
		if (EQUALS(Myname, xsys)) {
			...
			setbuf(fp, CNULL);
			gename(DATAPRE, syspart,_Grade, dfile);
...
register int code;
{
	...
	if (code) {
		wfabort();
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1061, "uux failed. code %d\n")), code);
	}
	DEBUG(1, (catgets(nlmsg_fd,NL_SETN,1062, "exit code %d\n")), code);
	exit(code);
}
