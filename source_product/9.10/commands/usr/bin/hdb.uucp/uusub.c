/*	@(#) $Revision: 70.3 $	*/
static char *RCS_ID="@(#)$Revision: 70.3 $ $Date: 93/12/17 15:15:40 $";
#ifndef NLS
...
/* HDB version of uusub */
#include "uucp.h"
#ifdef UUSUB
#include <sys/types.h>
#include "uusub.h"
#define	rid	0		/* user id for super user */
#define cno    strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,1000, "uusub: cannot open ")))
/*char cno[] = (catgets(nlmsg_fd,NL_SETN,1000, "uusub: cannot open "));*/
...
int argc;
{
	...
	while ((c=getopt(argc, argv, "x:a:d:c:u:rlf")) != EOF )
		...
		case 'f':
			...
			break;
		case '?':
			fprintf(stderr,(catgets(nlmsg_fd,NL_SETN,1001, "Usage:uusub -a* -d* -c* -u* -r -l-f\n")));
			exit(2);
		}
	 ...
	 */
	if (fflag) {	
		DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1002, "Flush file %s\n")), L_sub);
	...
	if (dflag) {	/* delete system "da" from "L_sub" */
		DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1003, "Delete system %s from subnetwork\n")),da);
		edit('d',da);
	}
	 ...
	 */
	if (aflag) {	
		DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1004, "Add system %s to subnetwork\n")),aa);
		...
		DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1005, "system %s is added\n")), aa);
	...
	if (rflag) {
		...
		printf((catgets(nlmsg_fd,NL_SETN,1007, "sysname\tsfile\tsbyte\trfile\trbyte\n")));
		while (fread((char *) &r, sizeof(r), 1, fp)!=NULL)
			printf((catgets(nlmsg_fd,NL_SETN,1008, "%s\t%d\t%ld\t%d\t%ld\n")),r.sys,r.sf,r.sb,r.rf,r.rb);
		fclose(fp);
	}
	...
	if (lflag) {	/* print the L_sub file */
		...
		printf((catgets(nlmsg_fd,NL_SETN,1009, "sysname\t#call\t#ok\tlatest-oktime\t#noacu\t#login")));
		printf((catgets(nlmsg_fd,NL_SETN,1010, "\t#nack\t#other\n")));
		while (fread((char *) &l, sizeof(l), 1, fp)!=NULL) {
			tp=localtime(&l.oktime);
			printf((catgets(nlmsg_fd,NL_SETN,1011, "%s\t%d\t%d")), l.sys,l.call,l.ok);
			printf((catgets(nlmsg_fd,NL_SETN,1012, "\t(%d/%d-%d:%2.2d)\t")), tp->tm_mon+1,
			        tp->tm_mday, tp->tm_hour, tp->tm_min);
			printf((catgets(nlmsg_fd,NL_SETN,1013, "%d\t%d\t%d\t%d\n")),l.noacu,l.login,l.nack,l.other);
		}
		fclose(fp);
	}
...
char *arg;
{
	...
	DEBUG(6, (catgets(nlmsg_fd,NL_SETN,1014, "enter edit, flag: %d\n")),flag);
	...
	while (fread((char *) &l, sizeof(l), 1, fp)!=NULL)
	 ...
	 case 'd':
		if (strncmp(l.sys, arg, SYSNSIZE)!=SAME) {
			DEBUG(7,(catgets(nlmsg_fd,NL_SETN,1016, "%s is retained\n")),l.sys);
			fwrite((char *) &l,sizeof(l),1,fq);
		}
		break;
	 }
...
main(argc, argv)
char **argv;
{	fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1018, "Uusub is not implemented on this system\n")));
}
#endif
update()
{
	 ...
	 * Flush R_sub file
	 */
	DEBUG(6,(catgets(nlmsg_fd,NL_SETN,1019, " Flush file %s\n")), R_sub);
	r.sf = r.rf = (long) 0;
	r.sb = r.rb = 0;
	while(fread((char *) &l,sizeof(l),1,fp)==1) {
		DEBUG(6,(catgets(nlmsg_fd,NL_SETN,1020, " l.sys: %s\n")), l.sys);
		strcpy(r.sys, l.sys);
	...
	while (fgets(s,sizeof(s),fp)!=NULL) {
		...
		if(flag)
			sscanf(p, "%ld",&bytes);
#ifndef HDBuucp  /* This cause the last entry not be read */
		if(fgets(s,sizeof(s),fp) == NULL)
			break;
#endif
		if(flag == 0)
			continue;
		DEBUG(9,(catgets(nlmsg_fd,NL_SETN,1022, " Rmt: %s ")),rmt);
		DEBUG(9,(catgets(nlmsg_fd,NL_SETN,1023, " s_r: %s")), s_r);
		DEBUG(9,(catgets(nlmsg_fd,NL_SETN,1024, " bytes: %ld ")),bytes);
		oldtick = timeod(-1, month, day, hr, min, sec);
		oldtick += timezone;
		DEBUG(9,(catgets(nlmsg_fd,NL_SETN,1025, " oldtick: %ld\n")),oldtick);
		dtick = tick - oldtick;
		if(dtick > utick)
			continue;
		DEBUG(8, (catgets(nlmsg_fd,NL_SETN,1026, " %s inside the time\n")),rmt);
		...
		while (fread((char *) &r,sizeof(r),1,fq)==1){
			if (strncmp(r.sys, rmt, SYSNSIZE)==SAME) {
				...
				fwrite((char *) &r,sizeof(r),1,fq);
				DEBUG(7,(catgets(nlmsg_fd,NL_SETN,1027, "%s is written\n")),r.sys);
...
cleanup(n)      /* need cleanup() so ASSERT works OK */
{
    exit(n);
}
