/*	@(#) $Revision: 70.1 $	*/
static char *RCS_ID="@(#)$Revision: 70.1 $ $Date: 91/11/07 10:48:49 $";
#ifndef NLS
...
#include "uucp.h"
#include <pwd.h>
#define LOGDEL "/usr/spool/uucp/.Admin/uuclean"
...
/*char *strcpy(); already include in uucp.h */
extern void mailst();
int	wrnf;
...
char	**envp;
{
	...
	while (argc>1 && argv[1][0] == '-') {
		...
		case 'w':
			...
			break;
		default:
			printf((catgets(nlmsg_fd,NL_SETN,696, "unknown flag %s\n")), argv[1]); break;
		}
		--argc;  argv++;
	}
	...
	if ((pdirf = opendir(Spool)) == (DIR *)NULL) {
		printf((catgets(nlmsg_fd,NL_SETN,697, "%s directory unreadable\n")), Spool);
		exit(1);
	}
	strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,698, "DEBUG# %s\n")));
        strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,699, "START")));
	DEBUG(4, msg1,msg2);
	ret = chdir(Spool);
	/*ASSERT(ret == 0, "UUCLEAN:CANNOT CHDIR TO SPOOL - ", Spool, errno);*/
        if ( ret != 0 ){
		printf((catgets(nlmsg_fd,NL_SETN,700, "cannot chdir to %s directory \n")), Spool);
                exit(1);
        } 
	...
	while (gnamef(pdirf, file)) {
		...
		if (stat(file, &stbuf) == -1) {
			DEBUG(4, (catgets(nlmsg_fd,NL_SETN,701, "stat on %s failed\n")), file);
			continue;
		}
...
/* 3 char seq #'s...  jjn = &file[strlen(file) - 3];  */
			notfyuser(file);
			rkill(jjn);
		}
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,702, "unlink file %s\n")), file);
		if(unlink(file) == -1)
			continue;
...
register char *file;
{
	...
	for (i = 0; i < Npre; i++) {
		if (PREFIX(Pre[i], file))
			...
			DEBUG(6,(catgets(nlmsg_fd,NL_SETN,703, "Prefix: %s\n")),Pre[i]);
	}
	DEBUG(5,(catgets(nlmsg_fd,NL_SETN,704, "chkpre failed - file : %s\n")), file);
	return(FALSE);
}
chksys(file)
register char *file;
...
register char *file;
{
	...
	fclose(fp);
	sprintf(msg,
		"uuclean: File %s deleted. \nCould not contact remote or work file has corrupted entry. \n%d requests deleted.\n",
		 file, numrq);
	if (numrq == 1) {
		strcat(msg, "REQUEST: ");
		strcat(msg, frqst);
	...
	getargs(frqst, args, D_MAX);  /* HDB version of getargs has 3 args */
	/*getargs(frqst, args);*/
	/*
...
char	*u, *f, *m;
{
	...
	chmod(mf, 0666);
DEBUG(4,(catgets(nlmsg_fd,NL_SETN,709, "STM %d\n")),Cf);
/*      fprintf(Cf, "uucp job %.4s ", xf + strlen(xf)-4);       */
	fprintf(Cf, "uucp job %.4s ", xf + strlen(xf)-3); /* 3 char seq # */
	time(&clock);
	tp = localtime(&clock);
