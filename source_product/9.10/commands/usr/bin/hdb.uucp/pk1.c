...
register struct pack *ipk;
{
	...
	if (tries >= GETRIES) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,607, "tries = %d\n")), tries);
		pkfail();
	}

	Connodata++;
	DEBUG(9, (catgets(nlmsg_fd,NL_SETN,608, "pkgetpack: Connodata=%d\n")), Connodata);
	...
	sum |= (unsigned) *p << 8;
	h->sum = sum;
	DEBUG(9, (catgets(nlmsg_fd,NL_SETN,609, "rec h->cntl %o\n")), (unsigned) h->cntl);
	k = h->ksize;
	if (hdchk != h->ccntl) {
		 ...
		 * bad header
		 */
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,610, "bad header %o,")), hdchk);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,611, "h->ccntl %o\n")), h->ccntl);
		return;
	}
	if (k == 9) {
		if (h->sum + h->cntl == CHECK) {
			...
			*/
			if(Debug >= 7) {
		            sprintf(msgline,(catgets(nlmsg_fd,NL_SETN,612, "state -")));
			    delimc = ' ';
			 ...
			 * bad header
			 */
			DEBUG(7, (catgets(nlmsg_fd,NL_SETN,613, "bad header %o\n")), h->cntl);
			pk->p_state |= BADFRAME;
		}
		return;
	}
	if (k && pksizes[k] == pk->p_rsize) {
		...
		if ((bp = pk->p_ipool) == NULL) {
			DEBUG(7, (catgets(nlmsg_fd,NL_SETN,614, "bp NULL %s\n")), "");
			return;
		}
		pk->p_ipool = (char **) *bp;
...
char cntl;
{
  ...
  * writes
  */
DEBUG(9, (catgets(nlmsg_fd,NL_SETN,615, "send %o\n")), (unsigned) cntl);
	p = (caddr_t) & pk->p_ohbuf;
	if (x < 0) {
...
register int fn;
{
	...
	if (setjmp(Getjbuf)) {
		Ntimeout++;
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,618, "alarm %d\n")), Ntimeout);
		return(-1);
	}
