/*	@(#) $Revision: 70.1 $	*/
static char *RCS_ID="@(#)$Revision: 70.1 $ $Date: 91/11/07 10:48:54 $";
#ifndef NLS
...
#include	"uucp.h"
VERSION(@(#)uucp:uucleanup.c	1.6.1.1);

#ifdef	V7
#define O_RDONLY	0
#endif

#define USAGE	strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,710, "[-oDAYS] [-mSTRING] [-Cdays] [-Ddays] [-Wdays] [-Xdays] [-xLEVEL] [-sSYSTEM]")))
...
char **envp;
{
	...
	while ((i = getopt(argc, argv, "C:D:W:X:m:o:s:x:")) != EOF) {
		...
		case 'm':
			...
			break;
		default:
			(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,711, "\tusage: %s %s\n"))),
			    Progname, USAGE);
			exit(1);
		...
		case 'o':
		    value = atoi(optarg);
		    if (value < 1) {
			fprintf(stderr,(catgets(nlmsg_fd,NL_SETN,712, " Options: CDWXo require value > 0\n")));
			exit(1);
		    }
		    switch(i) {
		    case 'C':
	...
	if (argc != optind) {
		(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,713, "\tusage: %s %s\n")), Progname, USAGE));
		exit(1);
	}

	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,714, "Progname (%s): STARTED\n")), Progname);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,715, "Myname (%s), ")), Myname);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,716, "_ShortLocal (%s)\n")), _ShortLocal);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,717, "Days C.(%d), ")), _Cdays);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,718, "D.(%d), ")), _Ddays);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,719, "W.(%d), ")), _Wdays);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,720, "X.(%d), ")), _Xdays);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,721, "other (%d)\n")), _Odays);

	if (chdir(SPOOL) != 0) {
	    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,722, "CAN'T CHDIR (%s): errno (%d)\n")),
		SPOOL, errno);
		exit(1);
	}
	if ( (spooldir = opendir(SPOOL)) == NULL) {
	    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,723, "CAN'T OPEN (%s): errno (%d)\n")),
		SPOOL, errno);
		exit(1);
	}
	...
	while (gnamef(spooldir, f) == TRUE) {
		...
		while ( (++pass < 5) && (subdir = opendir(f)) ) {
		    DEBUG(7, (catgets(nlmsg_fd,NL_SETN,724, "Directory: %s\n")), f);
		    while (gnamef(subdir, subf) == TRUE) {
			DEBUG(9, (catgets(nlmsg_fd,NL_SETN,725, "file: %s\n")), subf);
...
char *fullname;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,726, "xprocess(%s), ")), fullname);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,727, "unlink(%s)\n")), fullname);
...
#define CMFMT  (catgets(nlmsg_fd,NL_SETN,729, "\n\t%s!%s -> %s!%s   (Date %2.2d/%2.2d)\n\nCan't be executed."))
#define XFMT  (catgets(nlmsg_fd,NL_SETN,730, "\n\t%s!%s  (Date %2.2d/%2.2d)\n"))
#define WFMT  (catgets(nlmsg_fd,NL_SETN,731, "\n\t%s!%s -> %s!%s   (Date %2.2d/%2.2d)\n"))
void
cprocess(fullname)
char *fullname;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,732, "cprocess(%s)\n")), fullname);


	fp = fopen(fullname, "r");
	if (fp == NULL) {
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,733, "Can't open file (%s), ")), fullname);
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,734, "errno=%d -- skip it!\n")), errno);
		return;
	}
	...
	if (s.st_size == 0) { /* dummy C. for polling */
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,735, "dummy C. -- unlink(%s)\n")), fullname);
	...
	DEBUG(3, (catgets(nlmsg_fd,NL_SETN,740, "text (%s)\n")), text);
...
char *dir, *file;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,741, "wprocess(%s)\n")), fullname);

	fp = fopen(fullname, "r");
	if (fp == NULL) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,742, "Can't open file (%s), ")), fullname);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,743, "errno=%d -- skip it!\n")), errno);
		return;
	}
	if (fstat(fileno(fp), &s) != 0) {
	    /* can't happen; _age() did stat of this file and file is opened */
	...
	if (s.st_size == 0) { /* dummy C. for polling */
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,744, "dummy C. -- skip(%s)\n")), fullname);
		(void) fclose(fp);
		return;
	}
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
	    ...
	    if (sscanf(buf,"%s%s%s%s%s%s", type, file1, file2,
	      user, opt, file3) <5) {
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,745, "short line (%s): ")), buf);
		DEBUG(5, (catgets(nlmsg_fd,NL_SETN,746, "bad D. -- skip(%s)\n")), fullname);
		(void) fclose(fp);
		return;
	    }
		...
		if (EQUALSN(file2, "X.", 2)) {
			...
			DEBUG(3, (catgets(nlmsg_fd,NL_SETN,748, "Can't read %s\n")), xfile);
			break;
		    }
		    *retaddr = *uline_u = *uline_m = *text = NULLCHAR;
			...
			case 'R':	/* save return address */
			    ...
			    DEBUG(7, (catgets(nlmsg_fd,NL_SETN,749, "retaddr (%s)\n")), retaddr);
			    break;
	...
	DEBUG(3, (catgets(nlmsg_fd,NL_SETN,751, "text (%s)\n")), text);
...
char *fullname;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,752, "oprocess(%s), ")), fullname);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,753, "unlink(%s)\n")), fullname);
...
char *fullname;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,755, "dDprocess(%s), ")), fullname);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,756, "unlink(%s)\n")), fullname);
...
char *fullname;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,758, "dXprocess(%s), ")), fullname);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,759, "  unlink(%s)\n")), fullname);
...
char *dir, *file;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,761, "dMprocess(%s)\n")), fullname);


	if (PREFIX(_ShortLocal, &file[2])) {
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,762, "  Local file %s: ")), file);
	}
	else {
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,763, "  Remote file %s: ")), file);
	}
	if (toWho(fullname, &toUser, &toSystem)) {
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,764, "toUser %s, ")), toUser);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,765, "toSystem %s  ")), toSystem);
	    ret = sendMail(toSystem, toUser, fullname, _Undeliverable);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,766, "Mail sent, unlink(%s)\n")), fullname);
	    (void) sprintf(text,
...
char *dir, *file;
{
	...
	(void) sprintf(fullname, "%s/%s", dir, file);
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,768, "dNprocess(%s)\n")), fullname);


	if (PREFIX(_ShortLocal, &file[2])) {
	    /* just delete it, the C. is gone */
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,769, "  Local file %s, ")), file);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,770, "unlink(%s)\n")), fullname);
	    (void) unlink(fullname);
	    ...
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,772, "  Remote file %s, ")), file);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,773, "exec rnews(%s), ")), fullname);
	    ret = execRnews(fullname);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,774, "unlink(%s)\n")), fullname);
	    (void) sprintf(text,
...
char *fullname;
{
	...
	fp = fopen(fullname, "r");
	if (fp == NULL) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,776, "Can't open file (%s), ")), fullname);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,777, "errno=%d -- skip it!\n")), errno);
		return(FAIL);
	}
	type = D_DATA;
	...
	for (i=0; i<NLINES; i++) {
	    DEBUG(9, (catgets(nlmsg_fd,NL_SETN,778, "buf: %s\n")), buf);
...
char *mtext[];
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,782, "Mail %s to ")), file);
	DEBUG(5, "%s\n", user);
	...
	DEBUG(7, (catgets(nlmsg_fd,NL_SETN,785, "sendMail: %s\n")), cmd);
...
char *file;
{
	...
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,789, "Rnews %s\n")), file);
...
char **user;	/* pointer to the user name */
{
	...
	*fuser = NULLCHAR;
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,791, "toWho(%s)\n")), file);
	fp = fopen(file, "r");
	for (i=0; i<NLINES; i++) {
	    if (fgets(buf, BUFSIZ, fp) == NULL)
		break;	/* no more lines */
	    DEBUG(9, (catgets(nlmsg_fd,NL_SETN,792, "buf: %s\n")), buf);
