...
#define RMESG(m, s) if (rmesg(m, s) != 0) {if (Turnoff) (*Turnoff)(); return(FAIL);}
#define RAMESG(s) if (rmesg('\0', s) != 0) {if (Turnoff) (*Turnoff)(); return(FAIL);}
#define WMESG(m, s) if(wmesg(m, s) != 0) {if (Turnoff) (*Turnoff)(); return(FAIL);}
...
top:
	...
	statfopt = 0;
	*Jobid = '\0';
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,293, "*** TOP ***  -  role=%d, ")), role);
	setline(RESET);
	if (role == MASTER) {
		...
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,295, "Wfile - %s,")), Wfile);
		strncpy(Jobid, BASENAME(Wfile, '/')+2, NAMESIZE);
		Jobid[NAMESIZE-1] = '\0';
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,296, "Jobid = %s\n")), Jobid);
		wrktype = W_TYPE[0];
		...
		if (wrktype == SNDFILE ) {
			...
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,301, "Request: %s\n")), rqstr);
			mf = W_SFILE;
			...
			if ( !READSOME(filename) && !READSOME(W_DFILE)) {
				...
				CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,305, "Failed: Access Denied\n")), 0);
				goto top;
			}
			...
			if ((fp == NULL )&& 
				...
				(void) sprintf(msg, "CAN'T READ %s %d",
					filename, errno);
				logent(msg, "FAILED");
				CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,311, "Failed: Can't Read %s\n")), filename);
				goto top;
			}
			Seqn++;
			setline(SNDFILE);
		...
		if (wrktype == RCVFILE) {
			...
			logent(rqstr,  "REQUEST");
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,314, "Request: %s\n")), rqstr);
			mf = W_RFILE;
			...
			if (chkperm(W_FILE1, filename, strchr(W_OPTNS, 'd'))) {
				...
				logent("DENIED", "ACCESS");
				lnotify(User, rqstr, "access denied");
				CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,318, "Failed: Access Denied--File: %s\n")), 
				    filename);
				goto top;
			}
			...
			if (((fp = fopen(Dfile, "w")) == NULL)
				...
				CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,322, "Failed: No Space!\n")), 0);
				unlinkdf(Dfile);
...
sendmsg:
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,323, "wrktype - %c\n ")), wrktype);
		WMESG(wrktype, msg);
		RMESG(wrktype, msg);
		goto process;
	}
...
process:
	 ...
	 */
	ultouch();
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,324, " PROCESS: msg - %s\n")), msg);
	switch (msg[0]) {

	case RQSTCMPT:
		DEBUG(4, "%s\n", (catgets(nlmsg_fd,NL_SETN,326, "RQSTCMPT:")));
		if (msg[1] == 'N') {
			i = atoi(&msg[2]);
			if (i < 0 || i > EM_MAX) 
				i = 0;
	...
	case HUP:
		DEBUG(4, "%s\n", (catgets(nlmsg_fd,NL_SETN,329, "HUP:")));
		if (msg[1] == 'Y') {
			WMESG(HUP, YES);
			(*Turnoff)();
			Rdmsg = imsg;
		...
		if ( (switchRole() == FALSE) || !iswrk(Wfile) ) {
                        strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,331, "SLAVE-switchRole (%s)\n")));
                        strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,332, "TRUE")));
                        strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,333, "FALSE")));
			DEBUG(4,msg1, switchRole() ? msg2:msg3);
			WMESG(HUP, YES);
			RMESG(HUP, msg);
			goto process;
		}
	...
	case SNDFILE:
		 ...
		 */
		DEBUG(4, "%s\n", (catgets(nlmsg_fd,NL_SETN,335, "SNDFILE:")));
		if (msg[1] == 'N')
		   {
		    i = atoi(&msg[2]);
		    if (i < 0 || i > EM_MAX)
		...
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,344, "msg - %s\n")), msg);
		CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,345, "Remote Requested: %s\n")), rqstr);
		(void) strcpy(filename, W_FILE2);
		expfile(filename);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,346, "SLAVE - filename: %s\n")), filename);
		if (chkpth(filename, CK_WRITE)
		     || chkperm(W_FILE1, filename, strchr(W_OPTNS, 'd'))) {
			WMESG(SNDFILE, EM_RMTACC);
			logent("DENIED", "PERMISSION");
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,349, "Failed: Access Denied\n")), 0);
			goto top;
		}
		(void) sprintf(User, "%s", W_USER);

		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,351, "chkpth ok Rmtname - %s\n")), Rmtname);
		TMname(Dfile, pnum); /* get TM file name */

		if  (((fp = fopen(Dfile,  "w")) == NULL) || nospace(Dfile) ) {
			WMESG(SNDFILE, EM_NOTMP);
			logent("CAN'T OPEN", "DENIED");
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,355, "Failed: Can't Create Temp File\n")), 0);
			unlinkdf(Dfile);
			goto top;
		}
		Seqn++;
		...
		if (status == 0) {
			...
			status = putinpub(filename, Dfile,
			    BASENAME(W_USER, '!'));
			DEBUG(4, (catgets(nlmsg_fd,NL_SETN,361, "->PUBDIR %d\n")), status);
			if (status == 0)
	...
	case RCVFILE:
		 ...
		 */
		DEBUG(4, "%s\n", (catgets(nlmsg_fd,NL_SETN,363, "RCVFILE:")));
		if (msg[1] == 'N') {
			i = atoi(&msg[2]);
			if (i < 0 || i > EM_MAX)
				i = 0;
		...
		logent(rqstr, "REMOTE REQUESTED");
		CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,375, "Remote Requested: %s\n")), rqstr);
		mf = W_RFILE;
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,376, "msg - %s\n")), msg);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,377, "W_FILE1 - %s\n")), W_FILE1);
		(void) strcpy(filename, W_FILE1);
		expfile(filename);
		...
		if (requestOK() == FALSE) {
			...
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,382, "Failed: Access Denied\n")), 0);
			goto top;
		}
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,383, "requestOK for Loginuser - %s\n")), Loginuser);

		if (chkpth(filename, CK_READ) || !READANY(filename)) {
			WMESG(RCVFILE, EM_RMTACC);
			logent("DENIED", "PERMISSION");
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,386, "Failed: Access Denied\n")), 0);
			goto top;
		}
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,387, "chkpth ok Loginuser - %s\n")), Loginuser);

		if ((fp = fopen(filename, "r")) == NULL) {
			WMESG(RCVFILE, EM_RMTACC);
			logent("CAN'T OPEN","DENIED");
			CDEBUG(1, (catgets(nlmsg_fd,NL_SETN,391, "Failed: Can't Open %s\n")), filename);
			goto top;
		}
...
char *msg, c;
{
	...
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,393, "rmesg - '%c' ")), c);
	if ((*Rdmsg)(msg, Ifn) != 0) {
                
		strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,394, "got %s\n")));
                strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,395, "FAIL")));
		DEBUG(4, msg1,msg2);
	...
	if (c != '\0' && msg[0] != c) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,398, "got %s\n")), msg);
		(void) sprintf(str, "expected '%c' got %s", c, msg);
		logent(str, "BAD READ");
		return(FAIL);
	}
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,401, "got %s\n")), msg);
	return(0);
}
...
char *s, m;
{
	CDEBUG(4, (catgets(nlmsg_fd,NL_SETN,402, "wmesg '%c'")), m);
	CDEBUG(4, (catgets(nlmsg_fd,NL_SETN,403, "%s\n")), s);
...
register char *msgcode;
{
	...
	DEBUG(4,(catgets(nlmsg_fd,NL_SETN,404, "mailopt %d, ")), mailopt);
	DEBUG(4,(catgets(nlmsg_fd,NL_SETN,405, "statfopt %d\n")), statfopt);
...
char	*f, *m;
{
	...
	DEBUG(4,(catgets(nlmsg_fd,NL_SETN,413, "STMES %s\n")),mf);
...
register char *c;
{
	...
	for (p = Ptbl; p->P_id != '\0'; p++) {
		if (*c == p->P_id) {
			...
			if ((*Turnon)() != 0)
				break;
			CDEBUG(4, (catgets(nlmsg_fd,NL_SETN,417, "Proto started %c\n")), *c);
			return(SUCCESS);
		}
	}
	CDEBUG(4, (catgets(nlmsg_fd,NL_SETN,418, "Proto start-fail %c\n")), *c);
	return(FAIL);
}
