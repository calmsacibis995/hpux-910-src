...
#define DVCELEN 3       /* S300, 400, 700 device file is ttydxx */
	...
	if (dcf == -1) {
		...
		/* If device file starts with cua/cul, create a lock file
		   with name prefixed with ttyd and the same file extension.
		   This is done to detect if there is any incoming call
		   already in place on this line, and also to block uugetty
		   from accepting an incoming call. */

		(void) sprintf (dvc, dev[D_LINE]);
		if (( dvc[0] == 'c') && (dvc[1] == 'u'))
		{
		   ...
		   if (mlock (dvcttyd) == FAIL) {
			DEBUG(5, (catgets(nlmsg_fd,NL_SETN,11, 
			     "mlock %s failed\n")), dvcttyd);
		  	Uerror = SS_LOCKED_DEVICE;
			/* Unlock the cu[a/l] lock file */
			delock (sdev[D_LINE]);
			return(FAIL);
		   }
		   DEBUG(5, (catgets(nlmsg_fd,NL_SETN,12,
			     "mlock %s succeeded\n")), dvcttyd);
		}
		/*
		 * Open the line
		...
		(void) alarm(10);
                ...
                if ((tmp = open(dcname,O_RDWR | O_NDELAY)) == -1)
		{
			errnosav=errno;
		        (void) alarm(0);
			DEBUG(1, (catgets(nlmsg_fd,NL_SETN,16, "generic open (%d) failed, errno = %d\n")), 1,errnosav);
			(void) close(nullfd);
			logent("generic open", "FAILED");
			Uerror = SS_CANT_ACCESS_DEVICE;
			goto bad;
		}
                /* set VMIN value to 1 char */
                onechar++;
...
Success:
#ifdef DATAKIT
	if(strcmp(Dc, "DK") != 0)	/* if (!DK) {re-open}  -MJF	*/
	{
#endif /*DATAKIT*/
...
char *flds[], *dev[];
{
	...
	if (*dev[D_LINE] == '0')
#endif
		fd = dkndial(dev[D_ARG], 0);	/* new libdk.a format - MJF */
#ifndef STANDALONE
	else
		fd = dkndial(dev[D_ARG], atoi(dev[D_LINE]));  /* new libdk.a format - MJF */
#endif
	(void) alarm(0);
	(void) strcpy(Dc, "DK");
	if (fd < 0) {
...
char *flds[], *dev[];
{
    ...
    /* \T,\D in the dev[] translate to the phone and \S translate to the
       speed and \P translate to the protocol as specified in Systems
     */
   j=0;
   sleep(2);
   strcpy(prog,dev[D_CALLER]+4);   /* Skip the keyword PROG */
   dev[D_CALLER] = dev[D_CALLER] + 4;
   CDEBUG(1,(catgets(nlmsg_fd,NL_SETN,69,"Dialing using program %s\n")),prog);
   for (i=D_CALLER+1; dev[i] != NULL; i++){
        if (!strcmp(dev[i],"\\P")){
                 strcpy(protocol,protoString());  /* get protocol */
                 dev[i] = protocol;
        }
        if (!strcmp(dev[i],"\\S"))
                 dev[i] = dev[D_CLASS];
