...
register struct pack *pk;
{
	...
	if ( ! ISCNTL(c) ) {
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,600, "not cntl\n")));
		return;
	}
	...
	case INITA:
		if (val==0 && pk->p_state&LIVE) {
			fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,601, "alloc change not implemented\n")));
			break;
		}
		if (val) {
			pk->p_state |= INITa;
	...
	case RR:
		pk->p_rpr = val;
		(void) pksack(pk);
		DEBUG(9, (catgets(nlmsg_fd,NL_SETN,602, "pkcntl: RR/RJ: Connodata=%d\n")), Connodata);
		break;
	case SRJ:
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,603, "srj not implemented\n")));
		break;
	case CLOSE:
...
char **bp;
	...
	if (rcheck  != pk->p_rwindow) {
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,604, "r short %d want %d\n")),rcheck,pk->p_rwindow);
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,605, "rcount = %d\n")),pk->p_rcount);
		fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,606, "xcount = %d\n")),pk->p_xcount);
	}
	free((char *) pk);
}
