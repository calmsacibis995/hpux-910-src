 ...
 *	or	unpack filename...
 */
/*  see code for pack.c for more detail in comments */
...
getdict ()
{
	...
	if (inbuff[1] == US) {		/* oldstyle packing */
		if (setjmp (env))	/* in case of failure on decode
					of old style set for return of
					0  */
			return (0);
		expand ();
		return (1);
	...
	inp = &inbuff[2];
	origsize = 0;
	for (i=0; i<4; i++)	/* reconstruct the integer */
...
goof:		eprintf (".z: not in packed format");
	...
	for (i=1; i<=maxlev; i++)
		intnodes[i] = *inp++ & 0377;
	for (i=1; i<=maxlev; i++) {		/* get the meat of the dictary*/
		tree[i] = eof;
...
uggh:			eprintf (".z: unpacking error");
		...
		c = *inp++;
		bitsleft = 8;
		while (--bitsleft >= 0) {	/* break out the bit codes
						 from their encasing bytes */
			i *= 2;
			if (c & 0200)
				i++;
			c <<= 1;
	...
	*argv = p1 + 1;
	...
	for (k=1; k<argc; k++) {
		...
		*cp = '\0';
		...
		if (pcat)
		...
		else {			/* otherwise checkout unpack file */
			...
			if ((outfile = creat (argvk, status.st_mode&07000)) == -1) {
				eprintf (": cannot create");
				goto done;
			}
#ifdef ACLS
			/*
		 	* If the packed file has optional ACL entries, they
			* should be copied to the unpacked file.  Note this
			* does an fstat to get the uid and gid of the unpacked
			* file in case the file already existed and has
			* different ownerships (in this case the cpacl or
			* chmod may fail).
		 	*/
			if (status.st_acl) {
			...
			{
			/*
		 	* File has no optional ACL entries, or is remote.
			* Chmod is then more efficient to use than cpacl.
		 	*/
