...
char string[LINE_MAX+1], *str = {string};
int crs = '0';
...
e	:  e '+' e
	...
	|  LETTER DECR
		= bundle(4, "l", $1, "d1-s", $1 );
	| LETTER '[' e ']' INCR
		= bundle(7,$3,";",geta((char *)$1),"d1+",$3,":",
				geta((char *)$1));
	| INCR LETTER '[' e ']'
		= bundle(7,$4,";",geta((char *)$2),"1+d",$4,":",
				geta((char *)$2));
	| LETTER '[' e ']' DECR
		= bundle(7,$3,";",geta((char *)$1),"d1-",$3,":",
				geta((char *)$1));
	...
	| LETTER '[' e ']' EQOP e
		= { bundle(9,$3,";",geta((char *)$1),$6,$5,"d",$3,":",
				geta((char *)$1)); }
	| LENGTH '(' e ')'
		= bundle(2,$3,"Z");
	| SCALE '(' e ')'
...
restart:
	...
	case '=':
		...
		case '+': c=EQPL; goto obsolete;
		case '-': c=EQMI; 
			  if (!equal_minus) {
				fprintf(stderr,"old style assignment operator ambiguous; unary minus assumed >> =- <<\n");
				equal_minus=1;
			  }
			  return( '=' );
		case '*': c=EQMUL; goto obsolete;
		case '/':
			c=EQDIV;
			peekc = -1;
                	if ((peekc = getch()) == '*') {
                        	peekc = -1;
                        	while (((c = getch()) != '*') || ((peekc = getch()) != '/'))
                                	if (c == '\n') ln++;
                        	peekc = -1;
                        	return( '=' );
                	}
                	else {
				fprintf(stderr,"old style assignment operator causes syntax error >> =/ <<\n");
                        	return( _FOR ); /* cause syntax error! */
			}
		case '%': c=EQREM; goto obsolete;
		case '^': c=EQEXP; goto obsolete;
		default:   return( '=' );
		obsolete:
			fprintf(stderr,"old style assignment operator causes syntax error >> =%c <<\n",peekc);
                        return( _FOR ); /* cause syntax error! */
		gotit:
			peekc = -1; return(c);
		  }
...
va_dcl
{
	...
	q = b_sp_nxt;
	if ( bdebug )
		(void)fprintf(stderr,"bundle %d elements at %o\n",i,  q );
	while(i-- > 0){
		if( b_sp_nxt >= & b_space[b_sp_max] ) yyerror( "bundling space exceeded" );
		* b_sp_nxt++ = va_arg(p, int);
	}
	* b_sp_nxt++ = 0;
	yyval = (int)q;
}
...
routput(p) int *p; {
	if( bdebug ) (void)fprintf(stderr,"routput(%o)\n", p );
	...
	else (void)printf( (char *)p );	 /* character string */
}
...
yyerror( s ) char *s; {
	...
	(void)printf("c[%s on line %d, %s]pc\n", s ,ln+1,ss);
	(void)fflush(stdout);
	cp = cary;
	crs = rcrs;
	bindx = 0;
...
yyinit(argc,argv) int argc; char *argv[];{
	(void)signal( 2, (int(*)())1 );	/* ignore all interrupts */
	sargv=argv;
	sargc= -- argc;
	if(sargc == 0)in=stdin;
