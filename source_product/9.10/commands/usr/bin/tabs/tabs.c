...
struct ttab termtab[] = {
	"",		tclrhp,	132,  /** changed to default to hp -dds */
	"1620-12",	tclrsh,	158,
...
char **argv;
{
	...
	int x;
	char dashx [ 256 ];		/* used to hold fake -x option string */
	int num_tabspec = 0;	/* # of tabspec's specified, only 1 is allowed */
	int optionc;			/* getopt return value */
	char argstr [ 256 ];	/* string to hold manufactured parameters */
	char n_str [ 8 ];		/* string to hold -n values temporarily */

#ifdef NLS16			/* initialize to the current locale */
	if (!setlocale(LC_ALL, "")) {
		fputs(_errlocale("tabs"), stderr);
		putenv("LANG=");
	}
#endif NLS16
	...
	/* The following for loop alters the argv strings so they agree
     * with getopt syntax conventions.  
	 * The n1,n2,n3,,, tabspec (if used) is changed to a -xn1,n2,n3 option.  
	 * This option is only for the benefit of the getopt function.  It is 
	 * not documented externally.  
	 *
	 * The +m option is changed to -m and a default value is added 
	 * The -a and -c options are converted to -a0 and -c0.  
	 * The -n options are changed to -n-number 
	 */
	for ( x=1 ; x < argc ; x++ ) {
		/* change +m to -m */
		if ( strncmp ( argv [ x ], "+m", 2 ) == 0 ) 
			if ( strcmp ( argv [ x ], "+m" ) == 0 )
				argv [ x ] = "-m10"; 	/* set default = 10 */
			else 
				argv [ x ] [ 0 ] = '-';

		/* change n1,n2,n3,, to -xn1,n2,n3,, */
		if ( argv [ x ] [ 0 ] != '-' ) {
			strcpy ( dashx, "-x" );
			strcat ( dashx, argv [ x ] );
			argv [ x ] = dashx; 
		...
		/* change -a and -c to -a0 and -c0 */
		if ( strcmp ( argv [ x ], "-a" ) == 0 )
			argv [ x ] = "-a0";
		if ( strcmp ( argv [ x ], "-c" ) == 0 )
			argv [ x ] = "-c0";

		/* change -Number to -nNumber for getopt */
	...
	while ((optionc = getopt(argc, argv, "a:c:x:fpsuT:m:n:-:")) != EOF){
		switch ( optionc ) {
		case 'm':	margflg++;
					margin = atoi ( argv [ optind ] );
					if ( margin == 0 )
						margin = 10;
					break;
		...
		case '-':	tabspec = optarg -= 2;
					num_tabspec++;
					break;

		case 'n':	strcpy ( argstr, optarg );
					tabspec = optarg;
					num_tabspec++;
					break;

		case 'x':	tabspec = optarg;
					num_tabspec++;
					break;
		...
		default:  error ((catgets(catd,NL_SETN,1, "unknown tab code")) );
		}
	} 

	if ( num_tabspec > 1 )
		error ( (catgets(catd,NL_SETN,2, "only one tabspec allowed")) );
	...
	if (err <= 0 || columns <= 0 || strcmp(set_tab,"f(") == 0) {
		...
		tt = termtab;
		tt->tclr = clear_all_tabs;
		/* ensure we start at beginning of line */
		if (tt->tclr) strcat(tt->tclr,"\r");
	}
...
int tabvect[NTABS], level;
{
    ...
    if (*scan == '-')
        if ((c = *++scan) == '-')
            ...
            error((catgets(catd,NL_SETN,1, "unknown tab code")));
        else;
    else
...
int tabvect[NTABS];
{
	...
	for (i = 0; i<NTABS-1;) {
		if (*scan == '+') {
			...
			if (t = getnum(&scan))
				tabvect[i++] = last += t;
			else error((catgets(catd,NL_SETN,3, "illegal increment")));
		}
		else {
			if ((t = getnum(&scan)) > last)
				tabvect[i++] = last = t;
			else error((catgets(catd,NL_SETN,4, "illegal tabs")));
		}
		if (*scan++ != ',') break;
	}
	if (last > NCOLS)
		error((catgets(catd,NL_SETN,4, "illegal tabs")));
	tabvect[i] = 0;
}
...
#define	BSTATUS		b_status = BYTE_STATUS((unsigned char)c, b_status)
    int	b_status = ONEBYTE;		/* initialization */
#endif	NLS16
...
int tabvect[NTABS];
{
	...
	char *temp;
	if (level)
		error((catgets(catd,NL_SETN,5, "file indirection")));
	if ((fildes = open(scan,0)) < 0)
		error((catgets(catd,NL_SETN,6, "can't open")));
...
register int	tabvect[] ;
{
	...
	if(ioctl(1, U370TABS, &hbuf) == -1){
		fputs((catgets(catd,NL_SETN,7, "Error return from ioctl.\n")), stderr);
		perror("tabs") ;
	}
}
#endif
...
char *qq;
{
	...
	register char *q;
	q = qq;
	if (tt->tclr == TCLRLN) {	      /* if repetitive sequence */
		*p++ = CR;
