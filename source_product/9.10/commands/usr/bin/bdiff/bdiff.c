...
char *argv[];
{
	...
	char oline[BUFSIZ], nline[BUFSIZ], diffline[TWOBUFSIZ];
	char *olp, *nlp, *dp;
	int i, otcnt, ntcnt;
	int pfd[2];
	...
	*/
	while (1) {
		...
		*/
		...
		if ( fclose(poldtemp) != 0 )
		  {								/* fclose does a flush which can fail */
			Fflags |= FTLMSG;
			fatal("bdiff: can not close temporary file (bd7)");
		  }
		...
		*/
		...
		if ((i = fork()) < 0) {
			...
			*/
			...
			Process 'diff' output.
			*/
			while ((dp = fgets(diffline,TWOBUFSIZ,pipeinp))) {
				if (numeric(*dp))
					fixnum(diffline);
				else
					printf("%s",diffline);
...
FILE *iptr;
{
	...
	if ( fclose(temptr) != 0 )  /* If fclose does not return 0, then      */
		{						/* fflush has failed, 'cause /tmp is full */
		Fflags |= FTLMSG;
		fatal(" bdiff: can not close temporary file (bd7)");
		}
}
...
char *file;
{
	...
	if ( iop == NULL )			/* If /tmp is full, can't create new file */
		{
		sprintf( Error,"bdiff: can not create temporary file (bd7)");
		fatal( Error );
		}
