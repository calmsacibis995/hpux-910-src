...
char * argv[];
{
        ...
        char * cmd_args[6];	 /* holds rdiff command arguments	    */
	char * rdiff = RDIFF;	 /* rdiff command name			    */
		...
		int headnumlength;		 /* length of current head rev number       */
	...
	check_in = true;
        ...
        while (--argc,++argv, argc>=1 && ((*argv)[0] == '-')) {
                ...
                case 'm':
                        if ((*argv)[2]!='\0'){
                                ...
                                msglen=strlen(msg);
                                if (msglen >= logsize) {
                                   warn1i("log message truncated to %d characters",
                                        logsize);
                                   msg[logsize-2]='\n';
                                   msg[logsize-1]='\0';
                                }
        ...
        diagnose2s("%s  <--  %s", RCSfilename,workfilename);

        if (access(workfilename,4)!=0) {
                error1s("working file %s not readable or nonexistent",
                       workfilename);
                continue;
        }
        ...
        if (keepflag) {
                ...
                if (rev==nil && *(rev=prevrev)=='\0') {
                        error1s("Can't find a revision number in %s",workfilename);
                        continue;
                }
                if (*prevdate=='\0') {
                        error1s("Can't find a date in %s",workfilename);
                        continue;
                }
                if (*prevauthor=='\0')
                        warn1s("Can't find an author in %s", workfilename);
                if (*prevstate=='\0')
                        warn1s("Can't find a state in %s", workfilename);
		...
		headnumlength = (Head == nil) ? 0 : countnumflds(Head->num);
        ...
        if (initflag || rcsinitflag ) {
                ...
                if (newdnumlength==0) strcpy(newdelnum,"1.1");
                        ...
                        error1s("Branch point does not exist for %s",
                        newdelnum);
                        continue;
                ...
                } elsif ((targetdelta->next==nil) &&
        		  (countnumflds(olddeltanum)>headnumlength)) {
        ...
        } elsif (newdnumlength<headnumlength-1) {
				error1s("Revision number %s is not a branch or trunk revision",
				newdelnum);
				continue;
		  } elsif ( newdnumlength<=headnumlength) {
                /* add new head per given number */
                /* put new revision on trunk */
                olddeltanum=Head->num;
                if(newdnumlength==headnumlength-1) {
                        /* make a two-field number out of it*/
                        if (cmpnumfld(newdelnum,olddeltanum,headnumlength-1)==0)
                                incnum(olddeltanum,newdelnum);
                        else    strcat(newdelnum, ".1");
                }
                if (cmpnum(newdelnum,olddeltanum) <= 0) {
                        error2s("deltanumber %s too low; must be higher than %s",
                              newdelnum,Head->num);
                        continue;
                }
...
/*                if(!(targetdelta=removelock(caller,nil))) continue; */
                ...
                *(tp-1) = '\0'; /* kill final dot */
                ...
                if (cmpnum(targetdelta->num,branchpointnum)!=0) {
                        error1s("Cannot find branchpoint %s",branchpointnum);
                        continue;
                }
                if (!addbranch(targetdelta,newdelnum)) continue;
        }
        if (initflag||rcsinitflag) {
                diagnose1s("initial revision: %s",newdelnum);
        } else  diagnose2s("new revision: %s; previous revision: %s",
        ...
        if (!keepflag) {
                ...
                if (newdelta.next!=nil &&
                    cmpnum(prevdate,newdelta.next->date)<=0) {
                        error2s("Date in %s is older than existing revision %s",
        ...
        if (initflag||rcsinitflag) {
                ...
                if (&newdelta==Head) {
                        ...
                        newdelta.log=getlogmsg();
			if ((cmd_args[0] = rindex(rdiff, '/')) != NULL)
				cmd_args[0]++;
			else cmd_args[0] = rdiff;
			cmd_args[1] = "-n";
			cmd_args[2] = workfilename;
			cmd_args[3] = expfilename;
			cmd_args[4] = NULL;
			setruser();
			exit_stats = execstdout(rdiff, cmd_args, diffilename);
			seteuser();
                        if ((exit_stats > 1) || (exit_stats < 0))
                            faterror1s("%s failed", cmd_args[0]);
                        /* -1 = execstdout failure; 2.. = rdiff failure */
                        if(!putdtext(newdelnum,newdelta.log,workfilename,frewrite)) continue;
			...
			cmd_args[1] = "-n";
			cmd_args[2] = expfilename;
			cmd_args[3] = workfilename;
			cmd_args[4] = NULL;
			setruser();
			exit_stats = execstdout(rdiff, cmd_args, diffilename);
			seteuser();
	...
	rcsholdsigs();
        if (RENAME(newRCSfilename,RCSfilename)<0) {
                error2s("Can't write new RCS file %s; saved in %s",
	...
	rcsallowsigs();
		...
		rcsholdsigs();
                ...
                if (link(newworkfilename,workfilename)<0) {
                    error1s("Can't expand keywords in %s",workfilename);
		    rcsallowsigs();
                    continue;
                }
 ...
 */
{
        ...
        while (next!=nil) {
                ...
                if (num==nil) {
                        ...
                        if ((whomatch!=0)&&(nummatch==0)) {
                            error2s("revision %s locked by %s",num,next->login);
                            return false;
                        }
                }
                trail=next;
        ...
        if (next!=nil) {
                ...
                if (!owner) {
                        if (num==nil) error1s("no lock set by %s",who);
                        else          error2s("no lock set by %s for revision %s",who,num);
                        return nil;
                } elsif (num!=nil) {
                        return delta;
...
{       char * targetfname;
        ...
        if ((unexfile=fopen(unexfname,  "r" ))==NULL ||
            (exfile  =fopen(targetfname,"w"))==NULL) {
                error1s("Can't create local copy of %s.  Continuing...",unexfname);
                return nil;
        }
...
{       register int c;
        ...
        /* files are the same */
        diagnose2s("File %s is unchanged with respect to revision %s",
        ...
        if (result==false) {
                if (quietflag) {
                    warn3s("checkin aborted since %s was not changed; %s %sdeleted.",
                             workfilename,workfilename,keepworkingfile?"not ":"");
                } else {
                    diagnose2s("checkin aborted; %s %sdeleted.",
	...
	int cin;
        ...
        for (;;) {
                ...
                if (tp>=logmsg+logsize-2) { /* overflow */
                        if (!isatty(fileno(stdin))) {
                                warn1i("log message truncated to %d characters",logsize-2);
                                logmsg[logsize-2]='\n';logmsg[logsize-1]='\0';
                                return logmsg;
                        }
