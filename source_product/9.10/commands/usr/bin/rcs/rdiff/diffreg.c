...
{
	register int k;
	register char **startargv, **nextargv;

	if (hflag) {
		startargv = diffargv;
		diffargv[0] = "diffh";
		for (nextargv = diffargv; *nextargv != NULL; nextargv++)
			if (**nextargv != '-')
				*diffargv++ = *nextargv;
		*diffargv = NULL;
		execv(diffh, startargv);
	...
	if ((stb1.st_mode & S_IFMT) == S_IFDIR)
		...
		if (!strcmp(file2, "-")) {
			fprintf(stderr, "rdiff: can't specify - -\n");
			done();
		}
		file1 = copytemp();
...
copytemp()
{
	...
	if (signal(SIGHUP, SIG_IGN) != SIG_IGN) signal(SIGHUP, done);
	if (signal(SIGINT, SIG_IGN) != SIG_IGN) signal(SIGINT, done);
	if (signal(SIGQUIT, SIG_IGN) != SIG_IGN) signal(SIGQUIT, done);
	signal(SIGPIPE,done);
	signal(SIGTERM,done);
	...
	char *dir, *file;
{
	...
	if (!strcmp(file, "-")) {
		fprintf(stderr, "rdiff: can't specify - with other arg directory\n");
		done();
	}
	tail = rindex(file, '/');
	if (tail == 0)
...
change(a,b,c,d)
{
	...
	if(opt == D_CONTEXT) {
		...
		printf("\n");
		if (ch == 'a')
			fetch(ixold,lowa,upb,input[0],"  ",0);
		else {
		...
		printf(" -----\n");
		if (ch == 'd')
			fetch(ixnew,lowc,upd,input[1],"  ",0);
		else {
