...
char * argv[];
{
	...
	check_in = false;
        ...
        while (--argc,++argv, argc>=1 && ((*argv)[0] == '-')) {
                ...
                case 'A':  /*  append access list according to accessfile  */
                        ...
                        *argv = *argv+2;
                        if( pairfilenames(1, argv, true, true|no_pipe) > 0) {
        ...
        if ( initflag ) {
            switch( pairfilenames(argc, argv, false, true|no_pipe) ) {
                case -1: break;
                case  0: continue;     /*  can't open  */
                case  1: error1s("file %s exists already", RCSfilename);
                         fclose(finptr);
                         continue;
            }
	}
        else  {
            switch( pairfilenames(argc, argv, true, true|no_pipe) ) {
            ...
            if ( expandsym(curstate->revno, &numrev[0]) ) {
                ...
                if ( target )
                   if ( !(countnumflds(&numrev[0])%2) && cmpnum(target->num, &numrev[0]) )
                        error2s("Can't set state %s of a nonexistent revision %s",
	...
	putdesc(initflag,textflag,textfile,quietflag);
	    ...
	    rcsholdsigs();		/* ignore interrupts */
            if(RENAME(newRCSfilename,RCSfilename)<0) {
                error2s("Can't create RCS file %s; saved in %s",
		...
		rcsallowsigs();
            ...
            if (!initflag) /* preserve mode bits */
                if (chmod(RCSfilename,filestatus.st_mode & ~0222)<0)
                        warn1s("Can't set mode of %s",RCSfilename);

	    rcsallowsigs();		/* catch them all again */
            diagnose("done");
        } else {
            diagnose1s("%s unchanged.",RCSfilename);
        }
        } while (cleanup(),
                 ++argv, --argc >=1);
...

{
        ...
        sp = temp2; colon = ((c = *sp) == ':');   *sp = '\0';
        while( c == ' ' || c == '\t' || c == '\n')  c = *++sp;

        if ( c != ':' && c != '\0') {
	    error1s("Invalid string %s after option -n or -N",sp);
            return;
        }
...
{
        struct  access  * curpt, * pt;
	static struct access *pre;
        char    *temp;
        register c;
...
/*              and place in rplaccessor                */
{
        ...
        if ( (finptr=fopen(accessfile, "r")) == NULL) {
            faterror1s("Can't open file %s", accessfile);
        }
        Lexinit();
        nextp = &accessorlst[0];

        if ( ! getkey(Khead)) faterror1s("Missing head in %s", accessfile);
        getnum();
        if ( ! getlex(SEMI) ) serror1s("Missing ';' after head in %s",accessfile);
        ...
        if (getkey(Ksuffix)) {
            ...
            if ( ! getlex(SEMI) ) serror1s("Missing ';' after suffix in %s",accessfile);
        }
#endif

        if (! getkey(Kaccess))fatserror1s("Missing access list in %s",accessfile);
        curaccess = nil;
        while( id =getid() ) {
        ...
        if ( ! getlex(SEMI))serror1s("Missing ';' after access list in %s",accessfile);
        return;
}
...
/*              and place in delrev                             */
{
        ...
        if ( c == '<' || c == '-' ) {  /*  -o  -rev  or <rev  */
            ...
            *sp = '\0';
            ...
            if ( c != '-' && c != '<') {
                faterror2s("Invalid range %s %s after -o", pt->strt, sp);
                free(pt);
                return;
            }
            while( (c = *++sp) == ' ' || c == '\n' || c == '\t')  ;
...
{       struct hshentry * nextdelta;
        ...
        do {
                ...
                if (!(nextdelta=getnum())) {
                    if(delta)
                        faterror1s("Can't find delta for revision %s", delta->num);
...

{
        ...
        if ( flag ) warn1s("Can't remove a nonexisting accessor %s",who->login);
        return sourcelst;
}
 ...
 */
{
        ...
        if ( (mailmess = fopen(messagefile, "w")) == NULL) {
            faterror1s("Can't open file %s", messagefile);
        }

        fprintf(mailmess, "Subject: Broken lock on %s\n\n",RCSfilename);
	if (ferror(mailmess)) faterror("write error");
	...
	SENDMAIL	/* system dependent - defined in system.h */
 ...
 */
{
               ...
               if ((whor=strcmp(who,next->login))==0 &&
                ...
                if (num!=nil && numr==0 && whor !=0) {
                        if (!sendmail( num, next->login)){
                            diagnose2s("%s still locked by %s",num,next->login);
                            return nil;
        ...
        if (next!=nil) {
                /*found one */
                diagnose1s("%s unlocked",next->delta->num);
                ...
                if (delta)
                    error2s("no lock set by %s for revision %s", who, num);
                else
                    error1s("no lock set by %s",who);
                return nil;
        }
}
	...
	struct lock  *lockpt;
        ...
        while( pt != tail) {
            if ( pt->branches ){ /*  a branch point  */
                flag = true;
                error1s("Can't remove branch point %s", pt->num);
            }
	    lockpt = Locks;
	    ...
	    if ( lockpt ) {
		flag = true;
		error1s("Can't remove locked revision %s",pt->num);
	    }
            pt = pt->next;
        }
        ...
        if ( ! flag ) {
            ...
            while( pt != tail ) {
                pt->selector = DELETE;
                diagnose1s("deleting revision %s ",pt->num);
                pt = pt->next;
            }
        }
        return flag;
...

{
        ...
        if ( length % 2 ) {   /*  invalid branch after -o   */
            error1s("Invalid branch range %s after -o", &numrev[0]);
            return 0;
        }

        if ( delrev->code == 1 )  {  /*  -o  -rev   */
            if ( length > headnumlength ) {
                temp = searchcutpt( target->num, length-1, gendeltas);
                cuttail = target->next;
            }
            else {
                ...
                cuttail = target;
                while( cuttail && ! compartial(target->num,cuttail->num,headnumlength-1) )
        ...
        if ( delrev->code == 2 )  {   /*  -o  rev-   */
            if ( length == headnumlength ) {
                temp = searchcutpt(target->num, headnumlength-1,gendeltas);
                if ( flag)
                    cuttail = target;
                else
                    cuttail = target->next;
        ...
        if ( length != countnumflds( &numrev[0] ) ) {
            error2s("Invalid revision range %s-%s", target->num, &numrev[0]);
            return 0;
        }
        if ( length > headnumlength && compartial( &numrev[0], target->num, length-1) ) {
            error2s("Invalid revision range %s-%s", target->num, &numrev[0]);
            return 0;
        }
        ...
        if ( length > headnumlength) {  /* delete revisions on branches  */
            ...
            if ( flag ) {
                if ( ! cmpnum(target->num, target2->num) ) {
                    error2s("Revisions %s-%s don't exist", delrev->strt,delrev->end);
                    return 0;
                }
                cuthead = target;
                temp = target->next;
	...
	char *assocrev;
		...
		if ( pt )
			...
			pre->nextassoc = pt->nextassoc;
                    ...
                    warn1s("Can't delete nonexisting symbol %s",curassoc->ssymbol);
	    }
            else {
		if (*curassoc->revno == ':')
			assocrev = Head->num;
		else
			assocrev = curassoc->revno;
		if ( expandsym(assocrev, numrev) ) {
	    	/*   add symbol  */
			target = (struct hshentry *) malloc(sizeof(struct hshentry));
			target->num = strsav(numrev);
...

{
        ...
        while( lockpt ) {
            if (expandsym(lockpt->revno, &numrev[0]) ) {
                ...
                if ( target )
                   if ( !(countnumflds(&numrev[0])%2) && cmpnum(target->num,&numrev[0]) )
                        error1s("Can't unlock a nonexisting revision %s",lockpt->revno);
                   else
        ...
        while( lockpt ) {
            if (expandsym(lockpt->revno, &numrev[0]) ){
                ...
                if ( target )
                   if ( !(countnumflds(&numrev[0])%2) && cmpnum(target->num,&numrev[0]))
                        error1s("Can't lock a nonexisting revision %s",lockpt->revno);
                   else
                        if(lpt=addlock(target, caller))
                            diagnose1s("%s locked",lpt->delta->num);
            }
            lockpt = lockpt->nextrev;
        }
...
/*              change to delta text                                */
{
        ...
        if ( cuthead )  {
            ...
            if ( (fcut = fopen(cutfilename, "w")) == NULL) {
                faterror1s("Can't open temporary file %s", cutfilename);
            }
            ...
            finishedit(nil);    fseek(fcopy, 0L, 0);
        ...
        if ( cuthead ) {
            ...
            sprintf(command, "%s -n %s %s > %s", RDIFF,cutfilename, resultfile, diffilename);
