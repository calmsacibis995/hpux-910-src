...
char * argv[];
{
	...
	onlyRCSflag = false;
        ...
        do {
            ...
            if (!pairfilenames(argc, argv, true,true)) continue;
 ...
 */
{
	...
	if ( partime(source, &parseddate) == 0) {
	    error1s("Can't parse date/time: %s", source);
	    *target= '\0';
	    return nil;
	}
	if ( (unixtime = maketime(&parseddate)) == 0L) {
	    error1s("Inconsistent date/time: %s", source);
	    *target='\0';
	    return nil;
	}
	ftm = gmtime(&unixtime);
...

{
        ...
        if ( (length = countnumflds(num1)) != countnumflds(num2) ) {
            error2s(" Invalid branch or revision pair %s : %s", num1, num2);
            return false;
        }
        if ( length > 2 )
...

{
        ...
        while( c != '\0') {
            ...
            * argv = '\0';
            if ( c == '<'){
                error1s("seperator expected near %s", nextrevpair->endrev);
                while( (c = *++argv) != ',' && c != ' ' && c != '\0' &&
                        c != '\t' && c != '\n' && c != ';' ) ;
