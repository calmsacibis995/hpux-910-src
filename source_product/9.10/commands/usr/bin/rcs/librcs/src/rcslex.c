 ...
 *                     RCS file input
 *
 *  $Header: rcslex.c,v 66.1 90/11/14 14:46:58 ssa Exp $ Purdue CS
 */
/*****************************************************************************
 *                     Lexical Analysis.
 ...
 *                     checkid, serror, fatserror, error, faterror, warn, 
 *                     diagnose
...
/*	The following variables are used to prevent hashtable overflow
 *	from locking up the RCSfile so that the user cannot access it.
 *	The quick and dirty solution is to prevent a check-in operation
 *	from adding a delta if it would fill the hashtable.  This should
 *	allow check-out, rlog, rcsdiff, etc. to continue accessing the
 *	file.
 */
 ...
 */
	...
	/* this should prevent RCSfile lock-out */
	if ((check_in == true) && (Hash_count+1 >= hshsize))
		faterror1s("Hashtable overflow.\n%s",
		"Delete old revisions and try again!");
...
loop:
        ...
        case IDCHAR:
        case PERIOD:
                serror1c("unknown Character: %c",nextc);
 ...
 */
{
		...
		*tp++ =c;
                ...
                if (tp >= max) {
                        ...
                        nextc = c;
                        error1c("Can't find %c to terminate string before end of file",SDELIM);
                        return length;
                }
        }
        nextc = c;
...
/*              a legal identifier. Useful for checking commands*/
{
        ...
        if ( ctab[*id] == LETTER ) {
            ...
            if ( c!=' ' && c!='\t' && c!='\n' && c!='\0' && c!=delim) {
                ...
                while( (c=(*++id))!=' ' && c!='\t' && c!='\n' && c!='\0' && c!=delim) ;
                *id = '\0';
                faterror1c1s("Invalid character %c in identifier %s",tc,temp);
                return nil ;
            } else
                return id;
        } else {
            ...
            while( (c=(*++id))!=' ' && c!='\t' && c!='\n' && c!='\0' && c!=delim) ;
            *id = '\0';
            faterror1s("Identifier %s does not start with letter",temp);
            return nil;
        }
}
