...
char 	**argv;
{
	...
	tempfile = mktemp("/tmp/m4XXXXXX");	/* to work with our mktemp */
...
/* end of warning */
/* The above just had to go for NLS reasons.  Other languages (even Roman8)
 * may not use the ctype array to determine character type
 */
	...
	for (;;) {
		...
		/* Test for left-quote and left-comment markers first
		 * to handle the possibility the user redefining either to
		 * start with an alphabetic character.  The old scheme
		 * would not handle that at all.
		 * Defining the right- markers to start with an alpha char
		 * will only work if a non-alpha char precedes it.  This is
		 * because the name-recognizer below only deals with whole
		 * words.
		 */
...
initalloc()
{
	...
	/* Using the 8th bit to indicate a built-in function just isn't
	 * going to fly in the era of foreign language support.  So,
	 * the special sequence 0001 <index> 0001 will be used to represent
	 * the definition of a built-in macro.  The trailing 0001 is necessary
	 * because macro definitions are always stored in reverse order and
	 * parsed the same direction.
	 */
	for (t=1; barray[t].bname; ++t) {
		static char	p[4] = {BFUNC, 0, BFUNC, EOS};
...
char	**a1;
{
	...
	for (; *dp; ++dp) {
		/* The tests for built-in macros has changed due to
		 * 8-bit NLS support requirements.  See initalloc() for
		 * details on the new format.
		 */
		if ((*dp == BFUNC) && (dp[2] == BFUNC)) {
			(*barray[dp[1]].bfunc)(a1,c);
			dp += 2;
		} else if (dp[1]=='$') {
			if (isdigit(*dp)) {
				register	n;
...
register FILE	*iop;
{
	...
	if (nflag || ifx!=cfile) {
		...
		fprintf(iop, "%s", fname[ifx]);  /* Should only print current */
						 /* file, not all files in    */
						 /* path.		      */
		fprintf(iop,"\"\n");
...
char	*s;
{
	...
	if ((*s == BFUNC) && (s[2] == BFUNC)) {
		sprintf(buf,"<%s>",barray[s[1]].bname);
		return buf;
	}
