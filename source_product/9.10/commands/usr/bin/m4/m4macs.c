...
register char 	**ap;
{
	...
	s = ap[1];
	if (isalpha(*s) || (*s == '_')) 
		while (isalnum(*++s) || (*s == '_'))  /* test '_' separately */
			;
	if (*s || s==ap[1])
		...
		*defnn;
{
	...
	while (s>defnn)
		/* The form of a built-in function definition now
		 * takes three chars and a little more processing
		 */
		if ((*--s == BFUNC) && (s[-2] == BFUNC))
		{
			fprintf(stderr,"<%s>",barray[s[-1]].bname);
			s -= 2;
		}
		else
			fputc(*s,stderr);
...
/* First entry is now zero because indices into this table now start
 * at 1.  Changed because of NLS requirements.
 */
struct bs	barray[] = {
	0,		0,
	dochcom,	"changecom",
