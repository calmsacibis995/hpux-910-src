...
struct n_file {
	long n_time;
	char n_name[MAXNAMLEN+1];
} *n_list;

char NEWS[] = "/usr/news";
char NEWSFILE[] = ".news_time";
...
read_dir()
{
	...
	n_count = 0;
	while ((nf = readdir (dirp)) != (struct direct *)NULL) {
		if (nf->d_ino != 0 && stat (nf->d_name, &sbuf) >= 0
	...
	int (*emit)(), update;
{
	long cutoff;
	int i;
	char fname[MAXPATHLEN], *getenv(), *cp;
