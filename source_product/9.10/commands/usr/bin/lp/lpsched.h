...
/* Exit codes returned by 1st level child of scheduler */

#define SCHED_ACTIVE	'A'
#define SCHED_RUNNING	'B'
#define SCHED_FAULT	'C'
#define SCHED_INIT	'X'

#define MAXFILEAGE	43200	/* 12-hour delay for cleanfiles() */
...
#define	EX_TERM	017		/* child was SIGTERM'd */
...
struct dest {		/* structure of a destination */
	...
	struct prilist *d_priority;  /* list of output requests for dest */
};
...
struct prilist {
	short	priority;
	struct outlist	*pl_output;
	struct prilist	*pl_next;
	struct prilist	*pl_prev;
};
...
struct outlist {		/* structure of an output request list */
	...
	short ol_priority;	/* priority of this request */
#ifdef REMOTE
	char *ol_host;		/* host name where request originated */
	int ol_ob3;		/* HP-UX or BSD request */
#endif REMOTE
