...
int	tabtbl[500] = {		/* Table containing tab stops		*/
	...
	*spectbl[40] = {	/* Table of pointers into tabtbl	*/
	...
	savek,			/* Stores char count stripped from
				   front of line.			*/
...
void	options(), process(), append(), prepend(), begtrunc(),
	endtrunc(), inputtabs(), sstrip(), sadd(), outputtabs();
...
struct f *flp = optl;		/* Pointer to next open slot		*/
...
register char	**argv;		/* Array of pointers to arguments	*/
{
	...
	register char 	c;	/* Option character			*/
...
int
inputn(scan)			/* Read a command option number		*/
...
int
cnvtspec(p)		/* Convert tab specification to tab positions.	*/
register char	*p;		/* Pointer to spec string.		*/
{
	...
	if (spectype == 10) {
		...
		*nexttab = 1;
	 	...
	 	if (number[0] == 0)
			number[0] = 1; /* Prevent infinite loop. */
...
int
readspec(p)			/* Read a tabspec from a file		*/
...
char
*readline(fp, area)		/* Read one line from the file.	*/
...
char
type(c)				/* Determine type of a character	*/
