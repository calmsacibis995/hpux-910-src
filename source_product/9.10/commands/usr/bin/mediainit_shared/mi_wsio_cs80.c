...
#define SET_TVA(tva, c, h, s) \
	{ \
		tva.cyln[0] = (unsigned8) (c >> 16); \
		tva.cyln[1] = (unsigned8) (c >> 8); \
		tva.cyln[2] = (unsigned8) c; \
		tva.head = h; \
		tva.sect = s; \
	}
