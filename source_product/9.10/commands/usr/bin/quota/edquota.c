 ...
 */
#ifndef lint
static	char *sccsid = "@(#)$Revision: 66.7 $";
#endif
	...
	char **argv;
{
	...
	if (quotactl(Q_SYNC, NULL, 0, NULL) < 0 && errno == ENOSYS) {
		fprintf(stderr, "Warning: Quotas are not compiled into this kernel\n");
		sleep(3);
	}
	if (getuid()) {
	...
	setupfs();
	if (fsqlist == NULL) {
		fprintf(stderr, "%s: no HFS filesystems with %s file\n",
		    MOUNTED, qfname);
		exit(1);
	}
	mktemp(tmpfil);
	...
	while ((option = getopt(argc,argv,"p:t")) != EOF)
		switch(option) {
		case 'p':
			if (tflag) usage(arg0);
			pflag++;
