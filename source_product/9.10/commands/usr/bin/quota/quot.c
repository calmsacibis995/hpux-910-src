#ifndef lint
static char *sccsid = "@(#)$Revision: 66.7 $";	/* from 4.11 (Berkeley) 85/09/09 */
#endif
	...
	char *argv[];
{
	...
	now = time(0);
	while ((option = getopt(argc,argv,"nfcvha")) != EOF)
		switch (option) {
		case 'n':
			nflg++; break;
		case 'f':
...
quotall()
{
	...
	while (mntp = getmntent(fstab)) {
		if  (strcmp(mntp->mnt_type, MNTTYPE_43) != 0)
			continue;
		if (check(makerawname(mntp->mnt_fsname), mntp->mnt_dir) == 0)
			report();
	}
	endmntent(fstab);
}
...
report()
{
	...
	for (dp = du; dp < &du[ndu]; dp++) {
		...
		if (dp->blocks == 0)
			return;
		printf("%5ld\t", dp->blocks);
		if (fflg)
			printf("%5ld\t", dp->nfiles);
