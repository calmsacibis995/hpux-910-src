...
#define	UID      4
#define	GID      6
#define FSSTART  (BSIZE*2)	/*skip blks 0 (boot) and 1 (superblk) */
#define	INOSZ    (sizeof(struct dinode))
#define	DIRSZ    (sizeof(struct direct))
	...
	for(;;) {
		...
		case '\n': /* command end */
			...
			case DIRSZ:
				...
				prt_flag = 0;
				continue;
			case INOSZ: fprnt('i',1);
				cur_ino = addr;
				prt_flag = 0;
				continue;
			}
			printf("%6.6lx%s: %6.6lx (%ld)\n",addr,cptr,temp,temp);
			continue;
		...
		case '=': /* assignment operation	*/
			...
			case '+': /* =+ operator */
				temp = getnumb();
				value = kget(objsz);
				if(!error) kput(value+temp,objsz);
				continue;
			case '-': /* =- operator */
				temp = getnumb();
				value = kget(objsz);
	...
	short ment;
{
	...
	case 4: if(addr & 01L) break;
		return(0);
	case DIRSIZ: if((addr & (DIRENT-1)) != 02) break;
		return(0);
	case DIRSZ:
	case INOSZ:
	...
	register short	count;
{
	...
	case 'd': /* print as directories */
		...
		addr = addr & ~(DIRENT-1);
		offset = offset / DIRSZ;
		if(count == 0) count = DIRENT - offset;
		if(count + offset > DIRENT) break;
		type = DIR; 
		objsz = DIRSZ;
	...
	case 'x': /* print as hexadecimal words */
		addr = addr & ~01;
		offset = offset >> 1;
		iptr = ((short *)cptr) + offset;
		if(count == 0) count = BSIZE/sizeof(short) - offset;
		if(offset + count > BSIZE/sizeof(short)) break;
		objsz = 2;
		for(i=0; count--; i++) {
  			if(i % 8 == 0) {
			...
			else if(style == 'x') printf("  %04.4x",*iptr++ & 0xffff);
			else printf("  %6d",*iptr++);
			erraddr = addr;
			addr = addr + 2;
		}
...
reload()
{
	...
	else printf("fsize = %ld, isize = %u (blocks)\n",fsize,isize-2);
}
	...
	long	bn;
{
	...
	if(bn < NADDR - 3) {
		addr += A0 + bn * 3;
		return(nullblk(nb=kget(3)) ? 0L : nb);
	}
	...
	addr += A0 + (NADDR - j) * 3;
	if(nullblk(nb=kget(3)))
		return(0L);
	for(; j<=3; j++) {
		sh -= NSHIFT;
		addr = (nb << BSHIFT) + 4 * ((bn >> sh) & NMASK);
		if(nullblk(nb=kget(4)))
			return(0L);
	}
	return(nb);
}
