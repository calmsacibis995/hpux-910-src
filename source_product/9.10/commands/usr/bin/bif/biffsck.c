...
struct bufarea {
	...
	union {
		...
		struct HPUX_vol_type volhdr;	/* block 0 header */
	} b_un;
	char	b_dirty;
};
...
#define	MAXDATA ((MEMSIZE)200*ctob(1))  /* arbitrary to some extent */
...
char	*argv[];
{
	...
	while(memsize >= 2*sizeof(BUFAREA) &&
		(membase = sbrk((int)memsize)) == (char *)-1)
		memsize -= ctob(1);
...
register DIRECT *dirp;
{
	...
	if((inum = dirp->d_ino) == 0)
		return(KEEPON);
	thisname = pathp;
	for(p = &dirp->d_name[DIRSIZ-1];
		*p == 0 && p >= &dirp->d_name[0]; p--) ;
	if (p < &dirp->d_name[0]) {
		error("FILE/DIR NAME FOR I = %d IN %s IS NULL.",
			inum, pathname);
		if (reply("RENAME") == YES) {
			mkdir_entry(dirp,inum);
			alterd = ALTERD;
		}
	}
	for(; *p != '/' && *p != 0 && p >= &dirp->d_name[0]; p--) ;
	if (p >= &dirp->d_name[0]) {
		error("FILE/DIR NAME %s/%.14s HAS ILLEGAL CHARACTER(S).",
			pathname, dirp->d_name);
		if (reply("FIX") == YES) {
			for(;p >= &dirp->d_name[0]; p--) {
				if (*p == 0 || *p == '/') *p = '?';
			}
			alterd = ALTERD;
		}
...
char *dev;
{
	...
	rmscr = 0;
	/* getblk says its already done its job if the bno field
	   matches the requested block.  For block zero, that's a
	   loose. */
	tempblk.b_bno = -1;
	if (getblk(&tempblk, 0) == NULL) {
		error("Can't read block 0\n");
		ckfini();
		return(NO);
	}
	if (tempblk.b_un.volhdr.HPUXid != HPUXID) {
		error("Not an HP-UX volume\n");
		if (reply("CONTINUE") == NO) {
			ckfini();
			return(NO);
		}
	...
	if (getblk(&tempblk, fmax-1) == NULL) {
		error("Can't read last block\n");
		ckfini();
		return(NO);
	}
...
{
	fprintf(stderr,"Unexpected trap %d, quitting\n",n);
	ckfini();
	exit(4);
}
#endif
