...
#define HP-UX	/* define for HP-UX machines */
#include <setjmp.h>
#include <signal.h>
#include <locale.h>
int setjmp();
...
struct Comd *p;
{
	...
	if((d=mygetc()) == '\n') return(err(1,"syntax"));
	if(peekc() == d) mygetc();
	else if((tmp=getstr(1,currex,d,0,2))==2) {
		comdlist[0] = 'p';
		comdlist[1] = '\0';      
	}
	else if(tmp) return(-1);
	if((ptr = (char *) compile(currex,glbuf,&glbuf[REBUF],'\0'))
            == (char *) -1)
...
int prt;
{
	...
	r = 0;
	while(peekc() == ' ') mygetc();		/* Ignore leading spaces */
	switch(c = peekc()) {
		case '\n':
		case ',':
		case ';':	return(0);
...
char buf[], brk, ignr;
{
	...
	for(b=buf; c=peekc(); prevc=c) {
		if(c == '\n') {
			...
			else if(nonl==1) break;
			else if(nonl==2) return(2); /*used in gcomd only*/
			else return(*b='\0');
		}
		else {
			mygetc();
