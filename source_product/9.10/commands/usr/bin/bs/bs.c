...
int 	*E_Instr = &Instr[INSTRUCTIONS-1];
int	Sinstr[256];
int 	*E_Sinstr = &Sinstr[256-1];
int	*Ip = Instr;
int	*Oip = 0;
...
double	atof();
extern char	*Atof;
FILE	*popen();  /* In release 2 this is in stdio.h; delete if an error! */
...
intr()
{
	...
	if (Input) fclose(Input); fclose(stdin);
...
{
	/*
	** According to the man page: upon the return from a setjmp call 
	** caused by a longjmp, the values of any non-static local
	** variables belonging to the routine from which setjmp was
	** called are undefined.  The following stuff is declared as
	** static to avoid being blown away by setjmp - longjmp with
	** the optimizer on.
	*/
	...
	static char get[]	= "get";
	static char put[]	= "put";
	static char puterr[]	= "puterr";
	static double dfd0	= 0.0;
	static double dfd1	= 1.0;
	static double dfd2	= 2.0;
	...
	*/
	...
	if (!setlocale(LC_ALL,"")) {
		...
		*/
		/*
		 * There is a conflict between the grammar of bs and the radix
		 * character if it is allowed to change to a comma as is done
		 * in certain languages.  An expedient (although possibly not
		 * ideal) solution is simply to not allow the radix character
		 * to change.  Hence the following call.
		 */
...
again:
	(void)setjmp(Reset);
	io(arg3, get, dfd0, nullstr, rstr);
	io(arg3, put, dfd1, nullstr, wstr);
	io(arg3, puterr, dfd2, nullstr, wstr);
	...
	lc = 0;
	(void)setjmp(Afterr);
	while(Input && fgets(&Line[lc], 512-lc, Input) != NULL) {
		lc = strlen(Line) - 2;
		if(Line[lc] == '\\') {
			++Cont;
			continue;
...
elseif:
	...
	case ELSE:
	case ELIF:
		if((Nest <= 0) || (s[--Nest].tp != IF && s[Nest].tp != ELIF))
			cerror("Else: no if");
		save1.ip = s[Nest].loc;
		op(JUMP);
...
fifi:
		if((Nest <= 0) || (s[--Nest].tp != IF && s[Nest].tp != ELSE && s[Nest].tp != ELIF))
			cerror("Fi: no if");
		*(s[Nest].loc) = (int)Ip;
...
{
	register char *s, *st;
	static char rv[3][320];   /* 308 digits, +10 for fraction, + decimal +null */
	static which = 0;
...
char Plotdata[8 + NPLOTCHR] = "tplot -T";  /* 8 is length of the text */
char *Plot = Plotdata;
...
register struct estack *estack;
{
	...
	if(fcn == 0) {		/* pipe output through plot(I) */
		Plot[8] = '\0';
		if(strlen(stp = to_str(estack)) >  NPLOTCHR)
