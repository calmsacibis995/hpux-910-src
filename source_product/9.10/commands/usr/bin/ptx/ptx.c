...
char **argv;
{
	...
	if (!setlocale(LC_ALL, "")) {
		fputs(_errlocale("ptx"), stderr);
		/* This is to avoid duplicate printing of _errlocale messages */
		/* by the command (i.e. sort) exec'ed in this program.        */
		putenv("LANG=");
		putenv("LC_COLLATE=");
		putenv("LC_CTYPE=");
		putenv("LC_MONETARY=");
	...
	while(argc>1 && **argv == '-') {
		...
		case 'i':
			...
			if (argc>2){
				...
				diag("No ignore file name given", empty);
			}
			break;
		...
		case 'o':
			...
			if (argc>2){
				...
				diag("No only file name given", empty);
			}
			break;
		...
		case 'b':
			if(argc>2) {
				...
				diag("No break file name given", empty);
			}
			break;
...
*/
	...
	case 0:		/* child */
		execl(SORT, SORT, sortopt, "+0", "-1", "+1", "-o", sortfile, sortfile, (char *) 0);
		diag("Cannot exec",SORT);
...
char *strt, *end;
{
	...
	for(cp=strt; cp<end; cp++)
		if (putc(*cp, sortptr) == EOF)
			diag("sort file cannot be grown", empty);
	/* Add extra blank before TILDE to sort correctly
	   with -fd option */
	if (putc(' ',sortptr) == EOF)
		diag("sort file cannot be grown", empty);
	if (putc(TILDE,sortptr) == EOF)
		diag("sort file cannot be grown", empty);
