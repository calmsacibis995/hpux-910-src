...
char	**argv;
{
	...
	*/
	...
	n = 0;
	while(fgets(diffbuf,LINE_MAX,diffdes) != NULL){
		change = ' ';
		com = cmd(diffbuf);
...
put1()
{
	...
	if(fgets(buf1,LINE_MAX,fdes1) != NULL){
		len1 = getlen(0,buf1);
...
put2()
{
	...
	if(fgets(buf2,LINE_MAX,fdes2) != NULL){
		getlen((hlen+WGUTTER)%8 - 8,buf2);
...
FILE *fromdes,*todes;
{
	...
	while((fgets(tempbuf,LINE_MAX,fromdes) != NULL))
		fputs(tempbuf,todes);
	fclose(fromdes);
}
