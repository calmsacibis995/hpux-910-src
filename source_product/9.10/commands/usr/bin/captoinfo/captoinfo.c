static char *HPUX_ID = "@(#) $Revision: 27.4 $";
/*
...
static char *progname;		/* argv [0], the name of the program */
...
{
    while (*codes)
	if (strncmp(*codes++, cap, 2) == 0)
	    return 1;

    while (*ocodes)
	if (strncmp(*ocodes++, cap, 2) == 0)
	    return 1;
...
int filltables ()
{
	...
	if ( oboolval[uselevel][i] != 0 )
	    fprintf(stderr, "Warning: cannot translate %s (unsupported in terminfo)\n", oboolcodes[i]);
	if (verbose > 1)
	    {
...
char *usename;
{
	...
	if (boolval [0] [i] && !boolval [1] [i])
	    ...
	    pr_boolean (boolnames[i], (char *)0, (char *)0, -1);
	/* print out capabilities in common: */
	else if ( boolval[0][i] && boolval[0][i] == boolval[1][i] )
	    pr_boolean(boolnames[i], (char *)0, (char *)0, 1);
	...
	/* print out capabilities in common: */
	else if ( numval[0][i] != -1 && numval[0][i] == numval[1][i] )
	    pr_number (numnames[i], (char *)0, (char *)0, numval[0][i]);
	...
	/* print out capabilities in common: */
	else if ( strval[0][i] && strcmp(strval[0][i], strval[1][i]) == 0 )
	    pr_string (strnames[i], (char *)0, (char *)0, strval[0][i]);

    pr_sfooting();

    /* print out entire "minimal" definition instead of "use=.." */
    /*(void) printf ("\tuse=%s,\n", usename);*/
}

captoinfo ()
...
register char *line;
{
	...
	for ( ; *lineptr ; lineptr++)
		...
		*lineptr = '\0';
		if (verbose)
		    (void) fprintf (trace, "extracted name from '%s'.\n", line);
		return line;
		}
    if (verbose > 1)
...
char **argv;
{
    ...
    while ( (c = getopt (argc, argv, "1vw:")) != EOF)
	switch (c)
	    {
	    case '1':
		pr_onecolumn (1);
