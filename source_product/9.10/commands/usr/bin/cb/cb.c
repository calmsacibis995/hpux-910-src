#ifndef lint
static const char *HPUX_ID = "@(#) $Revision: 72.4 $";
#endif
...
#define boolean int                 /* for decfect CLLca01099 fix */
#define TRUE 1			    /* Christopher Hoang - HP-C/CLL */
#define FALSE 0

#ifdef NLS16
#include <nl_ctype.h>
#undef	_U		/* turn off the defines from <ctype.h> */
#undef	_L		/* cause cb needs to redefine below    */
#undef	_N
#undef	_S
#undef	_P
#undef	_C
...
#define MAXTABS	8
#define TABLENG	8
#define TEMP	4096	/* increased length of comment space by 4x */
...
int	inswitch	=0;
int	lbegin	 = 1;
boolean end_of_comment = FALSE;           /* TRUE when the current character */
					  /* is a '*' and the next character */
					  /* is a '/'.                       */
					  /* defect # CLLca01099             */		

...
char	*argv[];
{
	...
	while (--argc > 0 && (*++argv)[0] == '-'){
		...
		case 'j':
			...
			continue;
		case 'l':
                        /* 901128 choang: CLLca01701 Should be able to handle
                         * -lnn (-l nn works).
                         */
                        if ( (*argv)[2] == '\0' ) {
                           lopt_num = *++argv;          /* -l nn */
                           ...
                           lopt_num =  &(*argv)[2];     /* -lnn  */
                        if (!(maxleng = atoi(lopt_num))) {
                           fprintf(stderr, "cb: badly formed l option\n");
                           exit(1);
                        }
...
work(){
	...
	while ( NOTEOF && ((c = getch()) != EOF)) {
		...
		case '}':
			...
			c = *pt;
			if(*pt == ';' || *pt == ','){
                                /* CLLca01554 - At the end of a statement, 
                                 * reset clev. choang.
                                 */
                                if (*pt == ';') {       
                                   if (--clev < ind)
...
*/
			...
			getnl();
			OUTK;
			/* DTS # CLLbs00181. klee 12/10/92 */
			/* DTS # CLLbs00394. klee 12/10/92 */
			if (*pt == -1) { 
			/* EOF, check if we need to fflush the buffer */
				char*q = temp;
				while (isspace(*q) || *q == '\n')
					q++;
				if ((*q == '#' && q < tp) ||
				    (*q == '/' && *(q+1) == '*' && q < tp))
				{
				/* Unget the last byte that is just read.
			 	 * And reset pointers.
				 */
					ungetc(*(pt-1),input);
					tp = pt;
		...
		case '\n':
			if ((lptr = lookup(lastlook,p)) != 0){
				...
				else if (lptr->type == WHILE  &&
					  (dolevel && clev->tabs <= dotabs[dolevel]) ){
					 /* Add this section to handle do loops properly */
					 putspace(' ', YES);
					 keyflag = 1;
				}
				else {
		...
		case '"':
#ifdef NLS16
/*  check for double quote as 2nd byte of a kanji  */
/*  logic is basically same as for single quote    */
			putch(c, NO);
			while ((c = getch()) != '"' && c != EOF) {
				putch(c, NO);
				if (FIRSTof2(c))
					if ((c = getch()) != EOF)
						putch(c, NO);	/* skip 2nd byte of a kanji */
					else
						unget(c);	/* unless its an EOF */
				else if (c == '\\')
					if ((c = getch()) == '"')
						putch(c, NO);	/* eat quoted quotes */
                                /* If the previous character was a '\' AND the
                                 * current character is a end-of-line marker,
                                 * turn on the flag. choang: CLLca01717 900104
                                 */
                                        else if ( c == '\n' ) {
						...
						unget(c);	/* but force possible quoted kanji thru full loop */
				else if (c == '\n') {
                                /* If still in the middle of a continuous line,
                                 * do NOT output a tab character ( in a string
                                 * literal ). choang: CLLca01717 - 900104
                                 */
                                        if (continuous_line)
			...
			continue;
/* no 'else NLS16' cause the 8 bit double quote is handled by the same */
/* routine as the single quote below                                   */
#endif /* NLS16 */
		case '\'':
			putch(c,NO);
		...
		case '#':
			...
			while (NOTEOF && ((cc = getch()) != '\n')) {
#ifdef NLS16
/* if a kanji appears within a #define, then dump both bytes */
				if (FIRSTof2(cc & 0377)) {
					putch(cc,NO);
					cc = getch();
				} else
...
/* if a kanji appears within a #define, then dump both bytes */
			...
			continue;
		default:
			/* If the current character is an asterisk AND the   */
			/* next character is a slash, turn ON end_of_comment */
			/* so that they will not be treated as operators in  */
			/* function keep(). Defect # CLLca01099              */
			...
			if (c == ','){
				...
				if (lastlook == 0) lastlook = p;
				putch(c,NO);
				while ((cc=getch()) != EOF) {
					if ((cc == 'E') || (cc == 'e')) {
						putch(cc,NO);
						if ((cc=getch()) == EOF) break;
						if ((cc != '+') && (cc != '-')){
							unget(cc);
							continue;
						}
					}
					else if (!isalnum(cc) && (cc != '.')){
						unget(cc);
						break;
					}
					putch(cc,NO);
...
gotop(c)
{
	...
	*op_ptr = '\0';
	...
	end_of_comment = FALSE;   /* Turn OFF when fisnish processing an end */
				  /* of comment,  defect # CLLca01099        */
}

keep(o)
struct op *o;
{
	...
	/* If strict is ON (-s option) and the asterisk currently looked at */
        /* is part of the end of comment, do NOT put a space between the    */
        /* asterisk and the next character (the slash).                     */
        /* This fixes defect # CLLca01099.                                  */

	if (strict && !end_of_comment && ((o->blanks & ALWAYS)
	    || ((opflag == 0 && o->blanks & SOMETIMES) && clev->tabs != 0)))
		putspace(' ',YES);
	for(s=o->name; *s != '\0'; s++){
		ok = NO;	/* switched the order of these two lines in  */
		putch(*s,ok);	/* order to prevent the splitting of a token */
	}
...
comment(ok)
{
	...
	while ((ch  = getch()) != EOF){
		...
		if (FIRSTof2(ch))
			if ((ch = getch()) != EOF)
				putch(ch, NO);	/* eat 2nd byte of kanji */
			else
				unget(ch);	/* unless its an EOF */
		else if (ch == '*')
			if ((ch = getch()) == '/') {
				putch(ch, NO);	/* same as original code below, but ... */
				return(hitnl);
			} else
				unget(ch);	/* put it back so it goes thru the kanji check next time around */
		else
#else /* NLS16 */
		if (ch == '*'){
gotstar:
...
getch(){
/*
	return's have been masked off to eliminate sign extension
	from the 8th bit.
*/
	register char c;
	if(inswitch){
		...
		if(tptr <= lastplace){
			if(*tptr != '\0')return(*tptr++ & 0377);
			else if(++tptr <= lastplace)return(*tptr++ & 0377);
		}
		inswitch=0;
		lastplace = tptr = temp;
	}
...
cont:
			while(NOTEOF && ((c=getc(input)) != '*')) {
				puttmp(c,0);
#ifdef NLS16
				if (FIRSTof2(c))
					if ((c=getc(input)) != EOF)
						puttmp(c,0);	/* eat both bytes of a kanji */
					else {
						fprintf(stderr,"comment not terminated before EOF - quitting\n");
						exit(1);
					}
				else
#endif /* NLS16 */
			...
			if ((c=getc(input)) == '/') {
				...
				ungetc(c, input);	/* ensure all bytes are tested as possible kanji */
				goto cont;
			}
#else /* NLS16 */
	star:
	...
	if(c == '#' && tp > temp+1 && *(tp-2) == '\n'){
		...
		while(NOTEOF && (puttmp((c=getc(input)),1) != '\n'))
