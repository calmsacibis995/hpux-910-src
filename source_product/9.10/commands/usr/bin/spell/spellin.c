static char *HPUX_ID = "@(#) $Revision: 37.3 $";
#include <stdio.h>
#include "hash.h"
...
/***** check added 8/14/85 during code porting to prevent floating
 ***** exception.  While a negative number should not be given
 ***** to spellin under normal circumstances, we should check
 ***** anyway. A number of zero could happen by accident when
 ***** doing some automatic process on an empty file.
 ***** dds.
 */
	if ( atof(argv[1]) <= 0) {
	    fprintf(stderr,"spellin: invalid number argument:\"%s\"\n",argv[1]);
	    exit(1);
	}
