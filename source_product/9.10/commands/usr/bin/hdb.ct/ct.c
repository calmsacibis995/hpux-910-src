...
#define ROOT	0
#define SYS	3
#define TTYMOD	0666		/* so that it can be accessed by kermit,etc.*/
/*#define TTYMOD	0644*/       
#define DEV	"/dev/"
#define TELNOSIZE	32		/* maximum phone # size is 31 */
#define LEGAL	"0123456789-*#="
#define USAGE	strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,1120, "[-h] [-v] [-w n] [-x n] [-s speed] telno ...")))
...
static 
FILE	*_Fdl;
/* C2 additions to cleanup enviroment variables */
extern	char	**environ;
extern	int	cleanenv();
...
/*
 * C2 security changes to clean up 
 * enviroment variables (i.e. any env varibles needed in 
 * ct should be declared here or they will zeroed out! )
 */
	cleanenv(&environ, "LANG", "LANGOPTS", "NLSPATH", "TZ", "LOCALDOMAIN", "HOSTALIASES", "TEMPDIR", 0);

    nlmsg_fd = catopen("ct",0);
	...
	(void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1121, "ct: can't open %s\n")), DEVFILE);
	cleanup(101);
    }
	...
	switch (c) {
		...
		if (minutes < 1) {
		    (void) fprintf(stderr,
			strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1122, "\tusage: %s %s\n"))), Progname, USAGE);
		    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1123, "(-w %s) Wait time must be > 0\n")),
			optarg);
		    cleanup(101);
		}
		break;
	    ...
	    case '?': 
		(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1124, "\tusage: %s %s\n"))), Progname, USAGE);
		cleanup(101);
		/* NOTREACHED */
	}
    }
    ...
    if (optind == argc) {
	(void) fprintf(stderr, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1125, "\tusage: %s %s\n"))), Progname, USAGE);
	(void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1126, "No phone numbers specified!\n")));
	cleanup(101);
    }
	...
	if (strlen (_Num) >= sizeof tbuf - 1) {
	    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1127, "ct: phone number too long -- %s\n")), _Num);
	    ++errors;
	}
	if (strspn (_Num, LEGAL) < strlen (_Num)) {
	    (void) fprintf(stderr, (catgets(nlmsg_fd,NL_SETN,1128, "ct: bad phone number -- %s\n")), _Num);
	    ++errors;
	}
    }
    if (errors)
	...
	if ( (found = gdev (dfp, _Flds)) > 0) {  /* found a dialer */
	    (void) fprintf(stdout, (catgets(nlmsg_fd,NL_SETN,1129, "Allocated dialer at %s baud\n")),
		_Flds[F_CLASS]);
	    break;
	}
	else if (found == 0) {	/* no dialers of that on system */
	    (void) fprintf(stdout, (catgets(nlmsg_fd,NL_SETN,1130, "No %s dialers on this system\n")),
		fdig(_Flds[F_CLASS]) );
    	    cleanup(101);
	}
	...
	if (!first) { /* not the first time in loop */
	    ...
	    strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1131, "%s busy")));
            strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,1132, "Dialer is")));
            strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,1133, "Dialers are")));
	    VERBOSE((catgets(nlmsg_fd,NL_SETN,1134, " (%d minute(s))\n")), count);
	    VERBOSE(msg1, (found == -1) ? msg2 : msg3);
	    VERBOSE((catgets(nlmsg_fd,NL_SETN,1134, " (%d minute(s))\n")), count);
	    if (count < minutes) {
	        sleep(60);
	        continue;
	...
	/**************************************************************/
	first = FALSE;
	(void) fprintf(stdout, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1135, "The (%d) %s dialer%s busy\n"))), -found,
	    _Flds[F_CLASS], (found == -1 ? strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,1136, " is"))) : strcpy(msg3,(catgets(nlmsg_fd,NL_SETN,1137, "s are")))));
	if (minutes) {	/* -w already set wait minutes */
	    (void) fprintf(stdout, strcpy(msg1,(catgets(nlmsg_fd,NL_SETN,1138, "Waiting for %d minute%s\n"))), minutes,
		(minutes > 1 ? strcpy(msg2,(catgets(nlmsg_fd,NL_SETN,1139, "s"))) : "") );
	    sleep(60);
	    continue;
	}
	...
	(void) fputs((catgets(nlmsg_fd,NL_SETN,1140, "Do you want to wait for dialer? (y for yes): ")), stdout);
	if ((c = getchar ()) == EOF || tolower (c) != 'y')
	    cleanup(101);
	while ( (c = getchar()) != EOF && c != '\n')
	    ;

	(void) fputs ((catgets(nlmsg_fd,NL_SETN,1141, "Time, in minutes? ")), stdout);
	...
	(void) fputs ((catgets(nlmsg_fd,NL_SETN,1142, "Waiting for dialer\n")), stdout);
	sleep(60);
	continue;
    ...
    if (found < 0) {	/* no dialer found - get out */
        (void) fputs((catgets(nlmsg_fd,NL_SETN,1143, "*** TIMEOUT ***\n")), stdout);
        cleanup(101);
    }
    ...
    if (hangup) {  /* -h option not specified */
        (void) fputs ((catgets(nlmsg_fd,NL_SETN,1144, "Confirm hang-up? (y to hang-up): ")), stdout);
        if ((c = getchar ()) == EOF || tolower (c) != 'y')
    	    cleanup(101);
        while ( (c = getchar()) != EOF && c != '\n')
    	    ;
    ...
    VERBOSE((catgets(nlmsg_fd,NL_SETN,1145, "Connected\n")), "");
	...
	(void) fputs((catgets(nlmsg_fd,NL_SETN,1146, "Hit carriage return ")), _Fdl);
	(void) fclose(_Fdl);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1147, "there is a uugetty; exit\n")), 0);
	exit(0);
    }

    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1148, "start getty -h -t60 %s ")), Dc );
    DEBUG(4, "%s\n", fdig(_Flds[F_CLASS]));
    for (;;) {
	switch(_Pid = fork()) {
	case -1:	/* fork failed */
	    if ((!hangup || Verbose))
		(void) fputs ((catgets(nlmsg_fd,NL_SETN,1149, "ct: can't fork for getty\n")), stderr);
	    cleanup(101);
	    /*NOTREACHED*/
	...
	if ((_Status & 0xff00) < 0) {
	    if (!hangup)
		VERBOSE((catgets(nlmsg_fd,NL_SETN,1150, "ct: can't exec getty\n")), "");
	    cleanup(101);
	}

        rewind (_Fdl);	/* flush line */
        (void) fputs ((catgets(nlmsg_fd,NL_SETN,1151, "\nReconnect? ")), _Fdl);
...
struct termio   termio;
    ...
    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,1152, "Disconnect(%d)\n")), code);
    VERBOSE((catgets(nlmsg_fd,NL_SETN,1153, "Disconnected\n")), "");
...
register int    code;
{
    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,1154, "cleanup(%d)\n")), code);
    rmlock (CNULL);
    if (*_Tty != '\0') {
	DEBUG(5, (catgets(nlmsg_fd,NL_SETN,1155, "chmod/chown %s\n")), _Tty);
	if (chown (_Tty , UUCPUID, UUCPGID) < 0 || chmod (_Tty , TTYMOD) < 0) {
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,1156, "Can't chown/chmod on %s ")), _Tty);
	    DEBUG(5, (catgets(nlmsg_fd,NL_SETN,1157, "to %d\n")), UUCPUID);
	}
    }
...
char *line;
{
	...
	if (u->ut_type == LOGIN_PROCESS
		...
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,1158, "ut_line %s, ")), u->ut_line);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,1159, "ut_user %s, ")), u->ut_user);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,1160, "ut_id %.4s, ")), u->ut_id);
		DEBUG(7, (catgets(nlmsg_fd,NL_SETN,1161, "ut_pid %d\n")), u->ut_pid);

		/* see if the process is still active */
		if (kill(u->ut_pid, 0) == 0 || errno == EPERM) {
		    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,1162, "process still active\n")), 0);
		    return(1);
		}
	}
    }
