...
/*line.c copied from HDB uucp 	Was version 51.3
*/
#include "uucp.h"
...
int	tty, spwant, type;
{
	...
	DEBUG(6, (catgets(nlmsg_fd,NL_SETN,524, "fixline(%d, ")), tty);
	DEBUG(6, (catgets(nlmsg_fd,NL_SETN,525, "%d)\n")), spwant);
	...
	if (spwant > 0) {
		...
		ttbuf.c_cflag &= ~CBAUD;  /* so that it won't mess up */
		ttbuf.c_cflag |= speed;   /* parity field */
	} else
		ttbuf.c_cflag &= CBAUD;
	...
	if (type == D_DIRECT) {
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,527, "fixline - direct\n")), "");
...
register char	type;
{
	...
	if (ioctl(Ifn, TCGETA, &tbuf) != 0)
		return;
	DEBUG(2, (catgets(nlmsg_fd,NL_SETN,529, "setline - %c\n")), type);
	switch (type) {
	case RCVFILE:
...
int tty, spwant;
{
	...
	for (ps = spds; ps->sp_val >= 0; ps++)
		if (ps->sp_val == spwant)
			speed = ps->sp_name;
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,530, "sytfixline - speed= %d\n")), speed);
	ASSERT(speed >= 0, "BAD SPEED", "", speed);
...
int tty, spwant, type;
{
	...
	DEBUG(6, (catgets(nlmsg_fd,NL_SETN,534, "fixline(%d, ")), tty);
	DEBUG(6, (catgets(nlmsg_fd,NL_SETN,535, "%d)\n")), spwant);
