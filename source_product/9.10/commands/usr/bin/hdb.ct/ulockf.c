...
/*
	ulockf.c copied from HDB uucp. Was version 51.3
*/
#include "uucp.h"
...
time_t atime;
{
	...
	if (onelock(pid, tempfile, file) == -1) {
		...
		if (checkLock(file))
			...
			(void) unlink(tempfile);
			DEBUG(4,(catgets(nlmsg_fd,NL_SETN,641, "ulockf failed in onelock()\n")),"");
			return(FAIL);
		    }
		}
	}
...
register char *file;
{
	...
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,642, "ulockf file %s\n")), file);
	if (fd == -1) {
	    if (errno == ENOENT)  /* file does not exist -- OK */
		return(0);
	    DEBUG(4,(catgets(nlmsg_fd,NL_SETN,643, "Lock File--can't read (errno %d) --remove it!\n")), errno);
	    goto unlk;
	}
#ifdef	ASCIILOCKS
	...
	if (ret != sizeof(int)) {
	    ...
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,644, "Lock File--bad format--remove it!\n")), "");
	    goto unlk;
	}
#ifdef	ASCIILOCKS
	lpid = atoi(alpid);
	...
	if ((ret=kill(lpid, 0)) == 0 || errno == EPERM) {
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,645, "Lock File--process still active--not removed\n")),"");
	    return(FAIL);
	}
	else { /* process no longer active */
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,646, "kill pid (%d), ")), lpid);
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,647, "returned %d")), ret);
	    DEBUG(4, (catgets(nlmsg_fd,NL_SETN,648, "--ok to remove lock file (%s)\n")), file);
	}
unlk:
	
	if (unlink(file) != 0) {
		DEBUG(4,(catgets(nlmsg_fd,NL_SETN,649, "ulockf failed in unlink()\n")),"");
		return(FAIL);
	}
	return(0);
}
...
register char *file;
{
	...
	fd = open(file, 0);
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,650, "ulockf file %s\n")), file);
	if (fd == -1) {
	    if (errno == ENOENT)  /* file does not exist -- OK */
		return(0);
	    DEBUG(4,(catgets(nlmsg_fd,NL_SETN,651, "Lock File--can't read (errno %d) --remove it!\n")), errno);
	    goto unlk;
	}
	ret = stat(file, &stbuf);
	if (ret != -1) {
...
unlk:
	DEBUG(4, (catgets(nlmsg_fd,NL_SETN,652, "--ok to remove lock file (%s)\n")), file);
	
	if (unlink(file) != 0) {
		DEBUG(4,(catgets(nlmsg_fd,NL_SETN,653, "ulockf failed in unlink()\n")),"");
		return(FAIL);
	}
	return(0);
}
...
{	
	...
	if(link(tempfile,name)<0){
		...
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,657, "link(%s, ")), tempfile);
		DEBUG(4, (catgets(nlmsg_fd,NL_SETN,658, "%s)\n")), name);
