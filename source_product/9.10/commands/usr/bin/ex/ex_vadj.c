	...
	int p, cnt;
{
...
#ifndef NONLS8 /* 8bit integrity */
	copys(tlines, vtube, sizeof(vtube)/sizeof(*vtube[TUBECOLS]));	/*SASSIGN*/
#else NONLS8
	...
	for (to = p; from <= WECHO; from++, to++) {
		...
		vclrbyte(vtube[to], BTOCRATIO * WCOLS);
#else
	...
	bool doclr;
{
	...
	if (scroll_forward) {
		...
		if (state == ONEOPEN || state == HARDOPEN) {
			outline = destline = 0;
#if defined NLS16 && defined EUC
			vclrbyte(vtube[0], BTOCRATIO * WCOLS);
#else
	...
	register int cnt;
{
	...
	if (cnt < 0 || cnt > TUBELINES)
		error((nl_msg(1, "Internal error: vscroll")));
	if (cnt == 0)
		return;

#ifndef NONLS8 /* 8bit integrity */
	copys(tlines, vtube, sizeof(vtube)/sizeof(*vtube[TUBECOLS]));
#else NONLS8
	...
	CHAR *curs;
{
	...
	if (vcnt == 0 || vcline < 0 || vcline > vcnt || holdupd && state != VISUAL) {
		...
		if (noteit(1) == 0 && odol == zero) {
			CATCH
				error((nl_msg(2, "No lines in buffer")));
			ENDCATCH
			linebuf[0] = 0;
			splitw = 0;
		}
	...
	register int p;
{
	...
	if (state == HARDOPEN || splitw)
		return;
	if (p < 0 /* || p > WECHO */)
		error((nl_msg(3, "Internal error: vredraw")));
	...
	for (; l < vcnt && Peekkey != ATTN; l++) {
		 ...
		 */
#ifdef SIGWINCH
		if (LINE(l) != LINE(l + 1) && LINE(l) != p && !got_winch) {
#else 
