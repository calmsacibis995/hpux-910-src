...
#ifndef NONLS8 /* Character set features */
#if defined NLS16 && defined EUC
	else if ( ( (c&TRIM) < ' ' && (c != '\b' || !over_strike) && c != '\n' && c != '\t' ) || ( (c&TRIM) == DELETE ) || ( FIRSTof2( c & TRIM ) ) )
#else
	else if ((c&0177) < ' ' && (c != '\b' || !over_strike) && c != '\n' && c != '\t' || (c&0177) == DELETE)
#endif
#else NONLS8
	...
	int i;
{
	...
	if (right_to_left) {
		...
		if (rl_mode == NL_LATIN) {
			...
			sprintf(n,"%d",i);
			len = 6 - strlen(strord(rl_num,alt_num(n1,n),rl_mode));
			while (len--) printf("%c",alt_space);
			printf("%s",rl_num);
			printf("%c%c",alt_space,alt_space);
		}
	} else {
		printf("%6d  ", i);
	}
...
flush1()
{
	...
	int rem_col ;	/* the number of the column remaining in the line	*/
#endif
...
#ifdef EUC
				/*
				 * Produces a dummy_blank in ex mode.
				 */
				if ( IS_FIRST(c) ) {
					cw_ch = C_COLWIDTH( ( (int)c ) & TRIM ) ;
					if ( columns ) {
						rem_col = columns - ( destcol % columns ) ;
						if ( cw_ch > rem_col) {
							while( rem_col > 0 ) {
		...
		if (j > costRP && parm_right_cursor) {
...
#if defined NLS || defined NLS16
#if defined NLS16 && defined EUC
			col_vtub = vcolumn( vtube[outline], outcol ) ;
#ifdef TRACE
			if(trace)
				fprintf( trace, "\n plod, the value of inopen =%d, outline=%d, outcol=%d,vcolumn = %d \n", inopen, outline, outcol, col_vtub) ;
#endif TRACE
			if (!inopen || vtube[outline]==NULL ||
				(i=vtube[outline][ col_vtub ]) < ' ')
#else
			if (!inopen || vtube[outline]==NULL ||
				(i=vtube[outline][outcol]) < ' ')
#endif
				i = right_to_left ? rl_mode == NL_NONLATIN ? alt_space : ' ' : ' ';
			if(i & QUOTE) 	/* mjm: no sign extension on 3B */
				i = right_to_left ? rl_mode == NL_NONLATIN ? alt_space : ' ' : ' ';
			if ((insmode && cursor_right) || !opp_fix)
#else
			if (!inopen || vtube[outline]==NULL ||
				(i=vtube[outline][outcol]) < ' ')
				i = ' ';
			...
			if (insmode && cursor_right)
				...
				if (!inopen || vtube[outline] == NULL) {
					...
					if (destcol - outcol > 1)	{
					/* If vtube[outline][outcol] is the
					** first byte of the Kanji code and
					** its second byte should be written,
					** writing the subsequent second byte
					** of a Kanji code.
					*/
						plodput(i);	/* first byte */
#ifdef EUC
						outcol++ ;
						plodput(vtube[outline][col_vtub + 1 ]);	/* second byte */
						outcol -= ( 2 - C_COLWIDTH( i & TRIM ) ) ;
#else EUC
						plodput(vtube[outline][++outcol]);	/* second byte */
#endif EUC
					} else	{
						if (cursor_right)	{
							tputs(cursor_right, 0, plodput);
...
#ifdef EUC
							outcol++ ;
							plodput(vtube[outline][col_vtub + 1]);
							outcol -= ( 2 - C_COLWIDTH( i & TRIM ) )  ;
#else EUC
							plodput(vtube[outline][++outcol]);
#endif EUC
							outcol++;
							goto dontcr;
						}
				...
				/* If vtube[outline][outcol] is the second
				** byte of a Kanji code, trying to the
				** various operations as the following
				** sequence. (1) If cursor_right is available,
				** moving cursor with one column.
				** (2) If cursor_left is available,
				** rewriting from its first byte of a Kanji
				** code. (3) If cariage_return is available,
				** rewriting from the beginning of the line.
				** (4) Neither (1), (2) nor (3),
				** it is impossible.
				 */
					if (cursor_right) {
...
flusho()
{
	if (obp != obuf) {
		/**************
		* If vi loses its connection with its tty, we don't 
		* want vi hanging around chewing up cpu cycles (this can
		* occur in a modem disconnect), so let's exit as
		* quickly as possible.
		**************/
		if ( write(1, obuf, obp - obuf) < 0 && errno == EIO)
			exit(1);
