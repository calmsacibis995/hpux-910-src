...
#ifdef SIGWINCH
bool 	got_winch;		/* Indicates we are in SIGWINCH handler */
bool	got_winch_insert;	/* Indicates we got SIGWINCH in insert mode */
var 	struct	winsize save_win;	/* Current window size values */
#endif SIGWINCH
...
var	short	b_doomed ;	/* the number of bytes of doomed char	*/
#endif
...
var	bool	no_map_;	/* Don't map '_' when doing Y (y_) or S (c_) */
...
var	bool	tepe_ok;	/* Did peek ahead issue an extra tepe prompt? */
#endif
var	bool	tilde_cmd;	/* Processing a '~' (change case) command? */
...
/*
 * Only go through the overhead of the vputch subroutine call if TRACE is
 * defined.
 */
#ifdef TRACE
#define	vputp(cp, cnt)	tputs(cp, cnt, vputch)
#else TRACE
#define	vputp(cp, cnt)	tputs(cp, cnt, putch)
#define	vputch(c)	putch(c)
#endif TRACE
