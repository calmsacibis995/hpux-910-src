	...
	char **environ;
{
	...
	register int b, i;
        ...
        cleanenv( &environ, "LANG", "LC_COLLATE", "LC_CTYPE", "LC_TIME",
		"LC_NUMERIC", "LC_MONETARY", "LANGOPTS", "NLSPATH", "TZ", 0 );
#ifndef NONLS8 /* User messages and so on */
	if (!setlocale(LC_ALL,"")) {
		fprintf(stderr, _errlocale("exrecover"));
		putenv("LANG=");
		_nl_fn = (nl_catd)-1;
	} else
		nl_catopen("exrecover");
#endif NONLS8
	...
	if (argc != 3)
		error((nl_msg(1, " Wrong number of arguments to exrecover")), 0);
...
#ifndef NONLS8 /* Country customs */
	cp = nl_cxtime(&H.Time, (nl_msg(2, "%a %h %d %T")));
	/* e.g. Sun Sep 16 01:03:52 */
#else NONLS8
	cp = ctime(&H.Time);
	cp[19] = 0;
#endif NONLS8
...
#ifndef NONLS8 /* User messages */
	if (vercnt > 1)
		fprintmsg(stderr, (nl_msg(3, " [Dated: %1$s, newest of %2$d saved]")), cp, vercnt);
	else
		fprintf(stderr, (nl_msg(4, " [Dated: %s]")), cp);
#else NONLS8
	...
	if ((int) sbrk((int) (H.Flines * sizeof (line))) == -1)
		 ...
		 * Good grief.
		 */
		error((nl_msg(5, " Not enough core for lines")), 0);
#ifdef DEBUG
	...
	char *dirname;
{
	...
	while (dirent = readdir(dir)) {
		...
		if ((dirent->d_name[0] != 'E') || (dirent->d_name[1] != 'x') )
			continue;
/* 
 * make sure only working with 
 * regular files
 */
		if( (stat(dirent->d_name, &stat_buf) == -1) ||
	 ...
	 */
	if (ecount == 0) {
		fprintf(stderr, (nl_msg(6, "No files saved.\n")));
		return;
	}
	...
	for (fp = &svbuf[0]; fp < &svbuf[ecount]; fp++) {
		...
		strcpy(cpday, nl_cxtime(&fp->sf_time, (nl_msg(7, "%a %h %d"))));
		/* e.g. Sun Sep 16 */
		strcpy(cptime, nl_cxtime(&fp->sf_time, (nl_msg(8, "%H:%M"))));
		/* e.g. 01:52 */
		fprintmsg(stderr, (nl_msg(9, "On %1$s at %2$s saved %3$d lines of file \"%4$s\"\n")),
		    cpday, cptime, fp->sf_lines, fp->sf_name);
#else NONLS8
		cp = ctime(&fp->sf_time);
		cp[10] = 0;
	...
	char *fname;
{
	...
	for (cp2 = fp->sf_name, cp = savedfile; *cp;)
		*cp2++ = *cp++;
	*cp2++ = 0;
	for (cp2 = fp->sf_entry, cp = fname; *cp && cp-fname < MAXNAMLEN;)
		*cp2++ = *cp++;
	*cp2++ = 0;
}
	...
	char *dir;
{
	 ...
	 * Extreme lossage...
	 */
	error((nl_msg(10, " File not found")), 0);
}
...
#ifndef NONLS8 /* User messages */
				fprintf(stderr, " [");
				fprintf(stderr, (nl_msg(11, "Lost line(s)")));
				fprintf(stderr, ":");
#else NONLS8
...
#ifndef NONLS8 /* User messages */
			fprintf(stderr, " [");
			fprintf(stderr, (nl_msg(12, "Lost line(s)")));
			fprintf(stderr, ":");
#else NONLS8
			fprintf(stderr, " [Lost line(s):");
#endif NONLS8
	...
	int iof;
{
	...
	off = (atl << SHFT) & LBTMSK;
	if (bno >= NMBLKS)
		error((nl_msg(13, " Tmp file too large")));
	nleft = BUFSIZ - off;
	if (bno == iblock) {
		ichanged |= iof;
		return (ibuff + off);
...
syserror()
{
	...
	dirtcnt = 0;
	write(2, " ", 1);
	if (*(err = strerror( errno)) != '\0')
		error( err);
	else
		error((nl_msg(14, "System error %d")), errno);
	exit(1);
}
