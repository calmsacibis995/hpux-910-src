...
char **argv;
{
	...
	int x;
	char fmt[20], precedes[MAXPATHLEN+1], follows[MAXPATHLEN+1];
	...
	for(;;) {
		/* create a format the limits the size of the strings */
		sprintf(fmt, "%%%ds%%%ds",
			sizeof(precedes)-1, sizeof(follows)-1);
		x = fscanf(input, fmt, precedes, follows);
		if(x==EOF)
			break;
		if(x!=2)
			error("odd data",empty);

		if (strlen(precedes) >= sizeof(precedes)-1)
			error("Name too long: ", precedes);
		if (strlen(follows)  >= sizeof(follows)-1)
			error("Name too long: ", follows);
