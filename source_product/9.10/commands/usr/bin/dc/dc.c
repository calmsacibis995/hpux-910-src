...
commnds(){
	...
	while(1){
		...
		case EOF:
			continue;
		case 'Y':
			if (stkptr && *stkptr) sdump("stk",*stkptr);
			else printf("no stk\n");
...
sempty:
		...
		case ';':
			...
			if(sptr == 0) {
				...
				q=p;
				/*****************/
				sptr->val = p =  salloc((n+PTRSZ)*PTRSZ);
				zero(p);
				salterwd(p,q);
			}
			{
...
onchildexit(sig,code,scp)
int sig,code;
struct sigcontext *scp;
{
	int info;
    /*		Force the system call to restart if signal arrived during a read */
    scp->sc_syscall_action=SIG_RESTART;
    /*		Get the exist status from the child that died */
    waitpid((pid_t)-1, &info, 0);
	if (WIFEXITED(info)){
		exitstatus = WEXITSTATUS(info);  /* use child's status for parent */
        }
	}
pushp(p)
struct blk *p;
...
command(){
	int c;
	char line[100],*sl;
	register (*savint)(),(*savchld)(),pid,rpid;
	int retcode;
	...
	default:
		...
		*sl = 0;
		...
		savchld = signal(SIGCHLD, SIG_IGN);
		while((rpid = wait(&retcode)) != pid && rpid != -1);
		signal(SIGINT,savint);
		signal(SIGCHLD,savchld);
		printf("!\n");
		return(0);
	}
}
