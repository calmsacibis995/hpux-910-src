...
# endif
		...
		case CARAT:
			cfoll(left[v]);
			break;
		case STAR: case PLUS: case QUEST: case RSCON: case RNOSCON:
			cfoll(left[v]);
			break;
...
pfoll()
	...
	register int i,k,*p;
	int j;
	/* print sets of uchars which may follow positions */
	printf("pos\tchars\n");
	for(i=0;i<tptr;i++)
		if(p=foll[i]){
			j = *p++;
...
add(array,n)
  ...
  int n; {
	register int i, *temp;
	register uchar *ctemp;
	register int *poslimit;
	temp = nxtpos;
	ctemp = tmpstat;
	poslimit = positions+maxpos;
  ...
  int v;
	...
	switch(name[p]){
		...
		case RCAT: case DIV: 
			...
			break;
		case RSCON: case CARAT: case RNOSCON:
			follow(p);
			break;
# ifdef DEBUG
		default:
			printf("bad switch follow %d",p);
# endif
		}
	return;
	}
  ...
  int v; {
	...
	switch(i){
		...
		case RSCON:
			...
			break;
		case RNOSCON:
			/* a pattern with no start condition.  If the
			 * current state is a %x state, the pattern
			 * is not active.
			 */
			i = stnum/2;
# ifdef DEBUG
			printf("stnum = %d  i = %i  stype[i] = %c\n",
				stnum, i, stype[i]);
# endif
			if (stype[i]=='s' || stype[i]=='S')
				first(left[v]);
			break;
...
# ifdef DEBUG
			...
			case S2FINAL:
				break;
			default:
				printf("bad switch cgoto %d state %d",curpos,s);
				break;
# endif
			}
		}
...
# endif
		/* for each uchar, calculate next state */
		n = 0;
		for(i = 1; i<NCH; i++){
			if(symbol[i]){
				nextstate(s,i);		/* executed for each state, transition pair */
				xstate = notin(stnum);
				if(xstate == -2) warning(BADSTATE, itos(b_1,s), itos(b_2,i));
				else if(xstate == -1){
					if(stnum+1 >= nstates){
						message(MANYSTATES);
						if (nstates == NSTATES)
...
nextstate(s,c)
  int s,c; {
	...
	register uchar *temp, *tz;
	int *pos, i, *f, num, curpos, number;
	/* state to goto from state s on uchar c */
	num = *state[s];
	temp = tmpstat;
	pos = state[s] + 1;
	for(i = 0; i<num; i++){
...
# define CHECK_NPTR if (nptr >= ntrans) { if (ntrans==NTRANS) error(ENTRANS); else error(ENTRANS1); }
...
# endif
	...
	for(i=0; i<st; i++){	/* get most similar state */
				/* reject state with more transitions, state already represented by a third state,
					and state which is compressed by uchar if ours is not to be */
...
# ifdef PS
	if(cmin != -1){ /* if we can use st cmin */
		...
		while(ach[j]){
			...
			if(ach[j] > nchar[p-1]){
				warning(BADTRANS, itos(b_1,st), itos(b_2,cmin));
				goto nopack;
				}
			/* ach[j] == nchar[p-1] */
...
acompute(s)	/* compute action list = set of poss. actions */
  int s; {
	...
	int cnt, m;
	int k, n;
	int *temp=actemp; 	/* arrays are allocated in get2core to */
	int *neg = acneg; 	/* allow size to be settable. */
	k = 0;
	n = 0;
	p = state[s];
	cnt = *p++;
