...
/* both of these start out the same because their initial values are equal */
int	ofmt_stamp=1;
int	convfmt_stamp=1;
...
syminit()
{
	...
	recloc = &fldtab[0];
        ...
        /* NLS16 - initialize FS with trailing blank to allow enough */
	/* space for two-byte value from command line (see top of main()) */
	/* FS is always retrieved by dereferencing, so 2nd byte is invisible */
	/* unless 1st byte starts a kanji */
	FS = &setsymtab("FS", tostring("  "), 0.0, STR, symtab)->sval;
	*((*FS)+1) = '\0';

	FS = &setsymtab("FS", " ", 0.0, STR|DONTFREE, symtab)->sval;
	RecordSep = &setsymtab("RS", "\n", 0.0, STR|DONTFREE, symtab)->sval;
...
{
	if (vp->tval & ARR)
		error(FATAL,ERR25, rw, vp->nval);
	if (vp->tval & FCN)
		error(FATAL,ERR24, rw, vp->nval);
	error(FATAL,ERR42, vp, vp->nval, vp->sval, vp->fval, vp->tval);
}
...
uchar *s;
{
	if (!strcmp(vp->nval,"OFMT")) {
		if (++ofmt_stamp==convfmt_stamp)
			ofmt_stamp++;
	} else if (!strcmp(vp->nval,"CONVFMT")) {
		if (++convfmt_stamp==ofmt_stamp)
			convfmt_stamp++;
	}
	if ((vp->tval & (NUM | STR)) == 0)
		funnyvar(vp, "assign to");
...
uchar **fmt;
{
	...
	if (vp->tval & NUM) {
	if ((vp->tval & STR) == 0 ||
	    (!(vp->tval & (STRCON|FLD|REC)) && 
	    ((fmt==OFMT && vp->stamp!=ofmt_stamp) ||
	    (fmt==CONVFMT && vp->stamp!=convfmt_stamp)))) {
		if (!(vp->tval&DONTFREE))
			xfree(vp->sval);
		...
		vp->tval &= ~DONTFREE;
		vp->tval |= STR;
		vp->stamp=((fmt==OFMT)?ofmt_stamp:convfmt_stamp);
	}
	}
...
*/
			...
			*q++ = c;
			...
			switch (c = *++s) {
			case '\0':	/* in case '\' is last character */
				s--;
			case '\\':	*q++ = '\\'; break;
			case 'n':	*q++ = '\n'; break;
			case 't':	*q++ = '\t'; break;
