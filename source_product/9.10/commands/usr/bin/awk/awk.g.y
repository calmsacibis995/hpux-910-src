...
int	asgn_op = 0;	/* = 1 if next token could be /= */
uchar	*curfname = 0;
Node	*arglist = 0;	/* list of args for current function */
uchar	*strnode();
Node	*notnull();
extern int nosub;	/* compile with REG_NOSUB? */
%}
...
program:
	...
	| error	{ yyclearin; bracecheck(); error(SYNTAX,ERR21); }
	;
...
pa_stat:
	...
	| FUNC funcname '(' varlist rparen {infunc++;} lbrace stmtlist '}'
		{ infunc--; curfname=0; defn($2, $4, $8); $$ = 0; }
	;

pa_stats:
	  pa_stat
	| pa_stats pa_stat	{
				/* allows multiple pattern-action */
...
pe_list:
	...
	| pe_list comma pattern		{ $$ = linkum($1, $3); }
	| '(' pe_list ')'		{ $$ = $2; }
	;

ppattern:
	  var MAYBEDIVEQ {asgn_op=1;} ASGNOP ppattern	{$$ = op2($4, $1, $5);}
	...
	| ppattern MATCHOP reg_expr	{
					nosub = 1;
					$$ = op3($2, NIL, $1, makedfa($3));
					}
	| ppattern MATCHOP ppattern
		{ if (constnode($3)) {
			nosub=1;
			$$ = op3($2, NIL, $1, makedfa(strnode($3)));
		  }
		  else
			$$ = op3($2, (Node *)1, $1, $3); }
	...
	| reg_expr
		{
		nosub=1;
		$$ = op3(MATCH, NIL, rectonode(), makedfa($1));
		}
	| term
	;

pattern:
	  var MAYBEDIVEQ {asgn_op=1;} ASGNOP pattern	{$$ = op2($4, $1, $5);}
...
reg_expr:
	  '/' {startreg();} REGEXPR '/'		{ $$ = (Node *)$3; }
	| MAYBEDIVEQ {startreg2();} REGEXPR '/'		{ $$ = (Node *)$3; }
	;
...
simple_stmt:
	  print prarg '|' pattern	{ $$ = stat3($1, $2, (Node *) $3, $4); }
	| print prarg APPEND pattern	{ $$ = stat3($1, $2, (Node *) $3, $4); }
	| print prarg GT pattern	{ $$ = stat3($1, $2, (Node *) $3, $4); }
	| print prarg			{ $$ = stat3($1, $2, NIL, NIL); }
	| DELETE varname '[' patlist ']' { $$ = stat2(DELETE, makearr($2), $4); }
	| DELETE varname		{ yyclearin; error(SYNTAX,ERR119); $$ = stat1(DELETE, $2); }
	| pattern			{ $$ = exptostat($1); }
	| error				{ yyclearin; error(SYNTAX,ERR53); }
	;
...
term:
	...
	| MATCHFCN '(' pattern comma reg_expr ')'
		{ $$ = op3(MATCHFCN, NIL, $3, makedfa($5)); }
	| MATCHFCN '(' pattern comma pattern ')'
		{ if (constnode($5))
			$$ = op3(MATCHFCN, NIL, $3, makedfa(strnode($5)));
		  else
	...
	| SPLIT '(' pattern comma varname comma reg_expr ')'    /* const /regexp/ */
		{ $$ = op4(SPLIT, $3, makearr($5), makedfa($7), (Node *)REGEXPR); }
	| SPLIT '(' pattern comma varname ')'
		{ $$ = op4(SPLIT, $3, makearr($5), NIL, (Node*)STRING); }  /* default */
	| SPRINTF print_list		{ $$ = op1($1, $2); }
	| STRING	 		{ $$ = valtonode($1, CCON); }
	| subop '(' reg_expr comma pattern ')'
		{ $$ = op4($1, NIL, makedfa($3), $5, rectonode()); }
	| subop '(' pattern comma pattern ')'
		{ if (constnode($3))
			$$ = op4($1, NIL, makedfa(strnode($3)), $5, rectonode());
		  else
			$$ = op4($1, (Node *)1, $3, $5, rectonode()); }
	| subop '(' reg_expr comma pattern comma pattern ')'
		{ $$ = op4($1, NIL, makedfa($3), $5, $7); }
	| subop '(' pattern comma pattern comma var ')'
		{ if (constnode($3))
			$$ = op4($1, NIL, makedfa(strnode($3)), $5, $7);
		  else
			$$ = op4($1, (Node *)1, $3, $5, $7); }
...
{
	if (isarr(p))
		error(SYNTAX,ERR08, p->nval);
	else if (isfunc(p))
		error(SYNTAX,ERR90, p->nval);
	curfname = p->nval;
}
