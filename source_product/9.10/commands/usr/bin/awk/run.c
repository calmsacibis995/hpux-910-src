...
#pragma OPT_LEVEL 1     /* program() doesn't optimize well... */
...
Cell *call(a, n) Node **a;
{
	...
	dprintf("calling %s, %d args (%d in defn), fp=%d\n", s, ncall, ndef, fp-frame);
        if (ncall > ndef)
		error(WARNING,ERR122,s,ncall,ndef);
        if (ncall + ndef > NARGS)
		error(FATAL,ERR123,s,ncall+ndef,NARGS);
	...
	Cell *x;
{
	...
	y->fval = x->fval;
	y->tval = x->tval & ~(STRCON|NUMCON|CON|FLD|REC|DONTFREE);	/* copy is not constant or field */
							/* is DONTFREE right? */
...
Cell *arg(a) Node **a;
{
	...
	if (n+1 > fp->nargs)
		error(FATAL,ERR19,n+1, fp->fcncell->nval);
	return fp->args[n];
}
...
Cell *array(a,n) register Node **a;
{
	...
	for (np = a[1]; np; np = np->nnext) {
		...
		s = getsval(y);
		if (strlen(s)+strlen(*SUBSEP)+1>RECSIZE)
			error(FATAL,ERR75,s);
		strcat(buf, s);
		if (np->nnext)
			strcat(buf, *SUBSEP);
...
Cell *relop(a,n) Node **a;
{
	...
	if (x->tval&NUM && y->tval&NUM) {
		j = x->fval - y->fval;
		i = j<0? -1: (j>0? 1: 0);
	} else if (x->tval&NUM && (ynum=isnumber(y->sval))) {
		j = x->fval - getfval(y);
		i = j<0? -1: (j>0? 1: 0);
	} else if ((xnum=isnumber(x->sval)) && y->tval&NUM) {
		j = getfval(x) - y->fval;
		i = j<0? -1: (j>0? 1: 0);
	} else {
...
Cell *sindex(a, nnn) Node **a;
{
	...
	if (_CS_SBYTE) {
		q=strstr(s1,s2);
		if (q) {
			v = (Awkfloat) (q-s1+1); /* origin of substring */
		}
	} else {
		register uchar *p1, *p2;
		register i = 1;
		for (p1 = s1; *p1 != '\0'; ADVANCE(p1)) {
			for (q=p1, p2=s2; *p2 != '\0' && *q == *p2; q++, p2++)
				;
			if (*p2 == '\0') {
				v = (Awkfloat) (i);       /* origin 1 */
				break;
			}
			i++;
		}
...
format(buf, bufsize, s, a) uchar *buf, *s; int bufsize; Node *a;
{
	...
	while (*s) {
		...
		*t = '\0';
		...
		a = a->nnext;
		switch (flag) {
		case 1:	nbytes=sprintf((char *)p, (char *)fmt, getfval(x)); break;
		case 2:	nbytes=sprintf((char *)p, (char *)fmt, (long) getfval(x)); break;
		case 3:	nbytes=sprintf((char *)p, (char *)fmt, (int) getfval(x)); break;
		case 4:	nbytes=sprintf((char *)p, (char *)fmt, getsval(x)); break;
		case 5: if (isnum(x))
				nbytes=sprintf((char *)p, (char *)fmt, (int) getfval(x));
			else
				nbytes=sprintf((char *)p, (char *)fmt, getsval(x)[0]);
			break;
		}
		tempfree(x, "");
		p += nbytes;
...
Cell *aprintf(a,n) Node **a;
{
	...
	y = a[0]->nnext;
	x = execute(a[0]);
	if ((nbytes=format(buf,sizeof buf, getsval(x), y)) == -1)
		error(FATAL,ERR125,buf);
	tempfree(x,"");
	if (a[1] == NULL)
...
Cell *assign(a,n) Node **a;
{
	...
	if (n == ASSIGN) {	/* ordinary assignment */
		if ((y->tval & (STR|NUM)) == (STR|NUM)) {
			setsval(x, getsval(y));
			x->fval = getfval(y);
			x->tval |= NUM | (y->tval&(STRCON|NUMCON)) /*|(y->tval&FLD)*/;

		}
		else if (y->tval & STR) {
			setsval(x, getsval(y));
			x->tval |= (y->tval&(STRCON|CON));
		} else if (y->tval & NUM) {
			setfval(x, getfval(y));
			x->tval |= (y->tval&(NUMCON|CON));
		} else
...
Cell *cat(a,q) Node **a;
{
	...
	if (s == NULL)
		error(FATAL,ERR126,x->sval,y->sval);
	strcpy(s, x->sval);
	strcpy(s+n1, y->sval);
	tempfree(y,"");
	z = gettemp("");
...
static int ctest[0400];		/* NLS: increased from 0200 */
...
Cell *split(a,nnn) Node **a;
{
	Cell *x, *y, *ap;
	register uchar *s;
	register unsigned sep;	/* NLS16: may be kanji */
...
#ifndef NLS16
			ctest[sep] = ctest['\n'] = ctest['\0'] = 1;
			while(!ctest[*(unsigned char *)s])      /* NLS: unsigned char */
			/*while (*s != sep && *s != '\n' && *s != '\0')*/
				 s++;
			ctest[sep] = ctest['\n'] = ctest['\0'] = 0;
#else
...
Cell *bltin(a,n) Node **a;
{
	...
	case FSYSTEM:
		...
		if (WIFEXITED(status))
			u = (Awkfloat)WEXITSTATUS(status);
		else
			u = (Awkfloat)(WTERMSIG(status) + 128);
		break;
	case FRAND:
	...
	case FSRAND:
        ...
        case FTOLOWER:
			...
			if (_CS_SBYTE) {
                        	...
                        	for (p = buf; *p; CHARADV(p))
					if (!FIRSTof2(*p) && islower(*p))
                                        	*p = toupper(*p);
			}
                } else {
			if (_CS_SBYTE) {
	...
	for (i = 0; i < FOPEN_MAX; i++)
		if (files[i].fname && strcmp(x->sval, files[i].fname) == 0) {
			if (ferror(files[i].fp))
				error(WARNING,ERR129,files[i].fname);
			if (files[i].mode == '|' || files[i].mode == LE)
				stat = pclose(files[i].fp);
			else
				stat = fclose(files[i].fp);
			if (stat == EOF)
				error(WARNING,ERR130,files[i].fname);
			xfree(files[i].fname);
