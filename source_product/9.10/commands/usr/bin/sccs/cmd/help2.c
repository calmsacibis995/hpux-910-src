...
char *p;
{
...
#ifdef NLS16
	while ((c=CHARAT(q)) && (c > 255 || !numeric(*q)))
		ADVANCE(q);
#else NLS16
	while (*q && !numeric(*q))
		q++;
	...
	if((iop = fopen(hfile,"r")) == NULL)
	  {
	    fprintf(stderr, nl_msg(2, "There is not a place to look for help on the subject of %s\n"), key);
	    return(1);
	  }
	...
	*/
	...
	if (q == NULL) {	/* endfile? */
	    fclose(iop);
	    fprintf(stderr,nl_msg(3, "There is not any help available on the subject of %s\n"), key);
	    return(1);
	}
...
ask()
{
	...
	printf(nl_msg(4, "What is the message number or SCCS command name? "));
	fgets(resp,51,iop);
	return(repl(resp,'\n','\0'));
}
