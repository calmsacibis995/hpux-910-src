...
static char	*NVarg[NVARGS];   /* The MR's from split_mrs */
extern char *Sflags[];
int D_serial;
...
char *argv[];
{
	...
	if (!setlocale(LC_ALL, "")) {
		fputs(_errlocale(argv[0]), stderr);
		putenv("LANG=");
	}
	nl_catopen("sccs");
#endif NLS || NLS16
	...
	*/
	...
	for(i=1; i<argc; i++)
		if(argv[i][0] == '-' && (c = argv[i][1])) {
			...
			case 'r':
				if (!(*p))
					{ sprintf(Error,"%s (rc11)",nl_msg(1,"r has no sid"));
					fatal(Error);  }
				Sidhold=p;
			...
			case 'z':
				break;
			default:
				{ sprintf(Error,"%s (cm1)",nl_msg(2,"unknown key letter"));
				fatal(Error);  }
			}
	...
	if(HADM && HADZ)
	{
		fatal((nl_msg(4,"Cassi not compatable with cmrs on command line")));
	}
	if(num_files == 0)
		{ sprintf(Error,"%s (cm3)",nl_msg(5,"missing file arg"));
		fatal(Error);  }
	...
	if (equal(p,"rmdel"))
		...
		{ sprintf(Error,"%s (rc10)",nl_msg(6,"bad invocation"));
		fatal(Error);  }
	if (! logname())
		{ sprintf(Error,"%s (cm9)",nl_msg(7,"User ID not in password file"));
		fatal(Error);  }
...
char *file;
{
	...
	if (!exists(file)) {
		sprintf(Error,nl_msg(9,"file %s does not exist"),file);
		fatal(strcat(Error," (rc2)"));
	}
	...
	*/
	if (sid.s_rel != 0)
		{ sprintf(Error,"%s (rc3)",nl_msg(11,"nonexistent sid"));
		fatal(Error);  }
	/*
	...
	*/
	...
	if (user != fowner || user != downer)
		if (!equal(Pgmr,logname())) {
			...
			sprintf(Error,
				nl_msg(12,"you are neither owner nor '%s'"),Pgmr);
			fatal(strcat(Error," (rc4)"));
		}
	...
	if (D_type == 'R') {
		...
		for (n = maxser(&gpkt); n > D_serial; n--) {
			...
			if (ptr->i_pred == D_serial)
				{ sprintf(Error,"%s (rc5)",nl_msg(13,"not a 'leaf' delta"));
				fatal(Error);  }
		}
		...
		*/
		if (Mrs && *Mrs) {
			if (!(p = Sflags[VALFLAG - 'a']))
				{ sprintf(Error,"%s (rc6)",nl_msg(14,"MRs not allowed"));
				fatal(Error);  }
			if (*p && valmrs(&gpkt,p))
				{ sprintf(Error,"%s (rc7)",nl_msg(15,"invalid MRs"));
				fatal(Error);  }
		}
	...
	*/
	if((HADZ && !Sflags[CMFFLAG - 'a']) ||
		...
		{
			{ sprintf(Error,"%s\n",nl_msg(16,"invalid use of 'z' flag \n CASSI incompatablity"));
			fatal(Error);  }
		}
	else
		{
		if(HADZ)
			...
			/*now process the testcmr table and send messages*/
				...
				{
					fatal((nl_msg(17,"uneditable cmr for CASSI controlled file- Change not allowed")));
				}
			}
		}
...
struct packet *pkt;
{
	...
	if((HADZ) && (pkt->p_line[1] == MRNUM) && (D_type == 'D') && (!doneesc))
		{
		 ...
		 if(!esccmfdelt(pkt))
			{
			 fatal((nl_msg(18,"bad cdc replace of cmr's (CASSI)")));
			}
		 else
			return;
		}
	...
	if (pkt->p_line[1] == MRNUM) {		/* line read is MR line */
		...
		while (*p)
			p++;
		if (*(p - 2) == DELIVER)
			{ sprintf(Error,"%s (rc9)",nl_msg(19,"delta specified has delivered MR"));
			fatal(Error);  }
...
*/
	esccmfdelt(pkt)
		struct packet *pkt;
		{
		 ...
		 if(!holdptr[1])
			{
			 ...
			 printf("%s \n",(nl_msg(21,"only one cmr left for this delta no change possible")));
			 return(1);
			}
		 numcmrs = i;
		 n=0;
		 ...
		 while (holdptr[j])
			{
			 printf(nl_msg(22,"do you want to delete %s"),holdptr[j]);
			 printf("\n");
			 gets(answ);
#ifdef NLS
			 if(imatch(nl_langinfo(YESSTR),answ))
#else
			 if(imatch("y",answ))
#endif
				{
				 if(numcmrs <= 1)
					{
						printf(nl_msg(24,"%s  is not editable now"),holdptr[j]);
						printf("\n");
...
struct packet *pkt;
{
	...
	if(!(fred=(char *)gf(Sflags[CMFFLAG - 'a'])))
		fatal((nl_msg(25,"No fred file found- see sccs administrator")));
	/* check validity of values in test */
	for(i=0;test[i];i++)
		{
			if(!(testfred(test[i],fred)))
				{
					sprintf(Error,nl_msg(26," %s is not editable"),test[i]);
					fatal(strcat(Error,"\n"));
					return(0);
				}
		}
...
msg(app,name,cmrs,stats,sids,fred)
	...
	*ptr=(char)NULL;
	...
	if(!(fd=fopen(holdfred,"a")))
		{
			sprintf(Error,"%s\n",nl_msg(27,"CASSI msg not writable"));
			fatal(Error);
			return(0);
		}

#ifdef NLS 
	fprintmsg(fd,nl_msg(28,"%1$s chpost %2$s q %3$s sw MID=%4$s MFS=%5$s MPA=%6$s q q"),
	...
	fclose(fd);
	 if(noexist) /*new termLOG make owner of /BD/source owner of file */
	 {
		if(stat(dir,&stbuf) == -1)
		{
			sprintf(Error,"%s\n",nl_msg(29,"Cassi BD/source not writeable"));
			fatal(Error);
		}
		chmod(holdfred,0666);
...
/*
** Varg is a pointer to a pointer to a character.  Varg actually points
** into an array of pointer to integers (Qarg, DVarg, or NVarg).
** Varg is defined in dohist.c in the libraries.
*/
extern char **Varg;    
/*
** This routine apparently shifts the contents of Qarg as
** pointed at by Varg into the NVarg array (declared above).
** IF MRs_are_not_allowed THEN (*argv)==empty
*/
split_mrs()
{
	register char **argv;
...
/*
** One rather interesting way this routine can fail is if the
** *assumed* contents of NVarg are not initialized to empty and
** then stay that way (Note that a setjmp is done which could
** possibly overwrite into the NVarg array).  Varg will point
** at the contents of NVarg if split_mrs() (above) was executed.
*/
putmrs(pkt)
struct packet *pkt;
{
	...
	while (fgets(rline,sizeof(line),in) != NULL) {
		...
		if (sp->s_rel == pf.pf_gsid.s_rel &&
			...
			sp->s_seq == pf.pf_gsid.s_seq) {
				fclose(in);
				sprintf(Error,"%s (rc12)",nl_msg(30,"being edited -- sid is in p-file"));
				fatal(Error);
		}
	}
	fclose(in);
...
#ifndef hp9000s800
			putline(pkt,"c ");
#else hp9000s800	/* nscc currently can't handle control characters */
			putline(pkt,"\001c ");
#endif hp9000s800
			putline(pkt,(nl_msg(31,"*** LIST OF DELETED MRS ***")));
			putline(pkt,"\n");
			first = 0;
		}
