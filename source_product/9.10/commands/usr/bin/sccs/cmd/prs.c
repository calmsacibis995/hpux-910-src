...
char *argv[];
{
	...
	*/
	for (j = 1; j < argc; j++)
		if (argv[j][0] == '-' && (c = argv[j][1])) {
			...
			case 'r':	/* specified SID */
				if (*p) {
					if (invalid(p))
						{ sprintf(Error,"%s (co8)",nl_msg(1,"invalid sid"));
						fatal(Error);  }
					sid_ab(p,&sid);
				}
				break;
			case 'c':	/* cutoff date[time] */
				if((*p) && (date_ab(p,&Date_time) != -1))
					...
					{ sprintf(Error,"%s (prs4)",nl_msg(2,"invalid cutoff date"));
					  fatal(Error);  }
			...
			if (had[c - 'a']++)
				{ sprintf(Error,"%s (cm2)",nl_msg(5,"key letter twice"));
				fatal(Error);  }
			argv[j] = 0;
		}
		else
	...
	if (num_files == 0)
		{ sprintf(Error,"%s (cm3)",nl_msg(6,"missing file arg"));
		fatal(Error);  }

	if(HADR && HADC)
		{ sprintf(Error,"%s (prs5)",nl_msg(7,"can't specify cutoff date and SID"));
		fatal(Error);  }
	if(HADC && (!HADL) && (!HADE))
		{ sprintf(Error,"%s (prs6)",nl_msg(8,"must specify -e or -l with -c"));
		fatal(Error);  }
	...
	while (getstats(pkt,&stats) && !stopdel) {
		...
		*/
		...
		if(HADC) {
			...
			/*
			** Original code did not work for:
			**      -cDATE -e
			** when DATE was earlier than the most recent delta.
			**
			** Original code also did not work when both
			** -e & -l options were specified at the same time.
			*/
			if(!HADL && (dcomp < 0)) {
				read_to(EDELTAB,pkt);
				continue;
			}
			else if(!HADE && (dcomp > 0)) {
				stopdel = 1;
				continue;
			}
		}
			...
			/*
			if !HADL keyletter skip delta entry (see comment above)
			*/
			if (!HADL) {
				read_to(EDELTAB,pkt);
				continue;
		...
		if(lp[0] == ':' && lp[1] != 0 && lp[2] == ':') {
...
#if u370 || u3b || u3b5 || hp9000s200 || hp9000s500 || hp9000s800
			u.str[3] = *++lp;
			u.str[2] = *++lp;
#else
			u.str[0] = *++lp;
			...
			case 256*'N'+'P':	/* :PN: Full pathname of File */
				...
				if ( getcwd( Olddir, BUFSIZ ) == (char *) 0 )
					{ sprintf(Error,"%s (prs2)",nl_msg(27,"getcwd failed"));
					fatal(Error);  }
				if(chdir(Dir) != 0)
					{ sprintf(Error,"%s (prs3)",nl_msg(28,"cannot change directory"));
					fatal(Error);  }
				if ( getcwd( Pname, BUFSIZ ) == (char *) 0 )
					{ sprintf(Error,"%s (prs2)",nl_msg(29,"getcwd failed"));
					fatal(Error);  }
				if(chdir(Olddir) != 0)
					{ sprintf(Error,"%s (prs3)",nl_msg(30,"cannot change directory"));
					fatal(Error);  }
				printf("%s/",Pname);
				printf("%s",sname(gpkt.p_file));
				break;
...
register struct packet *pkt;
{
	...
	*/
	if (odt.d_sid.s_rel == 0)
		{ sprintf(Error,"%s (prs1)",nl_msg(31,"nonexistent SID"));
		fatal(Error);  }
	else
...
*********************************************************************/
	...
	if (HAD_BD)
		fclose(BDiop);
	if (pkt->p_q)
		{ sprintf(Error,"%s (co5)",nl_msg(32,"premature eof"));
		fatal(Error);  }
	return(0);
}
...
struct packet *pkt;
{
	...
	*/
	...
	/* Flush stdout so output will be correct if redirected
	   to a disc file.
	   KAH  1/6/82
	*/
	fflush(stdout);
	if ((i = fork()) < 0)
		fatal((nl_msg(33,"cannot fork, try again")));
	if (i == 0) {
		/*
		...
		*/
		...
		sprintf(Error,(nl_msg(34,"cannot execute '%s'")),Getpgm);
		fatal(Error);
	}
	else {
		wait(&status);
