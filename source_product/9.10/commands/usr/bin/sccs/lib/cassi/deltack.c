...
deltack(pfile,mrs,nsid,apl)
	...
	if(!pfile)
		{
		 error((nl_msg(11,"Pfile non existant at deltack ")));
		 return(0);
		}
	/*if no application serious error */
	if(!apl)
		{
		 error ((nl_msg(12,"no application found with -fz flag")));
		 return(0);
		}
	/*check for and retrieve FRED given the application name*/
	if(!getinfo(&fred,apl))
		{
		 error((nl_msg(13,"no FRED file or system name in admin directory ")));
		 return(0);
		}
	...
	if(!(mrs))
		{
		 if (efd=fopen(errorlog,"a"))
			{fprintf(efd,(nl_msg(15,"***CASSI REPORTS ERROR: no CMRS in pfile: %s")),pfile);
			 fprintf(efd,"\n");
			 fclose(efd);
			}
		 error((nl_msg(16,"CMRs not on P.file -serious inconsistancy")));
		 return(0);
		}
...
promdelt(cmrs,stat,type,fred)
	...
	if(HADZ) /*force delta flag on */
	{
		...
		{
		 if (efd=fopen(errorlog,"a"))
			{fprintf(efd,"%s\n",(nl_msg(17,"***CASSI REPORTS ERROR: no CMRS at sd")));
			 fclose(efd);
			}
			sprintf(Error,"%s\n",(nl_msg(18,"no CMR's left, delta forbidden")));
			fatal(Error);
		}
		strcpy(stat,"sd");
	...
	while(FOREVER)
	  {
	   if(!badflag)
		{
		 printf("%s\n",(nl_msg(19,"the CMRs for this delta now are:")));
		 ...
		 gets(answ);  
#ifdef NLS
		 if((!strcmp(answ,(langinfo((currlangid()),YESSTR)))))
#else
		 ...
		 {
		   printf("%s\n",(nl_msg(24,"you must input a least 1 valid cmr number")));
		 }
		 /*now prompt for new cmrs to add to the list*/
		while(FOREVER)
			{
			 eqflag = 0;
			 printf((nl_msg(25,"enter new CMR number or 'CR' ")));
			 gets(answ);
			 if(answ[0] == NULL)
				if(!badflag)
					{
...
************************************************/
	 ...
	 *ptr=NULL;
	 ...
	 if(!(fd=fopen(holdfred,"a")))
		{
		 if (efd=fopen(errorlog,"a"))
			{fprintf(efd,(nl_msg(32,"***CASSI REPORTS ERROR: can't write to FRED : %s")),pname);
			 fprintf(efd,"\n");
			 fclose(efd);
			}
		 sprintf(Error,"%s\n", (nl_msg(33," Cassi Interface Msg not writable")));
		 fatal(Error);
		 return(0);
		}
#ifdef NLS
	 fprintmsg(fd,(nl_msg(34,"%1$s chpost %2$s q %3$s %4$s MID=%5$s MFS=%6$s MPA=%7$s q q")),
	 ...
	 if(noexist) /*new termLOG make owner of /BD/source owner of file */
	 {
		...
		{
		 if (efd=fopen(errorlog,"a"))
			{fprintf(efd,(nl_msg(35,"***CASSI REPORTS ERROR: can't write to BD/source : %s")),pname);
			 printf(efd,"\n");
			 fclose(efd);
			}
			sprintf(Error,"%s\n",(nl_msg(36,"Cassi BD/source not writeable")));
			fatal(Error);
		}
		chmod(holdfred,0666);
...
getinfo(freddy,sys)
	...
	*freddy=gf(sys);
	if(!(**freddy))
		{
		 printf((nl_msg(37,"got to bad FRED file %s")),*freddy);
		 printf("\n");
		 return(0);
		}
	 if(stat(*freddy,&buf))
