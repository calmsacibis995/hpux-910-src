 ...
 */
#if hp9000s200
#define ISMAGIC(x) ((((unsigned short)x)==(unsigned short)RELOC_MAGIC)|| \
			(((unsigned short)x)==(unsigned short)EXEC_MAGIC)|| \
			(((unsigned short)x)==(unsigned short)SHARE_MAGIC)|| \
			(((unsigned short)x)==(unsigned short)DEMAND_MAGIC)|| \
			(((unsinged short)x)==(unsigned short)SHL_MAGIC)|| \
			(((unsinged short)x)==(unsigned short)DL_MAGIC))
...
#if !(vax || pdp11 || u3b || u3b5 || hp9000s200)
...
type_t	gmask = STABTYPES | N_TYPE | N_EXT | EXTERN2;
...
#if pdp11          /* no flexnames on pdp11 */
...
#if hp9000s200
	if (N_BADMAG(fhead)) {            /* Verify proper "magic" number. */
#else
...
#if (pdp11 || hp9000s200)    /* rdg */
	/* on pdp11 (and hp), manipulate file directly */
	(void)fseek(sym_iop, symaddr, 0);
#endif
	n_syms = 0;			/* Init count of qualified symbols. */
#if hp9000s200
	n = 0;                          /* Total symbols. */
	while (1) {                     /* Scan symbol table. */
	    int ret;
	    ret = readnl(++n, 0);       /* don't bother doing malloc's */
	    if (ret == -1) {
		symttl = n - 1;
		break;
	    }
...
#if hp9000s200
		if (readnl(symct, 1)) {    /* malloc space this time */
#else
...
#if (pdp11 || hp9000s200)   /* rdg */   /* on PDP11, long is enough and unsigned long not supported*/
...
PROC
#if hp9000s200
readnl(symindex, mallocflag)
int symindex, mallocflag;
#else
readnl(symindex)
int symindex;
#endif
{
#if hp9000s200
    struct nlist_ nli;          /* real structure in a.out, not nlist */
#endif

#if (u3b || u3b5 || vax)      /* for COFF, special care for auxiliary entries */
	static int numaux = 0;
...
#endif
#if hp9000s200
	if (fread((char *)&nli, sizeof(struct nlist_), 1, sym_iop) != 1)
		return(-1);
	nl.n_value = nli.n_value;
	nl.n_type = nli.n_type;
	nl.n_length = nli.n_length;
	nl.n_almod = nli.n_almod;
    {                           /* got length, now read symbol */
	int i;
	char *p, dummy[42];
	if (mallocflag || nli.n_length > 42)        /* don't do malloc */
	    nl.n_name = Malloc(1, nli.n_length + 1);    /* on first pass */
	else
	    nl.n_name = dummy;
	p = nl.n_name;
	for (i=0; i <= nl.n_length; i++)
	    *p++ = '\0';
    }
	if (fread(nl.n_name, nli.n_length, 1, sym_iop) != 1)
	    return(-1);
#endif
#if (pdp11 || hp9000s200)       /* rdg */
DEBUG(if (debug & 020)
...
int item_size;
{
	...
	if ((p = (char *)malloc((unsigned)item_count * (unsigned)item_size)) 
	     == NULL)  
	{
...
PROC char *
Realloc(oldblk, item_count, item_size)
char *oldblk;
int item_count;
int item_size;
{
	...
	p = (char *)realloc(oldblk, (unsigned)item_count * (unsigned)item_size);
	if (p == NULL)  {
		(void)fprintf(stderr, "%s: Out of space\n", cmdname);
		exit(1);
	}
...
#if pdp11                       /* fixed size name with not terminater on pdp11 */
...
SYMENT symbol;
{
	...
	if ((sp_used + lth) > size)  {	 /* if need more space */
		if (strtable == NULL)  { /* if first allocation */
			/* just in case very long name */
			get = lth > STRSPACE ? lth : STRSPACE;
			strtable = Malloc(1, get);
			size = get;
		} else {
			get = (lth > STRSPACE ? lth : STRSPACE) + size;
			strtable = Realloc(strtable, 1, get);
			size += get;
		}
	}
	(void)strcpy(&(strtable[sp_used + 1]), name);
	name = &(strtable[sp_used + 1]);
	sp_used += lth;
	return name;
}
#endif
