	...
	char **argv;
{
...
#ifndef hpux
	default:
		fputs("trouble reading termcap", stderr);
		/* fall through to ... */
	...
	case 0:
		/* No such terminal type - assume dumb */
		fputs("cannot access terminfo file for ", stderr);
		fputs(termtype, stderr);
		fputs("\ntrying 'dumb' terminfo description\n");
		switch(tgetent(termcap, "dumb")) {

		case 1:
			break;
		case 0:
			/* No terminfo file for "dumb" */
			fputs("cannot read 'dumb' terminfo file ... quitting\n", stderr);
			exit(1);
		}

	default:
		fputs("cannot access terminfo directory ... quitting\n", stderr);
		exit(1);
#endif hpux
	}
	initcap();
...
FILE *f;
{
	...
	case IESC:
		...
		default:
			fputs("Unknown escape sequence in input: ", stderr);
			fputs(ltostr(IESC, 8), stderr);
			fputs(", ", stderr);
			fputs(ltostr(c, 8), stderr);
			fputc('\n', stderr);
			exit(1);
		}
		continue;
