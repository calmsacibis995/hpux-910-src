 ...
 *	consistent, and writes to the standard output. A - can be used
 *	in place of files to indicate the standard input. If time/date
 *	corrections are not performed, acctcon1 will fault when it
 ...
 *	the flag NEW_TIME placed in the type field. Wtmpfix uses these
 *	records to synchronize all time stamps in the file. Wtmpfix
 *	nullifies date change records when writing to the standard
 *	output by setting the time field of the <utmp.h> structure in
 *	the old date change record equal to the time field in the new
 *	date change record. In this way, wtmpfix and acctcon1 will not
 *	factor in a date change record pair more than once.
 *
 *	In addition to correcting time/date stamps, wtmpfix will check
 *	the validity of the name field to ensure that it consists solely
 *	of alphanumeric characters or spaces. If it encounters a name
 *	that is considered invalid, it will change the login name to
 *	INVALID and write a diagnostic to the standard error. In this
 *	way, wtmpfix reduces the chance that acctcon1 will fail when
...
char	*name;
{
	...
	for(i=0; i<NSZ; i++) {
		if(name[i] == '\0')
			return(VALID);
		/* DSDe406933:
		 * Changed to follow current behavior of login(1) to allow
		 * all printable characters in the user name.
		 */
		/*
		...
		*/
		if (!isgraph(name[i])) return(INVALID);
	}
	return(VALID);
}
