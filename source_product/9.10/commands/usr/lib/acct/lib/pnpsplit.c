...
static int	holidays[NHOLIDAYS+1];	/* holidays file day-of-year table. */
					/* the array size should be NHOLIDAY+1*/
					/* to hold the "-1" at the end.     */
					/* by Ming Chen                     */
...
inithol()
{
	...
	fclose(holptr);
	if(!errflag && holindx <= NHOLIDAYS) {/*Since the index is incremented*/
