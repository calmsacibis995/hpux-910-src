 ...
 *
 *	Acctcms reads one or more files, normally in the form described
 *	in acct(5). It adds all records for processes that executed
 *	identically-named commands, sorts them, and writes them to the
 *	standard output, normally using an internal summary format. The
 *	options are:
 *
 *	-a	Print output in ASCII rather than in the internal
 *		summary format. The output includes command name, number
 ...
 *		transfered, and blocks read and written, as in
 ...
 *		default summary format splits each field into prime and
 *		non-prime time parts. This option combines the prime and
 *		non-prime time parts into a single field that is the
 *		total of both, and provides upward compatibility with
 *		old (i.e. UNIX System V) style acctcms internal summary
 *		format records.
 *
 ...
 *
 *	When -p and -o are used together, a combination prime and non-
 *	prime time report is produced. All the output summaries will be
...
struct	acct	ab;
/*
 * If your compiler doesn't gag on this, worry!
 * the first element of struct [pc]ms is only eight chars
 * -- where shall the null terminator go?
...
char **argv;
{
	...
	(void) memcpy(tcmtmp.tcm_comm, "***other", 8);
	(void) memcpy(pcmtmp.pcm_comm, "***other", 8);
	if ((pflg || oflg) && !aflg) {
		fprintf(stderr,"%s -%c option must be used with -a option\n",argv[0], pflg ? 'p' : 'o');
		exit(1);
	}
	if(tflg) {
...
register struct tcms *p;
{
	...
	for (i %= CSIZE, j = 0; tcm[i].tcm_comm[0] && j != CSIZE; i = (i+1)%CSIZE, j++)
		if (EQN(p->tcm_comm, tcm[i].tcm_comm))
			break;
	if(j == CSIZE) {
		fprintf(stderr,"acctcms: Command hash table overflow\n");
		return(-1);
	}
