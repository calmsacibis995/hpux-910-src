...
#ifdef SIGWINCH
	short	alloc_ch;	/* number of chtype allocated in body */
#endif
};
...
struct map {
	...
	unsigned char sends[16];/* Characters sent when key is pressed */
...
#ifdef		SIGWINCH
	short alloc_ln;		/* number of "lines" allocated in the screen */
#endif
};
...
/* following for improved performance when setting screen attributes */
#define	NHASHKEYS		11
#define MAX_ATTR_STRLEN		15
#define hashattr(key)		(((unsigned) (key) >> 24) % NHASHKEYS)
...
struct _winlink {
	WINDOW *win;
	struct _winlink *next;
};
extern	struct _winlink *_winlist;

extern WINDOW *_sumscr;			/* the summation screen containing */
					/* the body of the SP->cur_body   */
#endif
