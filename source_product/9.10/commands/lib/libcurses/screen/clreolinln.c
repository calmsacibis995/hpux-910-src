...
_clreol()
{
	...
	if (!move_standout_mode) 
	    /*
	     * Optimization - setting display attributes and then
	     *    clearing-to-eol wipes out the attributes on certain
	     *    magic cookie terminals so don't do it
	     */
	    if (magic_cookie_glitch || !clr_eol)
		_clearhl ();
