...
static char TOOLONG[] = "Terminfo entry too long:\n";
...
char **argv;
{
	...
	if (argc == 1) {
		rv = compfile(stdin, "stdin");
	}
	else for (i=1; i<argc; i++) {
		rv = compfile(fopen(argv[i], "r"), argv[i]);
	}
...
char *fname;
{
	...
	p = tbuf;
	if (++hopcount > MAXHOP) {
		fputs("Infinite use= loop '", stdout);
		fputs(tbuf, stdout);
		fputs("'\n", stdout);
		return (0);
	}
...
char *cap;
{
	...
	char *tname = cap;
	char *tnp;
	char tnbuf[2048], names[2048];
	char fnbuf[128], lnbuf[128];
	char strtab[4096];
	register char *strtabptr;
	FILE *fd;
	int sname, sbool;

	while ( *cap != '\0' && *cap != SEPARATE )  /* skip over names */
		cap++;
	*cap = 0;
	strcpy(tnbuf, tname);
	strcpy(names, tname);
	...
	if (cp-fp > 2) {
		...
		*cp = 0;
		if (stat(fn, &stbuf) < 0) {
			...
			if (mkdir(fn, 0777) == -1) {
				fputs("tic: can't make directory ",
				    stderr);
				perror(fn);
				exit(1);
			}
		...
		*cp++ = '/';
		...
		fputs(fp+1, stdout);
		fputs(": terminal name too short; check input file for proper format\n", stdout);
		exit(1);
	}
	if (stat(fn, &stbuf) < 0)
		return;
	if (stbuf.st_mtime < starttime) {
	    /* "old" terminfo file; it was built before  */
	    /* this session started so we can destroy it */
	    if (verbose > 1) {
		    fputs("unlink ", stdout);
                    fputs(fn, stdout);
		    fputc('\n', stdout);
	    ...
	    /* "new" terminfo file; this file has just been */
	    /* built in this session and should not be      */
	    /* destroyed. This is an error in the terminfo  */
	    /* source file.                                 */
	    fputs("tic: error in terminfo source, <", stdout);
	    fputs(fn, stdout);
	    fputs("> exists already and is new\n", stdout);
	}
}
