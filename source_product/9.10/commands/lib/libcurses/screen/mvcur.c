...
_loc_right(oldcol, newcol, domotion, row)
{
	...
	int	out_1st;	/* write out first byte of 16-bit character */
	...
	row, tabcol, i, tabcol+i, SP->cur_body[row+1]->body[tabcol+i]);
						...
						if (cursor_right && (!notinsmode || rp && SP->phys_gr != (rp->body[tabcol+i] & A_ATTRIBUTES) || rp && rp->body[tabcol+i] & A_NLSATTR)) { /* dont know */
							 ...
							 */
							if (rp->body[tabcol+i] & A_NLSATTR) {
								if (rp->body[tabcol+i] & A_SECOF2) {
									if ( out_1st ) {
										out_1st=0;
									} else {
										tputs(cursor_left,1,_outch);
										_outch(rp->body[tabcol+i-1]&A_CHARTEXT);
									}				
								} else {
									out_1st=1;
								}
				...
				for (i=oldcol,out_1st=0; i<newcol; i++) {
#ifdef DEBUG
	if (outf && SP->cur_body[row+1]) fprintf(outf, "nd2, row %d col %d, char '%c'\n",
	row, i, SP->cur_body[row+1]->body[i]);
#endif
					rp = SP->cur_body[row+1];
					if (cursor_right && (!notinsmode || rp && SP->phys_gr != (rp->body[i] & A_ATTRIBUTES) || rp && rp->body[i] & A_NLSATTR)) { /* dont know */
						tputs(cursor_right,1,_outch);
						out_1st=0;
					} else if (rp && rp->length > i) {
						if (rp->body[i] & A_NLSATTR) {
							if (rp->body[i] & A_SECOF2) {
								if ( out_1st ) {
									out_1st=0;
								} else {
									tputs(cursor_left,1,_outch);
									_outch(rp->body[i-1]&A_CHARTEXT);
								}
							} else {
								out_1st=1;
							}
