...
#**********************************************************
#							  *
# SINGLE PRECISION X**Y					  *
#							  *
# float fpow(x,y) float x,y;				  *
#							  *
...
c_pow1:
	fmov.l	%d0,%fpcr
	mov.l	&0x3F800000,%d0		#0.0**0.0 = 1.0
	unlk	%a6
	rts
c_pow2:
...
c_pow3:	
	...
	fneg	%fp0
c_pow4:
	fmov.s	%fp0,%d1		#get overflow/underflow cond.
	fmov.l	%fpsr,%d1
	andi.w	&0x1800,%d1		#overflow or underflow?
	bne.w	c_pow7
	fmov.s	%fp0,%d1
...
c_pow4a:
	...
	fmov.l	%d0,%fpcr		#restore control register
ifdef(`libM',`
	call_error(fpow,%a0,%a1,FLOAT_TYPE,NAN_RET,DOMAIN)
	',`
	call_error(fpow,%a0,%a1,FLOAT_TYPE,ZERO_RET,DOMAIN)
	')
	unlk	%a6
	rts
c_pow5:
	fmov.l	%d0,%fpcr		#restore control register
ifdef(`libM',`
	call_error(fpow,%a0,%a1,FLOAT_TYPE,NHUGE_RET,DOMAIN)
	',`
	call_error(fpow,%a0,%a1,FLOAT_TYPE,ZERO_RET,DOMAIN)
	')
	unlk	%a6
...
c_pow7:
	...
	call_error(fpow,%a0,%a1,FLOAT_TYPE,ZERO_RET,UNDERFLOW)
	unlk	%a6
	rts
c_pow8:
	call_error(fpow,%a0,%a1,FLOAT_TYPE,HUGE_RET,OVERFLOW)
	unlk	%a6
	rts
