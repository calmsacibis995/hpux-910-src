/* $Source: /misc/source_product/9.10/commands.rcs/lib/libc/rpc/clnt_kudp.c,v $
 * $Revision: 12.1 $	$Author: gomathi $
 * $State: Exp $   	$Locker:  $
 * $Date: 92/03/05 13:48:15 $
 */
...
static char rcsid[] = "@(#) $Header: clnt_kudp.c,v 12.1 92/03/05 13:48:15 gomathi Exp $ (Hewlett-Packard)";
#endif
	...
	struct timeval	wait;
{
	...
	if ( (p->cku_flags & CKU_RETRANSMIT) == 0) {
		...
		rpc_sbdrops++;
		(void) sbdrop( &so->so_rcv, (int)so->so_rcv.sb_cc, MA_SEQ_WILD);
	    }
	}
...
#ifdef KERNEL_TUNE_FG
		save_mask = P_sigmask(u.u_procp);
#else  KERNEL_TUNE_FG
		save_mask = u.u_procp->p_sigmask;
#endif KERNEL_TUNE_FG
	...
	struct socket *so;
{
	...
	u_short i;
	int error;
	struct kopt kopt_space;
	struct kopt *opt = &kopt_space;
	struct ucred *tcred;
