...
/*
	...
	convert an ASCII string to a double precision floating point number
	...
	NLS radix support added, 12/8/86, Chris Butler, Contractor

	Algorithm: The variable "ptr" for strtod simply returns a pointer to
	the character that terminated the scan. Therefore, atof is simply
	implemented as a call to strtod with the pointer ignored.
	The ASCII string is scan up until a terminating character with three
	items being generated: the mantissa sign, an unsigned integer, and
	a signed exponent. The signed exponent is then decomposed into two
	numbers: 10 raised to a power of ten which is a multiple of 25 and
	10 raised to a positive integer. These powers come from tables table1 
	and table2 respectively. The final mantissa is then the product of these
	two numbers and the unsigned integer. The final exponent is based on the
	size of the unsigned integer, the exponents associated with the powers 
	of 10 (which come from tables table1e and table2e), and any final
	normalization. The final sign comes directly from the scan.
	The retain maximum accuracy, extended precision arithmetic is needed and
	is provided by the routine mult32 which multiplies two 64 bit integers 
	and gives a 128 bit result. It was necessary to write this routine
	in assembly language to achieve necessary performance.
	If a number cannot be formed, a 0 is returned. Overflows return HUGE_VAL
	with the proper sign and set errno to ERANGE. Underflows return 0 and
	also set errno to ERANGE.
*/
...
/*  nl_rdxcvt() returns the string position which was converted, or
    NULL if no position changed.  The caller's string is temporarily
    modified at the position where the local radix character or '.'
    is encountered, on the assumption that the calling routine
    will "unconvert" later.
 
    No "syntax check" is performed, so in some cases more of a string
    may be scanned than necessary, looking for radix contenders.
    The assumption is that most strings will conform to floating
    point notation, so in most cases it will be more efficient
    not to make a syntax check.
*/
...
#ifdef _NAMESPACE_CLEAN
#undef atof
#pragma _HP_SECONDARY_DEF _atof atof
#define atof _atof
#endif /* _NAMESPACE_CLEAN */
...
double atof(str) char *str;
{
	...
	/* convert the first instance of a radix char into a
	*  decimal point for internal use, then reconvert before
	*  returning to caller
	*/ 
	if (_nl_radix == '.')
	    converted = (char *) 0;
	else
	    converted = nl_rdxcvt(str, _nl_radix, '.');
	result = xatof(str);
	if (converted) {
	    if (*converted == '.')
	        *converted = _nl_radix;
...
#ifdef NLS
static double xatof(str) char *str;
#else /* NLS */
double atof(str) char *str;
#endif /* NLS */
...
#ifdef _NAMESPACE_CLEAN
#undef strtod
#pragma _HP_SECONDARY_DEF _strtod strtod
#define strtod _strtod
#endif /* _NAMESPACE_CLEAN */
...
#ifdef NLS
static double xstrtod(str,ptr) char *str, **ptr;
#else /* NLS */
double strtod(str,ptr) char *str, **ptr;
#endif /* NLS */

{
...

	{
		...
		if ((c | 32) == 'e') { /* Is next character a "E" or "e" for an exponent? */
			{
				char digits = 0;
				short exponent = 0;
				char expsign = 0;
				...
				while (c >= '0' && c <= '9') { /* Look for digits */
					c -= '0'; /* Convert from ASCII */
					...
					exponent += c;
#else /* __hp9000s300 */
					exponent = 10 * exponent + c;
#endif /* __hp9000s300 */
					c = *p++; /* get next character */
				};
		...
		if (ptr) *ptr = --p; /* If termination pointer is desired, indicate what it is */
	};
...
nonumber: /* ASCII pattern is not a number: return pointer to start of string and return 0 */
	if (ptr) *ptr = str;
	return(0);
