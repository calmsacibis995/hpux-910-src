...
#ifdef LINE_MAX
#   define	MAXLINE	LINE_MAX	/* max message size */
#else  /* LINE_MAX */
#   define	MAXLINE	2048		/* default */
#endif /* LINE_MAX */
	...
	va_dcl
{
	...
	/* see if we priority out of range */
	if (pri <= 0 || PRIFAC(pri) >= LOG_NFACILITIES)
		return(-1);

	/* see if message is masked out */
	if ((PRIMASK(pri) & LogMask) == 0)
		return(0);
