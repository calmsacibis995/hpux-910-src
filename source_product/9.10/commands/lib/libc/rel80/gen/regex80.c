...
#define increase \
	if (flags&_REG_NOALLOC ||					\
		(p=Realloc(preg->c_re,bufsize+REG_BUF_SIZE))==NULL)	\
		return(REG_ESPACE);					\
	else {								\
		offset = (p - preg->c_re);				\
		ep += offset;		/* move current pointer */	\
		preg->c_re = p;		/* reset start of RE */		\
		bufsize += REG_BUF_SIZE;/* increment bufsize */		\
		endbuf = preg->c_re + bufsize - 1;/* reset endbuf */	\
...
unsigned		flags;
{
	...
	register	newline	= flags & REG_NEWLINE;
	...
	if (flags & _REG_NOALLOC) {
		ep	= preg->c_re;
		endbuf	= preg->c_buf_end;
	} else {
		if ((preg->c_re = ep = Malloc(REG_BUF_SIZE))==NULL)
			return(REG_ESPACE);
	...
	while (1) {
		...
		if (c == '\0') {
			...
			*ep++ = END_RE;				/* add final token to compiled RE */
			if (!(flags&_REG_NOALLOC)) {
				p=Realloc(preg->c_re,ep-preg->c_re);	/* release unused memory */
				if (p) {
					offset= p-preg->c_re;
					preg->c_re = p;
					ep += offset;
					preg->c_buf_end = ep;	/* ptr to byte past end of buffer1 */
				}
			}
			preg->flags	= flags;		/* save the flags */
			preg->anchor	= 0;			/* null out forced anchor option */
			preg->c_re_end	= ep;			/* ptr to end of compiled RE + 1 byte */
		 ...
		 */
		if (c == '\n' && !newline)
			return(REG_ENEWLINE);			/* if flag not set return error */

		if (c == '\\') {
			c = (*pp++) | BACKSLASH;
			if (c_esc) {				/* use C escaping conventions */
				switch (c) {
					case 'a'|BACKSLASH: c='\a';break;
		...
		case ')'|ERE:
			...
			*ep++ = *(token[level-1]+1);		/* paren index */
			...
			if (re) {				/* REs are simpler and we can quit now */
			...
			if ((*pp == '*' || *pp == '+' || *pp == '?') && non_null[level+1] == 0) {
				/*
		...
		default:
			...
			if (last_token == preg->c_re)		/* if first token of RE leave alone */
				continue;
			...
			if (opt_token && *last_token == BYTE) {
				...
				if (*opt_token == BYTE_PLUS) {
					if ((icase && icasecmp(*(opt_token-1),c))||*(opt_token-1)==c) {
						ep -= 2;	/* redundant "a+a" expression reduces to "a+" */
					} else {
...
unsigned	eflags;
{
	...
	register uchar	*start_ep = preg->c_re;
	register uchar	*ep = start_ep;
	register	nosub = preg->flags & REG_NOSUB;
	register 	icase = preg->flags & REG_ICASE;
	...
	if (*ep == BYTE && not_multibyte && !preg->anchor) {
		start_c = *++ep;
		start_ep = ++ep;
		if (icase) {
	...
	} else if (!not_multibyte && (*ep == KANJI || *ep == BYTE) && !preg->anchor) {
		if (*ep == KANJI) {
		...
		do {
			string_p = lp;
		} while (_CHARADV(lp) != start_kanji && *string_p);
		if (!*string_p)
			goto errcleanup;
	}
	...
	while (1) {
		...
		case ANCHOR_BEGIN:
			if (eflags&REG_NOTBOL) { /* can't match ^ if not BOL */
				if (preg->flags & (REG_EXTENDED|_REG_ALT))
					goto fail; /* ERE's live to try again */
				goto errcleanup;	/* RE's die */
			} else if (lp == string)
				continue;
			else {
				if (preg->flags & (REG_EXTENDED|_REG_ALT))
		...
		case ANCHOR_END:
			if (eflags&REG_NOTEOL) { /* can't match $ if not EOL */
				if (preg->flags & (REG_EXTENDED|_REG_ALT))
					goto fail; /* ERE's live to try again */
				goto errcleanup;
			}
		...
		success:
				if (!nosub && nmatch>0) {
					...
					for (cnt=nmatch-1;cnt>nparens;cnt--)
						pmatch[cnt].rm_sp = pmatch[cnt].rm_ep = (char *)0;

					while (cnt) {
						pmatch[cnt].rm_sp = (char *)best_parens.lparens[cnt-1];
						pmatch[cnt].rm_ep = (char *)best_parens.rparens[cnt-1];
						cnt--;
					}
				pmatch[0].rm_sp = (char *)string_p;		/* start of matched string */
				pmatch[0].rm_ep = (char *)best_lp;		/* end of matched string */
				}
...
regex_t	*preg;
{
	if (!(preg->flags&_REG_NOALLOC) && preg->c_re) {
		Free(preg->c_re);
		preg->c_re=NULL;
	}
}
